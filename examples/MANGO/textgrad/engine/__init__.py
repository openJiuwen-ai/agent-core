...
def get_engine(engine_name: str, is_async = False, **kwargs) -> EngineLM:
    ...
    if engine_name.startswith("azure"):
        ...
        from .openai import ChatOpenAI, ChatAsyncOpenAI
        if is_async:
            return ChatAsyncOpenAI(model_string=engine_name, is_multimodal=_check_if_multimodal(engine_name), **kwargs)
        else:
