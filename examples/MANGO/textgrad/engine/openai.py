...
# from owl_async_browser import BrowserToolkit
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def _generate_from_single_prompt(
        self, prompt: str, system_prompt: str=None, temperature=0.75, max_tokens=2048, top_p=0.9, tools = None, file_path = "", memory=[]
    ):
        ...
        # print(111)
        # print(messages)
        # print(sys_prompt_arg)

        # cache_check_use = sys_prompt_arg
        # for message in memory:
        #     if message["role"] == "user":
        #         cache_check_use += message["content"]
        # cache_check_use += prompt

        # cache_or_none = self._check_cache(cache_check_use)
        ...
        json_schema = {
            ...
            "schema": {
                "type": "object",
                "properties": {
                    "response": {"type": "string", "description": "Response of the question"},
                    "successful": {"type": "boolean", "description": "Whether the execution is successful"}
                },
                "required": ["response", "successful"],
                "additionalProperties": False
            },
        }

        if tools is None:
            # if "JSON" in sys_prompt_arg:
            if "JSON" in prompt:
            ...
            if msg.tool_calls is not None:
                ...
                for tool_call in msg.tool_calls:
                    ...
                    if fn_name == "browse_url":
                        ...
                        content = browser_toolkit.browse_url(args["task_prompt"], args["start_url"], args["round_limit"])
                    elif fn_name == "ask_question_about_image":
                        image_toolkit = ImageAnalysisToolkit()
                        content = image_toolkit.ask_question_about_image(file_path, args["question"], args["sys_prompt"])
                    # elif fn_name == "extract_document_content":
                    #     document_tookit = DocumentProcessingToolkit()
                    #     _, content = document_tookit.extract_document_content(file_path, args["query"])
                    elif fn_name == "search_google":
                        search_tookit = SearchToolkit()
                        content = search_tookit.search_google(args["query"], args["num_result_pages"])
                    elif fn_name == "search_papers":
                        arxiv_toolkit = ArxivToolkit()
                        content = arxiv_toolkit.search_papers(args["query"])
                    # print(content)
                    messages.append({"role": "tool", "name": fn_name, "tool_call_id": tool_call.id, "content": str(content)})
...
class ChatAsyncOpenAI(EngineLM, CachedEngine):
    DEFAULT_SYSTEM_PROMPT = "You are a helpful, creative, and smart assistant."
    ...
    @retry(wait=wait_random_exponential(min=1, max=5), stop=stop_after_attempt(5))
    async def generate(self, content: Union[str, List[Union[str, bytes]]], system_prompt: str=None, **kwargs):
        if isinstance(content, str):
            
            return await self._generate_from_single_prompt(content, system_prompt=system_prompt, **kwargs)
    ...
    async def _generate_from_single_prompt(
        self, prompt: str, system_prompt: str=None, temperature=0.75, max_tokens=2048, top_p=0.9, tools = None, file_path = "", memory=[]
    ):
        ...
        TOKEN_USAGE.add_usage(input_tokens=input_token, output_tokens=output_token)
        # self._save_cache(sys_prompt_arg + prompt, response)
        return content
