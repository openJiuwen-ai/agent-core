...
class LLMCall(Function):
    ...
    def forward(self, input_variable: Variable, tools, temperature, file_path, memory, response_role_description: str = VARIABLE_OUTPUT_DEFAULT_ROLE) -> Variable:
        """
        ...
        # Make the LLM Call
        response_text = self.engine(input_variable.value, system_prompt=system_prompt_value, tools=tools, temperature=temperature, file_path=file_path, memory=memory)
    ...
    async def async_forward(self, input_variable: Variable, tools, temperature, file_path, memory, response_role_description: str = VARIABLE_OUTPUT_DEFAULT_ROLE) -> Variable:
        """
        The LLM call. This function will call the LLM with the input and return the response, also register the grad_fn for backpropagation.
        ...
        # Make the LLM Call
        response_text = await self.engine(input_variable.value, system_prompt=system_prompt_value, tools=tools, temperature=temperature, file_path=file_path, memory=memory)
...
class FormattedLLMCall(LLMCall):
    ...
    async def async_forward(self, 
        ...
        # Make the LLM Call
        response_text = await self.engine(formatted_input_string, system_prompt=system_prompt_value)
