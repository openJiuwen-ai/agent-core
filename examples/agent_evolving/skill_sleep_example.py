...
def main() -> None:
    ...
    if outcome.report.skill_groups:
        group_info = " groups=[" + ", ".join(
            f"{g.skill_name}:{g.status}:accepted={g.accepted}"
            for g in outcome.report.skill_groups
        ) + "]"

    adopt_info = ""
    if outcome.adopted_skills:
        parts = [
            f"{item.skill_name}:{item.previous_version}->{item.new_version}"
            for item in outcome.adopted_skills
        ]
