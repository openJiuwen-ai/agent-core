...
def _parse_args(default_env: str = "") -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run skill_train ReflACT training on SearchQA/DocVQA/OfficeQA."
    )
    parser.add_argument(
        "--env",
...
def main() -> None:
    ...
    llm_total_budget = float(
        _env("LLM_TOTAL_BUDGET", default=str(max(600.0, llm_attempt_timeout * 5)))
    )
    ...
    env_kwargs: Dict[str, Any] = {
        ...
        "max_completion_tokens": int(
            _env("MAX_COMPLETION_TOKENS", default=str(preset.max_completion_tokens))
        ),
    ...
    logger.info(
        "[skill_reflact_train] env=%s split_dir=%s train_size=%s "
        "batch_size=%s workers=%s exec_timeout=%s output=%s",
        env_name,
        split_dir,
        config.train_size,
        config.batch_size,
