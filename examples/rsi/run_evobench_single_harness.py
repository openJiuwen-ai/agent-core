...
def main(argv: list[str] | None = None) -> int:
    ...
    # Baseline preparation delegates to the official launcher, whose options
    # are populated from these resolved values.
    args.run_model_config_ref = str(runtime_inputs.evaluation_model)
    args.analysis_model_config_ref = str(runtime_inputs.analysis_model)
    args.member_optimization_model_config_ref = str(runtime_inputs.member_optimization_model)
    args.judge_model_config_ref = str(runtime_inputs.judge_model)
...
def _ensure_source_run(args: argparse.Namespace, source_run: Path) -> tuple[Path, Path | None]:
    ...
    command = [
        ...
        "--judge-model-config",
        str(Path(args.judge_model_config_ref).expanduser().resolve()),
    ]
