...
: "${SFT_E2E_CONCURRENCY:=2}"
: "${SFT_E2E_TIMEOUT:=900}"
: "${SFT_E2E_TRAIN_GPU:=4,5,6,7}"
: "${SFT_E2E_USER_ID:=sft-verl-e2e}"
...
HOST_IP="$(host_ip)"
DATASET_MAPPING_DEFAULT="${JIUWENRL_ROOT}/sft_e2e/data/sft_short_10_cases.json"
: "${SFT_E2E_CASES:=${DATASET_MAPPING_DEFAULT}}"
SFT_E2E_GATEWAY_LOCAL_URL="http://127.0.0.1:${GATEWAY_PORT}"
SFT_E2E_GATEWAY_DOCKER_URL="http://${HOST_IP}:${GATEWAY_PORT}"
SFT_E2E_SUPERVISOR_URL="http://${HOST_IP}:${VLLM_PORT}"
SFT_E2E_EXPORT_PATH="/tmp/sft_samples_${SFT_E2E_USER_ID}.json"
SFT_E2E_REQUEST="我想要用 sft-optimize 技能对模型进行微调：数据集是 ${SFT_E2E_CASES}，${SFT_E2E_CASE_LIMIT}个用例，并发 ${SFT_E2E_CONCURRENCY}，不训练，只采集 supervisor replay 轨迹。"
...
echo "[sft-verl-e2e] cleaning previous backend state"
"${DEPLOY_DIR}/clean_online_rl_env.sh"

echo "[sft-verl-e2e] activating ${SFT_E2E_CONDA_ENV}"
if [[ "${USE_CONDA:-1}" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
  # shellcheck disable=SC1090
  source "${SFT_E2E_CONDA_SH}"
  conda activate "${SFT_E2E_CONDA_ENV}"
else
  echo "[sft-verl-e2e] USE_CONDA=0, skip conda activation"
fi

echo "[sft-verl-e2e] starting backend"
...
echo "[sft-verl-e2e] collecting samples via skill wrapper"
...
echo "[sft-verl-e2e] exporting pending samples"
...
if [[ -z "${lora_path}" ]]; then
  echo "[sft-verl-e2e] missing adapter_model.safetensors under ${LORA_REPO}/${SFT_E2E_USER_ID}" >&2
  exit 1
fi
echo "[sft-verl-e2e] adapter=${lora_path}"
echo "[sft-verl-e2e] export=${SFT_E2E_EXPORT_PATH}"
echo "[sft-verl-e2e] backend logs=${LOG_DIR}"
