...
"""Validate scenario 2-1 SFT mock E2E Redis state and generated dataset."""
...
def _validate_sample(sample: dict[str, Any]) -> None:
    ...
    _assert(str(assistant.get("content") or sample.get("response_text") or ""), "sample assistant text is required")
...
def _validate_dataset(tmp_root: str, *, min_samples: int) -> Path:
    ...
    _assert(path is not None, f"missing dry-run train.json under {tmp_root}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tmp-root", default="/tmp/agent_rl_online_sft_mock_e2e")
