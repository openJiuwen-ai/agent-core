...
echo "[sft-e2e][stage3] collect original raw trajectories"
...
task_json="$(curl -sf -X POST "${SFT_E2E_GATEWAY_LOCAL_URL}/v1/training/tasks" \
  ...
  -d "{\"user_id\":\"${SFT_E2E_USER_ID}\",\"metadata\":{\"e2e\":\"sft_stage3_mock_train\"}}")"
task_id="$(python -c 'import json,sys; print(json.load(sys.stdin)["task_id"])' <<< "${task_json}")"
echo "[sft-e2e][stage3] triggered task ${task_id}"
