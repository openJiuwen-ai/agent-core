...
for run_idx in $(seq 1 "${SFT_E2E_REPEAT}"); do
  echo "[sft-e2e] ===== run ${run_idx}/${SFT_E2E_REPEAT} ====="
  ...
  task_json="$(curl -sf -X POST "${SFT_E2E_GATEWAY_LOCAL_URL}/v1/training/tasks" \
    ...
    -d "{\"user_id\":\"${SFT_E2E_USER_ID}\",\"metadata\":{\"e2e\":\"sft_scenario2_1_mock\",\"run_index\":${run_idx}}}")"
  task_id="$(python -c 'import json,sys; print(json.load(sys.stdin)["task_id"])' <<< "${task_json}")"
  echo "[sft-e2e] triggered task ${task_id}"
...
echo "[sft-e2e] all ${SFT_E2E_REPEAT} run(s) passed"
