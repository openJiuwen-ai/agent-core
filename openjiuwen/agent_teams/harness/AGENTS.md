...
| `AsyncToolRuntime` | 挂在 NativeHarness 上的后台任务注册表 + 完成注入器。`launch` / `has_running` / `cancel_all` + 管控面 `get` / `list_all` / `cancel(task_id)` / `wait(task_id, timeout)`；`_tasks` 是 `dict[task_id, Task]`（非 set）+ per-task `_events`；完成经 `_maybe_spill`（内联 vs 写盘）后调注入回调（= `harness.send(immediate=False)`） |
| `AsyncToolRecord` | 注册表行（task_id / tool_name / status / result / error / `output_file`） |
