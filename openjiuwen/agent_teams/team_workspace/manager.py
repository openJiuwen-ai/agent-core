...
class TeamWorkspaceManager:
    ...
    def _merge_existing_mount_contents(self, link_path: str) -> None:
        ...
        before the mount exists. Merge missing files so user artifacts are not
        stranded, flattening the old ``.team/<team_name>`` layout and skipping
        symlinks or junctions that already expose the canonical workspace.
        Existing canonical files win to avoid overwriting newer workspace data.
        """
        if not os.path.isdir(link_path) or self._is_directory_link(link_path):
            return
        for root, dirs, files in os.walk(link_path):
            dirs[:] = [
                dirname
                for dirname in dirs
                if not self._is_directory_link(os.path.join(root, dirname))
            ]
            rel_root = os.path.relpath(root, link_path)
            rel_parts = [] if rel_root == "." else rel_root.split(os.sep)
            if rel_parts and rel_parts[0] == self.team_name:
                rel_parts = rel_parts[1:]
            dst_root = os.path.join(self.workspace_path, *rel_parts)
    ...
    def mount_into_workspace(self, workspace_root: str) -> None:
        ...
        Agents use short relative paths such as ``.team/artifacts/...`` so the
        filesystem path does not repeat the session-scoped team name.
