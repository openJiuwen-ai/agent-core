...
fork 继承把源成员（如 `reader`）的对话历史原样注入目标成员（如 `dev-1`）。这份历史里包含源成员的 `<team-context>` 身份块（`你的 member_name: reader`）。目标成员 spawn 后 `TeamContextTracker` 又会投递一份**它自己**的身份块（`你的 member_name: dev-1`）——于是子代上下文里并存两份身份，模型无法判断"我到底是谁"。F_75 系列从未处理这份残留。
...
- **fork 时剥掉 / 替换源身份块**（导师最初建议，XML 前后匹配定位后手术）：身份块位置靠前，动一个 token，其后整个继承段的前缀 cache 全部失效 → fork 白做。**不采纳**。
