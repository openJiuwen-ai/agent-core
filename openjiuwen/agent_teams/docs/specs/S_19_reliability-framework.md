...
| 最近一次修订日期 | 2026-06-02 |
| 关联 feature | F_29_reliability-framework.md |
...
3. **anomaly 统一经 `AnomalyDetectedEvent` 上报**，leader 进程的 `ReliabilityHandler` 是唯一
   消费者。`on_anomaly_detected` / `on_message` 均以 `role == LEADER` 为前置门。
...
6. **handler 仅在 `enabled` 时注册**到 dispatcher；rail 仅在 `enabled` 且
   `role.value in monitor_roles` 且 messager 就绪时挂载。
7. **检测分级边沿触发**：同一异常 severity 不上升时不重复产出，避免告警风暴。

## 接口契约

- **配置入口** `ReliabilityConfig`（`TeamAgentSpec.reliability`，公开导出）：
  `enabled` / `monitor_roles: list[str]`（TeamRole values，默认 `["teammate"]`）/
...
- `Signal` / `SignalKind`：统一信号流（见 F_28）。
...
- 内置检测器（成员内 5 + 团队级 1）：
  ...
  | `RepeatToolCallDetector` | before/after_tool_call + tool_exception | REPEAT_TOOL_CALL / TOOL_CALL_LOOP | 重复10 / 交替10 / 连续20 / 全局30 |
...
成员进程：rail hook → Signal → ReliabilityMonitor.feed
            ...
            → (policy: REPORT_LEADER/ESCALATE_USER) reporter.report → AnomalyDetectedEvent → messager
