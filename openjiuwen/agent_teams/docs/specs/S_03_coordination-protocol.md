...
| 最近一次修订日期 | 2026-08-15 |
| 关联 feature | `F_01_coordination-protocol-cleanup.md`、`F_05_lifecycle-finalize-relocation.md`、`F_07_team-completion-events.md`、`F_14_human-agent-team-event-rendering.md`、`F_65_runtime-idle-clock-stall-nudge.md`、`F_73_team-debate-single-wrapup.md` |
...
21. **思辨收束不能越过 interrupt，也不能被内部输入重开**（F_73）。Leader 的 debate wakeup 回调返回 `bool`：存在 pending interrupt 时不调用 `deliver_input()` 并返回 `false`，`DebateRunState` 只撤销 `finalizing`、保持 `finalized=false`，后续 mailbox poll 再试。收束成功后，mailbox、调度消息和内部总结 prompt 都不能重置状态；只有下一条来自团队外部、实际投给 Leader 的输入才重置已 finalized 的上一轮，包括 `USER_INPUT` / `GodViewMessage`、Operator 广播和 Operator 直发 Leader；Operator 只定向其他成员时不重置。
...
|---|---|---|---|
| `AgentLifecycleHandler` | `InnerEventType.USER_INPUT` / `TeamEvent.STANDBY` / `TeamEvent.CLEANED` / `TeamEvent.TOOL_APPROVAL_RESULT` / `TeamEvent.TASK_PLAN_RESPONSE` | 无 | `on_user_input`：Leader 先重置已 finalized 的 debate state，再 `_round.deliver_input`；`on_standby` → `_poll.pause_polls`；`on_cleaned` → `_lifecycle.shutdown_self`（leader 跳过）；`on_tool_approval_result` → 构造 `InteractiveInput` → `_round.resume_interrupt` |
| `MemberHandler` | `InnerEventType.REFRESH_TEAM_CONTEXT` + 6 个 `MEMBER_*`：`SPAWNED` / `RESTARTED` / `STATUS_CHANGED` / `EXECUTION_CHANGED` / `SHUTDOWN` / `CANCELED` | `_team_clean_requested: bool`（私有） | `on_member_event` 按 role 分流；leader 观测成员生命周期，成员 `ERROR` / `FAILED` 时标记当前 debate 终态并尝试统一收束，**不再** stale-claim 跨进程 nudge；非 leader 仅对自身事件反应：`MEMBER_CANCELED` 触发 `_round.cancel_agent`，`MEMBER_SHUTDOWN` **仅在 `force` 时** `_lifecycle.shutdown_self()`。graceful shutdown 走 `MessageHandler` 的 drain + harness-input 闸门（机制 18） |
| `MessageHandler` | `TeamEvent.MESSAGE` / `TeamEvent.BROADCAST` / `InnerEventType.POLL_MAILBOX` / `TeamEvent.MEMBER_SHUTDOWN`（fan-out） | 无 | Leader 捕获当前 round 的显式 debate final report，按 sender 去重，聚齐终态后才统一 wake；wakeup 返回 `false` 时保留消息并由后续 poll 重试。其它路径：leader ack user-bound + 通知 human-agent inbound；所有 role `_poll.resume_polls` + `_process_unread_messages`；三条投递路统一先过 `_harness_input_blocked`（机制 18） |
