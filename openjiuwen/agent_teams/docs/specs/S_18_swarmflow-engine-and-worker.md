...
4. **无 DB roster 身份、不持 `team_backend`**：swarmflow worker **不是 teammate**，`TeamWorkerBackend` **不**经 `spawn_member` 往 team DB 写 member row。每个 worker 仅 mint 一个 member_name（形如 `wf-<label-slug>-<n>`，满足 `_MEMBER_NAME_PATTERN`），只用作 worker card / owner id / 工作区目录名——纯进程内身份，用完即弃，不污染团队成员表。worker 工作区落在 `team_home/workspaces/{member}_workspace`，而 `agent_configurator` 已把整个 `team_home` 登记进团队 cleanup，故 worker 工作区随 `clean_team` 一并删除，**无需** backend 单独登记。由此 `TeamWorkerBackend` / `run_swarmflow` / `SwarmflowTool` 整条链**不再注入 `team_backend`**——worker 路径与 team DB 彻底解耦。
