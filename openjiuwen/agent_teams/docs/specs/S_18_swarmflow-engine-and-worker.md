...
- **两级执法，缺一不可**：
  ...
  - **引擎（重试短路）**：`_attempt_calls` 的 except 分支在重试前查账本——ledger 干涸则 **fail-fast 不重试**（账本不退款，重试必败；human turn 的重试还会重新弹一次 `HUMAN_PROMPT`），失败结果直接带与 gate 同格式的 `workflow/session token budget exhausted: X/Y` 真因进 `AGENT_FAILED`。消息格式统一由 `_budget_exhaustion` 出。
...
- harness 无原生结构化输出。当 `agent(prompt, schema=...)` 的 `schema_json` 非空：backend 构造一个 `StructuredOutputTool` **对象**，其 `ToolCard.input_params == schema_json`，描述经 `tools/locales` i18n 解析（`descs/<lang>/common/structured_output.md`）并尾部拼接 `describe_schema_requirements` 渲染的逐层必填键摘要（`F_100`；`required_structure` 属性供各 backend 在 turn prompt 复述），**作为实例追加进 worker spec.tools**（`DeepAgentSpec._resolve_tools` 原样透传实例）。挂上 harness 时 ability_manager 把 id re-qualify 为 `structured_output_{owner_id}`（owner = worker card id，并发 worker 不撞），无需 backend 指定 per-call id。
- worker system prompt 要求「完成后必须调用 `structured_output` 提交结构化结果」。工具 `invoke(inputs)` 先 `jsonschema.validate(inputs, card.input_params)` 再捕获到 `self.captured` 并标 `called`——schema 违规抛 `ValidationError`（不置位），错误 tool_message 同 turn 回流模型修正（`F_100`；`StructuredOutputFinishRail` 只在无异常时 force-finish，失败调用天然不触发）。
...
- `TeamWorkerBackend.run` 读 `submit_tool.captured` → `AgentResult(structured=...)`；若 worker 未调用工具则抛 `BackendError` → 引擎按 `retries` 重试（**校验失败的重试带错误回灌**，`F_100`：`coerce` 的校验错误经 `make_call(feedback)` 拼进下一次 attempt 的 prompt，模型拿到「缺哪个必填属性」的具体信息而非盲重试）→ 耗尽后 `agent()` 返回 `None`（dw 控制流容忍）。例外：账本干涸的失败不重试（见 Token 预算节的"重试短路"），`AGENT_FAILED` 直接带 budget 真因。工具的注册/清理全由 harness 的 ability_manager + `run_once` 的 `teardown_tools` 负责，backend **不手动** `resource_mgr.add/remove`。
