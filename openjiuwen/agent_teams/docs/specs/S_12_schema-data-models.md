...
| 关联 feature | （见 `docs/features/` 中涉及 schema 的条目） |
...
### I-11 PAUSED ≠ SHUTDOWN

`MemberStatus.PAUSED`：teammate 协程已退出但状态保留，可经 `RESTARTING`
重新拉起。`MemberStatus.SHUTDOWN`：永久退出。两者语义在 `MEMBER_TRANSITIONS`
中体现 —— `PAUSED` 可转 `READY` / `RESTARTING` / `SHUTDOWN_REQUESTED` / `SHUTDOWN` /
`ERROR`；`SHUTDOWN` 仅可转 `RESTARTING`。

不要给 `MemberStatus` 加新枚举来"区分细微情形"，新增情形先证明现有 PAUSED /
RESTARTING / ERROR 不能覆盖。
...
READY      -> READY | BUSY | PAUSED | SHUTDOWN_REQUESTED | SHUTDOWN | ERROR
BUSY       -> READY | PAUSED | SHUTDOWN_REQUESTED | ERROR
PAUSED     -> READY | RESTARTING | SHUTDOWN_REQUESTED | SHUTDOWN | ERROR
RESTARTING -> READY | ERROR | SHUTDOWN
SHUTDOWN_REQUESTED -> SHUTDOWN | ERROR
SHUTDOWN   -> RESTARTING
ERROR      -> RESTARTING | READY | SHUTDOWN_REQUESTED | SHUTDOWN
```
...
- **`S_06_runtime-pool-dispatch.md`**：消费 `TeamLifecycle`（spec 字段）与
  ...
  S_06 钉死 9 路 dispatch truth table。
