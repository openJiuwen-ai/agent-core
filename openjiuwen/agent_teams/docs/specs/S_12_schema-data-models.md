...
- **verify 闸**（`IN_REVIEW`）：任务配了 `reviewer` 时，成员"做完"落 `IN_REVIEW` 交验证，
  reviewer 裁决 pass→`COMPLETED` / fail→`IN_PROGRESS`（返工，author 全程持有）。**Phase 1
  仅在枚举与转移表里预留 `IN_REVIEW`；reviewer 列、`verify_task`、框架分发等使其可达的实现
  属 Phase 2**（见 F_59 §reviewer）。
