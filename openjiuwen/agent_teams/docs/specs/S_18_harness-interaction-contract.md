...
TeamHarness/StreamController）的决策见 [[F_28_native-harness-team-adoption]]。
...
`MemberRuntime` 额外声明：`on_state_changed(cb)` / `on_round(cb)`（注册 phase/round
事件回调，kwargs 按回调声明参数 narrow）、`has_pending_interrupt` /
...
StreamController 经 `on_state_changed`/`on_round` 订阅并映射到 MemberStatus /
ExecutionStatus：
...
- **NativeHarness**：`class NativeHarness(DeepAgent)`，继承复用 DeepAgent 的
  ...
  DeepAgent 完全一致。`configure/run` 分离：`prepare_config()`（sync）+
  `prepare()`（async + ensure_initialized）+ `start(session)`。
- **TeamHarness**：组合单个 `NativeHarness`（per-cycle）。build 时构造 native +
  `prepare_config()`（供 configurator 在 sync 上下文跑 customizer / 读 workspace）；
...
- **StreamController**：瘦身为输出转发 + 状态映射层。`start()` 挂 `_forward_outputs`
  （消费 `runtime.outputs()`→`_tag_chunk`→stream_queue+observers）+ 注册
  `on_state_changed`/`on_round`；不再驱动 round / 排 pending / 自重启。transient
