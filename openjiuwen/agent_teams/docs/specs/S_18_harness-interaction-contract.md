...
[[F_61_cold-resume-across-stop-start]]；Codex SDK 成员的 thread id checkpoint 与冷续接见
[[F_67_codex-external-session-checkpoint]]；其成员级归属修订见
[[F_68_codex-member-session-checkpoint]]。
...
- **CLI runtime**：abort 信号单轮 `_drive` 自然返回并 fire `aborted` round 事件；
  ...
  `AsyncTurnHandle.steer()`；若 app-server 已完成 turn 而本地尚未消费终态通知，
  `no active turn to steer` 必须降级为同 thread 的 pending 后续 turn，其它 SDK 错误上抛；
  shutdown 走 `AsyncTurnHandle.interrupt()` 并关闭 SDK client。
...
首次 `thread_start()` 后，runtime 将 `thread.id` 写入由当前 Jiuwen team session
派生的成员 `AgentSession` checkpoint：
`state["external_runtime"] = {"backend": "codex", "external_session_id": ...}`。
成员 session 复用 team `session_id`，以稳定 `member_agent_id`（当前为
`f"{team_name}_{member_name}"`）作为 checkpointer 的 agent identity，因此不同成员的
resume id 天然隔离。冷重建仅在 `resume_external_backend=True` 时恢复该成员 checkpoint，
命中后调用 `thread_resume(thread_id)`。严格恢复不允许退化：checkpoint 缺少 id、backend
不匹配、resume RPC 失败或 SDK 返回不同 thread id 时直接报错，禁止静默创建 replacement thread。
