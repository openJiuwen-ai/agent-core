...
11. **run-bound interact 必经 gate**：普通消息 payload 与 `InteractiveInput` 都先 `admit` 拿 ticket，并在实际 dispatch / resume 完成后 `consume_done`；ticket 与 gate 的引用绑定，跨 gate 的 ticket 静默忽略。`ExternalTeamEvent` 与 swarmflow human reply 保持既有直达路由，不占 gate ticket。
...
- `interact` 的普通消息 payload 与 `InteractiveInput` 在 gate closed 时返回 `DeliverResult.failure("gate_closed")`，且不进入 dispatch / resume；外部 team event 与 swarmflow human reply 保持既有 bypass 语义。
...
- 普通消息 payload 持票到 `dispatch_payloads` 返回；`InteractiveInput` 持票到 `resume_interrupt` 返回。`close_and_drain()` 因此不会越过仍在执行的 resume。
