...
- 该目录下的产物（文档、代码、报告、轨迹）被**所有成员通过扁平挂载点 `.team/`**读写（junction/symlink → `team-workspace/`）。
...
- **不管成员私有 workspace**。成员私有目录是 `team_home(team_name)/workspaces/{member}/` 或 `independent_member_workspace(member)`，由 `agent_configurator.setup_agent` 处理；workspace manager 只把团队共享区**挂载**进成员 workspace。
...
3. **挂载点统一为扁平 `.team/`**。成员 workspace 通过 `mount_into_workspace(workspace_root)` 在 `workspace_root/.team` 上建符号链接/junction，指向共享 `team-workspace`。Agent 读写 `.team/artifacts/...`；`TeamWorkspaceRail` 与 prompts 中宣告的挂载路径必须一致。
...
- `mount_into_workspace`：在成员 workspace 下建扁平 `.team` 符号链接/junction → `team-workspace`。
...
- 路径规范化：`_resolve_workspace_relative` 剥掉 `.team/` 前缀得到 workspace 相对路径（扁平挂载，无嵌套 `team_name` 段）。
...
| `.team` 符号链接/junction | 成员 workspace | `setup_agent` 调 `mount_into_workspace` | 成员 workspace 清理时随之删除 |
...
└── workspaces/                           # 成员 workspace 容器（不归本子系统）
    └── {member}/
        └── .team → ../../team-workspace  # flat mount
```

成员若在独立 workspace 模式（`stable_base=True`），`independent_member_workspace(member)` 可通过符号链接纳入 `team_home(team_name)/workspaces/{member}`；`.team` 挂载点不变。
