...
- 该目录下的产物（文档、代码、报告、轨迹）被**所有成员通过扁平挂载点 `.team/`**读写。
...
3. **挂载点统一为扁平 `.team/`**。成员 workspace 通过 `mount_into_workspace(workspace_root)` 在 `workspace_root/.team` 上建符号链接或 junction，指向共享 `team-workspace`。`TeamWorkspaceRail` 与 prompts 中宣告的挂载路径必须严格一致；rail 仍能解析旧 hub 路径（`.team/{team_name}/...`）以兼容已有数据，但新代码只生成扁平路径。
...
- `mount_into_workspace`：在成员 workspace 下建扁平 `.team` 符号链接或 junction，指向共享 `team-workspace`。
...
- 路径规范化：`_resolve_workspace_relative` 同时支持旧 hub 路径（`.team/{team_name}/...`）与扁平路径（`.team/...`），新代码使用扁平路径。
...
| `.team` 符号链接或 junction | 成员 workspace | `setup_agent` 调 `mount_into_workspace` | 成员 workspace 清理时随之删除 |
