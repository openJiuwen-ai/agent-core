...
1. **每次特性更新必须归档 feature 文档，且代码与文档拆成两个连续提交**：在 `docs/features/`
   下新增一份 `F_NN_<slug>.md`，记录决策、拒绝的方案、验证基线、已知遗留。commit message 只写
   what，feature 文档负责写 why / why-not。落地顺序**固定为两个紧邻的提交**——提交 1 落代码 +
   单测（`feat(swarm): ...` 或对应 type），提交 2 落本次涉及的全部文档：`docs/features/F_NN_*.md`
   新增 + 下面 #2 要求的 `docs/specs/S_NN_*.md` 修订（`docs(swarm): ...`）。代码与文档不再混进
   同一次 commit，既不让大段文档 diff 淹没代码评审，也避免文档归档拖延导致设计上下文随时间漂移。
...
涉及 `docs/features/F_*` / `docs/specs/S_*` 文档更新的特性改动，**代码与文档拆成两个连续提交**（提交 1 代码 + 单测，提交 2 文档），细则见上文「设计文档归档与双向同步」约束 #1。
