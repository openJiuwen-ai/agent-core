...
- **初始消息只首启注入，空则不 send**（见 [`docs/features/F_33`](../docs/features/F_33_spawn-first-start-message-injection.md)）：spawn 不再 fabricate `"Join the team and wait..."` 占位符；`TeamAgent.invoke` / `stream` 仅在 `raw_query` 非空时 `enqueue_user_input`（首轮 send），空 query（首启无 prompt / restart / recover / resume）只起来订阅、靠 `enqueue_mailbox_after_first_iteration` 的 `POLL_MAILBOX` 投递真实消息——`MessageHandler` 收件箱为空即不 send。`restart_teammate` 因此恒传 `initial_message=None`，不 replay DB 里的 `teammate.prompt`。例外：external CLI 成员（`external_cli_spawn`）仍自带 join prompt，未纳入此模型。
