...
class StreamController:
    ...
    def _tag_chunk(self, chunk: Any) -> Any:
        ...
        returned untouched. Non-OutputSchema chunks (e.g. ``AgentResponseChunk``
        forwarded by web adapters) get ``role``/``source_member`` stamped as
        instance attributes so downstream team consumers (which key off
        ``chunk.role``) can attribute and enrich them instead of skipping.
        """
        bp = self._get_blueprint()
        ...
        # Non-OutputSchema chunk (AgentResponseChunk / dict-like): stamp
        # role + source_member as attributes when settable and not already
        # present, so the team consumer's role-based filter & enrich apply.
        try:
