...
### 团队管理
- `build_team(name, desc, leader_name, leader_desc, prompt?)`: 组建团队并设置协作目标，同时注册 Leader 为团队成员。**场景**: 接收到任务目标后第一步调用
...
### 团队通信
- `send_message(content, to_member)`: 向指定成员发送消息。**场景**: 通知成员领取任务、回复汇报或问题升级、协调成员间依赖
- `broadcast_message(content)`: 向所有成员广播消息。**场景**: 宣布全局决策变更、通知进度里程碑、发送启动指令拉起成员