...
| `recovery_manager.py` | `RecoveryManager` | 团队级容错：成员崩溃恢复、状态对齐；冷恢复跳过 `SHUTDOWN_REQUESTED` / `SHUTDOWN`，关闭收敛仍由成员 finalizer 负责 |
| `session_manager.py` | `SessionManager` | session checkpoint 读写、生命周期 |
| `stream_controller.py` | `StreamController` | runtime（NativeHarness/CLI runtime）输出转发 + 状态映射层。`start()` 挂 `_forward_outputs`（消费 `runtime.outputs()`→`_tag_chunk`→stream_queue+observers）+ 经 `subscribe(on_state=, on_round=)` 注册 phase/round 回调；不再驱动 round / 排 pending / 自重启（单 supervisor 模型接管）。`_map_state` RUNNING→BUSY、IDLE→READY+`_on_idle_settled`，并在这条边上维护 `state.idle_since`（IDLE 打戳 / RUNNING 清空）——自主停滞检测的计时源，见 [[F_65_runtime-idle-clock-stall-nudge]]；`_map_round` 走 EXECUTION_TRANSITIONS；transient retry（181001）在 `_handle_retry` swallow+重驱，exhausted/non-retryable 转发不再 raise；自动升级 chunk 为 `TeamOutputSchema` 并 `add_chunk_observer` fan-out；`emit_completion_and_close` 发完成标记 + 关流；`schedule_team_idle` / `cancel_team_idle` 管一个去抖 2s 的 `team.idle` 定时 task（至多一个 pending，先清引用再 cancel，`stop()` 与 `close_stream()` 两条 teardown 都取消，`CancelledError` 不 catch、done callback 取 exception——四条一起保证取消不泄露），到点先复查 `member_registry.is_idle()`、**再**经注入的 `task_board_probe`（leader 接 `TeamAgent.is_task_board_settled`，一次聚合 COUNT；非 leader 传 None 即跳过）确认任务板无非终态任务，两条都过才由 `emit_team_idle` 发标记**不关流**（团队只是安静了，随时可能被叫醒，见 [[F_74_leader-member-activity-and-team-idle]] / [[F_77_team-idle-requires-a-settled-task-board]]）；`_on_idle_settled` 依次经两个注入回调收尾——先 `member_startup_reconciler`（leader 接 `TeamAgent._reconcile_member_startup`，看板有活就补拉 UNSTARTED 成员，F_84），后 `request_completion_poll_callback`（触发 leader 完成评估）。**顺序是正确性必需**：拉起会让团队重新动起来，先判完成会把"成员正在起来"的团队判成已完成 |
