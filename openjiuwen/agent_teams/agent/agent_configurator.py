...
def _sys_operation_provider_registered() -> bool:
    """Return True when ``core.sys_operation`` has a rail provider.

    Builtin RailSpec injection is skipped when no provider is registered,
    because build would fail without one.
    """
    from openjiuwen.harness.schema.deep_agent_spec import _RAIL_PROVIDER_REGISTRY

    from openjiuwen.agent_teams.rails.builtin_elements import SYS_OPERATION

    return SYS_OPERATION in _RAIL_PROVIDER_REGISTRY
...
def _apply_team_worktree_shell_guard(rails: list[Any], *, enabled: bool) -> list[Any]:
    ...
    # Skip when the sys_operation provider is absent.
    if not _sys_operation_provider_registered():
        return rails
    ...
    for rail in rails:
        ...
        params.setdefault("enable_read_image_multimodal", False)
    ...
    if not found_sys_operation:
        guarded.append(
            RailSpec(
                type=SYS_OPERATION,
                params={
                    "bash_deny_patterns": list(_TEAM_WORKTREE_BASH_DENY_PATTERNS),
...
class AgentConfigurator:
    ...
    def setup_agent(
        ...
        *,
        ...
        # Platform member rails mount via swarm enrich providers on
        # ``spec.agents[*].rails`` (``swarm.platform_member_rails``).
