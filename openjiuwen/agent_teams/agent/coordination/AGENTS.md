...
| `MemberHandler` | 6 个 `MEMBER_*` | 无 | `on_member_event` 分流到 `_handle_leader_member_event` / `_handle_teammate_member_event`；leader 只观测/记录成员生命周期（**不再** stale-claim 跨进程 nudge，见 F_53）；自身 `MEMBER_SHUTDOWN` **只处理 `force`**（`_lifecycle.shutdown_self()`，角色无关，不给卡死成员收尾的 round）。graceful 退场不在此 teardown，走 `MessageHandler` 的 drain + harness-input 闸门（`S_03` 机制 18）——**`_shutdown_human_agent` 这个 human-agent 专属分支已删除** |
| `MessageHandler` | `MESSAGE` / `BROADCAST` / `POLL_MAILBOX` + `MEMBER_SHUTDOWN`（fan-out） | 无 | `on_message_or_broadcast`（leader 额外 ack user-bound + 通知 human-agent inbound，收件人按**可达**筛选 = status ∉ {`SHUTDOWN`}；`SHUTDOWN_REQUESTED` 必须留，否则"你已被移出团队"这条通知恰好送不到）/ `on_poll_mailbox` / `on_member_shutdown_drain`（非 leader 给自己 drain，不再限 teammate）。**三条路统一先过 `_harness_input_blocked` 闸门**（`S_03` 机制 18，状态驱动不是事件驱动）：已 `SHUTDOWN` → 丢弃投递；`SHUTDOWN_REQUESTED` 且无 in-flight round → 不唤醒 harness，直接 `shutdown_self()`（省掉一整轮"烧 token 说告别"）；`SHUTDOWN_REQUESTED` 且 round 在飞 → 放行 steer，靠 round-end close。投递前先 `_expand`（`message_template.expand_message`）：普通消息原样过，框架模板消息（`content` 空 + `meta`）在此刻按当前任务行渲染出正文（F_63）——**四个消费点都要展开**：`_format_message`、`_notify_human_agent_inbound`（HITT 人类成员可能是 assignee/reviewer）、`_bridge_deliverable_for`（relay 给无 DB 的远程执行者的必须是展开后文本）、以及 external 侧的 `read_inbox`。`_format_message` 经 `inbound_render.render_inbound` 渲染成 `<team-inbound>`（+ `<team-note>`）：human-agent 加 `for="controller"` + `hitt-silence` note；普通消息带 `reply-hint` note，**note 正文经 `i18n.reply_hint_for(sender)` 按发件人分化**——`from == "user"` 取无条件的 `dispatcher.reply_hint_user`（点明 user 是团队外部真人 + 不调工具即等于没回复），其余发件人取通用的条件式 `dispatcher.reply_hint`；选择逻辑收在 i18n 一处，`external/format.py` 的 `read_inbox` 复用同一函数；**模板消息不带 reply-hint**（框架指令的响应是调工具，不是回信）（见 F_46 / F_63） |
| `TaskBoardHandler` | `TASK_CLAIMED` / `TASK_REVOKED` / 7 个 board `TASK_*`（`CREATED` / `UPDATED` / `STARTED` / `COMPLETED` / `CANCELLED` / `UNBLOCKED` / `RELEASED`） + 3 个 verify 闸 `TASK_*`（`SUBMITTED_FOR_REVIEW` / `VERIFIED` / `REVISION_REQUESTED`）；调度模式子类 `ScheduledTaskBoardHandler` 额外监听 `TASK_REVIEW_VOTE`（纯 board 观测，F_62，见下文「调度模式 handler 变体」） | 无 | `on_task_claimed`（targeted assignment）/ `on_task_revoked` / `on_task_cancelled` / `on_task_updated`（任务被改派 / 取消 / 改内容时，各发带 `member_name` 的 `TASK_REVOKED` / `TASK_CANCELLED` / `TASK_UPDATED` 精准投给受影响成员 → `deliver_input(use_steer=True)`：改派·取消=停手，编辑=重看后继续且任务仍归你。改派经 DAO 原子 CAS 交换 assignee，不发 `TASK_RELEASED`；revoked 非自身忽略，cancelled·updated 非自身 fall-through，见 F_54 / F_56）/ `on_task_board_event` → `_nudge_idle_agent`。**verify 闸（F_59）**：`on_task_submitted_for_review`（payload `reviewer` 列含自己 → steer 去 `verify_task`，否则 board 兜底）/ `on_task_revision_requested`（`member_name`=author，自身 → steer 带 feedback 返工）/ `on_task_verified`（`member_name`=author，自身 → steer 释放去找新活；author 验证期被一活跃卡住）。**leader 收每个 board 事件都巡视全部未完成任务；teammate 只被 `_TEAMMATE_NUDGE_EVENTS`（`CREATED` / `UNBLOCKED` / `RELEASED`——会扩大可处理池的三类）唤醒，且 `_nudge_idle_agent` 只渲染 claimable（pending+未指派）任务和明确 assignee=自己的未完成任务，别人 in-flight 的任务不喂给 teammate**。`resume_polls` 对每个事件都触发（过滤只砍 nudge、不停 poll）。self 分支经 `inbound_render.render_event(kind="task-assigned")`：human-agent 加 `for="controller"` + `hitt-silence` note（best-effort 查 task title 内联），否则普通 event（见 F_46） |
| `StaleTaskHandler` | `POLL_TASK` | 独占 `_last_stale_nudge` / `_last_pending_nudge` / `_stale_claim_streak` / `_escalated_claims` | `on_poll_task` → `_check_stale_claimed_tasks`（**self-only**：本成员 idle 超 `stale_claim_idle_timeout` 且仍持 `{PLANNING, IN_PROGRESS}` 任务 → `_self_nudge_idle_claim`，`deliver_input(use_steer=False)`；连续 3 窗口无效则 `_escalate_stale_claim` 自报 leader，见 F_53 / F_65） + `_check_stale_pending_tasks`（leader idle 超 `stale_pending_idle_timeout` + 有无主 PENDING + roster 有 READY 成员，才自催，同样 minimal + 非 steer） |
...
stale-claim nudge 现在是 **self-only**（每个成员 poll 自己的活跃任务并喂自己的 loop），不再有 leader 跨进程催别人的路径，所以也不再需要跨 handler 共享 throttle。`_last_stale_nudge`（活跃任务）与 `_last_pending_nudge`（leader 自催 pending）都是 `StaleTaskHandler` 私有字段，各自节流本进程内的重复 nudge。见 [[F_53_self-only-stale-nudge-minimal-and-append]]。

**自主模式的停滞时长来自运行时 idle 时钟，不是 DB `updated_at`**（[[F_65_runtime-idle-clock-stall-nudge]]）：`TeamAgentState.idle_since`（`time.monotonic()`，进程本地、不持久化）由 `StreamController._map_state` 在 READY/BUSY 边写入，经 `AgentRoundController.idle_seconds()` 读取（忙时为 `None`，故正在干活的成员绝不会被判停滞），`kernel.start` 调 `TeamAgent.refresh_idle_baseline()` 在 poll 恢复前重置基线，使 pause 窗口不计入。理由：pause 冻结 `updated_at` 而墙钟继续走，长 pause→resume 后 `now - updated_at` 必然报假停滞；成员状态列本就不落时间戳。自催的活跃集收窄为 `{PLANNING, IN_PROGRESS}`——**排除 `IN_REVIEW`**，author 等 reviewer 裁决是正常 idle。阈值经 spec 的 `stale_claim_idle_timeout` / `stale_pending_idle_timeout` 可调（各默认 600s）。**调度模式仍走 `updated_at`**（见下）。

**调度模式 handler 变体（F_62）**：`task_board.py` / `stale_task.py` 各有一对模式类——`TaskBoardHandler` / `StaleTaskHandler` 是自主模式实现（原样，零模式分支），`ScheduledTaskBoardHandler` / `ScheduledStaleTaskHandler` 是调度模式实现。dispatch_mode 是**静态 spec 配置**：`EventDispatcher` 构造期按 `blueprint.spec.dispatch_mode` 查 `_TASK_BOARD_CLASS` / `_STALE_TASK_CLASS` 字面量表装配（未知值 KeyError），每个角色相同、运行期不变，**绝不在 handler 方法内判模式**。调度类重写的是组合点：`ScheduledTaskBoardHandler` 的 `EVENT_METHOD_MAP` 把 verify 闸三事件与 `TASK_REVIEW_VOTE` 全部路由到 `on_task_board_event`（重写为 resume_polls-only——交接是调度器的 leader 身份邮箱消息，自反应会双投递；leader 感知走调度器摘要/升级），继承 TASK_CLAIMED/REVOKED/CANCELLED/UPDATED 定向 steer（update_task 两模式同义）；`ScheduledStaleTaskHandler.on_poll_task` 只扫自己的 stale 活跃任务（漏投递兜底），不做 leader stale-pending 自催（排队中的 `PENDING(assignee)` 是常态）；它还**覆写 `_check_stale_claimed_tasks` / `_self_nudge_stale_claim` 钉住 pre-F_65 的 `task.updated_at` 计时**（活跃集含 `IN_REVIEW`），使自主基类切到 idle 时钟后调度模式行为逐字不变——同款 pause 缺陷在调度模式下仍在，记为 [[F_65_runtime-idle-clock-stall-nudge]] 的已知遗留。见 `S_22_scheduling-runtime`。
...
- **`kernel.pause` 中止 LLM 流、杀成员 handle、停 harness**：`mark_team_pausing` → `_abort_host_llm_stream` → `shutdown_all_handles` → `pause_agent_round` → `_await_harness_paused` → `harness.stop()`。Task board 行保留在 SQLite；`resume_paused_round()` 从 checkpoint cold resume（空 query 跳过），`start()` 重建 native。`contextlib.suppress(asyncio.CancelledError, Exception)` 在 `stream_controller.stop()`（await 已 cancel 的 forwarder task）。见 [[F_60_native-harness-pause-abort-resume]]。
