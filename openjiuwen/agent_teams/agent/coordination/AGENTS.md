...
| `MemberHandler` | `REFRESH_TEAM_CONTEXT`（inner）+ 6 个 `MEMBER_*` | `_team_clean_requested` | leader 侧纯观测成员生命周期（**不再** stale-claim 跨进程 nudge，见 F_53）+ 全员 SHUTDOWN 后自动 `clean_team`；非 leader 只反应自身事件（`MEMBER_CANCELED` 取消 round，`MEMBER_SHUTDOWN` **只处理 `force`**）；名册变动时给外部 CLI 成员 steer 团队上下文快照 |
