...
| `TaskBoardHandler` | `TASK_CLAIMED` / 6 个 `TASK_*`（`CREATED` / `UPDATED` / `COMPLETED` / `CANCELLED` / `UNBLOCKED` / `RELEASED`） | 无 | `on_task_claimed`（targeted assignment）/ `on_task_board_event` → `_nudge_idle_agent`。**leader 收每个 board 事件都巡视全部未完成任务；teammate 只被 `_TEAMMATE_NUDGE_EVENTS`（`CREATED` / `UNBLOCKED` / `RELEASED`——会扩大 claimable 池的三类）唤醒，且 `_nudge_idle_agent` 只渲染 claimable（pending+未指派）任务，别人 in-flight 的 claimed 任务不喂给 teammate**。`resume_polls` 对每个事件都触发（过滤只砍 nudge、不停 poll）。self 分支经 `inbound_render.render_event(kind="task-assigned")`：human-agent 加 `for="controller"` + `hitt-silence` note（best-effort 查 task title 内联），否则普通 event（见 F_46） |
