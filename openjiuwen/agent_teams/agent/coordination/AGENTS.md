...
| `TaskBoardHandler` | `TASK_CLAIMED` / `TASK_REVOKED` / 6 个 `TASK_*`（`CREATED` / `UPDATED` / `COMPLETED` / `CANCELLED` / `UNBLOCKED` / `RELEASED`） | 无 | `on_task_claimed`（targeted assignment）/ `on_task_revoked`（leader 改派时 `TASK_REVOKED` 投给失去任务的原 owner，`deliver_input(use_steer=True)` 令其停手，非自身忽略，见 F_54）/ `on_task_board_event` → `_nudge_idle_agent`。**leader 收每个 board 事件都巡视全部未完成任务；teammate 只被 `_TEAMMATE_NUDGE_EVENTS`（`CREATED` / `UNBLOCKED` / `RELEASED`——会扩大 claimable 池的三类）唤醒，且 `_nudge_idle_agent` 只渲染 claimable（pending+未指派）任务，别人 in-flight 的 claimed 任务不喂给 teammate**。`resume_polls` 对每个事件都触发（过滤只砍 nudge、不停 poll）。self 分支经 `inbound_render.render_event(kind="task-assigned")`：human-agent 加 `for="controller"` + `hitt-silence` note（best-effort 查 task title 内联），否则普通 event（见 F_46） |
...
stale-claim nudge 现在是 **self-only**（每个成员 poll 自己的 claimed 任务并喂自己的 loop），不再有 leader 跨进程催别人的路径，所以也不再需要跨 handler 共享 throttle。`_last_stale_nudge`（claimed）与 `_last_pending_nudge`（leader 自催 pending）都是 `StaleTaskHandler` 私有字段，各自节流本进程内的重复 nudge。见 [[F_53_self-only-stale-nudge-minimal-and-append]]。
