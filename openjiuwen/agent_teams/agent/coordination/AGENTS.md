...
| `MemberHandler` | `REFRESH_TEAM_CONTEXT`（inner）+ 6 个 `MEMBER_*` | `_team_clean_requested` | leader 侧纯观测成员生命周期（**不再** stale-claim 跨进程 nudge，见 F_53）+ 把观测到的状态喂给 leader 的活动登记（F_74）+ 全员 SHUTDOWN 后自动 `clean_team`；非 leader 只反应自身事件（`MEMBER_CANCELED` 取消 round，`MEMBER_SHUTDOWN` **只处理 `force`**）；名册变动时让外部 CLI 成员的 runtime 公告待发的团队状态（`announce_team_context()`，见 F_70）|
