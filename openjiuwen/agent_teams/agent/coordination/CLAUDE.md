...
| `WorkflowHandler` | `WORKFLOW_PROGRESS` | 无 | `on_workflow_progress`（leader-only）：渲染 swarmflow 里程碑（`workflow_started` / `phase` / `workflow_completed`）→ `deliver_input(use_steer=True)`；per-agent 事件不播报（归 4 层 `WorkflowRun`）。监听独立 event_key，无 fan-out 重叠 |
