...
| `render.py` | 交接消息渲染（开工/送审/催办/返工/汇报/摘要/升级），文案全部走 `i18n.py` 的 `scheduler.*` |
...
3. **`SchedulerHost.deliver_input` 只准注入 leader 自身**（终态摘要 / 升级 / 收尾），对成员永不直投。
4. **判定在本包，票据在 DB。** `verify_task`（scheduled 团队）只追加票行；`verdict.judge` 三值判定；settle 经 `task_manager.settle_review` 的 `IN_REVIEW` 源态 CAS——单判定者 + CAS 保证不双结算。
5. **轮数升级与停摆升级共用一条注入路径**，按 `(task_id, review_round)` 去重；升级后任务留 `IN_REVIEW`，决定性迟票仍可正常 settle（不再重复升级）。
6. **leader 自发事件的可见性靠 `SCHEDULER_SCAN` 回声**：kernel 的 `_filter_self` 丢弃 self 事件时，若调度器激活且是 `task_*` 事件，改投一个 `SCHEDULER_SCAN` inner 事件——coordination 无 handler 监听它，调度器把它当纯扫描提示。
7. **异常语义与 coordination 对齐**：`on_event` 吞普通异常（log + 下次触发重试），绝不让 bus loop 挂掉。
...
设计上下文见 `docs/features/F_62_scheduled-dispatch-runtime-and-review-voting.md` 与 `docs/specs/S_22_scheduling-runtime.md`。
