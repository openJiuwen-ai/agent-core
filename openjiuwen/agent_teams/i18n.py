...
STRINGS: dict[str, dict[str, str]] = {
    "cn": {
        ...
        "swarmflow.budget_exhausted.workflow_guidance": (
            ...
            "请重新设计工作流后以相同 META name 重试 swarmflow。"
        ),
    ...
    "en": {
        ...
        "swarmflow.budget_exhausted.workflow_guidance": (
            ...
            "model) and relaunch. Relaunch budget rules: relaunching under the SAME "
            "META name resets the ceiling (a terminal prior run carries no spent "
            "forward) and unchanged agent calls replay from the prior run's result "
            "cache (no model call, zero tokens) — only the changed parts are billed "
            "again, so edit the script in place, keep the META name, and fit the "
            "CHANGED parts' expected spend under the ceiling; only an interrupted "
            "(non-terminal) run continues its ledger on relaunch "
            "(remaining = ceiling − prior spent). Redesign the workflow and retry "
            "swarmflow under the same META name."
        ),
        "swarmflow.budget_exhausted.session_guidance": (
