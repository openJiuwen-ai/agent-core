...
STRINGS: dict[str, dict[str, str]] = {
    "cn": {
        ...
        "dispatcher.stale_claim_self": (
            "[催促] 你已认领的任务 [{task_id}] {title} "
            "已超过 10 mins 仍未完成，请继续推进：{content}"
        ),
    ...
    "en": {
        ...
        "dispatcher.stale_claim_self": (
            "[Nudge] Your claimed task [{task_id}] {title} "
            "has been open for over 10 mins. Please continue: {content}"
        ),
...
def t(key: str, **kwargs: object) -> str:
    ...
    if key not in table:
        raise KeyError(
            f"Missing i18n key '{key}' for language '{_current_language}'"
        )
    raw = table[key]
