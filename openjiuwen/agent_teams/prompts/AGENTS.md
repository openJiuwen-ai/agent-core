...
| 模板文件 | 触发条件 | 装配位置 | 主要内容 |
|---|---|---|---|
| （无模板，Python 装配） | `member_name` 或 `member_prompt` 非空 | `build_team_identity_section` | **唯一的 per-member section**（P:10）：自身 `member_name` 一行 + `## 私有工作约定` 子节（`member_prompt` 为空则只出名字）。两者生命周期与投递去向一致，故合成一个 attachment type。进程内成员由 `TeamPolicyRail` 挂 attachment，外部 CLI 由 `build_team_static_sections(include_member_specific=True)` 内联进静态 prompt——留在系统提示词会让每个成员各占一份前缀 KV cache。见 [[F_68]] |
...
| `attachment_notice.md` | 进程内成员（有 attachment 通道） | `build_team_attachment_notice_section` | 团队 attachment 说明：自身身份（含私有工作约定）/ 成员名册 / 团队信息以 `<prompt-attachment>`（type=`team_identity`/`team_members`/`team_info`）挂在消息尾部，`team_members`・`team_info` 逐轮刷新（`team_members` 里人类成员标 `[human]`）、`team_identity` 恒定；HITT 协作规则在系统提示词里、稳定不变 |
| `inbound_tags.md` | 常驻（每个成员，含外部 CLI） | `build_team_inbound_tags_section` | 入站消息 XML 标签体系（`<team-inbound>` / `<team-note>` / `<team-event>`、`for="controller"`）+ **`from="user"` 的角色定义**（团队外部真人 / 不在名册 / 无 agent 进程 / 不可派任务）。user 的身份是角色中立事实故放这里一处覆盖全角色；**怎么答复 user 由各自角色契约管**（teammate 必须 `send_message(to="user")`，leader 纯文本直达），此处不写。进程内成员与外部 CLI（`read_inbox`）都渲染这套 XML |
...
2. **动态值走 `{{name}}` 占位符** — 只有两个模板含占位符：`bridge_agent` 与 `hitt_human_agent`，都只用 `{{self_line}}`（"你的 member_name 是 X"这一自身名字行）。占位符用 `PromptTemplate` 默认的 `{{ }}` 定界符，由 builder 调 `load_template(...).format({"self_line": ...})` 渲染；`self_line` 由 `_self_member_line` 生成，builder **仅在 HUMAN_AGENT / BRIDGE_AGENT 时才算**（这两个角色单例，不构成前缀 cache 放大）。其余模板（policy / workflow / lifecycle / attachment_notice / inbound_tags / `hitt_leader`・`hitt_teammate`・`hitt_teammate_anonymous`）纯静态，`load_template` 原样返回。**不要往这些静态模板里加 per-member 变量**——普通 teammate 的自身名字走 `team_identity` attachment，见 S_09 不变量 6a。
