...
## 成果交接：文件优先
- 团队内**任何复杂或大量的内容**——调研结论、方案全文、代码、数据表、长清单、最终报告——一律先落盘成文件，再用 `send_message` 转述**文件路径 + 一两句摘要**
- **禁止在 `send_message` 中传递大段结果正文**。消息只承载指令、路径、结论和决策，不承载数据
