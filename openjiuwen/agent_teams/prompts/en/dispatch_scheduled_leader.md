...
The `reviewer` field of `create_task` accepts a list of structured objects. Whether the reviewer system is active depends on the `enable_task_verification` parameter of `build_team` (see the build_team tool description for details). Each entry has two fields — `type` and `description`: `reviewer_id` is auto-generated by the system (verifier_1, inspector_1, etc.) — do not provide it:

| Field | Required | Description |
|-------|:---:|------|
| `type` | ✅ | `"verifier"` / `"inspector"` / `"challenger"` |
| `description` | as needed | **verifier**: how to verify (which tests, what to focus on), never repeat numbers from criteria. **inspector**: scoring rubric table (see below), leave empty for default 6-dim rubric. **challenger**: not needed |

### Three Reviewer Types

- **verifier**: Checks the deliverable point-by-point against acceptance criteria; may run tests. pass if all criteria are met, fail otherwise. Any verifier fail → rework. Use `description` to specify *how* to verify
- **inspector**: Extracts evaluation dimensions from acceptance criteria, scores each 0–1, outputs a weighted overall score. All inspector scores averaged ≥ 0.85 to pass. `description` field is a scoring rubric table (writing guide below); leave empty for the default 6-dim generic rubric.
...
create_task(tasks=[{
  ...
  "assignee": "algo-dev",
  "reviewer": [
    {"type": "verifier", "description": "Run unit tests; focus on edge cases (empty, single, duplicates) and consistency with sorted()"},
    {"type": "verifier", "description": "Run perf tests, compare timing ratio vs sorted(), and verify true in-place behavior"},
    {"type": "inspector", "description": "| Dimension | Weight | Description |\n| --- | --- | --- |\n| Correctness | 0.3 | ..."}
  ]
}])
```
