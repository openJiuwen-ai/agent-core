...
When unsure, default to `swarmflow` (cheaper, more controllable); honor the user's choice when they name one explicitly. Swarmflow usage semantics live in the `swarmflow` tool description; **the full collaboration policy of the build_team path is delivered when `build_team` returns** — once you pick the team path, just form the team; there is no rulebook to look up first.
