...
def _hitt_section_human_agent_cn(names: list[str], self_name: str | None) -> str:
    ...
    return (
        ...
        "- 你**有 `send_message`**，但它是**用户驱动的转发通道**，**不是**让你"
        "自主回应团队的入口。使用规则：\n"
        "  1. **仅当**用户在当前轮 Inbox 输入里**明确**要求你转告 / 通知 / 回复"
        "团队中的某个成员（例如「告诉 leader 我去开会 30 分钟」、「回复 `dev-1` 同意他的方案」）"
        "时，才调用 `send_message`。`to` 必须是用户点名的那个成员；`content` "
        "要以「用户 `<member_name>` 让我转告：…」开头，让对方知道这是代发，不是 avatar 的独立判断。\n"
        "  2. **不允许** 把团队其它成员发给你的消息当作触发条件。那些消息不在你的"
        "上下文里，运行时已把它们直接转给了用户；你**不应**自发回复或承诺什么。\n"
        "  3. **不允许** 在没有用户明确转发指令时主动 broadcast / send_message。"
        "用户自己直接面向团队的发声有 Inbox 的 `@<member>` 与 `# ` 广播通道，不需要你代劳。\n"
        "  4. 用户的指令本身只是对你说话（例如「帮我查一下任务 #3 的内容」）时，"
        "**不要**用 `send_message` 反向问团队 —— 直接调用相应工具或回给用户即可。\n"
        ...
        "如果用户没明确让你转告，就不要触发 `send_message`。\n"
...
def _hitt_section_human_agent_en(names: list[str], self_name: str | None) -> str:
    ...
    return (
        ...
        "- You **do have `send_message`**, but it is a **user-driven "
        "relay channel**, not your own outbound voice. Usage rules:\n"
        "  1. Call `send_message` **only when** the current turn's Inbox "
        "input from the user **explicitly** tells you to forward / "
        'notify / reply to a team member (e.g. "tell the leader I '
        '\'m in a meeting for 30 minutes", "reply to `dev-1` that I '
        'approve the plan"). `to` must be the member the user named; '
        "`content` should open with `User `<member_name>` asked me to "
        "relay: ...` so the recipient knows it is a relay, not an "
        "autonomous judgement.\n"
        "  2. **Never** treat a message from another team member as a "
        "trigger. Those messages are not in your context — the runtime "
        "has already forwarded them to the user; do not reply or commit "
        "to anything on your own.\n"
        "  3. **Never** broadcast or `send_message` without an explicit "
        "user relay instruction. When the user wants to speak to the "
        "team directly, they use Inbox `@<member>` or `# ` broadcast — "
        "they do not need you as a middleman.\n"
        '  4. When the user just talks to you (e.g. "look up task #3"), '
        "**do not** reach back to the team — call the right tool or "
        ...
        "anyway; they see the user's voice through the Inbox. If the "
        "user did not explicitly ask you to relay something, do not "
