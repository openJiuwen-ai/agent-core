...
def resolve_embedding_config(config: Optional[TeamMemoryConfig]) -> Optional[EmbeddingConfig]:
    """Resolve embedding config for team memory (in-spec → env → None).

    Lazy-imports ``resolve_embedding_config_from_env`` so importing
    ``TeamMemoryConfig`` does not require team memory / lite memory to be
    enabled. Callers only hit this path when ``spec.memory.enabled`` is true.
    """
