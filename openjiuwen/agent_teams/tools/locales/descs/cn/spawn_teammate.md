...
必须先调用 build_team 组建团队，才能调用 spawn_teammate。spawn_teammate 只创建成员记录（状态为 UNSTARTED）；成员就位后按已经选定的分支继续：思辨分支用 `send_message` 启动成员输出或互论，任务协作分支才用 `create_task` 下发任务。成员必须先存在，消息或任务才能落到具体的人头上。任务协作中的“先建成员”顺序**按批次成立而非全局只走一遍**——可以先建一名调研成员摸清背景，再据此补齐其余成员和任务。成员何时被拉起取决于团队的调度模式（见系统提示词《任务下发与获取》一节）。成员完成后调用 shutdown_member 关闭。若 member_name 已存在，创建会失败，请使用不冲突的名称。
