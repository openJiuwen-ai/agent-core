...
必须先调用 build_team。调用顺序：build_team → create_task → spawn_bridge_agent → send_message。spawn_bridge_agent 只创建成员记录（状态为 UNSTARTED），首次 send_message 时系统自动拉起。`prompt` 是长期角色设定兼远程 briefing，不要绑定到具体任务——任务通过 create_task / send_message 下发。
