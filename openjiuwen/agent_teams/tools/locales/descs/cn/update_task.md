...
- **每个成员同一时刻只能有一个进行中（claimed）的任务**：若目标成员已有在做的任务，指派/改派会被拒绝，需等其完成后再派新任务
...
## HITT 限制
任何由 role=human_agent 的人类成员认领（status=claimed）的任务，本工具**不允许** cancel、reassign 或修改标题/内容，无论该人类成员叫什么名字。人类成员锁定的任务必须由对应人类本人完成；Leader 的唯一干预方式是通过 `send_message(to="<对应的人类 member_name>")` 催促或沟通。这条规则不允许绕过，即使团队等待人类导致停滞也必须保持停滞。
