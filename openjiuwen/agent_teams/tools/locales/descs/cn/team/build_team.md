...
**团队已存在时本工具不会失败，而是接管它**：不重建团队、不重复注册成员，名册与成员配置原样保留，生效的能力开关以团队已有配置为准（本次传入的对应参数不适用）。在新 session 里接手同一个团队时走的就是这条路径——照常调用即可。

## 返回内容
本工具的返回结果**除了建队结果，还包含你的完整协同准则**——核心职责、成果交接方式、决策原则、响应节奏、任务状态流转、任务如何下发到成员、团队生命周期收尾、入站消息标签体系。这些准则按你本次调用选定的参数裁剪，**是你后续全部行为的依据，读完照做**，不要当成一次性提示忽略掉。
...
- 成员自主规划：成员承接任务后自行制定计划，Leader 通过 approve_plan 审批（仅当成员执行模式为 plan_mode 时生效）
