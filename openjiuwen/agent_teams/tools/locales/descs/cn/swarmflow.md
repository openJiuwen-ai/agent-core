...
## 何时调用 / 何时不用
- **按协同性质选机制**：结构能预先想清、可写成确定性控制流（拓扑已知，扇出 / 流水线 / 验证 / 综合可编码）的多 agent 任务，**默认用本工具编排**，不需要用户显式点名。反之，协同是**涌现式**的——成员需自主相互通信 / 协商、没有固定信息流拓扑、任务 DAG 不明确、动态场景多、或需持久 / HITT 协作——那是 `build_team` 团队的领域。判断依据是任务的协同性质，不是用户措辞或任务复杂度。
- **用**：复杂、多步、可并行 / 流水线、需对抗验证、大规模排序、根因排查、探索、分诊的任务——这类真正需要多 agent 协同的工作。
- **不用**：简单单 agent 就能完成的任务别强行编排——不是每个任务都需要一群 worker。先问自己"它真的需要多个 agent 协同吗？"；需要就编排，不需要就一次 `agent()` 或直接自己做。默认反转的是"多 agent 协作用哪种机制"（swarmflow 优先于 build_team），**不是**"把简单任务也拆成编排"。
- **混合式最佳**：决定编排之前，常常先**内联侦察**（列出相关文件、找到数据源、圈定范围）得出"工作清单"，再让脚本对其流水线处理。你不必在*任务*之前知道全貌——只需在*编排那一步*之前知道。

> 本工具存在即 `enable_swarmflow` 已开。选 swarmflow 还是 build_team 取决于**任务的协同性质**（结构可确定性编排 → swarmflow；涌现式自主协同、拓扑 / DAG 不预定、动态多、需成员互相通信 → build_team），而非用户是否当轮点名。拿不准默认 swarmflow。
...
> **按复杂度选来源**：编排结构一眼看清的简单任务，**优先内联 `script`** 直接跑，或自己手写一个极简脚本文件走 `script_path`——不必惊动 `swarmskill-creator`。任务复杂（多阶段 / 多角色 / 需重试·降级·预算等可执行约束 / 要沉淀成可复用技能）才用 `swarmskill-creator` skill 走完整开发 + 验证流程产出脚本。
>
