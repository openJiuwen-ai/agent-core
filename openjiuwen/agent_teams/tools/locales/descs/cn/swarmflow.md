...
## 何时调用 / 何时不用
- **明确选择才编排**：工作流会派生数十个 worker、消耗大量 token，**必须由用户主动要这种规模，不能由你推断**。满足任一即可：用户用自己的话要求（"用 workflow"、"并行扇出"、"多 agent 编排"、"彻底审计"）；消息出现 `swarmflow` / `workflow` / `工作流` 关键字；或所在团队已开 `enable_swarmflow` 且任务确需多 agent 协作。
- **用**：复杂、多步、可并行 / 流水线、需对抗验证、大规模排序、根因排查、探索、分诊的任务——这类高价值、算力成本被证明合理的工作。
- **不用**：简单单 agent 就能完成的任务别强行编排——多数普通任务不需要一个 5 人评审团。**即便某任务明显能从并行受益，只要用户没明确要，就不要擅自编排**：改用一次 `agent()`，或在主流程里简述"多 agent 工作流能做什么、大致多少算力开销"并问用户是否运行（可顺带提示：下次直接说"用 workflow"即可跳过这层询问）。先问自己"它真的需要更多算力吗？"
- **混合式最佳**：决定编排之前，常常先**内联侦察**（列出相关文件、找到数据源、圈定范围）得出"工作清单"，再让脚本对其流水线处理。你不必在*任务*之前知道全貌——只需在*编排那一步*之前知道。

> 是否编排由 `enable_swarmflow` 能力位 + 用户当轮请求共同决定，而非一个持续生效的会话级标志。
...
## 脚本来源（四选一）与 args
- `script_path`（**当前可用**）：磁盘上的脚本文件路径。优先级最高。
...
> 当前仅 `script_path` 接通执行；其余三者会以**明确错误**提示拒绝（绝不静默无操作）。**迭代脚本**：编辑磁盘上的脚本文件后，用同一个 `script_path` 重新调用即可，无需重发整段源码。无现成脚本时可用 `swarmskill-creator` skill 生成脚本，拿到路径后再调本工具；该 skill 不可用时如实告知用户、不要硬调或手搓。
