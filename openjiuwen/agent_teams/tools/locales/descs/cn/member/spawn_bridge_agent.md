...
必须先调用 build_team。调用顺序：build_team → spawn_bridge_agent → create_task。成员先于任务存在。spawn_bridge_agent 只创建成员记录（状态为 UNSTARTED），何时被拉起取决于团队的调度模式（见系统提示词《任务下发与获取》一节）。`prompt` 是长期角色设定兼远程 briefing，不要绑定到具体任务——任务通过 create_task / send_message 下发。
