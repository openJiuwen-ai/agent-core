...
1. **Before creating**: when setting `assignee`, the owner must already exist and must not be the leader (`spawn_teammate` / `build_team` first); you MUST call `view_task` to inspect the current task board — prevents duplicates, surfaces missing dependencies, and reveals reusable task IDs
