...
- **Each member can hold only ONE in-progress (claimed) task at a time**: if the target member already has a task in progress, the assign/reassign is refused — wait for it to complete before assigning another
