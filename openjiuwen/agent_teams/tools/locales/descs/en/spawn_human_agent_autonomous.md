...
You must call build_team first. spawn_human_agent only creates the member record (status: UNSTARTED); after the member is in place, follow the branch already selected in the system prompt: use `send_message` to start participation on the **Debate branch**, and use `create_task` only on the **Task-collaboration branch**. Members must exist before messages or tasks can land on them. Startup depends on the team's dispatch mode. `desc` is a long-term role profile, not a request-specific instruction.
