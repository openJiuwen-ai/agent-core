Assemble a team and register yourself as Leader. Call immediately once you have chosen `build_team`.

## Call Order
`build_team` must be the first team tool used in this turn; no other team tool may precede it. After the team is assembled, follow the branch selected from the user's expected final result in the system prompt:

- **Debate branch**: put participants in place for the current team mode -> use `send_message` to start independent output or debate; do not call `view_task` or `create_task`
- **Task-collaboration branch**: put task owners in place -> `view_task` -> `create_task` -> `view_task` again for self-review -> start the members
