...
You must call build_team first. spawn_bridge_agent only creates the member record (status: UNSTARTED); after the member is in place, continue on the branch already selected: use `send_message` to start participation on the debate branch, and use `create_task` only on the task-collaboration branch. Members must exist before messages or tasks can land on named people. When the member gets started depends on the team's dispatch mode (see the "Task Dispatch" section of your system prompt). `prompt` is a long-term role setup and remote briefing — do not bind it to a specific request (request-specific content is delivered via create_task / send_message).
