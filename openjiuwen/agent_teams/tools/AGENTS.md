...
| 小节 | 目的 |
|---|---|
| 调用顺序 | 防止错序（`build_team` 必须最先；成员先就位，任务协作分支才调用 `create_task`） |
