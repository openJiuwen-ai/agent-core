...
| `tool_permissions.py` | 权限集合（`LEADER_*`、`MEMBER_*`、`SHARED_TOOLS`、`HUMAN_AGENT_TOOLS`）、`_MEMBER_NAME_PATTERN` |
...
| `tool_message.py` | `SendMessageTool`（点对点、多播、广播） |
...
- 文件命名：`descs/cn/build_team.md` → 对 lang `"cn"` 解析为 `build_team._desc`。
- 文件经 `PromptTemplate` 加载（与 `agent_teams/prompts/` 相同）并用 `@cache` 缓存。
