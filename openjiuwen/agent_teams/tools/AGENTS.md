...
| 文件 | 负责 |
|---|---|
| `team.py` | `TeamBackend` —— 每个工具都对话的后端对象（spawn/shutdown/clean/approve、名册查询、cleanup-path registry）。`startup_member`（单次 UNSTARTED→STARTING CAS + spawn）、`recover_member`（单次 ERROR→RESTARTING CAS + runtime replacement）、`startup`（经 `startup_member` 批量）、`_spawn_and_publish`（共享 helper）。ERROR 成员的 shutdown 直接收敛到 SHUTDOWN 并清 stale handle，不等待已退出的 runtime。`approve_tool` 写入 `protocol="json"` 的 DB 消息，作为解除中断的兜底投递 |
...
| `send_message` | ✓ | ✓ | **两个形态**：`SendMessageTool`（leader 全模式 + autonomous 成员）用 `to: string` 做单播 / `"*"` 广播、用 `targets: string[]` 做多播；两个字段必须二选一，schema 无 `anyOf`。leader 调用会经 `TeamBackend.autostart_unstarted()` 自动拉起 UNSTARTED 成员（**写库之前**——成员得先订阅 bus，MessageEvent 才有人收）；单播/多播把目标 ERROR 成员视为显式恢复 nudge，经 `recover_member` CAS 后重建 runtime，广播不盲目复活全部 ERROR。leader `to == "user"` 会被拒绝，直接普通输出即可。`ReportToLeaderTool`（scheduled 成员）只有 `to: enum ["leader", "user"]`（角色词，投递时翻译成真实 leader），无 `targets` / 多播 / 广播。也挂到 `human_agent` 作为一条用户驱动的转发通道 —— HITT prompt section 禁止自主使用；只有用户下达的"tell `<member>` …"指令才可触发。**`content` 硬上限** `MAX_CONTENT_CHARS`（2000 字符）：`invoke` 内、`_dispatch` 之前校验，一处覆盖两形态 × 三路径 + MCP 客户端；超限返回教 LLM 改走文件通道的错误（`write_file` → `.team/` → 重发路径 + 摘要）。**无收件人豁免**——约束的是内容形态，`user` 也一样（用户经自己的助手 agent 读文件）。见 F_64、F_109 与 `S_08` 不变量 22 / 24 |
...
- `send_message` 的两个形态共享 `_SendMessageBase`，因为它们共享的是**真实行为**
  ...
  子类只声明自己的收件参数 schema 与一条直线 `_dispatch`。基类内**零形态分支**。
...
**零机制**。扩展 = 新增 key（如 `create_task.task.assignee`）。语义变了的同名参数用**新的
desc_key 命名空间**（`send_message_scheduled.to`），其余（`content` / `summary`）继续复用
`send_message.*`；完整形态新增 `send_message.targets`，scheduled 形态不暴露该字段。
**绝不让 resolver 学会 variant**。

schema 复用走模块级纯函数，不是复制粘贴：
`_base_task_node_properties(t)` / `_autonomous_task_node_schema(t)` / `_scheduled_task_node_schema(t)`。
