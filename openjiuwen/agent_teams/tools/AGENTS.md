...
| `verify_task` | | ✓（成员，非 leader） | reviewer 对 `IN_REVIEW` 任务裁决：`decision=pass` → `COMPLETED`（解依赖，发 `TASK_VERIFIED`）/ `fail` → `IN_PROGRESS`（返工，`feedback` 定向 author，发 `TASK_REVISION_REQUESTED`）。守卫：任务须 `IN_REVIEW`、caller ∈ 任务 `reviewer` 列且 ≠ author。进 autonomous / scheduled / human_agent 成员集，不进 leader 集。多 reviewer v1「首个裁决即生效」。见 F_59 |
...
`send_message_scheduled`、`shutdown_member`、`spawn_bridge_agent`、`spawn_external_cli`、`spawn_human_agent`、
