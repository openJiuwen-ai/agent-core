...
class ViewTaskToolV2(TeamTool):
    ...
    async def invoke(self, inputs: dict[str, Any], **kwargs) -> ToolOutput:
        ...
        if action == "in_review":
            ...
            # Pending rows this member may start: unassigned pool OR reserved
            # for self (create-time assignee → claim_task starts via start_task).
            result = await self.task_manager.list_tasks_with_deps(
                status=TaskStatus.PENDING.value,
            )
            me = self.task_manager.member_name
            claimable = [
                task
                for task in result.tasks
                if not task.assignee or task.assignee == me
            ]
            result = result.model_copy(update={"tasks": claimable, "count": len(claimable)})
        else:
            result = await self.task_manager.list_tasks_with_deps(
