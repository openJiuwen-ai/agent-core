...
class TeamPermissionRail(PermissionInterruptRail):
    ...
    async def after_tool_call(self, ctx: AgentCallbackContext) -> None:
        """Refresh permission config after each tool call when host exposes a snapshot.

        Mirrors plan-mode ``PermissionInterruptRail`` behaviour: RPC / yaml updates
        become visible on the teammate's next tool boundary without remounting the
        rail. ``get_permissions_snapshot`` is optional — when absent this is a no-op.
        """
        _ = ctx
        ...
        try:
            ...
            team_logger.debug(
                "[TeamPermission] permission.rail.after_tool_call.snapshot_failed",
                exc_info=True,
            )
            return
        if isinstance(snap, dict):
            self.update_config(snap)
