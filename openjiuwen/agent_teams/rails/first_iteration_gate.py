...
class FirstIterationGate(AgentRail):
    ...
    External code can ``await gate.wait()`` to block until the agent is
    actually inside its loop and ready to receive steer / follow_up
    inputs.
    """
    ...
    def is_ready(self) -> bool:
        """Return ``True`` when the gate has been opened."""
        return self._event.is_set()

    async def before_task_iteration(self, ctx: AgentCallbackContext) -> None:
        """Open the gate the first time the agent enters its loop."""
