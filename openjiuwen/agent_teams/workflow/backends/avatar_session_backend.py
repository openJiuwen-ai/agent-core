...
class AvatarSessionManager:
    ...
    async def close_session(self, session_id: str) -> None:
        ...
        if state is None or state.harness is None:
            return
        release_kvc = None
        if kv_cache_harness_session_lifecycle_hook.is_harness_affinity_enabled(state.harness):
            try:
                session = state.harness.current_session()
                release_kvc = getattr(session, "release_kvc", None)
            except Exception as exc:
                team_logger.warning("KVC session lookup failed for %s: %s", session_id, exc)
        try:
