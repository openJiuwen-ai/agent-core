...
class PuctProgramProvider:
    ...
    def validate_input(self, artifact_path: str | None) -> ArtifactValidationResult:
        ...
        Deliberately cheap and synchronous: the contract's `rsi.dataset.validate`
        is a form check the user waits on. Whether the *scoring* can tell a good
    ...
    async def run(
        ...
        """Start a new search and stream its progress.
    ...
    async def terminate(self, task_id: str, on_event: OnEvent | None = None) -> EngineResult:
        """Stop at the next node boundary and keep everything already measured.
