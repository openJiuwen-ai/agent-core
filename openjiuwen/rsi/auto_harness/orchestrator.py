...
from openjiuwen.rsi.auto_harness.infra.session_budget import (
    SessionBudgetController,
)
from openjiuwen.rsi.auto_harness.infra.worktree_manager import (
    WorktreeManager,
)
from openjiuwen.rsi.auto_harness.registry import (
...
if TYPE_CHECKING:
    ...
    from openjiuwen.rsi.auto_harness.rails.cancellation_rail import (
        CancellationRail,
    )
...
class AutoHarnessOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self.stream_rails: List["AgentRail"] = list(stream_rails) if stream_rails else []
        self.agent = agent if agent is not None else _infer_agent_from_rails(self.stream_rails)
        ...
        self.project_profile: ProjectProfile = config.build_project_profile()
        ...
        self.experience_store = ExperienceStore(config.resolved_experience_dir)
        ...
        self.fix_loop = FixLoopController(
            phase1_max_retries=(config.fix_phase1_max_retries),
            phase2_max_retries=(config.fix_phase2_max_retries),
        )
    ...
    def _setup_cancellation_rail(self) -> None:
        ...
        logger.info("[AutoHarnessOrchestrator] CancellationRail bound and added to stream_rails")
    ...
    def record_cycle_result(self, result: CycleResult) -> None:
    ...
    def _dispatch_message(
        ...
        if interaction_id:
            return self._resolve_interaction(interaction_id, message)
        return False
    ...
    def _resolve_interaction(
        ...
        fut = self._pending_interactions.pop(interaction_id, None)
    ...
    async def _stream_session_pipeline(
        ...
        selected_pipeline = self._select_session_pipeline(tasks)
        self.runtime.selected_pipeline = selected_pipeline.pipeline_name
        logger.info(
        ...
        spec = self.pipeline_registry.require(pipeline_name)
        stages_payload = [{"slot": slot, "display_name": dn} for slot, dn in spec.pipeline_cls.stage_order]
        yield OutputSchema(
            type="message",
            index=0,
            payload={
                "content": (f"Session pipeline: {pipeline_name}"),
    ...
    def ensure_session_runtime_dir(self) -> Path:
        ...
        path = Path(self.paths.runtime_extensions_dir) / self.runtime.session_id
