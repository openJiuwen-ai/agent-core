...
class LearningsStage(SessionStage):
    ...
    async def stream(
        ...
        results_artifact = ctx.get_artifact("session_results")
        results = getattr(
            results_artifact,
            "results",
        ...
        async for chunk in run_learnings(
            ...
            yield chunk
        yield StageResult(artifacts={"session_results": SessionResultsArtifact(results=list(results))})
...
async def run_learnings(
    ...
    *,
    ...
    results_text = "\n".join(
        f"- {r.summary or r.pr_url or r.error or 'completed'} (success={r.success}, reverted={r.reverted})"
        for r in results
    )

    recent = await experience_store.list_recent(limit=10)
    existing_text = "\n".join(f"- [{m.type.value}] {m.topic}: {m.summary}" for m in recent) or "无"
