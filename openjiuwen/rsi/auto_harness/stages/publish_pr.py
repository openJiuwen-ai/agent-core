...
def _build_completion_summary(
    ...
    *,
    ...
    ci_summary = ", ".join(
        (f"{gate.get('name', 'unknown')}={'PASS' if gate.get('passed') else 'FAIL'}")
        for gate in ci_result.get("gates", [])
    ) or ("未执行" if not ci_result else ("PASS" if ci_result.get("passed") else "FAIL"))
    ...
    return f"{task.topic}: 已完成；CI={ci_summary}；变更文件={changed_files}{suffix}；交付={location}"
...
def _summarize_ci_result(ci_result: dict[str, Any]) -> str:
    ...
    if gates:
        ...
        return "\n".join(lines)
    return "- overall: PASS" if ci_result.get("passed") else "- overall: FAIL"
...
def _build_deterministic_pr_title(
    ...
    return _normalize_pr_title(
        task.topic or task.issue_ref or "auto-harness task",
        task,
    )
...
def _normalize_pr_body(body: str, task: OptimizationTask) -> str:
    ...
    if not re.search(
        rf"(?i)(关联\s*issue|对应\s*issue|closes|fixes|resolves).{{0,20}}#\s*{issue_number}\b", normalized
    ):
        normalized += f"\n\n## 关联 Issue\n- 对应 Issue: {issue_ref}\nCloses {issue_ref}\n"
...
def _normalize_self_checklist(body: str) -> str:
    ...
    if missing:
        suffix = "\n".join(f"- [x] **{item}**：{_SELF_CHECKLIST_NOTES[item]}" for item in missing)
        normalized = normalized.rstrip() + "\n\n" + suffix
    return normalized
...
def _build_deterministic_pr_body(
    ...
    *,
    ...
    changed_files = "\n".join(f"- `{path}`" for path in allowed_files) if allowed_files else "- 无"
    ...
    fallback_note = "\n\n> 注：PR draft agent 未返回可解析 JSON，本 PR 描述由 auto-harness 基于提交事实自动生成。"
    if draft_error:
        fallback_note += f"\n> draft 解析错误：{draft_error}"
    issue_link_section = f"## 关联 Issue\n- 对应 Issue: {issue_ref}\n" + (
...
def _build_pr_draft_query(
    ...
    *,
    ...
    related_text = (
        "\n".join(f"- [{item.type.value}] {item.topic}: {item.summary}" for item in ctx.runtime.related[:5]) or "无"
    )
    repair_text = ""
    if validation_error:
        repair_text = (
...
def _should_create_pr(ctx: TaskContext) -> bool:
    return bool(ctx.orchestrator.config.git_remote and ctx.orchestrator.config.fork_owner)
...
class PublishPRStage(TaskStage):
    ...
    async def stream(
        ...
        commit_result = ctx.require_artifact("commit_result")
        if not commit_result.committed or commit_result.facts is None:
            error = commit_result.error or "commit result missing"
        ...
        if _should_create_pr(ctx):
            pr_template = await fetch_pr_template(ctx.orchestrator.config)
            draft_error = ""
            previous_output = ""
            for attempt in range(1, _PR_DRAFT_MAX_ATTEMPTS + 1):
                if attempt == 1:
                    ...
                    yield ctx.message(f"修正 PR draft ({attempt}/{_PR_DRAFT_MAX_ATTEMPTS})")
            ...
            if not pr_draft.title or not pr_draft.body:
                ...
                messages.append(f"PR draft agent 未返回合法 JSON，已使用事实兜底生成: {error}")
        ...
        if ctx.orchestrator.config.git_remote:
            ...
            push_result = await ctx.orchestrator.git.push(branch_name=branch_name)
            if push_result.get("output"):
                messages.append("Push 诊断: " + str(push_result.get("output") or "")[:1200])
            if not push_result.get("success"):
                error = f"Git branch push failed: {push_result.get('output') or 'empty git output'}"
                ctx.task.status = TaskStatus.FAILED
                yield StageResult(
                    status="failed",
                    artifacts={
            ...
            if _should_create_pr(ctx):
                ...
                pr_url = pr_result.get("pr_url", "") or pr_result.get("manual_url", "")
                if not pr_url:
                    error = f"GitCode PR creation failed: {pr_result.get('error') or pr_result}"
                    ctx.task.status = TaskStatus.FAILED
                    yield StageResult(
                        status="failed",
                        artifacts={
                ...
                if pr_url:
                    if pr_result.get("pr_url"):
                        ...
                        messages.append(f"PR 自动创建未确认，分支已推送；请通过链接创建/确认 PR: {pr_url}")
        ...
        yield StageResult(
            ...
            + [
                ...
                (f"任务完成: {pr_url}" if pr_url else "任务完成（本地提交）"),
            ],
        )
