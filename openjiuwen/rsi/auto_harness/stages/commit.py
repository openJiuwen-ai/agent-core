...
def _derive_allowed_files(
    ...
    for path in facts.legacy_related_test_files:
        if path in edited_set and is_allowed_repo_edit_path(path):
            allowed.add(path)
...
async def _collect_commit_facts(
    ...
    *,
    ...
    if not edited_files:
        ...
        edited_files = [f for f in status["dirty_files"] if f not in pre_set]
...
async def _collect_existing_branch_commit_facts(
    ...
    *,
    ...
    edited_files = [path for path in branch_files if is_allowed_repo_edit_path(path)]
    facts = CommitFacts(
        branch_name=await git.current_branch(),
        task_declared_files=list(task.files),
        preexisting_dirty_files=list(preexisting_dirty_files),
        ...
        untracked_files=[],
        edited_files=edited_files,
        derived_test_files=[path for path in edited_files if is_derived_test_file(task.files, path)],
...
def _format_commit_failure(
    ...
    *,
    ...
    details = [reason]
    if status_text:
        details.append(f"当前 git status --porcelain:\n{status_text}")
    if last_commit_stat:
        details.append(f"最近一次提交摘要:\n{last_commit_stat}")
    return "\n".join(details)
...
async def _validate_issue_target_alignment(
    *,
    ...
    if not symbols:
        return ""
    diff_text = await git.diff_against_base() if against_base else await git.diff_against("HEAD")
    if not diff_text.strip():
        return ""
    matched = [symbol for symbol in symbols if re.search(rf"\b{re.escape(symbol)}\b", diff_text)]
    if matched:
        return ""
    if allowed_files:
        logger.warning(
            "Issue symbols %s not found in diff, but allowed_files %s has content — passing advisory check",
            symbols,
            allowed_files,
        )
        return ""
...
async def _run_deterministic_commit(
    *,
    ...
    if not add_result.get("success"):
        ...
        return CommitRoundResult(
            ok=False,
            reason=(f"Failed to stage allowed files:\n{add_result.get('output', '')}"),
            status_text=status_text,
            last_commit_stat="",
        )
    ...
    return CommitRoundResult(
        ok=False,
        reason=(f"Deterministic git commit did not create a new commit:\n{commit_result.get('output', '')}"),
        status_text=status_text,
        last_commit_stat="",
    )
...
class CommitStage(TaskStage):
    ...
    async def stream(
        ...
        messages.append("自动提交文件: " + (", ".join(facts.allowed_files) or "无"))
        ...
        if not facts.allowed_files and await ctx.orchestrator.git.has_commits_against_base():
            ...
            messages = [
                ...
                "分支已有提交文件: " + (", ".join(facts.allowed_files) or "无"),
            ]
            yield StageResult(
                artifacts={
                    "commit_result": CommitArtifact(
        ...
        if not commit_ok:
            ...
            yield StageResult(
                ...
                messages=messages + [f"提交失败: {formatted_error}"],
                error=formatted_error,
            )
            return
        yield StageResult(
