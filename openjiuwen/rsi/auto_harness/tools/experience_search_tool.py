...
class ExperienceSearchMetadataProvider(
    ...
    def get_description(
        ...
        descriptions = {
            "cn": ("搜索历史经验记录。输入关键词，返回相关的成功/失败/洞察经验，帮助避免重复错误、复用已验证的方案。"),
            "en": ("Search historical experiences by keyword and return relevant success/failure/insight entries."),
        }
...
register_tool_provider(ExperienceSearchMetadataProvider())
...
class ExperienceSearchTool(Tool):
    ...
    async def invoke(
        ...
        **kwargs: Any,
        ...
        try:
            store = ExperienceStore(self._experience_dir)
