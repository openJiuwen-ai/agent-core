...
def _quote_path(path: str, convert_slashes: bool = True) -> str:
    ...
    if sys.platform == "win32" and convert_slashes:
        ...
        is_relative = not (
            (len(path) >= 2 and path[1] == ":" and path[0].isalpha()) or path.startswith("\\\\") or path.startswith("/")
        )
        if is_relative:
            path = path.replace("/", "\\")
...
_DEFAULT_YAML = str(Path(__file__).resolve().parent.parent / "resources" / "ci_gate.yaml")
...
class CIGateRunner:
    ...
    def __init__(
        ...
        self._gates = self._load_gates(config_path or _DEFAULT_YAML)
    ...
    def _load_gates(path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Failed to load ci_gate.yaml: %s", path)
            return []

    def _match_gates(self, action: str) -> List[Dict[str, Any]]:
        """根据 action 筛选要执行的门控。"""
        if action == "all":
            return list(self._gates)
        name_map = {"check": "lint"}
        target = name_map.get(action, action)
        return [g for g in self._gates if g["name"] == target]
    ...
    def _resolve_python_executable(self) -> str:
        ...
        if self._workspace:
            ...
            if sys.platform == "win32":
                candidates.append(str(Path(self._workspace) / ".venv" / "Scripts" / "python.exe"))
            else:
                candidates.append(str(Path(self._workspace) / ".venv" / "bin" / "python"))
    ...
    def _normalize_command(self, cmd: str) -> str:
        ...
        if not stripped.startswith("make "):
            ...
            if shell_make in f" {stripped}":
                make_index = stripped.index("make ")
                make_segment = stripped[make_index:]
                normalized_make = self._normalize_command(make_segment)
                if normalized_make != make_segment:
                    prefix = stripped[:make_index]
        ...
        if target == "test":
            ...
            return (f"{python_executable} -m pytest {testflags}").strip()
    ...
    def _get_changed_files(self, commits: str) -> list[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                logger.warning("Failed to get changed files: %s", stderr_text.strip()[-500:] or "unknown error")
                return []

            stdout_text = decode_stdout(result.stdout)
            files = [
                f.strip() for f in stdout_text.splitlines() if f.strip() and (f.endswith(".py") or f.endswith(".pyi"))
            ]
            if files:
                return files
            ...
            if commits_val > 0:
                logger.info(
                    "No committed changes for COMMITS=%s; falling back to worktree diff (git diff HEAD)",
                    commits_val,
                )
    ...
    def _get_diff_line_ranges(self, commits: str) -> dict[str, set[int]]:
        ...
        try:
            ...
            if commits_val > 0:
                logger.info(
                    "No diff line ranges for COMMITS=%s; falling back to worktree diff (git diff HEAD -U0)",
                    commits_val,
                )
                wm_result = subprocess.run(
    ...
    def _make_repo_relative(self, filepath: str) -> str:
        ...
        if normalized.startswith(ws):
            workspace_end = len(ws)
            rel = normalized[workspace_end:]
            # Strip leading slash
            return rel.lstrip("/")
        return normalized
    ...
    def _filter_ruff_json_by_line_ranges(
        ...
        for v in in_range:
            ...
            lines.append(f"{filepath}:{line_num}:{col}: {code} {msg}")
        return True, "\n".join(lines)
    ...
    def _filter_pylint_json_by_line_ranges(
        ...
        for v in in_range:
            ...
            lines.append(f"{filepath}:{line_num}: [{msg_id}] {symbol}: {msg}")
        return True, "\n".join(lines)
    ...
    def _filter_codespell_by_line_ranges(
        ...
        for line in raw_output.splitlines():
            ...
            in_range.append(line.replace(raw_path, normalized_path, 1))
    ...
    def _filter_mypy_by_line_ranges(
        ...
        pattern = re.compile(r"^(\S+):(\d+):\s*(error|note|warning):\s+(.*)$")
        in_range: list[str] = []
        for line in raw_output.splitlines():
            m = pattern.match(line)
            if not m:
    ...
    def _filter_format_diff_by_line_ranges(
        ...
        for filepath, fmt_lines in format_hunks.items():
            ...
            if overlap:
                ...
                in_range_lines.append(f"{norm_filepath}: formatting differs on changed lines {sorted(overlap)}")

        if not in_range_lines:
            return False, ""
        return True, "\n".join(in_range_lines)

    async def _run_check_gate(self, commits: str) -> dict[str, Any]:
        ...
        if not line_ranges:
            logger.warning(
                "Diff line ranges empty for COMMITS=%s; falling back to whole-file scope",
                commits,
            )
            line_ranges = {f: set(range(1, 10**6)) for f in changed_files}
        ...
        ruff_cmd = f"{python_executable} -m ruff check --output-format=json {quoted_files}"
        ...
        fmt_cmd = f"{python_executable} -m ruff format --check --diff {quoted_files}"
        ...
        pylint_cmd = f"{python_executable} -m pylint --output-format=json {quoted_files}"
    ...
    async def _run_type_check_gate(self, commits: str) -> dict[str, Any]:
        ...
        mypy_cmd = f"{python_executable} -m mypy --show-error-codes --show-column-numbers {quoted_files}"
        code, mypy_output = await self._run_tool_command(mypy_cmd)
        has_errors, error_text = self._filter_mypy_by_line_ranges(
            mypy_output,
            line_ranges,
            self._make_repo_relative,
        )
    ...
    async def _ensure_environment(self) -> None:
        ...
        install_command = self._normalize_install_command(self._install_command)
        ...
        if re.search(r"(^|\s)uv(\s|$)", install_command):
            ...
            if not uv_ready:
                logger.warning(
                    "[CIGateRunner] skip uv install command; uv unavailable: %s",
                    uv_error,
                )
                self._prepared = True
                return
        ...
        if proc.returncode != 0:
            logger.warning(
                "[CIGateRunner] install command failed; continue with existing environment: %s",
                output.strip()[-1000:],
            )
            self._prepared = True
            return
    ...
    async def _run_gate(self, gate: Dict[str, Any]) -> Dict[str, Any]:
        """执行单个门控命令并返回结果。
        ...
        logger.info("Running CI gate '%s': %s", name, cmd)
        ...
        try:
            ...
            passed = proc.returncode == 0 and fail_marker not in output
    ...
    async def run(self, action: str = "all") -> Dict[str, Any]:
        """执行 CI 门控检查。
        ...
        if not gates:
            return {
                "passed": False,
                "gates": [],
                "errors": (f"No gate matched action={action}"),
            }
        ...
        failed = [gate for gate in results if not gate.get("passed", False)]
        errors = "\n\n".join(
            (f"[{gate.get('name', 'unknown')}]\n{gate.get('output', '').strip()}").strip()
            for gate in failed
