...
class GitOperations:
    ...
    def __init__(
        ...
        self._token = gitcode_token or os.getenv("GITCODE_ACCESS_TOKEN", "")
    ...
    async def _git(self, *args: str, extra_env: Dict[str, str] | None = None) -> tuple[int, str]:
    ...
    def _sanitize_git_args(self, args: list[str]) -> str:
        ...
        return "git " + " ".join(self._sanitize_git_output(str(arg)) for arg in args)

    def _authenticated_remote_url(self, remote_url: str) -> str:
        """Return an HTTPS remote URL carrying basic auth credentials."""
        if not remote_url.startswith("https://") or not self._gitcode_username or not self._token:
            return ""
    ...
    async def create_branch(self, branch_name: str) -> Dict[str, Any]:
        """创建并切换到新分支。"""
        code, out = await self._git("checkout", "-b", branch_name)
        return {
            "success": code == 0,
            "branch": branch_name,
            "output": out,
    ...
    async def collect_status(self) -> Dict[str, List[str]]:
        ...
        code, out = await self._git("status", "--porcelain", "--untracked-files=all")
    ...
    async def current_branch(self) -> str:
        ...
        _, out = await self._git("rev-parse", "--abbrev-ref", "HEAD")
        return out.strip()
    ...
    async def _diff_name_only_for_ref_against_base(self, ref: str) -> List[str]:
    ...
    async def find_existing_issue_fix_ref(
        ...
        *,
        ...
        for ref in dict.fromkeys(line.strip() for line in out.splitlines()):
            if not ref:
                continue
            normalized = ref.removeprefix("origin/").removeprefix("autoharness/")
            if normalized == current_branch:
                continue
            lower_ref = normalized.lower()
            if needle not in lower_ref and not any(value in lower_ref for value in exact_needles):
                continue
    ...
    async def status_porcelain(self) -> str:
        ...
        _, out = await self._git("status", "--porcelain", "--untracked-files=all")
        return out.rstrip()
    ...
    async def push(self, branch_name: str) -> Dict[str, Any]:
        """推送到 fork 远程。"""
        ...
        _, remote_url = await self._git("remote", "get-url", self._remote)
        _, remote_push_url = await self._git("remote", "get-url", "--push", self._remote)
        _, origin_push_url = await self._git("remote", "get-url", "--push", "origin")
        ...
        for label, args in attempts:
            ...
            logger.info(
                "[AutoHarnessGitPush] attempt end: label=%s code=%s elapsed=%.2fs output=%s",
                label,
                code,
                elapsed,
                sanitized,
    ...
    def _create_pr_sync(
        ...
        url = f"https://api.gitcode.com/api/v5/repos/{owner}/{repo}/pulls?access_token={self._token}"
        request_payload = {
            "title": title,
    ...
    async def create_pr(
        ...
        return await loop.run_in_executor(None, self._create_pr_sync, title, body, head_branch)
