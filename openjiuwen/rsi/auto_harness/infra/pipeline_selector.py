...
def choose_session_pipeline(
    *,
    ...
    preference = normalize_pipeline_preference(config.pipeline_preference)
    if preference != PIPELINE_PREFERENCE_AUTO and preference in available_pipelines:
        alternatives = [name for name in available_pipelines if name != preference]
    ...
    if signal and signal in available_pipelines:
        alternatives = [name for name in available_pipelines if name != signal]
        return PipelineSelectionArtifact(
    ...
    pipeline_name = META_EVOLVE_PIPELINE if META_EVOLVE_PIPELINE in available_pipelines else available_pipelines[0]
    alternatives = [name for name in available_pipelines if name != pipeline_name]
    return PipelineSelectionArtifact(
