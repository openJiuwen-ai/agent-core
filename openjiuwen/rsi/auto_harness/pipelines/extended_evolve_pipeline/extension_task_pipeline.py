...
def build_extension_task(
    ...
    return OptimizationTask(
        ...
        description=(f"Implement and verify runtime extension {design.extension_name}"),
        files=[
...
async def prepare_extension_task_runtime(
    ...
    *,
    ...
    wt_path = await orchestrator.worktree_mgr.prepare(f"extension-{design.extension_name}")
    logger.info(
...
class ExtensionTaskPipeline(BasePipeline):
    ...
    async def stream(
        ...
        if ctx.get_artifact("task_result") is None:
            ctx.put_artifact(
                ...
                CycleResult(
                    success=True,
                    summary=(f"Extension activated: {design.extension_name}"),
                ),
            )
        yield ctx.message(f"Extension activated: {design.extension_name}")
        logger.info(
    ...
    async def run_isolated_stream(
        ...
        try:
            ...
            result = CycleResult(
                success=False,
                error="timeout",
                error_log=("Extension task exceeded timeout"),
            )
        ...
        *,
        ...
        try:
            ...
            result = CycleResult(
                success=False,
                error="timeout",
                error_log=("Extension build/verify exceeded timeout"),
            )
        except Exception as exc:
            task.status = TaskStatus.FAILED
            logger.exception(
    ...
    async def run_activate_stream(
        ...
        async for chunk in cls()._run_activate_stage_stream(verified.ctx):
            yield chunk
    ...
    async def _run_task_stream(
        ...
        *,
        ...
        try:
            pipeline = cls()
            stream = pipeline.stream(ctx) if include_activate else pipeline.run_build_verify_stream(ctx)
            ...
            await orchestrator.worktree_mgr.cleanup(runtime.wt_path)
            orchestrator.task_contexts.pop(task_key(task), None)
            logger.info(
    ...
    def _resolve_task_result(
        ...
        if not isinstance(result, CycleResult):
            result = CycleResult(
                ...
                error_log=("No cycle result recorded for extension task"),
            )
        if result.success:
            task.status = TaskStatus.SUCCESS
    ...
    def _resolve_build_verify_result(
        ...
        if runtime_ext is None:
            return CycleResult(
                ...
                error_log=("No runtime_extension artifact recorded after build/verify"),
            )
        return CycleResult(
            success=True,
