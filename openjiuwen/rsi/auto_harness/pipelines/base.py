...
class PipelineStageMap:
    ...
    mapping: dict[str, type[BaseStage]] = field(default_factory=dict)

    def resolve(self, slot: str) -> BaseStage:
        """Instantiate the stage bound to *slot*."""
        cls = self.mapping.get(slot)
        if cls is None:
            raise KeyError(f"No stage bound for slot '{slot}'")
        return cls()
...
class BasePipeline:
    ...
    def _require_stage_result(
        ...
        if not result_holder:
            raise RuntimeError(f"Stage '{stage.name}' did not return a StageResult")
    ...
    def _did_stage_fail(
        ...
        return self._require_stage_result(stage, result_holder).status == "failed"
...
def _describe_context(ctx: "SessionContext | TaskContext") -> str:
    ...
    if task is not None:
        return f"task={getattr(task, 'topic', '')} status={getattr(task, 'status', '')}"
    orchestrator = getattr(ctx, "orchestrator", None)
    runtime = getattr(orchestrator, "runtime", None)
    return f"session={getattr(runtime, 'session_id', '')} pipeline={getattr(runtime, 'selected_pipeline', '')}"
