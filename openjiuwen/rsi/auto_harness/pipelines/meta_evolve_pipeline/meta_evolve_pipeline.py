...
class MetaEvolvePipeline(BasePipeline):
    ...
    async def stream(
        ...
        try:
            async for chunk in self.run_assess_and_plan_stream(ctx):
                yield chunk
        ...
        async for chunk in self.run_learnings_stage_stream(ctx):
            yield chunk
    ...
    def _explicit_issue_fix_tasks(
        ...
        input_tasks = ctx.orchestrator.artifacts.get("input_tasks", default=[])
    ...
    async def run_assess_and_plan_stream(
        ...
        if issue_tasks:
            ...
            yield ctx.message("检测到显式 GitCode issue 修复任务，跳过 assess/plan，直接进入实现/验证/提交/发布流程。")
            return
        async with self._readonly_assess_workspace(ctx):
            ...
            if self._did_stage_fail(assess_stage, assess_result_holder):
                raise _StopMetaEvolvePipeline
            ...
            if ctx.orchestrator.should_cancel:
                ...
                raise _StopMetaEvolvePipeline
            if self._did_stage_fail(plan_stage, plan_result_holder):
                raise _StopMetaEvolvePipeline
    ...
    async def run_task_pipeline_stream(
        ...
        capped_tasks = task_plan.tasks[: ctx.orchestrator.config.max_tasks_per_session]
    ...
    async def run_learnings_stage_stream(
        ...
        async for chunk in self._stream_stage(
            ...
            yield chunk
        self._require_stage_result(LearningsStage(), result_holder)
    ...
    async def _readonly_assess_workspace(
        ...
        original_workspace = ctx.orchestrator.config.workspace
        assess_workspace = await ctx.orchestrator.worktree_mgr.prepare_readonly_snapshot(label="assess")
        ctx.orchestrator.config.workspace = assess_workspace
        try:
            yield
        finally:
            ctx.orchestrator.config.workspace = original_workspace
            await ctx.orchestrator.worktree_mgr.cleanup(assess_workspace)
