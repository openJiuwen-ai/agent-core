...
def _target_regression(candidate: dict[str, Any], target_case_ids: set[str]) -> dict[str, Any]:
    if isinstance(candidate.get("regressed_target_case_ids"), (list, tuple, set)):
        ...
        regressed = {
            ...
            if _number(candidate_scores.get(case_id)) is not None
            and _number(source_scores.get(case_id)) is not None
            and float(candidate_scores[case_id]) < float(source_scores[case_id])
        }
    return {
        "availability": "observed",
...
def _evidence_refs(candidate: dict[str, Any]) -> dict[str, str]:
    ...
    return {output_key: value for output_key, keys in aliases.items() if (value := _first_text(candidate, *keys))}
