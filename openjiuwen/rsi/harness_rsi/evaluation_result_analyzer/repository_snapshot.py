...
def prepare_repository_snapshot(
    *,
    ...
    manifest = {"repository": "unavailable", "patch": "unavailable", "artifacts": "unavailable", "errors": []}
    if patch:
        try:
    ...
    for name, location in (("repository", workspace), ("artifacts", artifacts)):
        if not location:
            continue
        ...
        try:
            ...
            manifest[name] = "complete"
        except shutil.Error as exc:
            manifest[name] = "partial"
            for src, _dst, error in exc.args[0]:
                relative = Path(src).relative_to(source).as_posix()
                manifest["errors"].append(
                    {
                        "path": f"artifacts/{relative}" if name == "artifacts" else relative,
                        "error": str(error),
                    }
                )
        except OSError as exc:
            manifest["errors"].append({"path": f"{name}/", "error": str(exc)})
