...
from openjiuwen.rsi.harness_rsi.auto_harness.infra.gitcode_pr_template import load_pr_template_fallback
from openjiuwen.rsi.harness_rsi.auto_harness.pipelines import EXTENDED_EVOLVE_PIPELINE
from openjiuwen.rsi.harness_rsi.auto_harness.prompts.sections import (
...
def create_auto_harness_agent(
    ...
    *,
    ...
    if enable_task_planning:
        ...
        rails.append(TaskPlanningRail(enable_progress_repeat=enable_progress_repeat))
    if extra_rails:
    ...
    system_prompt = "\n\n".join(s.render(config.language) for s in sections)
    ...
    return create_deep_agent(
        ...
        max_iterations=config.resolve_agent_iterations("implement", 30),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness"),
    )
...
def _build_rails(
    ...
    *,
    ...
    from openjiuwen.rsi.harness_rsi.auto_harness.rails.experience_rail import (
    ...
    *,
    ...
    subagents: list[object] = [
        build_explore_agent_config(
            ...
            max_iterations=config.resolve_agent_iterations("explore_subagent", 20),
        ),
    ]
    try:
        subagents.append(
            build_browser_agent_config(
                ...
                workspace=subagent_workspace,
                language=resolved_language,
                max_iterations=config.resolve_agent_iterations("browser_subagent", 20),
            )
        )
...
def _build_skill_rail(
    ...
    is_extended = config.pipeline_preference == EXTENDED_EVOLVE_PIPELINE
    community_dirs = community_skill_cache_skill_dirs(config) if is_extended else []
    ...
    for root in roots:
        ...
        if not root_path.is_dir():
            continue
        if not any((root_path / skill_name).is_dir() for skill_name in skill_names):
            continue
...
def create_assess_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        language=config.language,
        enable_async_subagent=True,
        max_iterations=config.resolve_agent_iterations("assess", 30),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-assess"),
    )
...
def create_plan_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        language=config.language,
        enable_async_subagent=True,
        max_iterations=config.resolve_agent_iterations("plan", 15),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-plan"),
    )
...
def create_eval_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        language=config.language,
        enable_async_subagent=True,
        max_iterations=config.resolve_agent_iterations("eval", 10),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-eval"),
    )
...
def create_select_pipeline_agent(
    ...
    *,
    ...
    rails = _build_readonly_rails(config)
    rails.append(_build_skill_rail(config, ["select_pipeline"]))
    if extra_rails:
        rails.extend(extra_rails)
    return create_deep_agent(
        model=config.model,
        ...
        language=config.language,
        enable_async_subagent=True,
        max_iterations=config.resolve_agent_iterations("select_pipeline", 10),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-select-pipeline"),
    )
...
def create_design_ext_agent(
    ...
    *,
    ...
    rails = _build_readonly_rails(config)
    rails.append(_build_skill_rail(config, ["design_ext"]))
    if extra_rails:
        rails.extend(extra_rails)
    return create_deep_agent(
        model=config.model,
        ...
        language=config.language,
        enable_async_subagent=True,
        max_iterations=config.resolve_agent_iterations("design_ext", 15),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-design-ext"),
    )
...
def create_pr_draft_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        workspace=workspace,
        language=config.language,
        max_iterations=config.resolve_agent_iterations("pr_draft", 5),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-pr-draft"),
    )
...
def create_learnings_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        workspace=config.workspace,
        language=config.language,
        max_iterations=config.resolve_agent_iterations("learnings", 5),
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("auto-harness-learnings"),
    )
...
def create_activate_guide_agent(
    ...
    *,
    ...
    return create_deep_agent(
        ...
        completion_timeout=config.model_timeout_secs,
        auto_create_workspace=False,
        sys_operation=_build_trusted_local_sys_operation("activate-guide"),
    )
