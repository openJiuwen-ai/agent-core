...
def _platform_adaptation_section() -> PromptSection:
    ...
    cn = (
        ...
        "该目录属于机器上所有账号共享，先运行的账号会成为目录属主并保留默认的 0755 权限，"
        "此后其他账号写入时只会得到一个仅指明路径的权限错误。"
        "确需跨进程共享的稳定路径时，在目录名中加入 `os.getuid()`（无 `os.getuid` 的平台用登录名），"
        "并将目录权限设为 0700；不需要稳定路径时改用 `tempfile.mkdtemp()`\n"
    ...
    en = (
        ...
        "- Fixed names under the temp directory: Never create a fixed-name directory directly under "
        "`tempfile.gettempdir()`. That directory is shared by every account on the machine: the first "
        "account to run creates the entry, owns it, and leaves it at the default 0755, after which every "
        "later account fails to write into it with a permission error that names only the path. When a "
        "stable path is genuinely needed across processes, put `os.getuid()` in the directory name (the "
        "login name where `os.getuid` is unavailable) and chmod it to 0700; otherwise use "
        "`tempfile.mkdtemp()`\n"
