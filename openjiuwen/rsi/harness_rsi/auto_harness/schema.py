...
_CONFIG_TEMPLATE = Path(__file__).parent / "resources" / "config.yaml"
...
def _default_immutable_files() -> list[str]:
    ...
    return [
        ...
        "openjiuwen/rsi/harness_rsi/auto_harness/resources/ci_gate.yaml",
...
class ExtensionDesign:
    ...
    file_plan: Dict[str, str] = field(default_factory=dict)
    harness_config_patch: Dict[str, Any] = field(default_factory=dict)
    skill_source: str = ""
...
class ExtensionDesignArtifact:
    ...
    designs: List["ExtensionDesign"] = field(default_factory=list)
...
class CodeChangeArtifact:
    ...
    related: List[Experience] = field(default_factory=list)
...
class VerifyReportArtifact:
    ...
    ci_result: Dict[str, Any] = field(default_factory=dict)
    fix_errors: str = ""
    reverted: bool = False
    error: str = ""
...
class CommitFacts:
    ...
    branch_name: str = ""
    task_declared_files: List[str] = field(default_factory=list)
    preexisting_dirty_files: List[str] = field(default_factory=list)
    current_dirty_files: List[str] = field(default_factory=list)
    tracked_modified_files: List[str] = field(default_factory=list)
    untracked_files: List[str] = field(default_factory=list)
    edited_files: List[str] = field(default_factory=list)
    allowed_files: List[str] = field(default_factory=list)
    derived_test_files: List[str] = field(default_factory=list)
    legacy_related_test_files: List[str] = field(default_factory=list)
    verify_related_files: List[str] = field(default_factory=list)
    diff_stat: str = ""
...
class AutoHarnessConfig:
    ...
    stage_registrars: List[str] = field(default_factory=list)
    pipeline_registrars: List[str] = field(default_factory=list)
    ...
    def cache_repo_dir(self) -> str:
        ...
        if self.data_dir:
            return str(Path(self.data_dir) / "repo" / repo_name)
    ...
    def runtime_extensions_dir(self) -> str:
        ...
        if self.data_dir:
            return str(Path(self.data_dir) / "runtime_extensions")
    ...
    def resolved_community_skill_cache_dir(self) -> str:
        ...
        if self.data_dir:
            return str(Path(self.data_dir) / "skills-cache")
    ...
    def resolve_ci_gate_python_executable(self) -> str:
        ...
        if self.workspace:
            candidates.extend(_venv_python_candidates(self.workspace))
        if self.local_repo:
            candidates.extend(_venv_python_candidates(self.local_repo))
    ...
    def resolve_agent_iterations(
        ...
        try:
            return int(self.agent_iterations.get(stage_name, default))
    ...
    def build_project_profile(self) -> ProjectProfile:
        ...
        return ProjectProfile(
            ...
            high_impact_prefixes=list(self.high_impact_prefixes),
            default_base_branch=self.git_base_branch or "develop",
        )
    ...
    def build_paths(self) -> AutoHarnessPaths:
        ...
        return AutoHarnessPaths(
            ...
            runtime_extensions_dir=(self.runtime_extensions_dir),
        )
    ...
    def load_from_dict(
        ...
        if "skills_dirs" in data and isinstance(data["skills_dirs"], list):
            cfg.skills_dirs = [str(v) for v in data["skills_dirs"]]
        if "community_skill_repos" in data and isinstance(data["community_skill_repos"], list):
            cfg.community_skill_repos = [str(v) for v in data["community_skill_repos"]]
        if "community_skill_cache_dir" in data:
            cfg.community_skill_cache_dir = str(data["community_skill_cache_dir"])
        if "stage_registrars" in data and isinstance(data["stage_registrars"], list):
            cfg.stage_registrars = [str(v) for v in data["stage_registrars"]]
        if "pipeline_registrars" in data and isinstance(data["pipeline_registrars"], list):
            cfg.pipeline_registrars = [str(v) for v in data["pipeline_registrars"]]
        if "immutable_files" in data and isinstance(data["immutable_files"], list):
            cfg.immutable_files = [str(v) for v in data["immutable_files"]]
        if "language" in data:
            cfg.language = str(data["language"])
        if "pipeline" in data:
            cfg.pipeline_preference = normalize_pipeline_preference(data["pipeline"])
        if "pipeline_preference" in data:
            cfg.pipeline_preference = normalize_pipeline_preference(data["pipeline_preference"])
        # 兼容旧字段
        if "workspace" in data:
            cfg.workspace = str(data["workspace"])
        if "experience_dir" in data:
            cfg.experience_dir = str(data["experience_dir"])
        ...
        if isinstance(git, dict):
            ...
            if "base_branch" in git:
                cfg.git_base_branch = str(git["base_branch"])
            if "user_name" in git:
            ...
            if "upstream_owner" in git:
                cfg.upstream_owner = str(git["upstream_owner"])
            if "upstream_repo" in git:
                cfg.upstream_repo = str(git["upstream_repo"])

        # gitcode section
        gc = data.get("gitcode", {})
        if isinstance(gc, dict):
            if "username" in gc:
                cfg.gitcode_username = str(gc["username"])
            if "access_token_env" in gc:
                cfg.gitcode_token_env = str(gc["access_token_env"])
            if "access_token" in gc:
                cfg.gitcode_token = str(gc["access_token"])

        # budget section
        budget = data.get("budget", {})
        if isinstance(budget, dict):
            if "session_secs" in budget:
                cfg.session_budget_secs = float(budget["session_secs"])
            if "cost_limit_usd" in budget:
                cfg.cost_limit_usd = float(budget["cost_limit_usd"])
            if "task_timeout_secs" in budget:
                cfg.task_timeout_secs = float(budget["task_timeout_secs"])
            if "model_timeout_secs" in budget:
                cfg.model_timeout_secs = float(budget["model_timeout_secs"])
            if "max_tasks_per_session" in budget:
                cfg.max_tasks_per_session = int(budget["max_tasks_per_session"])

        # ci_gate section
        ci = data.get("ci_gate", {})
        if isinstance(ci, dict):
            if "config_path" in ci:
                cfg.ci_gate_config = str(ci["config_path"])
            if "python_executable" in ci:
                cfg.ci_gate_python_executable = str(ci["python_executable"])
            if "install_command" in ci:
                cfg.ci_gate_install_command = str(ci["install_command"])

        # fix_loop section
        fl = data.get("fix_loop", {})
        if isinstance(fl, dict):
            if "phase1_max_retries" in fl:
                cfg.fix_phase1_max_retries = int(fl["phase1_max_retries"])
            if "phase2_max_retries" in fl:
                cfg.fix_phase2_max_retries = int(fl["phase2_max_retries"])
        ...
        if isinstance(agent_cfg, dict):
            for key, value in agent_cfg.items():
                try:
                    cfg.agent_iterations[str(key)] = int(value)
                except (TypeError, ValueError):
                    continue

        extensions = data.get("extensions", {})
        if isinstance(extensions, dict):
            if "stage_registrars" in extensions and isinstance(extensions["stage_registrars"], list):
                cfg.stage_registrars = [str(v) for v in extensions["stage_registrars"]]
            if "pipeline_registrars" in extensions and isinstance(extensions["pipeline_registrars"], list):
                cfg.pipeline_registrars = [str(v) for v in extensions["pipeline_registrars"]]
    ...
    *,
    ...
    if not path.is_file():
        bootstrapped = _bootstrap_config_file(
            path,
            suggested_local_repo=_detect_local_repo(workspace_hint),
        )
        logger.info(
        ...
        cfg.suggested_local_repo = _detect_local_repo(workspace_hint)
        if not cfg.data_dir:
...
def _bootstrap_config_file(
    ...
    *,
    ...
    try:
        ...
        template = _CONFIG_TEMPLATE.read_text(encoding="utf-8")
