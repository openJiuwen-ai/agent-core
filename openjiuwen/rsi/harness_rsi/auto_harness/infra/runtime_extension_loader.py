...
from openjiuwen.rsi.harness_rsi.auto_harness.infra.runtime_manifest import load_runtime_manifest
...
def load_runtime_rails(
    ...
    *,
    ...
    rails: list[type[Any]] = []
    for spec in config.resources.rails if config.resources else []:
...
def load_runtime_tools(
    ...
    *,
    ...
    config = load_runtime_manifest(runtime_ext.config_path)
    tools: list[type[Any]] = []
    for spec in config.resources.tools if config.resources else []:
        if spec.type != "package":
            continue
        if not spec.module or not spec.class_name:
            continue
...
def _load_runtime_class(
    *,
    ...
    if not (module_name == prefix or module_name.startswith(f"{prefix}.")):
        raise ValueError(f"Runtime module does not belong to runtime extension '{extension_name}': {module_name}")

    prefix_end = len(prefix)
    relative_module = module_name[prefix_end:].lstrip(".")
    root = Path(runtime_ext.runtime_path).resolve()
    unique_base = f"{_RUNTIME_PREFIX}.{session_id}.{extension_name}"
    ...
    module = _load_module_from_path(
        ...
        package_path=(module_path.parent if module_path.name == "__init__.py" else None),
    )

    # Alias under the official prefix so cross-module
    # imports within the extension work.
    official_name = f"{prefix}.{relative_module}" if relative_module else prefix
...
def _ensure_official_namespace(
    *,
    ...
    for init_file in sorted(extension_root.rglob("__init__.py")):
        pkg_dir = init_file.parent
        if pkg_dir == extension_root:
            continue
        relative = pkg_dir.relative_to(extension_root)
        sub_fqn = f"{ext_fqn}." + str(relative).replace("/", ".").replace("\\", ".")
...
def _load_module_from_path(
    *,
    ...
    if not module_path.is_file():
        raise FileNotFoundError(f"Runtime extension module not found: {module_path}")
    kwargs: dict[str, Any] = {}
