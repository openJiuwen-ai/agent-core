...
def _validate_skill_frontmatter(skill_md_path: Path) -> list[str]:
    ...
    if not text.startswith("---"):
        errors.append(f"SKILL.md missing frontmatter: {skill_md_path}")
        return errors

    parts = text.split("---", 2)
    if len(parts) < 3:
        errors.append(f"SKILL.md malformed frontmatter: {skill_md_path}")
        return errors
    ...
    try:
        ...
        errors.append(f"SKILL.md frontmatter YAML error: {skill_md_path}: {e}")
        return errors

    if not isinstance(data, dict):
        errors.append(f"SKILL.md frontmatter not a dict: {skill_md_path}")
        return errors

    name = data.get("name")
    if not name or not isinstance(name, str) or not name.strip():
        errors.append(f"SKILL.md missing 'name' field: {skill_md_path}")

    description = data.get("description")
    if not description or not isinstance(description, str) or not description.strip():
        errors.append(f"SKILL.md missing 'description' field: {skill_md_path}")
...
async def check_ruff(
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            if not output:
                logger.warning(
                    "[check_ruff] rc=%s no output with CI=1, retrying without CI",
                    proc.returncode,
                )
                try:
                    diag_env = dict(env)
                    ...
                    output = _decode_subprocess_output(diag_stdout, diag_stderr)
                except Exception as exc:
                    logger.debug("[check_ruff] diagnostic re-run failed: %s", exc)
            if not output:
                output = f"ruff returned exit code {proc.returncode} with no output"
...
def _build_ruff_env() -> dict[str, str]:
    ...
    env["PATH"] = f"{bin_dir}{pathsep}{existing}" if existing else bin_dir
    return env
