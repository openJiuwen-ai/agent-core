...
async def _run_git(*args: str, cwd: str, env: dict[str, str] | None = None) -> tuple[int, str]:
    """执行 git 命令。
...
class WorktreeManager:
    ...
    def __init__(self, config: "AutoHarnessConfig") -> None:
        self._config = config
    ...
    def _base_repo(self) -> str:
        ...
        if self._config.local_repo:
            return str(Path(self._config.local_repo).resolve())
        return self._config.cache_repo_dir

    def _is_managed_worktree_path(self, worktree_path: str) -> bool:
        """Return whether a worktree lives under auto-harness root."""
        root = Path(self._config.worktrees_dir).resolve(strict=False)
    ...
    async def _list_worktrees(self, base: str) -> list[dict[str, str]]:
        ...
        if code != 0:
            raise RuntimeError(f"worktree list failed: {out}")
    ...
    async def _drop_existing_branch(self, *, base: str, branch_name: str) -> None:
        ...
        if code != 0:
            raise RuntimeError(f"worktree prune failed: {out}")
        ...
        for entry in await self._list_worktrees(base):
            ...
            if not self._is_managed_worktree_path(worktree_path):
                raise RuntimeError(
                    f"existing auto-harness branch is checked out in unmanaged worktree: {worktree_path}"
                )
            code, out = await _run_git(
                "worktree",
                "remove",
            ...
            if code != 0:
                raise RuntimeError(f"failed to remove existing managed worktree for branch {branch_name}: {out}")
            logger.info(
        ...
        if code != 0:
            raise RuntimeError(f"failed to delete existing branch {branch_name}: {out}")
        logger.info(
    ...
    async def _ensure_base_repo(self) -> str:
        ...
        if self._config.local_repo:
            ...
            if not base_path.exists():
                raise RuntimeError(f"local_repo not found: {base}")
            code, out = await _run_git("fetch", "origin", cwd=base, env=self._git_env)
            if code != 0:
                logger.warning(
                    "fetch failed in local_repo (continuing): %s",
                    out,
                )
            return base
        ...
        if git_dir.is_dir() or (base_path.is_dir() and (base_path / "HEAD").is_file()):
            # 已有缓存，fetch
            code, out = await _run_git("fetch", "origin", cwd=base, env=self._git_env)
            if code != 0:
                logger.warning(
                    "fetch failed in cache repo (continuing): %s",
                    out,
                )
            return base
    ...
    async def prepare(self, topic: str) -> str:
        ...
        async with self._git_lock:
            ...
            base_branch = self._config.git_base_branch or "develop"

            await self._drop_existing_branch(base=base, branch_name=branch_name)
            ...
            if self._config.git_remote:
                ...
                if rc != 0:
                    ...
                    fork_url = f"https://gitcode.com/{self._config.fork_owner}/{self._config.upstream_repo}.git"
                    await _run_git(
                        "remote",
                        "add",
                        self._config.git_remote,
    ...
    async def prepare_readonly_snapshot(
        ...
        *,
        ...
        async with self._git_lock:
            ...
            if code != 0:
                raise RuntimeError(f"readonly worktree add failed: {out}")
    ...
    async def cleanup(self, worktree_path: str) -> None:
        ...
        if code != 0:
            logger.warning(
                "worktree remove failed (manual cleanup needed): %s",
                out,
            )
        else:
            logger.info(
