...
def normalize_repo_path(path: str) -> str:
    ...
    project_root = Path(get_project_root() or str(current_cwd)).resolve()
    expanded = Path(raw).expanduser()
    resolved = expanded.resolve() if expanded.is_absolute() else (current_cwd / expanded).resolve()
    try:
...
def is_allowed_repo_edit_path(path: str) -> bool:
    ...
    return any(normalized.startswith(prefix) for prefix in ALLOWED_EDIT_PREFIXES)
...
def render_edit_scope(
    ...
    return (
        ...
        "- 不要修改 `openjiuwen/rsi/harness_rsi/auto_harness/**` 或其他源码目录\n"
