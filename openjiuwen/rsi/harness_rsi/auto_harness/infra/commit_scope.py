...
_TEST_FILE_RE = re.compile(r"(tests/(?:unit_tests|system_tests)/[^\s:'\"]+\.py)")
...
def is_allowed_documentation_file(path: str) -> bool:
    ...
    return is_documentation_file(path) and is_allowed_repo_edit_path(path)
...
def derive_legacy_related_test_files(
    ...
    verify_set = {path.strip().replace("\\", "/") for path in verify_related_files if path.strip()}
    related = []
