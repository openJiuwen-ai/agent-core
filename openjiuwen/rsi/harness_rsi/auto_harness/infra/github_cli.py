...
def ensure_github_cli_ready(
    ...
    if not gh_path:
        emit("未检测到 `gh`，正在尝试自动安装 GitHub CLI。")
        gh_path = _install_github_cli(emit)
        installed_now = bool(gh_path)
        if not gh_path:
            emit("自动安装 `gh` 失败。本轮会退回网页补充调研，无法执行 GitHub-first 源码策略。")
            emit("请先安装 GitHub CLI 后重试：https://cli.github.com/")
    ...
    if authenticated:
        ...
        emit("建议先执行 `gh auth login --web` 完成浏览器登录；若已有 token，也可使用 `gh auth login --with-token`。")
...
def _install_github_cli(
    ...
    for command, label in _install_commands():
        ...
        if stderr:
            emit(f"`{label}` 安装失败: {_truncate(stderr)}")
