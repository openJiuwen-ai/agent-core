...
class FixLoopController:
    ...
    async def run(
        ...
        evaluator: Optional[Callable[[], Coroutine[Any, Any, Any]]] = None,
    ) -> FixLoopResult:
        """执行两阶段修复循环。
        ...
        for i in range(1, self._p1_max + 1):
            ...
            result.error_log.append(f"Phase 1 attempt {i}: {errors[:200]}")
            try:
        ...
        for j in range(1, self._p2_max + 1):
            ...
            if review.approved:
                result.success = True
                logger.info(
                    "Evaluator approved on phase 2 attempt %d",
                    j,
                )
                return result

            result.error_log.append(f"Phase 2 attempt {j}: evaluator rejected")
            try:
                await asyncio.wait_for(
