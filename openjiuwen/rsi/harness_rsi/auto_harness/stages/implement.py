...
def _build_implement_prompt(
    ...
    for exp in related:
        context_parts.append(f"- [{exp.type.value}] {exp.topic}: {exp.summary}")
    context = "\n".join(context_parts) or "无"
    edit_scope = render_edit_scope("本轮实现阶段允许改动的路径")
    return (
        f"任务: {task.topic}\n"
...
def _artifact_contract_hint(name: str) -> str:
    ...
    if any(token in lowered for token in ("ppt", "pptx", "powerpoint")):
        return (
            common + " PPT/PPTX 生成必须产出真实 .pptx 文件；不得用 JSON、Markdown、"
    ...
    if any(token in lowered for token in ("docx", "word")):
        return (
            common + " DOCX 生成必须产出真实 .docx 文件，并用 zipfile 校验 "
...
async def run_implement_stream(
    ...
    if session is None:
        async for chunk in agent.stream({"query": effective_prompt}):
            yield chunk
        return

    await session.pre_run(inputs={"query": effective_prompt})
    try:
...
class MetaImplementStage(ImplementStage):
    ...
    async def stream(
        ...
        yield ctx.message(f"任务准备就绪: {ctx.task.topic}")
        async for chunk in run_implement_stream(
            ...
            yield chunk
            implement_error = _extract_controller_task_failed_error(chunk)
            if implement_error:
                break
        if implement_error:
        ...
        diff_files = await ctx.orchestrator.git.diff_name_only("HEAD")
        ...
        if not edited_files:
            if await ctx.orchestrator.git.has_commits_against_base():
                ...
                return
            issue_number = _extract_issue_number_from_task(ctx.task)
            existing_fix = (
            ...
            if existing_fix.get("success"):
                pick_result = await ctx.orchestrator.git.cherry_pick_ref_commits(str(existing_fix.get("ref") or ""))
...
def _build_implement_ext_prompt(
    ...
    *,
    ...
    file_plan_text = "\n".join(file_plan_lines) if file_plan_lines else "  (无)"

    # Build component-specific requirements
    requirements: list[str] = [
        "1. 在扩展根目录下创建完整的 Python 包结构",
        "2. 严格按 ExtensionDesign.components 实现组件；不要为了完整性自动补充未声明的 Rail、Tool 或 Skill。",
    ...
    if "skill" in components:
        if design.skill_source:
            skill_name = design.skill_source.removeprefix("community:")
            requirements.append(
    ...
    if "rail" in components and "tool" in components:
        ...
        step += 1
    requirements.append(f"{step}. 生成 harness_config.yaml manifest（只声明实际包含的组件类型）")
    step += 1
    requirements.append(
...
def _resolve_extension_root(
    ...
    root = design.file_plan.get(
        "root",
        (f"openjiuwen/extensions/harness/{design.extension_name}"),
    )
...
def _resolve_config_path(
    ...
    manifest = design.file_plan.get(
        "manifest",
        (f"openjiuwen/extensions/harness/{design.extension_name}/harness_config.yaml"),
    )
...
class ExtendImplementStage(ImplementStage):
    ...
    async def stream(
        ...
        if not isinstance(design, ExtensionDesign):
            raise TypeError("extension_target artifact must be ExtensionDesign")

        agent = ctx.runtime.task_agent
        if agent is None:
            error = "No task_agent available for implement_ext stage"
            logger.error(error)
            ctx.task.status = TaskStatus.FAILED
            yield StageResult(
                status="failed",
        ...
        if design.skill_source:
            ...
            if not copied:
                logger.warning(
                    "Community skill '%s' not found in cache, falling back to agent-generated skill",
                    skill_name,
                )
                design.skill_source = ""
        ...
        yield ctx.message(f"开始实现扩展: {design.extension_name}")
        ...
        async for chunk in agent.stream(
            ...
            **({"session": session} if session is not None else {}),
            ...
            implement_error = _extract_task_failed_error(chunk)
            if implement_error:
                break
        ...
        if not extension_root.exists():
            error = f"Agent did not create extension root: {extension_root}"
            ctx.task.status = TaskStatus.FAILED
            yield StageResult(
                status="failed",
                artifacts={
        ...
        yield StageResult(
            ...
            messages=[f"Implemented extension: {design.extension_name}"],
        )
...
async def promote_runtime(
    ...
    session_root = ctx.orchestrator.ensure_session_runtime_dir()
    ...
    config_path = (destination / "harness_config.yaml").resolve()
