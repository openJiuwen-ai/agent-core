...
from openjiuwen.rsi.harness_rsi.auto_harness.infra.edit_scope import (
    render_edit_scope,
)
...
class MetaPlanStage(PlanStage):
    ...
    async def stream(
        ...
        input_tasks = ctx.get_artifact("input_tasks", default=[])
        input_task_list = list(input_tasks) if isinstance(input_tasks, list) else []
        ...
        async for chunk in run_plan_stream(
            ...
            yield chunk
        if plan_text.strip():
            path = Path(ctx.orchestrator.paths.runs_dir) / "latest_plan.md"
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(plan_text, encoding="utf-8")
            messages.append(f"规划原始输出已保存: {path}")
        tasks = parse_tasks(plan_text)
        if len(tasks) > 1:
            tasks = tasks[:1]
            messages.append("规划阶段只保留最高优先级的 1 个任务")
        if not tasks:
            if input_task_list:
                tasks = input_task_list[:1]
                messages.append("规划阶段未生成任务，回退执行最高优先级输入任务")
            else:
...
class ExtendPlanStage(PlanStage):
    ...
    async def stream(
        ...
        if not isinstance(analysis, GapAnalysisArtifact):
            raise TypeError("gap_analysis artifact must be GapAnalysisArtifact")
        ...
        if analysis.gaps:
            ...
            try:
                ...
                query = _build_design_query(
                    analysis,
                    max_designs=(ctx.orchestrator.config.max_tasks_per_session),
                ...
                logger.exception("Agent extension design failed")
        ...
        if not designs:
            ...
            designs = _build_fallback_designs(
                analysis.gaps,
                max_capabilities=(ctx.orchestrator.config.max_tasks_per_session),
            )
        ...
        messages = [f"Extension design complete: {len(designs)} design(s)"]
        if designs:
            messages.append("Designs: " + ", ".join(design.extension_name for design in designs))
        if designs:
            path = _persist_extension_designs(
                ctx.orchestrator.paths.runs_dir,
                artifact,
            )
            messages.append(f"扩展设计已保存: {path}")

        yield StageResult(
            artifacts={"extension_design": artifact},
            messages=messages,
        )
...
def _persist_extension_designs(
    ...
    payload = {
        "designs": [asdict(design) for design in artifact.designs],
    ...
    stamped_path = runs_path / (f"extension_design_{int(latest_path.stat().st_mtime * 1000)}.json")
...
async def _build_plan_query(
    ...
    *,
    ...
    experiences_text = "\n".join(f"- [{e.type.value}] {e.topic}: {e.summary}" for e in recent) or "无"
    edit_scope = render_edit_scope("本轮任务规划必须遵守的范围")
...
def _build_design_query(
    ...
    *,
    ...
    for gap in analysis.gaps:
        source_tag = f" (来源/参考对象: {gap.competitor})" if gap.competitor else ""
        gap_lines.append(
...
def _cap_extension_designs(
    ...
    if limit == 0:
        return []
    constraints = [design for design in designs if design.kind == "constraint"]
    capabilities = [design for design in designs if design.kind != "constraint"]
...
def _infer_extension_kind(gap: Gap) -> str:
    ...
    if any(item in text for item in constraint_signals) and any(item in text for item in enforcement_signals):
        return "constraint"
    return "capability"
...
def _source_name_prefix(source: str) -> str:
    ...
    if not normalized or any(item in normalized for item in generic_sources):
        return ""
...
def _build_design(gap: Gap) -> ExtensionDesign:
    ...
    if competitor_slug and feature_slug:
        extension_name = f"{competitor_slug}_{feature_slug}"
    elif feature_slug:
        extension_name = feature_slug
    else:
        extension_name = _slugify(gap.id or "runtime_extension")
    module_base = f"openjiuwen.extensions.harness.{extension_name}"
    ...
    if "rail" in components:
        resources["rails"] = [
            {
                "type": "package",
                "module": (f"{module_base}.rails.extension_rail"),
                "class": "ExtensionRail",
            }
        ]
    if "tool" in components:
        resources["tools"] = [
            {
                "type": "package",
                "module": (f"{module_base}.tools.extension_tool"),
                "class": "ExtensionTool",
            }
        ]
    ...
    return ExtensionDesign(
        ...
        file_plan={
            "root": (f"openjiuwen/extensions/harness/{extension_name}"),
            "manifest": (f"openjiuwen/extensions/harness/{extension_name}/harness_config.yaml"),
        },
...
def _build_fallback_designs(
    ...
    *,
    ...
    constraints = [design for design in designs if design.kind == "constraint"]
    capabilities = [design for design in designs if design.kind != "constraint"][: max(0, max_capabilities)]
