...
async def _run_assess_with_fallback(
    ...
    try:
        return await _assess_with_agent(config, experience_store)
    except Exception:
        logger.warning(
            "Agent assess failed, using fallback",
            exc_info=True,
        )
        return await _fallback_assess(config, experience_store)
...
async def run_assess_stream(
    ...
    *,
    ...
    query = await _build_query(config, experience_store, input_tasks=input_tasks)
...
class MetaAssessStage(AssessStage):
    ...
    async def stream(
        ...
        task_list = list(tasks) if isinstance(tasks, list) else []
        ...
        if assessment.strip():
            ...
            artifacts["assessment"] = AssessmentArtifact(report=assessment)
        yield StageResult(
            artifacts=artifacts,
        )
...
async def _build_query(
    ...
    *,
    ...
    check_strategy = await _detect_python_check_strategy(workspace)
    edit_scope = render_edit_scope("本轮评估需要遵守的可落地变更范围")
    ...
    return (
        ...
        "不要把 `openjiuwen/rsi/harness_rsi/auto_harness/**` 或其他范围外源码目录"
        " 作为本轮建议修改目标。"
...
def _format_task_focus(
    ...
    for task in tasks:
        ...
        lines.append(f"- {task.topic}: {detail}; files={files}")
    return "\n".join(lines)
...
def _format_python_check_strategy(
    ...
    delta_files = list(dict.fromkeys(modified + untracked))
    if delta_files:
...
async def _detect_python_check_strategy(
    ...
    modified = [path for path in changed_since_head if path not in staged_set]
    return _format_python_check_strategy(staged, modified, untracked)
...
async def _assess_with_agent(
    ...
    query = await _build_query(config, experience_store)
    ...
    if not report or len(report) < 100:
        logger.warning(
            "Agent report too short (%d chars), falling back",
...
async def _collect_source_summary(
    ...
    for d in key_dirs:
        ...
        if p.is_dir():
            ...
            lines.append(f"- `{d}/`: {count} .py files")
        else:
...
def _format_experiences(
    ...
    for exp in experiences:
        lines.append(f"- [{exp.type.value}] **{exp.topic}**: {exp.summary or exp.outcome}")
    return "\n".join(lines)
...
def _derive_directions(
    ...
    failures = [exp for exp in experiences if exp.type.value == "failure"]
    if failures:
        topics = {exp.topic for exp in failures}
        lines = [f"- 修复近期失败: {t}" for t in sorted(topics)]
        return "\n".join(lines)
...
class ExtendAssessStage(AssessStage):
    ...
    async def stream(
        ...
        goal = ctx.orchestrator.config.optimization_goal or ""
        ...
        if task_list or goal:
            try:
                ...
                logger.exception("Agent gap analysis failed")
        ...
        artifact = GapAnalysisArtifact(
            gaps=gaps,
            competitor_summary="",
            raw_analysis=(output if output else "heuristic gap analysis"),
        )
        messages = [f"Gap analysis complete: {len(artifact.gaps)} gap(s)"]
        if artifact.gaps:
            messages.append("Gaps: " + "; ".join(_gap_summary(gap) for gap in artifact.gaps))
        yield StageResult(
...
def _build_gap_query(
    ...
    task_summary = "\n".join(f"- {t.topic}: {t.description or t.topic}" for t in tasks)
    query = (
        "当前阶段: assess_ext\n"
...
def _build_gaps(
    ...
    for index, task in enumerate(tasks, start=1):
        gap = Gap(
            ...
            competitor="",
            feature=task.topic,
            current_state=("missing capability absorption workflow"),
            gap_description=task.description or task.topic,
            impact=0.8,
            feasibility=0.8,
            suggested_approach=(task.expected_effect or task.description or task.topic),
