...
def _build_install_env() -> dict[str, str]:
    ...
    if python_path.name.startswith("python"):
        ...
        pathsep = os.pathsep
        env["PATH"] = f"{bin_dir}{pathsep}{existing_path}" if existing_path else bin_dir
    return env
...
def _iter_ci_gate_messages(
    ...
    *,
    ...
    if not gates:
        errors = _summarize_text(ci_result.get("errors", "")) or "未匹配到任何 CI 门禁"
        return [f"{prefix}CI 检查未执行: {errors}"]
    summary = ", ".join((f"{gate.get('name', 'unknown')}={'PASS' if gate.get('passed') else 'FAIL'}") for gate in gates)
    messages = [f"{prefix}CI 结果: {summary}"]
    for gate in gates:
        if gate.get("passed"):
            continue
        detail = _summarize_text(gate.get("output", "")) or "无错误输出"
        messages.append(f"{prefix}[{gate.get('name', 'unknown')}] {detail}")
    return messages
...
def _format_ci_status_for_evaluator(
    ...
    gates = ci_result.get("gates", [])
    if not gates:
        errors = _summarize_text(ci_result.get("errors", "")) or "未执行任何门禁"
        return f"结论: blocking failure\n详情: {errors}"
    lines = ["结论: pass" if ci_result.get("passed") else "结论: blocking failure"]
    for gate in gates:
        status = "PASS" if gate.get("passed") else "FAIL"
        detail = _summarize_text(gate.get("output", ""))
...
def _start_fix_loop(
    *,
    ...
    async def _fixer(errors: str) -> None:
        attempt_state["fix"] += 1
        await _emit_message(f"[修复循环] 第 {attempt_state['fix']} 次修复")
        detail = _summarize_text(errors)
        if detail:
            await _emit_message(f"[修复循环] 修复目标:\n{detail}")
        if agent is None:
            return
        prompt = f"CI 检查失败，请修复以下错误:\n{errors[:3000]}"
        async for chunk in agent.stream({"query": prompt}):
            await chunk_queue.put(chunk)

    async def _ci_runner() -> _CIResult:
        attempt_state["ci"] += 1
        await _emit_message(f"[修复循环] 第 {attempt_state['ci']} 次重跑 CI")
    ...
    async def _evaluator() -> _EvalResult:
        ...
        await _emit_message(f"[修复循环] 进入评审阶段，第 {attempt_state['eval']} 次评审")
        ...
        await _emit_message(f"[修复循环] 评审结果: {'PASS' if approved else 'REJECT'}")
        return _EvalResult(
            approved=approved,
            feedback=output,
        )
    ...
    async def _run_fix() -> Any:
        try:
            ...
            await _emit_message(f"[修复循环] {'修复成功' if fix_result.success else '修复耗尽'}")
    ...
    async def stream(
        ...
        if not ci_result.get("passed"):
            ...
            fix_task, chunk_queue, fix_done, last_ci_result = _start_fix_loop(
                ...
                task=ctx.task,
                agent=(ctx.runtime.fix_agent or ctx.runtime.task_agent),
...
class ExtendVerifyStage(VerifyStage):
    ...
    async def stream(
        ...
        if not isinstance(build, ExtensionBuildArtifact):
            raise TypeError("extension_build artifact must be ExtensionBuildArtifact")
        ...
        for attempt in range(1, max_attempts + 1):
            static_result = await run_static_checks_against_runtime(
                ...
                session_id_prefix=(f"verify_{ctx.orchestrator.runtime.session_id}_{uuid.uuid4().hex[:8]}"),
            )
            ...
            yield ctx.message(
                f"[verify_ext] 结构/静态校验失败，修复扩展实现后重试\n{_summarize_text(error_text, max_chars=800)}"
            )
            ...
            async for chunk in _stream_verify_ext_agent_turn(
                ...
                prompt,
                session_id_prefix=(f"verify-ext-{build.extension_name}-static-fix"),
            ):
        ...
        if static_result.errors:
            ...
            yield StageResult(
                ...
                artifacts={
                    ...
                    "task_result": CycleResult(
                        success=False,
                        error=(f"Extension verify failed: {error_text}"),
                        error_log=error_text,
                    ),
                },
                messages=[f"Extension verify failed: {error_text}"],
                error=error_text,
            )
            return
        ...
        if not acceptance_ok:
            yield StageResult(
                ...
                artifacts={
                    ...
                    "task_result": CycleResult(
                        success=False,
                        error=(f"Extension acceptance tests failed: {acceptance_error}"),
                        error_log=acceptance_error,
                    ),
                },
                messages=[f"Extension acceptance tests failed: {_summarize_text(acceptance_error, max_chars=600)}"],
                error=acceptance_error,
            )
            return
        ...
        yield StageResult(
            ...
            messages=[f"Verified extension scaffold: {build.extension_name}"],
        )
...
async def _run_agent_generated_ext_acceptance(
    *,
    ...
    test_dir = Path(ctx.runtime.wt_path) / ".auto_harness_verify" / build.extension_name
    test_file = test_dir / "test_runtime_extension_acceptance.py"
    python_executable = ctx.orchestrator.config.resolve_ci_gate_python_executable()
    last_error = ""
    max_attempts = 3
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if not test_generated:
            yield ctx.message(f"[verify_ext] 生成 runtime extension 验收测试 (attempt {attempt}/{max_attempts})")
            ...
            async for chunk in _stream_verify_ext_agent_turn(
                agent,
                prompt,
                session_id_prefix=(f"verify-ext-{build.extension_name}-generate"),
            ):
                yield scope_output_event_stage(
                    chunk,
                    "verify_ext",
            ...
            if not test_file.is_file():
                last_error = f"verify_ext_test_not_generated: expected test file at {test_file}"
            else:
                test_generated = True
        ...
        if result.passed:
            yield ctx.message("[verify_ext] runtime extension 验收测试通过")
            yield result
            return
        ...
        yield ctx.message(
            f"[verify_ext] 验收测试失败，修复扩展实现后复跑同一测试\n{_summarize_text(last_error, max_chars=800)}"
        )
        ...
        async for chunk in _stream_verify_ext_agent_turn(
            agent,
            fix_prompt,
            session_id_prefix=(f"verify-ext-{build.extension_name}-fix"),
        ):
            yield scope_output_event_stage(
                chunk,
                "verify_ext",
...
def _build_ext_acceptance_test_prompt(
    *,
    ...
    previous = f"\n\n上一次测试/生成失败信息:\n{previous_error[:4000]}" if previous_error else ""
    ...
    return (
        ...
        "   - 临时目录下的固定名称: 禁止在 tempfile.gettempdir() 下直接创建固定名称的目录。"
        "该目录由机器上所有账号共享，先运行的账号成为属主并保留默认的 0755 权限，"
        "此后其他账号写入时只会得到一个仅指明路径的权限错误。"
        "确需跨进程共享的稳定路径时，在目录名中加入 os.getuid()（无 os.getuid 的平台用登录名）"
        "并将权限设为 0700；否则改用 tempfile.mkdtemp()\n"
        "   ```python\n"
        ...
        "  ```\n"
        "  断言:\n"
        "  - record.refs only contains actually bound items (no status/target/phase)\n"
        "  - 每个 rail 在 labels 中有 rail:<ClassName>\n"
...
async def _run_pytest_file(
    *,
    ...
    if not python_path.is_file():
        return _CIResult(
            passed=False,
            errors=(f"verify_ext_python_executable_not_found: {python_executable}"),
        )
    if python_path.is_file():
        bin_dir = str(python_path.parent)
        env["VIRTUAL_ENV"] = str(python_path.parent.parent)
        pathsep = os.pathsep
        env["PATH"] = f"{bin_dir}{pathsep}{env.get('PATH', '')}" if env.get("PATH") else bin_dir
    proc = await asyncio.create_subprocess_exec(
        python_executable,
        "-m",
        "pytest",
...
def _check_imports(
    ...
    try:
        for py_file in py_files:
            ...
            module_suffix = str(relative.with_suffix("")).replace("/", ".").replace("\\", ".")
            ...
            try:
                ...
                if spec is None or spec.loader is None:
                    errors.append(f"Import failed for {py_file.name}: cannot create import spec")
                    continue
                ...
                errors.append(f"Import failed for {py_file.name}: {exc}")
    finally:
