...
class MergeActivationBlock:
    ...
    async def stream(
        ...
        if package_name:
            ...
            logger.info("[MergeActivate] using pre-generated package_name: %s", merged_name)
        else:
        ...
        logger.info(
            "[MergeActivate] merge success, name={}, tools_count: {}, rails_count: {}, skills_count: {}".format(
                merged_name, result.tools_count, result.rails_count, result.skills_count
            )
        )
...
async def _derive_merged_name(
    ...
    try:
        ...
        if name:
            logger.info("[MergeActivate] agent generated merged name: %s from designs: %s", name, ext_names)
            return name
    except Exception:
...
async def _call_naming_model(config: AutoHarnessConfig, prompt: str) -> str:
    """直接调用模型生成名称。"""
