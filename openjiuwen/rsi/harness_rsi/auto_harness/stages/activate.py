...
class ExtendActivateStage(TaskStage):
    ...
    async def stream(
        ...
        runtime_ext = ctx.require_artifact("runtime_extension")
        verify_report = ctx.get_artifact("verify_report", default={})
        session_runtime_path = str(ctx.orchestrator.ensure_session_runtime_dir())
        runtime_extensions = _runtime_extensions_snapshot(session_runtime_path)
        ...
        yield OutputSchema(
            ...
            payload={
                ...
                "extension_runtime_path": (runtime_ext.runtime_path),
                "config_path": (runtime_ext.config_path),
                "runtime_extensions": runtime_extensions,
                "verify_report": _safe_verify_report(verify_report),
                "components_summary": (_components_summary(verify_report)),
            },
        )

        interaction_id = f"activate:{runtime_ext.extension_name}"
        fut = ctx.orchestrator.create_interaction(interaction_id)
        yield OutputSchema(
            ...
            payload={
                ...
                "extension_name": (runtime_ext.extension_name),
                "runtime_path": (runtime_ext.runtime_path),
                "session_runtime_path": session_runtime_path,
                "extension_runtime_path": (runtime_ext.runtime_path),
        ...
        agent.enqueue_harness_config(runtime_ext.config_path)
        loaded = _preview_extension_components(runtime_ext)
        yield ctx.message(
        ...
        async for chunk in _stream_testing_guide(ctx, runtime_ext, loaded, verify_report):
            payload = chunk.payload
            if isinstance(payload, dict):
                guide_parts.append(payload.get("content", ""))
            yield chunk
...
async def _parse_decision(
    ...
    if isinstance(response, dict):
        ...
        return ActivateDecision(action=action, feedback=feedback)
...
def _preview_extension_components(
    ...
    from openjiuwen.rsi.harness_rsi.auto_harness.infra.runtime_manifest import (
    ...
    if resources is not None:
        for r in resources.rails:
            rails.append(r.class_name or r.name or r.module or "unknown")
        for t in resources.tools:
            tools.append(t.class_name or t.name or t.module or "unknown")
...
def unload_extension(
    ...
    prefix_runtime = f"openjiuwen_runtime_extensions.{session_id}.{runtime_ext.extension_name}"
    prefix_official = f"openjiuwen.extensions.harness.{runtime_ext.extension_name}"
    to_remove = []
...
async def _stream_testing_guide(
    ...
    from openjiuwen.rsi.harness_rsi.auto_harness.agents.factory import (
        create_activate_guide_agent,
    )

    design = ctx.get_artifact("extension_target")
    agent = create_activate_guide_agent(ctx.orchestrator.config)

    query = _build_testing_guide_query(runtime_ext, loaded, verify_report, design)
...
def _build_testing_guide_query(
    ...
    if design and hasattr(design, "gap_id"):
        design_info = (
            f"扩展设计目标: gap_id={design.gap_id}, components={design.components}, file_plan={design.file_plan}"
        )
    verify_info = ""
