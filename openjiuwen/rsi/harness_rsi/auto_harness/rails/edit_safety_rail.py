...
class EditSafetyRail(DeepAgentRail):
    ...
    async def after_tool_call(
        ...
        if count > self._max_files:
            logger.warning(
                "Atomic change limit exceeded: %d files (max %d)",
                count,
                self._max_files,
            )
            ctx.push_steering(
                f"You have modified {count} files (limit is {self._max_files}). Keep changes minimal and focused."
            )
    ...
    async def _run_ruff(
        ...
        try:
            ...
            if proc.returncode != 0 and stdout:
                ...
                ctx.push_steering(f"ruff check found issues in '{file_path}':\n{output}\nPlease fix these issues.")
