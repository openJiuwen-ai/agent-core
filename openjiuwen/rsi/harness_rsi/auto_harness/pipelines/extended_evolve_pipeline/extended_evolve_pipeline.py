...
class ExtendedEvolvePipeline(BasePipeline):
    ...
    async def stream(
        ...
        if self._did_stage_fail(
            ...
            return
        design_artifact = ctx.require_artifact("extension_design")
        if not isinstance(design_artifact, ExtensionDesignArtifact):
            raise TypeError("extension_design artifact must be ExtensionDesignArtifact")

        constraints = [design for design in design_artifact.designs if design.kind == "constraint"]
        capabilities = [design for design in design_artifact.designs if design.kind != "constraint"]
        designs_to_run = (constraints + capabilities)[: ctx.orchestrator.config.max_tasks_per_session]
        logger.info(
        ...
        if verified_tasks:
            activate_results = ctx.orchestrator.results[activate_started_at:]
            activate_failed = any(not result.success for result in activate_results)
            yield _top_stage_result(
                "activate",
        ...
        ctx.put_artifact(
            "session_results",
            SessionResultsArtifact(results=list(ctx.orchestrator.results)),
        )
        logger.info(
...
async def _run_dependency_waves(
    ...
    pending = {design.extension_name: design for design in designs}
    ...
    while pending:
        ...
        for design in skipped:
            ...
            yield ctx.orchestrator.message_output(
                f"Skipped extension {design.extension_name}: failed dependency {', '.join(unmet)}"
            )
        ...
        if not ready:
            for design in list(pending.values()):
                ...
                failed_extensions.add(design.extension_name)
                yield ctx.orchestrator.message_output(
                    f"Skipped extension {design.extension_name}: unresolved dependency {', '.join(unmet)}"
                )
            pending.clear()
            break
...
async def _run_build_verify_wave(
    ...
    async def _worker(design: ExtensionDesign) -> None:
        ...
        if ctx.orchestrator.budget.should_stop:
            wave_results[design.extension_name] = False
            return
        if not ctx.orchestrator.budget.check_task_budget(task_timeout_secs=1.0):
            wave_results[design.extension_name] = False
            return
        ...
        async with semaphore:
            try:
                stream = ExtensionTaskPipeline.run_build_verify_isolated_stream(
                    ctx.orchestrator,
                    design,
        ...
        if len(ctx.orchestrator.results) > result_count:
            wave_results[design.extension_name] = ctx.orchestrator.results[-1].success
            return
        wave_results[design.extension_name] = False

    async def _run_all() -> None:
        try:
            await asyncio.gather(*[asyncio.create_task(_worker(design)) for design in designs])
        finally:
...
def _record_skipped_dependency(
    ...
    ctx.orchestrator.record_cycle_result(
        CycleResult(
            success=False,
            summary=(f"skipped extension {design.extension_name}"),
            error="skipped dependency",
            error_log=(f"Skipped because dependency failed or was unavailable: {', '.join(unmet)}"),
        )
    )
