...
async def prepare_task_runtime(
    ...
    related = await orchestrator.experience_store.search(task.topic)
    wt_path = await orchestrator.worktree_mgr.prepare(task.topic)
    ...
    preexisting_dirty_files = await orchestrator.git.list_dirty_files()
...
class PRTaskPipeline(BasePipeline):
    ...
    async def stream(
        ...
        try:
            async for chunk in self.run_implement_stage_stream(ctx):
                yield chunk

            async for chunk in self.run_verify_stage_stream(ctx):
                yield chunk

            async for chunk in self.run_commit_stage_stream(ctx):
                yield chunk

            async for chunk in self.run_publish_pr_stage_stream(ctx):
                yield chunk
    ...
    def _resolve_task_result(
        ...
        if result is None:
            return CycleResult(
                ...
                error_log=("No cycle result recorded for completed task"),
            )
        if result.success:
