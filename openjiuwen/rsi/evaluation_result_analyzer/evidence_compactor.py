...
_MUTATING_TOOL_PATTERN = re.compile(
    r"(?:^|_)(?:assign|cancel|create|delete|disable|enable|finish|modify|order|patch|save|schedule|send|submit|update|write)(?:_|$)",
    re.IGNORECASE,
)
...
def public_task_contract_snapshot(task: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "tool_schemas": [
            ...
            if isinstance(schema, Mapping) and (normalized := _normalize_tool_schema(schema))
        ],
    }
