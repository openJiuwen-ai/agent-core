...
def run_loop(
    ...
    if config.holdout > 0:
        ...
        if config.verbose:
            logger.info("Split: %d train, %d test (holdout=%s)", len(train_set), len(test_set), config.holdout)
    else:
        train_set = eval_set
        test_set = []
    ...
    for iteration in range(1, config.max_iterations + 1):
        ...
        if paths.live_report_path:
            ...
            paths.live_report_path.write_text(generate_html(partial_output, auto_refresh=True, skill_name=name))
        ...
        if config.verbose:
            ...
            def print_eval_stats(label: str, results: list[dict], elapsed: float) -> None:
                ...
                logger.info(
                    "%s: %d/%d correct, precision=%.0f%% recall=%.0f%% accuracy=%.0f%% (%.1fs)",
                    label,
                    tp + tn,
                    total,
                    precision * 100,
                ...
                for r in results:
                    ...
                    logger.info(
                        "  [%s] rate=%s expected=%s: %s", status, rate_str, r["should_trigger"], r["query"][:60]
                    )
        ...
        if train_summary["failed"] == 0:
            ...
            if config.verbose:
                logger.info("All train queries passed on iteration %d!", iteration)
            break
        ...
        if config.verbose:
            logger.info("Proposed (%.1fs): %s", improve_elapsed, new_description)
    ...
    if config.verbose:
        ...
        logger.info("Best score: %s (iteration %d)", best_score, best["iteration"])
