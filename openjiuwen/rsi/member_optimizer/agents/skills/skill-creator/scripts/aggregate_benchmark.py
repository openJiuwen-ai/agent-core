...
def calculate_stats(values: list[float]) -> dict[str, float]:
    ...
    Returns:
        Dictionary with mean, stddev, min, max.
    """
    if not values:
...
def load_run_results(benchmark_dir: Path) -> dict[str, list]:
    ...
    Returns:
        Dictionary of config name to list of run results.
    """
    # Support both layouts: eval dirs directly under benchmark_dir,
    # or under runs/
    ...
    if runs_dir.exists():
        ...
        logger.warning("No eval directories found in %s or %s", benchmark_dir, benchmark_dir / "runs")
        return {}
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                if not grading_file.exists():
                    logger.warning("grading.json not found in %s", run_dir)
                    continue
                ...
                # Extract timing — check grading.json first, then sibling
                timing = grading.get("timing", {})
                duration_key = "total_duration_seconds"
                result["time_seconds"] = timing.get(duration_key, 0.0)
                ...
                for exp in raw_expectations:
                    if "text" not in exp or "passed" not in exp:
                        logger.warning(
                            "expectation in %s missing required fields (text, passed, evidence): %s", grading_file, exp
                        )
...
def aggregate_results(results: dict[str, list]) -> dict[str, Any]:
    """Aggregate run results into summary statistics.

    Returns run_summary with stats for each configuration and delta.

    Args:
        results: Dictionary of config name to list of run results.

    Returns:
        Dictionary with stats and delta.
    """
    ...
    for config in configs:
        ...
        run_summary[config] = {
            ...
            "tokens": calculate_stats([float(t) for t in tokens]),
        }
