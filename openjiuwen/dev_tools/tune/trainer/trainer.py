...
class Trainer:
    def __init__(self, optimizer: BaseOptimizer, evaluator: BaseEvaluator, **kwargs):
        ...
        TuneUtils.validate_digital_parameter(
            self._num_parallel, "num_parallel", TuneConstant.MIN_PARALLEL_NUM, TuneConstant.MAX_PARALLEL_NUM
        )
        self._early_stop_score = kwargs.get("early_stop_score", TuneConstant.DEFAULT_EARLY_STOP_SCORE)
        TuneUtils.validate_digital_parameter(self._early_stop_score, "early_stop_score", 0.0, 1.0)
        self._callbacks = Callbacks()

    def train(
        self, agent: BaseAgent, train_cases: CaseLoader, val_cases: Optional[CaseLoader] = None, **kwargs
    ) -> Optional[BaseAgent]:
        if not self._check_trainable(agent):
            raise build_error(
                StatusCode.TOOLCHAIN_TRAINER_EXECUTION_ERROR, error_msg="trainer only support current Agent right now"
            )
        ...
        if progress.best_score >= self._early_stop_score:
            logger.info(
                f"val set score {progress.best_score} already exceed target score {self._early_stop_score}, "
                f"skip optimization"
            )
    ...
    def evaluate(
        ...
        score = sum(case.score for case in evaluated_cases) / len(evaluated_cases) if evaluated_cases else 0.0
        return score, evaluated_cases

    def predict(self, agent: BaseAgent, cases: CaseLoader) -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            predicts = executor.map(forward, cases.get_cases())
    ...
    def _pre_train(self, agent: BaseAgent, **kwargs) -> Progress:
        ...
        TuneUtils.validate_digital_parameter(
            max_epoch, "num_iterations", TuneConstant.MIN_ITERATION_NUM, TuneConstant.MAX_ITERATION_NUM
        )
...
class ParameterSearcher:
    ...
    def search_best(
        ...
        candidates.pop(0)
        candidates = random.sample(candidates, k=min(DEFAULT_CANDIDATES_SAMPLE_NUM, len(candidates)))
        best_score = base_score
        best_parameters = base_parameters
        best_cases = None
        logger.info(
            f"start searching best parameter group from {len(candidates)} candidates, "
