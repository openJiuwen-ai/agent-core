...
def _format_layout_appendix_for_model(
    ...
    *,
    ...
    This appendix is merged into ``data['content']``, which is the whole of
    what the default ``Tool.render_for_llm`` shows the model on success; the
    sibling result fields never reach it. The skill's own absolute directory
    therefore has to be rendered here to be readable at all.
...
class SkillTool(Tool):
    ...
    async def invoke(self, inputs: Dict[str, Any], **kwargs) -> ToolOutput:
        ...
        try:
            ...
            # programmatic consumers; the model reads this. ``skill_directory`` is
            # among the keys the model would otherwise never see, so it is rendered
            # into the appendix.
