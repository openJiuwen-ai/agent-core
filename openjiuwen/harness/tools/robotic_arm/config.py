...
"""Runtime settings for the robotic-arm (VLM grounding + auto execution) subagent.
...
``report_plan`` -- turns the sub-task's description into the physical arm/gripper
action for whichever sub-task is ``in_progress``. Grounding the description to a
location, deciding whether to grip, depth estimation, coordinate transform, and
trajectory computation are entirely this object's own responsibility, not the
framework's. ``StepExecutorRail`` drives both halves automatically; the model
never calls a movement tool directly.
"""
...
class SubTaskExecutor(Protocol):
    ...
    def execute(self, frame: Any, sub_task: dict) -> str:
        ...
        submitted it via ``report_plan``: ``id``/``description``/``status`` only --
        no coordinates. Grounding the description to a location on ``frame`` (and
        deciding whether to grip) is entirely up to this implementation.
        """
        ...
