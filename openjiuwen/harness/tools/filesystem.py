...
_OFFICE_WRITE_EXTENSIONS: frozenset = frozenset({".docx", ".xlsx", ".pptx"})
...
def _office_write_unsupported_error(file_path: str, tool_name: str) -> str:
    ...
    return (
        ...
        "Convert to .docx / .xlsx / .pptx first, then retry."
    )
...
def _office_text_lines(content: str) -> list[str]:
    """Normalize Office write input into non-empty plain-text lines.

    ``read_file`` may render Office docs as Markdown with blank separators
    (``\\n\\n``). Writers intentionally do **not** parse that Markdown; they
    rebuild a plain-text container. Skipping blank lines avoids turning
    paragraph separators into real empty paragraphs / empty spreadsheet rows.
    """
...
def _write_office_document(file_path: str, content: str) -> None:
    """Write plain text into an Office container via temp file + atomic replace.

    This is a full overwrite rebuild, not a Markdown round-trip of ``read_file``
    output. Heading markers / table pipes are treated as literal text.
    """
    _, ext = os.path.splitext(file_path.lower())
    if ext not in _OFFICE_WRITE_EXTENSIONS:
    ...
    lines = _office_text_lines(content)
    tmp_path = f"{abs_path}.{os.getpid()}.{threading.get_ident()}.writing"
    try:
        if ext == ".docx":
            from docx import Document
...
class ReadFileTool(Tool):
    ...
    @staticmethod
    def _is_office_doc(file_path: str) -> bool:
        _, ext = os.path.splitext(file_path.lower())
        return ext in _OFFICE_DOC_EXTENSIONS
    ...
    def _is_binary_check_exempt(cls, file_path: str) -> bool:
        ...
        return (
            cls._is_pdf(file_path)
            or cls._is_notebook(file_path)
            or cls._is_image(file_path)
            or cls._is_office_doc(file_path)
        )
    ...
    async def _read_office_doc(self, file_path: str) -> str:
        ...
        try:
            ...
            pkg_map = {".docx": "python-docx", ".xlsx": "openpyxl", ".pptx": "python-pptx"}
            pkg_name = pkg_map.get(ext, exc.name or "unknown")
            raise RuntimeError(
            ...
            raise RuntimeError(
                f"Cannot read Office document '{os.path.basename(file_path)}': "
                "the file is not a valid Office (binary) document or is corrupt. "
                f"Details: {exc}"
            ) from exc
