...
def _build_batch_interact_script(payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "    });\n"
        "  };\n"
        "  const probeOutcome = async (work, timeoutMs) => {\n"
        ...
        "          (error) => ({\n"
        "            status: 'error',\n"
        "            error: compactText(error && error.message ? error.message : error, 240),\n"
        "          })\n"
        "        ),\n"
        "      page.waitForTimeout(timeoutMs).then(() => ({ status: 'timeout' })),\n"
        "    ]);\n"
        "  };\n"
        "  const diagnoseClick = async (locator, actionTimeout) => {\n"
        "    const probeTimeout = Math.max(250, Math.min(750, Math.floor(actionTimeout / 4)));\n"
        "    const [countProbe, visibleProbe, enabledProbe, boxProbe, domProbe, rafProbe] =\n"
        "      await Promise.all([\n"
        "        probeOutcome(() => locator.count(), probeTimeout),\n"
        "        probeOutcome(() => locator.isVisible(), probeTimeout),\n"
        "        probeOutcome(() => locator.isEnabled({ timeout: probeTimeout }), probeTimeout),\n"
        "        probeOutcome(() => locator.boundingBox({ timeout: probeTimeout }), probeTimeout),\n"
        "        probeOutcome(() => locator.evaluate((element) => {\n"
        ...
        "          } : null;\n"
        "          const hitNodes = point ? document.elementsFromPoint(point.x, point.y) : [];\n"
        "          const hitTarget = hitNodes.length ? hitNodes[0] : null;\n"
        ...
        "            ? element.getAnimations({ subtree: true }).slice(0, 5).map((animation) => ({\n"
        "                play_state: String(animation.playState || ''),\n"
        "                pending: !!animation.pending,\n"
        "                current_time: typeof animation.currentTime === 'number'\n"
        ...
        "            click_point: point,\n"
        "            receives_events: hitTarget\n"
        "              ? (hitTarget === element || element.contains(hitTarget))\n"
        "              : null,\n"
        "            hit_target: describeNode(hitTarget),\n"
        "            element: {\n"
        ...
        "          };\n"
        "        }), probeTimeout),\n"
        "        probeOutcome(() => locator.evaluate((element) => new Promise((resolve) => {\n"
        "          const samples = [];\n"
        ...
        "            resolve({\n"
        "              frames_observed: samples.length,\n"
        "              stable: (\n"
        ...
        "        })), probeTimeout),\n"
        "      ]);\n"
        "    const valueOf = (probe) => probe.status === 'ok' ? probe.value : null;\n"
        "    const dom = valueOf(domProbe);\n"
        "    const raf = valueOf(rafProbe);\n"
        "    const probeStatus = {};\n"
        "    for (const [name, probe] of Object.entries({\n"
        "      count: countProbe, visible: visibleProbe, enabled: enabledProbe,\n"
        "      bounding_box: boxProbe, dom: domProbe, animation_frames: rafProbe,\n"
        "    })) {\n"
        "      probeStatus[name] = probe.status === 'error'\n"
        "        ? { status: probe.status, error: probe.error }\n"
        "        : { status: probe.status };\n"
        "    }\n"
        "    return {\n"
        "      probe_timeout_ms: probeTimeout,\n"
        "      attached: countProbe.status === 'ok' ? Number(countProbe.value) > 0 : null,\n"
        "      locator_count: valueOf(countProbe),\n"
        "      visible: valueOf(visibleProbe),\n"
        "      enabled: valueOf(enabledProbe),\n"
        "      stable: raf ? !!raf.stable : null,\n"
        "      stable_source: 'three_requestAnimationFrame_bounding_box_samples',\n"
        "      receives_events: dom ? dom.receives_events : null,\n"
        "      receives_events_source: 'center_elementsFromPoint_approximation',\n"
        "      bounding_box: valueOf(boxProbe),\n"
        "      animation_frames: raf,\n"
        "      page: dom ? {\n"
        "        document_visibility: dom.document_visibility,\n"
        "        document_hidden: dom.document_hidden,\n"
        "        viewport: dom.viewport,\n"
        "        scroll: dom.scroll,\n"
        "      } : null,\n"
        "      element: dom ? dom.element : null,\n"
        "      in_viewport: dom ? dom.in_viewport : null,\n"
        "      click_point: dom ? dom.click_point : null,\n"
        "      hit_target: dom ? dom.hit_target : null,\n"
        "      animations: dom ? dom.animations : [],\n"
        ...
        "      if (op === 'click') {\n"
        "        clickLocator = locatorFromStep(step);\n"
        ...
        "      if (op === 'click' && clickLocator) {\n"
        "        try {\n"
        "          item.actionability = await diagnoseClick(clickLocator, timeout);\n"
        "        } catch (diagnosticError) {\n"
        "          item.actionability = {\n"
        "            diagnostic_error: compactText(\n"
        "              diagnosticError && diagnosticError.message ? diagnosticError.message : diagnosticError,\n"
        "              240\n"
        "            ),\n"
        "          };\n"
        "        }\n"
        ...
        for item in completed_steps:
            if isinstance(item, dict):
                ...
                if isinstance(actionability, Mapping):
                    actionability_summary = json.dumps(
                        actionability,
                        ensure_ascii=True,
                        separators=(",", ":"),
                        default=str,
                    ...
                    browser_agent_log_info(
                        "[BROWSER_BATCH] click_diagnostics index=%s visible=%s enabled=%s "
                        "stable=%s receives_events=%s details=%s",
                        item.get("index"),
                        actionability.get("visible"),
                        actionability.get("enabled"),
                        actionability.get("stable"),
                        actionability.get("receives_events"),
