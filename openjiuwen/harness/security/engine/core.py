...
class PermissionEngine:
    ...
    async def check_permission(
        ...
        if (
            self.config.get("findings_escalate") is not False
            and not matched_rule_uses_approval_override(matched_rule)
        ):
            permission = escalate_with_findings(
                permission,
                findings,
                mode=str(self.config.get("permission_mode") or "normal"),
            )
