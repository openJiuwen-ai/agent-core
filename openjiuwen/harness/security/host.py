...
class ToolPermissionHost:
    ...
    :func:`openjiuwen.harness.security.approve.persist.write_permissions_section_to_agent_config_yaml`
    ...
    resolve_workspace_dir: Callable[[], Path | None] | None = None
    """当前任务 workspace（file_guard 区内路径）；不是 agent 数据根。

    返回 ``None`` 表示本轮无法解析任务目录；rail 应保留引擎已绑定的 workspace，
    不得回退到 agent 数据根。
    """
