...
class PermissionConfirmResponse:
    ...
    - ``approved and auto_confirm and persist_allow``：永久记住，走 ``persist_allow_rule``。
    - ``approved and auto_confirm and not persist_allow``：会话内记住，走 ``persist_session_allow_rule``
      （并仍可写入 session state）。
