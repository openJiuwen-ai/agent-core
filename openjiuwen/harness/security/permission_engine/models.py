...
引擎 ``PermissionEngine.config`` 等。工具级策略由 ``tool_policy.evaluate_tiered_policy`` 评估。
"""
...
class PermissionConfirmResponse:
    ...
    - ``approved and auto_confirm and persist_allow is True``：永久记住，走 ``persist_allow_rule``。
    - ``approved and auto_confirm and persist_allow is False``：会话内记住，走 ``persist_session_allow_rule``。
    - ``approved and auto_confirm and persist_allow is None``：旧宿主未传该字段，按永久记住。
    ...
    def wants_permanent_persist(self) -> bool:
        if not self.approved or not self.auto_confirm:
            return False
        if self.persist_allow is None:
            return True
        return bool(self.persist_allow)

    def wants_session_persist(self) -> bool:
        return bool(self.approved and self.auto_confirm and self.persist_allow is False)
...
class NetGuardSection(TypedDict, total=False):
    """抓取网络防护配置（Pipeline C）。仅 ``allow`` / ``deny``，无 ASK。"""
...
class PermissionsSection(TypedDict, total=False):
    ...
    - 工具级策略由 :func:`openjiuwen.harness.security.permission_engine.toolguard.tool_policy.evaluate_tiered_policy` 评估。
    ...
    net_guard: NotRequired[NetGuardSection]
    shell_guard: NotRequired[ShellGuardSection]
