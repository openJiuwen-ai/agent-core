...
from openjiuwen.harness.security.common.builtin_platforms import filter_entries_for_platform
...
from openjiuwen.harness.security.toolguard.patterns import PathMatcher, match_wildcard
...
# Re-exported for jiuwenswarm (`_PATH_TOOLS`) and in-tree callers.
_SHELL_TOOLS = SHELL_TOOLS
_PATH_TOOLS = PATH_TOOLS
_NETWORK_TOOLS = NETWORK_TOOLS
...
def _package_builtin_rules_path() -> Path:
    from openjiuwen import harness as _harness

    return Path(_harness.__file__).resolve().parent / "resources" / "builtin_rules.yaml"
...
def tiered_policy_rule_matches(
    ...
    if cat == "path":
        ...
        return False
    if cat == "network":
        from openjiuwen.harness.security.toolguard.network_guard import network_url_text
