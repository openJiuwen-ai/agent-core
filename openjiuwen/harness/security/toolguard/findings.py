...
from openjiuwen.harness.security.toolguard.catalog import SHELL_TOOLS
...
def escalate_with_findings(
    ...
    *,
    ...
    INFO 仅展示、不升级。``permission_mode=strict``：LOW+；否则（normal）：MEDIUM+。
    Host 可通过 ``findings_escalate: false`` 跳过升级。
    """
    ...
    if mode_l in {"off", "none", "disabled"}:
        return permission
    if mode_l == "strict":
