...
# Some factories register extra default rails that the disposable
# PersonalContext sandbox agent must not carry.  ``agent_ras=False`` opts out
# of AgentRASRail when the parameter exists; LLMStabilityRail (a.k.a.
# ModelAnomalyDetectionRail) is hardwired on some factory versions and is
# dropped from the pending rail list after creation.
_UNREGISTER_DEFAULT_RAIL_TYPES = (LLMStabilityRail,)


def _build_sandbox_agent_kwargs() -> dict[str, Any]:
    kwargs = {_DEFAULT_RETRY_RAIL_FLAG: False}
    if "agent_ras" in inspect.signature(create_deep_agent).parameters:
        kwargs["agent_ras"] = False
    return kwargs


def _drop_unwanted_default_rails(agent: object) -> None:
