...
DEFAULT_BROWSER_AGENT_SYSTEM_PROMPT_EN = (
    ...
    "browser_snapshot only when compact probes are insufficient. "
    "If an older result has a <persisted-output> marker, recall it only when its preview "
...
DEFAULT_BROWSER_AGENT_SYSTEM_PROMPT_CN = (
    ...
    "旧结果出现 <persisted-output> 且预览不足时才恢复；"
    "导航后恢复内容中的目标不可继续操作。\n"
