...
class SignalInterruptKind(str, Enum):
    """Permission-interrupt origin of a lifecycle signal.

    ``None`` on the Signal means "not an interrupt pseudo-record"; detectors
    treat anything non-None as a candidate to skip from history.
    """

    PERMISSION_ASK_SIGNAL = "permission_ask_signal"
    PERMISSION_ASK_REDO = "permission_ask_redo"
...
class Signal:
    ...
    interrupt_kind: SignalInterruptKind | None = None
        ...
        *,
        ...
        for field in optional_fields:
            ...
            if value is not None:
                payload[field] = value
        if self.interrupt_kind is not None:
            payload["interrupt_kind"] = self.interrupt_kind.value
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Signal":
        ...
        interrupt_kind_raw = data.get("interrupt_kind")
        interrupt_kind = (
            SignalInterruptKind(interrupt_kind_raw)
            if isinstance(interrupt_kind_raw, str)
            else interrupt_kind_raw
        )
        return cls(
            kind=kind,
