...
robustness_prompts_cn: dict[str, str] = {
    ...
    "repeat_tool_generic_warning": (
        "{member_name} 的工具 {tool_name} 已重复调用 {count} 次，请检查是否有效。"
    ),
    "repeat_tool_generic_user": (
        "[工具调用异常] {member_name} 的工具 {tool_name} 已重复调用 {count} 次。"
    ),
    ...
    "repeat_tool_pingpong_warning": (
        "检测到 {member_name} 的工具 {tool_name} 与另一工具的交替循环（{count} 轮），"
        "请合并步骤或更换策略。"
    ),
    "repeat_tool_pingpong_user": (
        "[工具调用异常] {member_name} 的 Ping-Pong 交替调用已持续 {count} 轮。"
    ),
    ...
    "repeat_tool_unknown_tool_warning": (
        "{member_name} 的未知工具 {tool_name} 已连续失败 {count} 次，请停止重试。"
    ),
    "repeat_tool_unknown_user": (
        "[工具调用异常] {member_name} 的工具 {tool_name} 已连续失败 {count} 次。"
    ),
    ...
    "repeat_tool_global_breaker_critical": (
        "全局断路器: {member_name} 的工具 {tool_name} 连续 {count} 次无进展"
    ),
    "repeat_tool_pingpong_critical": (
        "Ping-Pong 循环: {member_name} 共 {count} 轮交替无进展，阻断"
    ),
    "repeat_tool_unknown_tool_critical": (
        "{member_name} 的未知工具 {tool_name} 连续调用 {count} 次，停止重试"
    ),
...
robustness_prompts_en: dict[str, str] = {
    ...
    "repeat_tool_generic_warning": (
        "{member_name}'s tool {tool_name} has been repeated {count} times; "
        "please verify it is effective."
    ),
    "repeat_tool_generic_user": (
        "[Tool Call Anomaly] {member_name}'s tool {tool_name} "
        "has been repeated {count} times."
    ),
    "repeat_tool_generic_steering": (
    ...
    "repeat_tool_pingpong_warning": (
        "Detected ping-pong alternation between {member_name}'s tool {tool_name} "
        "and another tool ({count} rounds). Merge steps or change strategy."
    ),
    "repeat_tool_pingpong_user": (
        "[Tool Call Anomaly] {member_name}'s ping-pong alternating calls "
        "have continued for {count} rounds."
    ),
    "repeat_tool_pingpong_steering": (
    ...
    "repeat_tool_unknown_tool_warning": (
        "{member_name}'s unknown tool {tool_name} has failed {count} times "
        "in a row; please stop retrying."
    ),
    "repeat_tool_unknown_user": (
        "[Tool Call Anomaly] {member_name}'s tool {tool_name} "
        "has failed {count} times in a row."
    ),
    "repeat_tool_unknown_steering": (
    ...
    "repeat_tool_global_breaker_critical": (
        "Circuit breaker: {member_name}'s tool {tool_name} made no progress "
        "for {count} consecutive calls"
    ),
    "repeat_tool_pingpong_critical": (
        "Ping-pong loop: {member_name} had {count} alternating calls "
        "with no progress, blocked"
    ),
    "repeat_tool_unknown_tool_critical": (
        "{member_name}'s unknown tool {tool_name} called {count} times "
        "in a row, stopping retries"
    ),
    "thinking_loop_lock_judge_intro": (
...
def _repeat_tool_fields(anomaly: Anomaly) -> dict[str, Any]:
    ...
    tool = str(evidence.get("tool_name") or "").strip()
    if not tool:
        # 兼容旧 anomaly：summary 形如 "... on read_file"
        summary = anomaly.summary or ""
        marker = " on "
        if marker in summary:
            tool = summary.rsplit(marker, 1)[-1].strip()
    return {
        "member_name": anomaly.member_name or "",
        "tool_name": tool or anomaly.member_name or "",
