...
class AgentRASMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self._abort_raise_armed = False
        self._notice_emitted_immediate: str | None = None
        self._language = "cn"

    @property
    def abnormal_committed(self) -> bool:
        return self._abnormal_committed

    @property
    def should_abort_stream(self) -> bool:
        return self._abnormal_committed and self._abort_raise_armed

    def arm_stream_abort_raise(self) -> None:
        """Raise on the next inspect_stream_chunk call (after truncated write)."""
        self._abort_raise_armed = True
    ...
    def bind_ctx(self, ctx: AgentCallbackContext | None) -> None:
        ...
        self._bind_repeat_tool_ability_manager(ctx)
    ...
    def _bind_repeat_tool_ability_manager(
        self,
        ctx: AgentCallbackContext | None,
    ) -> None:
        """Inject ``ctx.agent.ability_manager`` into RepeatTool detectors."""
        ...
        for detector in self._detectors:
            setter = getattr(detector, "set_ability_manager", None)
            if callable(setter):
                setter(ability_manager)
    ...
    async def on_stream_chunk(
        ...
        L1/L2 (``text_repetition``) abort immediately via inspector raise
        on the next stream chunk. L3 (``plan_execution``) keeps suppressing
        while a Reviewer skill runs; confirmed abnormal then aborts,
        otherwise fail-open flushes the buffer.
        """
        self.bind_ctx(ctx)
    ...
    async def _apply_abnormal_recovery(
        ...
        if self._executor is None or self._abnormal_committed:
            return
        # Prefer the invoke-bound ctx so steering / notice stay aligned
        # (ZZP-001). Stream abort is via inspector raise after this commit.
        # Cancel before commit must leave pending intact for
    ...
    def reset_detectors(self) -> None:
        ...
        Passes bound ``session`` / ``_resume_continuation`` when the detector
        ``reset`` accepts them (e.g. RepeatTool user-turn scope). Other
        detectors keep a no-arg ``reset()``.
        """
        ...
        resume_continuation = bool(
            isinstance(extra, dict) and extra.get("_resume_continuation")
        )
        for detector in self._detectors:
            try:
                self._call_detector_reset(
                    detector,
                    session=session,
    ...
    def _call_detector_reset(
        ...
        *,
        ...
        if not callable(reset):
            return
        try:
            params = inspect.signature(reset).parameters
        ...
        if "resume_continuation" in params:
            kwargs["resume_continuation"] = resume_continuation
        if kwargs:
            reset(**kwargs)
        else:
            reset()
