...
DESCRIPTION: Dict[str, str] = {
    "cn": (
        ...
        "务必根据实际文件编码指定读取方式：\n"
        " - `powershell`：优先使用 `-Encoding utf8`（适用于 UTF-8 文件）；如确知文件为 GBK，则用 `-Encoding Default`；\n"
        "   更可靠的方式：`[System.IO.File]::ReadAllText(path, [Text.Encoding]::UTF8)` "
        "   或 `[System.IO.File]::ReadAllText(path, [Text.Encoding]::GetEncoding('GB18030'))`。\n"
        " - `bash`/`sh`（Git Bash）：用 `file -i filename` 检测编码，再用 `iconv` 转换；\n"
        " - `cmd`：用 `chcp` 检查代码页，再用 `type file` 读取。\n"
        "系统的输出编码处理层已自动支持 GB18030 回退解码——你只需专注于以正确的编码读取文件即可。\n"
        "如果输出出现乱码，优先尝试添加 `-Encoding utf8` 重新读取。\n"
        "\n"
        ...
        "\n"
        "# 使用说明\n"
        " - 丢弃输出（重定向到空设备）：bash/sh 用 `/dev/null`（如 `cmd > /dev/null`），"
        "**不要用 `>nul`**——`>nul` 是 CMD 语法，在 Git Bash/MSYS 下不会指向空设备，"
        "而是创建一个无法删除的 `nul` 保留名文件。确需 `>nul` 时改用 shell_type='cmd'；"
        "PowerShell 丢弃输出用 `> $null`。\n"
    ...
    "en": (
        ...
        "misinterpreted as GBK, producing garbled output.\n"
        "Always specify the correct encoding based on the actual file encoding:\n"
        " - `powershell`: prefer `-Encoding utf8` (for UTF-8 files); use `-Encoding Default` if file is known GBK;\n"
        "   more robust: `[System.IO.File]::ReadAllText(path, [Text.Encoding]::UTF8)` or\n"
        "   `[System.IO.File]::ReadAllText(path, [Text.Encoding]::GetEncoding('GB18030'))`.\n"
        " - `bash`/`sh` (Git Bash): use `file -i filename` to detect encoding, then `iconv` to convert;\n"
        " - `cmd`: check code page with `chcp`, then use `type file` to read.\n"
        "The output encoding layer already handles GB18030 fallback decoding automatically —\n"
        "you only need to focus on reading the file with the correct encoding.\n"
        "If output contains mojibake, first try re-reading with `-Encoding utf8`.\n"
        "\n"
        ...
        "\n"
        "# Instructions\n"
        " - To discard output (redirect to the null device): use `/dev/null` for "
        "bash/sh (e.g., `cmd > /dev/null`) — **never `>nul`**. `>nul` is CMD syntax; "
        "under Git Bash/MSYS it does not target the null device but creates an "
        "undeletable `nul` reserved-name file. If you need `>nul`, use "
        "shell_type='cmd'; PowerShell discards output with `> $null`.\n"
