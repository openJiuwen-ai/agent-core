...
入参为 JSON 数组（每个任务必须包含 id 字段）：
    {"tasks": [
        {"id": "translate_doc", "content": "翻译文档", "activeForm": "正在翻译文档",
         "description": "将文档翻译为目标语言", "selected_model_id": "fast"},
        {"id": "analyze_arch", "content": "分析代码架构", "activeForm": "正在分析代码架构",
         "description": "梳理代码模块结构与依赖关系", "selected_model_id": "smart"}
    ]}

## 规则

- **id 字段为必填**：必须为每个任务指定简短、语义清晰的唯一字符串 ID
  （如 "translate_doc"、"analyze_code"），禁止使用随机字符或 UUID。同一会话内 ID 不得重复。
  此 ID 在后续 todo_modify 中用于精准定位任务，是跨轮次更新状态的唯一依据
...
Create a todo list for the current session to track progress, organize complex tasks,
and help the user understand overall execution status.
...
Input is a JSON array (each task must include an id field):
    {"tasks": [
        {"id": "translate_doc", "content": "Translate document", "activeForm": "Translating document",
         "description": "Translate the document into the target language", "selected_model_id": "fast"},
        {"id": "analyze_arch", "content": "Analyze code architecture",
         "activeForm": "Analyzing code architecture",
         "description": "Map out module structure and dependencies", "selected_model_id": "smart"}
    ]}

## Rules

- **id is required**: You must provide a short, semantically meaningful unique string ID for each task
  (e.g. "translate_doc", "analyze_code"). Do NOT use random characters or UUIDs.
  IDs must be unique within a session. This ID is used by todo_modify to precisely locate tasks
  and is the sole key for cross-turn status updates
...
- When the currently available model information is not obtained, the selected_model_id field
  should not be added; otherwise, selected_model_id must be added to specify the model ID
...
append：在列表末尾追加新任务（id 由你指定，须简短语义且唯一）：
    {
        "action": "append",
        "todos": [
            {"id": "write_report", "content": "新任务内容", "activeForm": "执行新任务",
             "description": "任务的详细描述", "status": "pending"}
        ]
    }
...
insert_after：在指定任务之后插入新任务（目标任务状态须为 in_progress 或 pending）：
    {
        "action": "insert_after",
        "todo_data": {
            "target_id": "translate_doc",
            "items": [{
                "id": "review_translation", "content": "插入的任务", "activeForm": "执行插入的任务",
                "description": "任务的详细描述", "status": "pending", "selected_model_id": "fast"
            }]
        }
    }
...
insert_before：在指定任务之前插入新任务（目标任务状态须为 pending）：
    {
        ...
        "todo_data": {
            "target_id": "analyze_code",
            "items": [{
                "id": "setup_env", "content": "插入的任务", "activeForm": "执行插入的任务",
                "description": "任务的详细描述", "status": "pending"
            }]
        }
    }
...
Modify todo items for the current session. Supports batch operations —
consolidate multiple changes into a single call whenever possible.
...
Support modifying selected_model_id: If the task's selected_model_id is not empty and the execution
result is of poor quality (inaccurate output, logical errors, or failure to meet expectations),
the model ID should be updated according to the model description to a higher-quality model:
    {
        "action": "update",
        "todos": [
            {"id": "translate_doc", "selected_model_id": "smart", "status": "pending"}
...
append: Add new tasks at the end of the list (id must be a short semantic string you choose, unique within the session):
    {
        "action": "append",
        "todos": [
            {"id": "write_report", "content": "New task content", "activeForm": "Executing new task",
             "description": "Detailed description of the task", "status": "pending"}
        ]
    }
...
insert_after: Insert new tasks after the specified task (target must be in_progress or pending):
    {
        ...
        "todo_data": {
            "target_id": "translate_doc",
            "items": [{
                "id": "review_translation", "content": "Inserted task",
...
insert_before: Insert new tasks before the specified task (target must be pending):
    {
        ...
        "todo_data": {
            "target_id": "analyze_code",
            "items": [{
                "id": "setup_env", "content": "Inserted task", "activeForm": "Executing inserted task",
                "description": "Detailed description of the task", "status": "pending"
            }]
        }
    }
...
返回：完整的任务信息，包括 id、content（任务摘要）、activeForm、description（任务详细内容）、
status、depends_on、result_summary、meta_data、selected_model_id。
"""
...
Returns: complete task info including id, content (task summary), activeForm,
description (detailed content), status, depends_on, result_summary, meta_data,
selected_model_id.
"""
