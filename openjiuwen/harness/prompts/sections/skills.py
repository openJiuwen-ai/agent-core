...
def build_skill_line(
    *,
    ...
    It replaces the former ``skill_md_path``, which rendered a ``Path:`` line
    naming the ``SKILL.md`` file. No caller passed it once ``skill_tool`` became
    the way a skill body is fetched, and the directory is the path the model
    actually has to join relative references onto.
    """
