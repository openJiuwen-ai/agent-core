...
DESCRIPTION: Dict[str, str] = {
    "cn": (
        ...
        "当任务只是把磁盘上已有的文本类文件做合并/拼接"
        "（无需推理改写）时，尤其在上下文资源紧张的情况下，"
        "推荐使用本工具在磁盘侧直接完成。\n\n"
        "多语言内容处理规则：\n"
    ...
    "en": (
        ...
        "When the task is only merging/concatenating existing on-disk text-like files "
        "with no reasoning or rewriting needed, especially under tight "
        "context, prefer completing it on disk with this tool.\n\n"
