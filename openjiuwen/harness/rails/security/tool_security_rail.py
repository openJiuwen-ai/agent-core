...
_FAIL_CLOSED_DEFAULTS = {"*": "ask"}
_FAIL_CLOSED_FILE_GUARD = {
    "enabled": True,
    "defaults": {"read": "ask", "write": "ask", "exec": "ask"},
}


def _materialize_baked_permissions(raw: dict[str, Any]) -> dict[str, Any]:
    """Pass through Host-baked config. Incomplete input Fail-Closes; never expands product modes."""
    cfg = deepcopy(raw) if isinstance(raw, dict) else {}
    has_defaults = isinstance(cfg.get("defaults"), dict)
    has_file_guard = isinstance(cfg.get("file_guard"), dict)
    if has_defaults and has_file_guard:
        return cfg
    logger.error(
        "[PermissionEngine] permission.rail.unbaked_config fail_closed "
        "has_defaults=%s has_file_guard=%s",
        has_defaults,
        has_file_guard,
    )
    if not has_defaults:
        cfg["defaults"] = dict(_FAIL_CLOSED_DEFAULTS)
    if not has_file_guard:
        cfg["file_guard"] = deepcopy(_FAIL_CLOSED_FILE_GUARD)
    return cfg
...
class PermissionInterruptRail(ConfirmInterruptRail):
    ...
    def __init__(
        ...
        self._static_config = _materialize_baked_permissions(raw)
        self.sandbox_intent = self._static_config.get("sandbox_intent")
        self.permission_mode = self._static_config.get("mode")
    ...
    def update_config(
        ...
        """Hot-update static permission config. Caller must pass already-baked permissions."""
        cfg_dict = _materialize_baked_permissions(cast(dict[str, Any], config))
        self._static_config = cfg_dict
        self.sandbox_intent = cfg_dict.get("sandbox_intent")
        self.permission_mode = cfg_dict.get("mode")
