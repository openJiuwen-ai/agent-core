...
"""Rail that attaches the published PCS context description for one model call."""
...
def _warn(operation: str, exc: BaseException | None = None) -> None:
    ...
    if exc is None:
        logger.warning("[PCSContextRail] %s", operation)
    else:
        logger.warning("[PCSContextRail] %s failed (%s)", operation, type(exc).__name__)
...
class PCSContextRail(DeepAgentRail):
