...
dev-stable 适配说明（相对 0708 ``jiuwenclaw...deep_agent/rails/skill_authorization_rail.py``）：

- 基类为 agent-core ``openjiuwen.harness.rails.interrupt.interrupt_base.BaseInterruptRail``；
  ``InterruptRequest`` 无 ``allow_auto_confirm`` 字段，审批请求不再携带该参数；
- ``PermissionEngine`` 经构造注入（不再回落 0708 的进程级 ``get_permission_engine``）；
  未注入时策略评估按失败处理（fail-closed，主 Agent 交还原权限 Rail，子 Agent 拒载）；
- DENY 预判改用 agent-core ``PermissionEngine.evaluate_global_policy_directly``
  （无 channel/session/overlay 参数；agent-core 引擎本身不感知 Skill overlay）；
- trust 判定通过调用方注入的可信 Skill 目录完成，core 不感知产品安装账本；
- file_guard 差分适配 native ``paths`` 条目口径（移植版
  ``effective_file_guard_axis_level(paths, raw_path, axis, *, defaults, workspace_root)``），
  不再依赖 0708 legacy ``file_guard.global`` 归一化助手；
- 正文 hash 复核改为磁盘重读 ``SKILL.md``（dev-stable 无
  ``context_engine.active_skill_bodies``，skill_tool 也不写 ``is_skill_body`` metadata；
  加载成功信号由移植版 ``skill_lifecycle_events`` 派生）。
"""
...
PENDING_SKILL_APPROVAL_CONTEXT_KEY = "jiuwenswarm_pending_skill_approval_contexts"
SKILL_AUTHORIZATION_CONTEXT_TOKEN_KEY = "jiuwenswarm_skill_authorization_context_token"
...
SessionConfigMerger = Callable[[dict[str, Any], str | None], dict[str, Any]]
SceneBypass = Callable[[], bool]
...
def build_skill_registry_resolver(
    ...
    *,
    ...
    """把 SkillUseRail 注册表访问器适配为 ``SkillResolver``。

    主 Agent 与子 Agent 共用；来源取目录绝对路径，注册表提供版本时一并纳入
    Manifest 身份，缺失时才返回 ``None``。

    dev-stable 注意：agent-core ``SkillUseRail.skills_meta`` 是 property（0708 为
    ``get_skills_meta()`` 方法），调用方应传 ``lambda: skill_use_rail.skills_meta``。
    """
...
def build_default_trust_resolver(
    prebuilt_dirs_provider: Callable[[], Any] | None = None,
    *,
    builtin_dirs_provider: Callable[[], Any] | None = None,
) -> TrustResolver:
    """默认可信判定：调用方声明的内置或预置目录视为 ``builtin``。

    判定失败一律按 ``other``（更严格）。
    """

    def resolve(skill_dir: Path) -> SkillTrustLevel:
        try:
            resolved = Path(skill_dir).resolve()
        except Exception:  # noqa: BLE001 — 判定失败一律按 other（更严格）
            logger.warning(
                "[skill_authorization] trust resolve failed, fallback=other",
                exc_info=True,
            )
            return SkillTrustLevel.OTHER
        for provider, label in (
            (builtin_dirs_provider, "builtin"),
            (prebuilt_dirs_provider, "prebuilt"),
        ):
            if provider is None:
                continue
            try:
                raw_dirs = provider() or []
                if isinstance(raw_dirs, (str, Path)):
                    raw_dirs = [raw_dirs]
                for raw_dir in raw_dirs:
                    root = Path(str(raw_dir)).resolve()
                    if resolved == root or root in resolved.parents:
                        return SkillTrustLevel.BUILTIN
            except Exception:  # noqa: BLE001 — 判定失败不放宽
                logger.warning(
                    "[skill_authorization] %s dirs resolve failed, fallback=other",
                    label,
                    exc_info=True,
                )
        return SkillTrustLevel.OTHER
...
def _default_session_config_merger(
...
class SkillAuthorizationRail(BaseInterruptRail):
    ...
    ``engine`` 由调用方构造注入（与 PermissionInterruptRail 同一实例）；未注入时
    DENY 预判不可用，按策略评估失败处理（fail-closed）。
    """
    ...
    def __init__(
        ...
        *,
        ...
        session_config_merger: SessionConfigMerger | None = None,
        scene_bypass: SceneBypass | None = None,
        agent_scope_id: str = MAIN_AGENT_SCOPE_ID,
    ) -> None:
        super().__init__(tool_names=[SKILL_TOOL_NAME, SKILL_COMPLETE_TOOL_NAME])
        self._engine = engine
        self._skill_resolver = skill_resolver
        self._trust_resolver = (
            trust_resolver
            if trust_resolver is not None
            else build_default_trust_resolver(
                prebuilt_dirs_provider,
                builtin_dirs_provider=builtin_dirs_provider,
            )
        )
        ...
        self._session_config_merger = (
            session_config_merger or _default_session_config_merger
        )
        self._scene_bypass = scene_bypass
    ...
    def _preserve_legacy_scene(self) -> bool:
        """产品声明的既有专用裁决场景，动态授权不得接管。"""
        if self._scene_bypass is None:
            return False
        try:
            return bool(self._scene_bypass())
    ...
    @staticmethod
    def _native_file_guard_parts(config: dict[str, Any]) -> tuple[list[Any], Any]:
        """native file_guard 的 ``paths`` 条目列表与 ``defaults``（缺失时空列表 / None）。"""
        fg = config.get("file_guard") if isinstance(config.get("file_guard"), dict) else {}
        paths = fg.get("paths")
        return (paths if isinstance(paths, list) else []), fg.get("defaults")
    ...
    def _file_guard_adjudication_view(
        ...
        """裁决口径的 file_guard 视图（dev-stable native ``paths`` 形态）。

        返回 ``(base_paths, merged_paths, base_defaults, merged_defaults,
        workspace_root)``，供差分按移植版 ``effective_file_guard_axis_level``
        的真实裁决语义计算 before/after。
        """
        base_paths, base_defaults = self._native_file_guard_parts(base)
        merged_paths, merged_defaults = self._native_file_guard_parts(merged)
        ws_root = getattr(self._engine, "_workspace_root", None)
        return base_paths, merged_paths, base_defaults, merged_defaults, ws_root
    ...
    def _compute_permission_diff(
        ...
        # overlay 声明沿用 0708 的 ``file_guard.global``（path -> 轴）形态；
        # 裁决侧已投影为 native ``paths`` 条目，差分按裁决语义计算 before/after。
        ...
        (
            ...
            merged_defaults,
            ws_root,
        ) = self._file_guard_adjudication_view(base, merged)
        for path, entry in overlay_global.items():
            ...
            for axis, declared in entry.items():
                ...
                before = effective_file_guard_axis_level(
                    base_paths, str(path), str(axis),
                    defaults=base_defaults, workspace_root=ws_root,
                )
                after = effective_file_guard_axis_level(
                    merged_paths, str(path), str(axis),
                    defaults=merged_defaults, workspace_root=ws_root,
                )
                if before is None or after is None:
                    # 路径无法规范化时回退到同路径条目口径。
                    before = _native_axis_at(base_paths, str(path), str(axis))
                    after = _native_axis_at(merged_paths, str(path), str(axis))
    ...
    def _evaluate_skill_tool_deny(
        ...
        dev-stable：agent-core ``PermissionEngine`` 不感知 Skill overlay，
        ``evaluate_global_policy_directly`` 天然等价 0708 的
        ``apply_skill_overlay=False``；引擎未注入时按评估失败处理。
        """
        if self._engine is None:
            logger.warning(
                "[skill_authorization] gate policy evaluation skipped: engine not injected",
            )
            return _POLICY_EVALUATION_FAILED
        try:
            level, matched_rule = self._engine.evaluate_global_policy_directly(
                SKILL_TOOL_NAME,
                tool_args,
            )
    ...
    async def before_tool_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if tool_name not in (SKILL_TOOL_NAME, SKILL_COMPLETE_TOOL_NAME):
            return
        ...
        if not self._feature_enabled():
            return
        if tool_name == SKILL_COMPLETE_TOOL_NAME:
        ...
        deny_rule = self._evaluate_skill_tool_deny(tool_args)
    ...
    async def _run_approval_flow(self, call: _SkillApprovalCall) -> None:
        ...
        if user_input is None:
            ...
            # dev-stable InterruptRequest 无 allow_auto_confirm 字段。
    ...
    def _base_effective_config(self, session_id: str | None) -> dict[str, Any]:
        ...
        优先取注入引擎的实时配置（``engine.config``）；引擎未注入或配置形态
        非法时回落 ``config_provider``。
        """
        base_source: Any = None
        if self._engine is not None:
            base_source = getattr(self._engine, "config", None)
        if not isinstance(base_source, dict):
            try:
                base_source = self._config_provider()
            except Exception:  # noqa: BLE001
                logger.warning("[skill_authorization] base config provider failed", exc_info=True)
                base_source = {}
        try:
            base = self._session_config_merger(
                base_source if isinstance(base_source, dict) else {},
                session_id,
            )
    ...
    async def after_tool_call(self, ctx: AgentCallbackContext) -> None:
        ...
        # dev-stable 无 active_skill_bodies 会话态：磁盘重读 SKILL.md 复核正文 hash。
        body = self._read_skill_md_bytes(event.skill_name)
    ...
    def _read_skill_md_bytes(self, skill_name: str) -> bytes | None:
        """磁盘重读 ``SKILL.md`` 正文（激活前 hash 复核）；无法定位或读取返回 ``None``。"""
        if self._skill_resolver is None or not skill_name:
            return None
        try:
            location = self._skill_resolver(skill_name)
            ...
            logger.warning(
                "[skill_authorization] skill body resolve failed skill=%s",
                skill_name,
                exc_info=True,
            )
            return None
        if location is None:
            return None
        try:
            return (Path(location[0]) / ROOT_SKILL_FILE).read_bytes()
        except OSError:
            logger.warning(
                "[skill_authorization] skill body read failed skill=%s dir=%s",
                skill_name,
                location[0],
            )
            return None
...
def _native_axis_at(paths: Any, raw_path: str, axis: str) -> str:
    """同路径 prefix 条目的轴档位（差分展示的回退口径）；未命中按 ``ask``。"""
    normalized = str(raw_path).replace("\\", "/").strip().rstrip("/")
    if not isinstance(paths, (list, tuple)):
        return "ask"
    for entry in paths:
        raw_entry_path = entry.get("path") if isinstance(entry, dict) else getattr(entry, "path", None)
        if not isinstance(raw_entry_path, str):
            continue
        if raw_entry_path.replace("\\", "/").strip().rstrip("/") != normalized:
            continue
        match = entry.get("match") if isinstance(entry, dict) else getattr(entry, "match", "prefix")
        if match == "glob":
            continue
        raw_level = entry.get(axis) if isinstance(entry, dict) else getattr(entry, axis, None)
        level = str(getattr(raw_level, "value", raw_level) or "").strip().lower()
        return level or "ask"
    return "ask"
