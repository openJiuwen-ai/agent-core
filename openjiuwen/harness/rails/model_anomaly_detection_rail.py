...
_EMPTY_CONTENT_ERROR_MARKER = "LLM empty content detected"
...
class ModelAnomalyDetectionRail(DeepAgentRail):
    ...
    Handles four failure modes:
      ...
      - empty ``content`` with no tool calls (nudge context and retry)
    ...
    def __init__(
        ...
        *,
        ...
        self.empty_content_retry_count = 0
    ...
    async def after_model_call(self, ctx: AgentCallbackContext) -> None:
        """Detect empty-content responses on the non-stream (invoke) path.

        Streaming empty-content is handled inside ``inspect_stream_chunk`` when
        ``finish_reason`` arrives, so the exception is raised from the model
        call body and can be retried via ``on_model_exception`` without changing
        ``@rail``. Non-stream invoke has no chunk inspectors, so this hook is
        the fallback: nudge context and abort (no whole-call retry on this path).
        """
        response = getattr(ctx.inputs, "response", None) if ctx.inputs is not None else None
        if response is None:
            return
        if not self._is_empty_content_response(response):
            return

        self._raise_empty_content_anomaly(ctx, via_callback=True)

    async def inspect_stream_chunk(self, ctx: AgentCallbackContext, chunk: Any) -> None:
        """Inspect stream chunks for repeated suffixes and empty-content finals."""
        ...
        # AssistantMessage defaults finish_reason to the string "null" (truthy).
        # Only real terminal reasons mean the stream is complete.
        finish_reason = getattr(chunk, "finish_reason", None)
        if finish_reason in (None, "", "null"):
            return
        if state.get("has_tool_calls"):
            return
        if str(state.get("content") or "").strip():
            return

        # Raise inside the model-call body (same path as repeat detection) so
        # the existing @rail on_model_exception retry loop can handle it.
        self._raise_empty_content_anomaly(ctx, via_callback=False)
    ...
    async def on_model_exception(self, ctx: AgentCallbackContext) -> None:
        ...
        if self._is_empty_content_exception(ctx.exception):
            self._request_retry_or_reset(ctx, "empty_content")
    ...
    def _request_retry_or_reset(self, ctx: AgentCallbackContext, reason: str) -> None:
        if reason == "repeat":
            ...
            counter_attr = "empty_content_retry_count"
            label = "empty model content"
        else:
            return
    ...
    @staticmethod
    def _is_empty_content_exception(exc: Optional[BaseException]) -> bool:
        return exc is not None and _EMPTY_CONTENT_ERROR_MARKER in str(exc)

    def _raise_empty_content_anomaly(
        self, ctx: AgentCallbackContext, *, via_callback: bool
    ) -> None:
        removed, nudged = self._nudge_context_for_empty_content(ctx)
        error_msg = (
            f"{_EMPTY_CONTENT_ERROR_MARKER}: content is empty without tool_calls"
        )
        logger.warning(
            "[ModelAnomalyDetectionRail] empty model content detected "
            "(no tool_calls); removed_empty_assistant=%s appended_newline=%s",
            removed,
            nudged,
        )
        # Stream inspectors run inside the model-call body: plain build_error
        # propagates to @rail. after_model_call runs inside the callback
        # framework, which swallows plain exceptions — use AbortError there.
        error = build_error(
            StatusCode.MODEL_CALL_FAILED,
            error_msg=error_msg,
        )
        if via_callback:
            raise AbortError(error_msg, cause=error)
        raise error

    @staticmethod
    def _is_empty_content_response(response: Any) -> bool:
        ...
        if isinstance(content, list):
            # Multimodal content: treat as non-empty if any text/media part exists.
            for part in content:
    ...
    @classmethod
    def _nudge_context_for_empty_content(
        cls, ctx: AgentCallbackContext
    ) -> Tuple[bool, bool]:
        """Drop trailing empty assistant (if any) and append ``\\n`` to previous.

        Returns ``(removed_empty_assistant, appended_newline)``.
        Mutates ``ctx.context`` (not ``ctx.inputs.messages``) so the next
        ``get_context_window`` rebuild picks up the change.

        At ``after_model_call`` time the anomalous assistant is usually not in
        ModelContext yet (ReAct writes it only after a successful return). The
        remove step still covers the case where history already contains such a
        trailing empty assistant (e.g. reasoning-full + empty content).
        """
        context = ctx.context
        if context is None:
            return False, False
        ...
        removed = False
        last = messages[-1]
        if cls._is_empty_content_assistant_message(last):
            messages.pop()
            removed = True

        nudged = cls._append_newline_to_messages(messages)
        if removed or nudged:
            set_messages(messages)
        return removed, nudged

    @classmethod
    def _is_empty_content_assistant_message(cls, message: Any) -> bool:
        role = getattr(message, "role", None)
        is_assistant = role == "assistant" or isinstance(message, AssistantMessage)
        if not is_assistant:
            return False
        return cls._is_empty_content_response(message)

    @staticmethod
    def _append_newline_to_messages(messages: List[Any]) -> bool:
        """Append ``\\n`` to the last message with writable content (in-place)."""
        if not messages:
            return False
        ...
        for index in range(len(messages) - 1, -1, -1):
            ...
            if isinstance(content, list) and content:
                ...
                elif isinstance(last_part, dict) and last_part.get("type") == "text":
                    new_content[-1] = {
                        **last_part,
                        "text": str(last_part.get("text") or "") + "\n",
                    }
                else:
