...
def _normalize_tool_name_list(names: Iterable[str] | None) -> list[str]:
    """Deduplicate tool names while preserving first-seen order."""
    ordered: list[str] = []
    seen: set[str] = set()
    for name in names or []:
...
class ContextAssembleRail(DeepAgentRail):
    ...
    def __init__(
        ...
        if tool_name_allowlist is None:
            ...
            self._tool_name_allowlist = _normalize_tool_name_list(tool_name_allowlist)
    ...
    def set_tool_name_allowlist(self, tool_names: Iterable[str] | None) -> None:
        ...
        if tool_names is None:
            ...
            new_allowlist = _normalize_tool_name_list(tool_names)
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if tools_section is not None:
            ...
            if fingerprint != self._tools_section_fingerprint:
