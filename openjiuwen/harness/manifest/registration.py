...
# Declaration modules loaded by ``ensure_builtin_elements_registered``.
# ENT may lack some rail/tool implementations that DEV declares; import
# failures are skipped so the registration pipeline still completes and
# upper layers (team / swarm) can register their own catalog entries.
# Missing builtins are treated as an empty rail/tool set until migrated.
_BUILTIN_DECLARATION_MODULES = (
    "openjiuwen.harness.manifest.builtin_elements",
    "openjiuwen.harness.manifest.harness_elements",
    "openjiuwen.harness.manifest.meta_elements",
)
...
def ensure_builtin_elements_registered() -> None:
    ...
    for module_name in _BUILTIN_DECLARATION_MODULES:
        try:
            importlib.import_module(module_name)
        except ImportError as exc:
            # Soft-load: keep the DEV registration flow, but allow ENT to
            # proceed with a partial (possibly empty) builtin catalog when
            # declaration modules cannot import yet.
            logger.warning(
                "Skipping harness manifest declarations in %s: %s",
                module_name,
                exc,
            )
