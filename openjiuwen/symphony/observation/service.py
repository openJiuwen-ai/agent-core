...
MIN_RUNTIME_WEIGHT = 0.2
MAX_RUNTIME_WEIGHT = 2.0
...
class GraphObservationService:
    ...
    def _edge_outcome(
        ...
        if value.task.outcome.label == TaskOutcomeLabel.VERIFIED_FAILURE:
            if (
                edge_success is False
                and edge_failure_domain == FailureDomain.ORCHESTRATION
    ...
    def _apply_evidence(
        ...
        for item in payload.get("edges") or []:
            ...
            stats["runtime_weight"] = _runtime_weight(stats)
...
def _runtime_weight(stats: Mapping[str, Any]) -> float:
    success_count = int(stats.get("success_count") or 0)
    failure_count = int(stats.get("failure_count") or 0)
    raw = 1.0 + RUNTIME_WEIGHT_STEP * (success_count - failure_count)
    return max(MIN_RUNTIME_WEIGHT, min(MAX_RUNTIME_WEIGHT, raw))
