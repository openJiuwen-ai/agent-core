"""One-shot fast Symphony graph planner."""
...
- When runtime_weight and planner_weight are present, use them only as tie-breakers
  between otherwise relevant relationships. planner_weight already combines static and
...
class FastOneShotPlanner:
    """Ask the LLM for one validated plan from a compact graph subgraph."""
    ...
    async def plan(self, query: str) -> dict[str, Any]:
        ...
        base = {
            ...
            "graph_dir": str(self.artifacts.graph_dir),
    ...
    def _retrieval_candidate_subgraph(self, query: str) -> dict[str, Any]:
        dynamic_edges = self._known_edges(self._sorted_eligible_edges(query))
        runtime_summary = self._runtime_summary
        dynamic_selected, dynamic_candidates = self._retrieval_candidate_selection(dynamic_edges)
        if not self._overlay_edges():
            return self._subgraph_payload(dynamic_selected, dynamic_candidates)

        static_edges = self._known_edges(
            RuntimeEdgeResolver(
                self.artifacts,
        ...
        static_selected, static_candidates = self._retrieval_candidate_selection(static_edges)

        dynamic_by_key = {
            (skill_id(edge.get("source")), skill_id(edge.get("target"))): edge for edge in dynamic_candidates
        }
        candidate_edges = []
        candidate_edge_keys: set[tuple[str, str]] = set()
        for static_edge in static_candidates:
            key = (skill_id(static_edge.get("source")), skill_id(static_edge.get("target")))
            self._append_candidate_edge(
                candidate_edges,
                candidate_edge_keys,
                dynamic_by_key.get(key, static_edge),
            )
        for dynamic_edge in dynamic_candidates:
            self._append_candidate_edge(candidate_edges, candidate_edge_keys, dynamic_edge)

        selected = list(static_selected)
        selected_set = set(static_selected)
        for current_skill_id in dynamic_selected:
            self._append_skill(selected, selected_set, current_skill_id)
        self._runtime_summary = runtime_summary
        return self._subgraph_payload(selected, candidate_edges)

    def _retrieval_candidate_selection(
        self,
        sorted_edges: Sequence[dict[str, Any]],
    ) -> tuple[list[str], list[dict[str, Any]]]:
        edge_by_key = {(skill_id(edge.get("source")), skill_id(edge.get("target"))): edge for edge in sorted_edges}
        ...
        for left_index, left_seed in enumerate(seeds):
            for right_index in range(left_index + 1, len(seeds)):
                ...
                ranked_paths = [(self._path_rank(path), path) for path in paths if path]
                if ranked_paths:
        ...
        return selected, candidate_edges
    ...
    def _best_directed_path(
        ...
        *,
        ...
        queue: list[tuple[int, float, float, tuple[str, ...]]] = [(0, -1.0, 0.0, (source_id,))]
        while queue:
            hops, negative_minimum, negative_total, nodes = heapq.heappop(queue)
            current_id = nodes[-1]
            if current_id == target_id and hops:
                return [edge_by_key[(nodes[index], nodes[index + 1])] for index in range(len(nodes) - 1)]
    ...
    def _subgraph_payload(
        ...
        return {
            ...
            "edge_by_key": {
                (skill_id(edge.get("source")), skill_id(edge.get("target"))): edge for edge in candidate_edges
            },
            "runtime_summary": {
                **getattr(self, "_runtime_summary", {}),
                "available": bool(self._overlay_edges()),
                "applied_edges": len(runtime_edges),
                "runtime_only_edges": sum(1 for edge in runtime_edges if edge.get("runtime_only")),
            },
        }
    ...
    def _path_rank(cls, path: Sequence[dict[str, Any]]) -> tuple[Any, ...]:
        ...
        edge_ids = tuple((skill_id(edge.get("source")), skill_id(edge.get("target"))) for edge in path)
        return (
            len(path),
            -min(confidences),
            -sum(confidences),
        ...
        *,
        ...
        if status == "no_plan" or not step_ids:
            return {
                ...
                "detail": "",
                "plan": {
                    "title": str(selection.get("title") or default_fast_no_plan_title(self.language)).strip(),
                    "status": "no_plan",
                    "steps": [],
                    "stages": [],
                    "produced_artifacts": [],
        ...
        unknown_ids = [current_skill_id for current_skill_id in step_ids if current_skill_id not in candidate_skill_ids]
        if unknown_ids:
            return {
                "valid": False,
        ...
        selected_edges_result = self._normalize_selected_edges(selection.get("can_feed_edges") or [])
        selected_edges = selected_edges_result["edges"]
        if selected_edges_result["invalid"]:
            return {
                "valid": False,
                "detail": (f"Fast planner returned malformed can_feed edges: {selected_edges_result['invalid']}"),
            }
        if selected_edges_result["duplicates"]:
            return {
                "valid": False,
                "detail": (f"Fast planner returned duplicate can_feed edges: {selected_edges_result['duplicates']}"),
            }
        if len(step_ids) > 1 and not selected_edges:
            selected_edges = [(step_ids[index], step_ids[index + 1]) for index in range(len(step_ids) - 1)]

        step_id_set = set(step_ids)
        unknown_edge_endpoints = [
            edge for edge in selected_edges if edge[0] not in step_id_set or edge[1] not in step_id_set
        ]
        if unknown_edge_endpoints:
            return {
                "valid": False,
                "detail": (f"Fast planner selected edges outside plan steps: {unknown_edge_endpoints}"),
            }
        order = {current_skill_id: index for index, current_skill_id in enumerate(step_ids)}
        backward_edges = [
            (source, target) for source, target in selected_edges if order.get(source, -1) >= order.get(target, -1)
        ]
        if backward_edges:
            return {
                "valid": False,
                "detail": (f"Fast planner selected edges that violate step order: {backward_edges}"),
            }
            ...
            steps.append(
                {
                    ...
                    "capability_type": str(skill.get("type") or skill.get("capability_type") or "skill"),
        ...
        return {
            ...
            "plan": {
                ...
                "status": status,
                "steps": steps,
                "stages": [{"stage": index, "skills": [step]} for index, step in enumerate(steps, start=1)],
                "produced_artifacts": produced_artifacts,
                "missing_inputs": missing_inputs,
                "can_feed_edges": edge_items,
                "reason": str(selection.get("reason") or "").strip(),
                "plan_classification": ("executable" if status == "ready" else "structurally_valid_but_incomplete"),
                "connectivity_trace": (
                    ["can_feed", "fast_llm_inferred"] if has_inferred_edges else (["can_feed"] if edge_items else [])
                ),
    ...
    def _normalize_step_ids(raw_steps: Any) -> list[str]:
        ...
        for item in raw_steps:
            current_skill_id = skill_id(item.get("skill_id") if isinstance(item, dict) else item).strip()
    ...
    def _step_reason(raw_steps: Any, target_skill_id: str) -> str:
        if not isinstance(raw_steps, list):
            return ""
        for item in raw_steps:
            if isinstance(item, dict) and skill_id(item.get("skill_id")) == target_skill_id:
