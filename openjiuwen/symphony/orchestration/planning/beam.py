"""Bidirectional beam planner for Symphony graph orchestration."""
...
from openjiuwen.symphony.orchestration.planning.plan_builder import (
...
_EXPLICIT_FILE_EXTENSION_RE = re.compile(r"(?<![A-Za-z0-9])[\w.-]+\.([A-Za-z0-9]{1,12})\b")
...
class BeamSearchGraph:
    ...
    def apply_judgement(
        ...
        status = "selected" if judgement.score >= SEMANTIC_SCORE_THRESHOLD else "rejected"
        self._ensure_node(
            expansion.candidate_skill_id,
            status=status,
        )
...
class BidirectionalBeamPlanner:
    ...
    async def plan(self, query: str) -> dict[str, Any]:
        ...
        unique_skill_ids = {current_skill_id for state in all_states for current_skill_id in state.skill_ids}
        return {
            "query": query,
    ...
    def _final_rerank_payload(
        ...
        *,
        ...
        return {
            ...
            "candidate_can_feed_edges": self._candidate_edges_for_rerank(candidate_skill_ids),
            "candidate_plans": [
        ...
        *,
        ...
        if raw_steps is None:
            return {
                ...
                "plan": {
                    **selected_plan,
                    "reason": _short_text(str(selection.get("reason") or "")) or selected_plan.get("reason", ""),
        ...
        eligible_edge_keys = {
            (skill_id(edge.get("source")), skill_id(edge.get("target"))) for edge in self.eligible_edges
        }
        ...
        return {
            "valid": True,
            "detail": "",
            "cross_plan_skill_merge": not set(step_ids) <= set(_dict_plan_signature(selected_plan)),
    ...
    def _default_rerank_edges(
        ...
        for item in self.eligible_edges:
            ...
            if edge[0] in order and edge[1] in order and order[edge[0]] < order[edge[1]]:
                output.append(edge)
        return output
    ...
    def _final_rerank_plan_payload(
        ...
        *,
        ...
        edge_by_key = {
            (skill_id(edge.get("source")), skill_id(edge.get("target"))): edge for edge in self.eligible_edges
        }
    ...
    def _default_seed_skill_ids(self) -> tuple[str, ...]:
        ...
        for edge in self.eligible_edges:
            for key in ("source", "target"):
                ...
                if current_skill_id and current_skill_id in self.skill_by_id and current_skill_id not in seen:
    ...
    def _expansion_groups(
        ...
        pending_keys: dict[tuple[str, str, str, str], str] = {}
                ...
                if misses:
                    ...
                    for expansion in misses:
                        ...
                        if existing_candidate_id is not None:
                            aliases[existing_candidate_id].append(expansion.candidate_id)
                            continue
    ...
    def _neighbor_expansions(
        ...
        *,
        ...
        for edge_index in edge_indices:
            ...
            candidate_skill_id = (
                skill_id(edge.get("target")) if direction == "forward" else skill_id(edge.get("source"))
            )
            if (
            ...
            expansions.append(
                NeighborExpansion(
                    candidate_id=(f"{direction}:{edge_index}:{_state_signature(state)}:{candidate_skill_id}"),
                    state=state,
                    direction=direction,
                    current_skill_id=current_skill_id,
    ...
    def _merge_state_group(self, states: list[BeamState]) -> BeamState | None:
        ...
        edge_indices = tuple(sorted({edge_index for state in states for edge_index in state.edge_indices}))
        ordered = self._topological_order(skill_ids, edge_indices)
        if ordered is None:
            return None
        seed_skill_ids = tuple(dict.fromkeys(current for state in states for current in state.seed_skill_ids))
        semantic_scores = tuple(score for state in states for score in state.semantic_scores)
        unique_reasons = []
        for state in states:
    ...
    def _topological_order(
        ...
        ready = sorted(current_skill_id for current_skill_id, count in incoming.items() if count == 0)
        ordered = []
        while ready:
    ...
    def _rank_and_trim(
        ...
        *,
        ...
        for state in self._dedupe_states(states):
            ...
            ranked.append((state, self._state_score(state, plan=plan), len(plan.missing_inputs)))
        ranked.sort(
            key=lambda item: (
                -item[1],
                item[2],
    ...
    def _state_score(
        ...
        *,
        ...
        semantic_score = (
            sum(state.semantic_scores) / len(state.semantic_scores) if state.semantic_scores else SEED_BASELINE_SCORE
        )
        ...
        return 0.45 * semantic_score + 0.25 * edge_confidence + 0.20 * input_coverage - 0.03 * depth_penalty
        ...
        payload.update(
            {
                ...
                "reason": reason,
                "plan_classification": (
                    "executable" if plan.status == "ready" else "structurally_valid_but_incomplete"
                ),
...
def _candidate_plan_skill_ids(plans: list[dict[str, Any]]) -> set[str]:
    return {current_skill_id for plan in plans for current_skill_id in _dict_plan_signature(plan)}
...
def _rerank_plan_stages(
    ...
    incoming: dict[str, set[str]] = {current_skill_id: set() for current_skill_id in step_by_id}
    for edge in edges:
    ...
    while remaining:
        ready = [current_skill_id for current_skill_id in remaining if incoming[current_skill_id] <= completed]
        if not ready:
            ready = [remaining[0]]
        stages.append(
            {
...
def _sorted_expansions(
    expansions: Iterable[NeighborExpansion],
) -> list[NeighborExpansion]:
    return sorted(
        expansions,
        key=lambda item: (
