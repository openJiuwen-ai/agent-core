...
from openjiuwen.symphony.orchestration.planning.input_matching import (
    missing_inputs,
    output_keys,
)
...
from openjiuwen.symphony.orchestration.planning.utils import skill_id
    ...
    for current_skill_id in state.skill_ids:
        ...
        steps.append(
            PlanStep(
                ...
                inputs=[{"name": item.get("name"), "type": item.get("type")} for item in skill.get("inputs", [])],
                outputs=[{"name": item.get("name"), "type": item.get("type")} for item in skill.get("outputs", [])],
                missing_inputs=missing,
            )
        )

    edges = [can_feed_edges[index] for index in state.edges]
    edge_confidence = sum(float(edge.get("confidence") or 0.0) for edge in edges) / len(edges) if edges else 1.0
    status = "ready" if not all_missing else "needs_input"
    return OrchestrationPlan(
        steps=steps,
        produced_artifacts=[
            ArtifactRef(name=name, type=type_, source="skill_output") for name, type_ in sorted(produced)
        ],
...
def plan_stages(
    ...
    while remaining:
        ready = sorted(current_skill_id for current_skill_id in remaining if incoming[current_skill_id] <= completed)
        if not ready:
