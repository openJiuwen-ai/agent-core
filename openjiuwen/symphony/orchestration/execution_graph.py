...
def build_execution_graph(
    ...
    skill_ids = [str(step.get("skill_id") or "").strip() for step in steps if str(step.get("skill_id") or "").strip()]
...
def _project_skill(skill: dict[str, Any]) -> dict[str, Any]:
    return {
        "capability_id": skill.get("id") or skill.get("capability_id"),
        "capability_type": skill.get("type") or skill.get("capability_type") or "skill",
        "name": skill.get("name"),
...
def _edge_touches_selected_skills(edge: dict[str, Any], skill_ids: set[str]) -> bool:
    source = _normalize_capability_id(edge.get("source") or edge.get("source_id"))
    target = _normalize_capability_id(edge.get("target") or edge.get("target_id"))
