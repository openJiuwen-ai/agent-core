...
MutationItem = SkillGraphAdd | SkillGraphUpdate | SkillGraphDelete
...
def prepare_mutation_request(
    ...
    *,
    ...
    normalized_expected_version = str(expected_graph_version or "").strip()
    try:
        requested_snapshot = SourceSnapshot.model_validate(source_snapshot)
    except (PydanticValidationError, TypeError) as exc:
        raise_mutation_error("source_snapshot_mismatch", "source_snapshot is invalid", cause=exc)
    affected_ids = validate_mutation_batch(
        operation,
        items,
        request_id=normalized_request_id,
        expected_graph_version=normalized_expected_version,
    )
...
def validate_mutation_batch(
    ...
    *,
    ...
    if not expected_graph_version or re.fullmatch(r"[A-Za-z0-9._-]+", expected_graph_version) is None:
        raise_mutation_error("invalid_request", "expected_graph_version is invalid")
    if not items:
        raise_mutation_error("empty_batch", "a Skill graph mutation batch must not be empty")
    expected_type = {
        "add": SkillGraphAdd,
        "update": SkillGraphUpdate,
        "delete": SkillGraphDelete,
    }[operation]
    capability_ids = [_validate_item(item, operation=operation, expected_type=expected_type) for item in items]
...
def mutation_request_digest(
    ...
    *,
    ...
    item_payloads = sorted(
        (_mutation_item_payload(item) for item in items),
        key=lambda item: (str(item["capability_type"]), str(item["capability_id"])),
    )
    return stable_sha256(
        {
            "schema_version": "Symphony-skill-graph-mutation-request-v1",
            "request_id": request_id,
            "operation": operation,
            "expected_graph_version": expected_graph_version,
            "source_snapshot": snapshot_stability_payload(source_snapshot),
            "skills": item_payloads,
        }
    )
...
def validate_mutation_against_inventory(
    ...
    *,
    ...
    requested_keys = {
        _validate_item_against_inventory(
            item,
            operation=operation,
...
def canonical_fingerprint_payload(fingerprint: CapabilityFingerprint) -> dict[str, Any]:
    """Return the complete provider fingerprint payload used for exact comparison."""

    return fingerprint.model_dump(mode="json", exclude_none=False)
...
def _validate_item(item: MutationItem, *, operation: MutationOperation, expected_type: type[Any]) -> str:
    if not isinstance(item, expected_type):
        raise_mutation_error(
            "invalid_request",
            f"{operation}_skills() accepts only {expected_type.__name__} items",
        )
    if isinstance(item, SkillGraphDelete):
        capability_id = str(item.capability_id or "").strip()
        if not capability_id:
            raise_mutation_error("invalid_request", "deleted Skill capability_id must not be empty")
        if str(item.capability_type or "").strip() != "skill":
            raise_mutation_error("invalid_request", "batch graph mutation accepts only capability_type=skill")
        if not str(item.expected_content_hash or "").strip():
            raise_mutation_error("invalid_request", "deleted Skill expected_content_hash must not be empty")
        return capability_id
    try:
        fingerprint = CapabilityFingerprint.model_validate(item.fingerprint)
    except (PydanticValidationError, TypeError) as exc:
        raise_mutation_error("invalid_request", "Skill fingerprint is invalid", cause=exc)
    if fingerprint.capability_type != "skill":
        raise_mutation_error("invalid_request", "batch graph mutation accepts only capability_type=skill")
    if not fingerprint.content_hash.strip():
        raise_mutation_error("invalid_request", "Skill fingerprint content_hash must not be empty")
    if isinstance(item, SkillGraphUpdate) and not str(item.expected_content_hash or "").strip():
        raise_mutation_error("invalid_request", "updated Skill expected_content_hash must not be empty")
    return fingerprint.capability_id
...
def _mutation_item_payload(item: MutationItem) -> dict[str, Any]:
    if isinstance(item, SkillGraphDelete):
        return {
    ...
    fingerprint = CapabilityFingerprint.model_validate(item.fingerprint)
    payload = canonical_fingerprint_payload(fingerprint)
    if isinstance(item, SkillGraphUpdate):
        payload["expected_content_hash"] = item.expected_content_hash
    return payload


def _validate_item_against_inventory(
    item: MutationItem,
    *,
    operation: MutationOperation,
    current_hashes: dict[str, str],
    ...
    if isinstance(item, SkillGraphDelete):
        key = capability_identity(item.capability_type, item.capability_id)
        _require_existing_hash(
            key,
            capability_id=item.capability_id,
        ...
        if key in target_by_identity:
            raise_mutation_error(
                "change_set_mismatch",
                f"deleted Skill {item.capability_id!r} is still present in the target inventory",
                current_version=current_version,
                affected_ids=affected_ids,
            )
        return key

    requested = CapabilityFingerprint.model_validate(item.fingerprint)
    key = capability_identity(requested.capability_type, requested.capability_id)
    if operation == "add" and key in current_hashes:
        raise_mutation_error(
            "already_exists",
            f"Skill {requested.capability_id!r} already exists in the active graph",
            current_version=current_version,
            affected_ids=affected_ids,
        )
    if isinstance(item, SkillGraphUpdate):
    ...
    if target is None or canonical_fingerprint_payload(requested) != canonical_fingerprint_payload(target):
        raise_mutation_error(
            "source_snapshot_mismatch",
            f"requested Skill {requested.capability_id!r} does not match the provider target inventory",
            current_version=current_version,
            affected_ids=affected_ids,
        )
    return key
...
def _require_existing_hash(
    ...
    *,
    ...
    if current_hashes[key] != expected_hash:
        raise_mutation_error(
            "stale_content_hash",
            f"Skill {capability_id!r} content hash changed",
            current_version=current_version,
            affected_ids=affected_ids,
        )
