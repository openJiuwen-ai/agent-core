...
class GraphMutationCoordinator:
    ...
    def _read_active_state(self, request: MutationRequest) -> _ActiveMutationState:
        ...
        if current_version != request.expected_graph_version:
            raise_mutation_error(
                "stale_graph_version",
                "expected_graph_version does not match the active graph",
                current_version=current_version,
                affected_ids=request.affected_ids,
                details={"expected_graph_version": request.expected_graph_version},
            )
    ...
    def _validate_target_snapshot(
        ...
        if snapshot_stability_payload(request.source_snapshot) != snapshot_stability_payload(target_snapshot):
            raise_mutation_error(
                "source_snapshot_mismatch",
                "the requested source snapshot does not match the provider target snapshot",
                current_version=current_version,
                affected_ids=request.affected_ids,
                details={
                    "requested_snapshot_id": request.source_snapshot.snapshot_id,
