...
from openjiuwen.symphony.shared.data_types import (
    DataTypeVocabulary,
)
...
LOGGER = logging.getLogger("openjiuwen.symphony.orchestration.graph.candidates")
...
class CandidateGenerator:
    ...
    def _add_exact_io_candidates(
        ...
        for name in sorted(set(indexes.by_output_name) & set(indexes.by_input_name)):
            if self.lexicon.is_generic_io_name(name):
                ...
                continue
            pair_fanout = len(indexes.by_output_name[name]) * len(indexes.by_input_name[name])
                ...
                for target_id in sorted(indexes.by_input_name[name]):
                    ...
                    self._merge_candidate(
                        ...
                        RelationCandidate(
                            ...
                            evidence={
                                "matched_terms": [name],
                                "source_outputs": [item.to_dict() for item in source_outputs],
                                "target_inputs": [item.to_dict() for item in target_inputs],
            ...
            for target in skills:
                ...
                for output in source.outputs:
                    for parameter in target.inputs:
                        if output.name == parameter.name and self.lexicon.is_generic_io_name(output.name):
                            continue
                        ...
                        self._merge_candidate(
                            ...
                            RelationCandidate(
                                ...
                                evidence={
                                    ...
                                    "matched_types": [f"{output.type}->{parameter.type}"],
                                },
                            ),
                        )
    ...
    def _merge_candidate(
        ...
        relation_hints = [hint for hint in candidate.relation_hints if hint in ALLOWED_RELATION_TYPES]
    ...
    def _limit_per_skill_relation(self, candidates: List[RelationCandidate]) -> List[RelationCandidate]:
        ...
        for key in sorted(buckets):
            ...
            limit = self.max_candidates_per_skill_relation
            kept = ordered_bucket[:limit]
            dropped = ordered_bucket[limit:]
            limited.extend(kept)
...
class CandidateIndexes:
    def __init__(self) -> None:
        ...
        self.outputs_by_skill_name: Dict[Tuple[str, str], List[ArtifactSpec]] = defaultdict(list)
        self.inputs_by_skill_name: Dict[Tuple[str, str], List[ParameterSpec]] = defaultdict(list)

    @classmethod
    def from_skills(cls, skills: Iterable[FingerprintLike]) -> "CandidateIndexes":
        indexes = cls()
        for skill in skills:
            ...
            for parameter in skill.inputs:
                ...
                indexes.inputs_by_skill_name[(skill.id, parameter.name)].append(parameter)
        return indexes
...
class CandidateEvidenceMerger:
    ...
    def merge(
        ...
        priority = (
            candidate.priority
            if PRIORITY_RANK.get(candidate.priority, 0) > PRIORITY_RANK.get(existing.priority, 0)
        ...
        return self._limit_port_mappings(
            RelationCandidate(
                ...
                candidate_methods=sorted(set(existing.candidate_methods) | set(candidate.candidate_methods)),
                priority=priority,
                evidence=evidence,
            )
        )
...
def _merge_evidence(
    ...
    for key, value in right.items():
        ...
        if isinstance(existing, list) and isinstance(value, list):
            merged_value = _dedupe_list(existing + value)
...
def _merge_directional_evidence(
    ...
    merged = dict(left)
    left_directions = merged.get("directions", {})
    right_directions = right.get("directions", {})
    directions = dict(left_directions) if isinstance(left_directions, dict) else {}
    for direction, evidence in dict(right_directions).items() if isinstance(right_directions, dict) else ():
...
def _limit_port_mappings_for_candidate(
    ...
    for direction, evidence in dict(candidate.evidence.get("directions", {})).items():
        ...
        mappings = [mapping for mapping in limited_evidence.get("port_mappings", []) if isinstance(mapping, dict)]
        limited_evidence["port_mappings"] = _dedupe_port_mappings(sorted(mappings, key=_port_mapping_sort_key))[
            :max_port_mappings
        ]
...
def _candidate_quality_rank(candidate: RelationCandidate) -> tuple[int, int, int, int]:
    ...
    for evidence in dict(candidate.evidence.get("directions", {})).values():
        if not isinstance(evidence, dict):
            continue
        mappings = [mapping for mapping in evidence.get("port_mappings", []) if isinstance(mapping, dict)]
...
def _is_generic_content_input(parameter: ParameterSpec, input_terms: Set[str]) -> bool:
    return parameter.name in GENERIC_CONTENT_INPUT_NAMES or bool(input_terms & GENERIC_CONTENT_INPUT_NAMES)
...
def _match_reason_exact_io(output_type: str, input_type: str) -> str:
    ...
    return f"source output type '{output_type}' is a subtype of target input type '{input_type}'"
