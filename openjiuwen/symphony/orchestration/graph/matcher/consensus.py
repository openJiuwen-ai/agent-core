...
def consensus_matches(
    ...
    first_by_key = {_match_key(match): match for match in first_matches if match.accepted}
    second_by_key = {_match_key(match): match for match in second_matches if match.accepted}
            ...
            GraphDiagnostic(
                ...
                details={
                    ...
                    "first_run": _consensus_run_details(
                        ...
                        same_candidate_matches=first_by_candidate.get(match.candidate_id, []),
                    ),
                    "second_run": _consensus_run_details(
                        accepted_match=second_by_key.get(key),
                        same_key_matches=second_all_by_key.get(key, []),
                        same_candidate_matches=second_by_candidate.get(match.candidate_id, []),
                    ),
                    "match": match.to_dict(),
                },
            )
...
def _consensus_run_details(
    *,
    ...
    return {
        "status": status,
        "accepted_match": (accepted_match.to_dict() if accepted_match is not None else None),
        "same_key_matches": [match.to_dict() for match in same_key_matches],
        "same_candidate_matches": [match.to_dict() for match in same_candidate_matches],
    }
...
def _match_key_details(key: tuple[str, str, str, Optional[str]]) -> Dict[str, Optional[str]]:
