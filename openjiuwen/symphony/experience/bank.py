...
class ExperienceBank:
    ...
    def __init__(
        ...
        *,
        ...
        # Items that actually have an embedding and thus live in the FAISS
        # index. ``search_by_embedding`` indexes THIS list (not ``_items``)
        # because FAISS returns positions in the trained array, which only
        # covers items with embeddings — indexing the full ``_items`` would
        # misalign when some items lack an embedding.
        self._indexed_items: list[ExperienceItem] = []
    ...
    def search_by_embedding(
        ...
        """
        # Embed outside the lock — it is a slow network/local call and
        # does not touch shared mutable state.
        query_emb = self._embedder.embed(query)
        query_vec = np.asarray([query_emb], dtype=np.float32)

        # Take the write lock for the FAISS search + item lookup so a
        # concurrent _rebuild_index cannot swap self._faiss_index to None
        # (AttributeError) or replace self._indexed_items out from under us
        # (TOCTOU returning a misaligned item). This sacrifices read
        # concurrency for correctness; the actual search is fast.
        with self._write_lock:
            if self._faiss_index is None or not self._indexed_items:
                return []

            k = min(top_k, len(self._indexed_items))
            ...
            for i in range(k):
                ...
                if score >= threshold and 0 <= idx < len(self._indexed_items):
                    results.append((score, self._indexed_items[idx]))
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            # If the manifest itself is corrupt/unreadable but a backup
            # snapshot exists, try restoring from it before giving up.
            if not self._try_restore_from_backup(meta_path, scalar_path, faiss_path, emb_path):
            ...
            try:
                ...
                LOGGER.error("ExperienceBank: failed to parse manifest after backup restore: %s", exc2, exc_info=True)
                raise

        if not self._validate_hashes(meta, scalar_path, faiss_path, emb_path):
            # Live files disagree with the manifest — the most likely cause
            # is a crash mid-persist that left some files new and others
            # stale. Restore from the pre-write snapshot if one survives.
            if self._try_restore_from_backup(meta_path, scalar_path, faiss_path, emb_path):
                try:
                    meta_data = json.loads(meta_path.read_text(encoding="utf-8"))
                    meta = _KnowledgeMeta(**meta_data)
                except Exception as exc:
                    LOGGER.error("ExperienceBank: manifest unreadable after backup restore: %s", exc, exc_info=True)
                    raise
                if self._validate_hashes(meta, scalar_path, faiss_path, emb_path):
                    LOGGER.warning("ExperienceBank: integrity failed on live files, restored from backup")
                else:
                    raise ValueError(
                        "ExperienceBank: integrity check failed even after restoring from backup"
                    )
            else:
                raise ValueError(
                    "ExperienceBank: integrity check failed — manifest does not match on-disk files "
                    "and no backup snapshot is available"
                )
        ...
        try:
            ...
            # Items are rehydrated from the embedding matrix above (one row
            # per item, guaranteed by _validate_counts), so every loaded item
            # has an embedding and is searchable. Mirror that in
            # ``_indexed_items`` so search_by_embedding maps FAISS positions
            # back to the right items.
            self._indexed_items = list(self._items)
                ...
                for item in self._items:
                    if item.id.startswith("exp_"):
                        try:
                            ...
                            # One malformed id must not abort loading the whole
                            # KB — skip it and keep scanning the rest.
                            LOGGER.warning(
    ...
    def _try_restore_from_backup(
        ...
        """Restore the four live files from the ``_backup/`` snapshot.

        Used by ``_load`` when live files fail integrity (typically a crash
        mid-persist left them inconsistent). Returns True if a full backup
        snapshot was found and restored, False if no snapshot is available.
        Does not re-run validation itself — the caller re-validates after.
        """
        backup_dir = self._dir / _BACKUP_DIR
        backup_names = [_SCALAR_FILE, _FAISS_FILE, _EMBED_FILE, _META_FILE]
        if not all((backup_dir / n).exists() for n in backup_names):
            return False

        targets = {
            _SCALAR_FILE: scalar_path,
            _FAISS_FILE: faiss_path,
            _EMBED_FILE: emb_path,
            _META_FILE: meta_path,
        }
        try:
            for name, live in targets.items():
                live.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(backup_dir / name, live)
        except Exception:
            LOGGER.warning(
                "ExperienceBank: failed to restore files from backup", exc_info=True,
            )
            return False
    ...
    def _persist(self) -> None:
        ...
        Four files must be replaced together (scalar/faiss/embed/meta); the
        OS cannot atomically swap multiple files, so a crash mid-replace
        leaves on-disk files inconsistent with the manifest — next ``_load``
        would fail integrity and the KB would be unusable. To survive that:

        1. Before writing, snapshot the current four files (if any) into
           ``_backup/``.
        2. Write ``.tmp`` files, then ``os.replace`` each into place.
        3. On success, delete ``_backup/``.
        4. On failure (or a crash before step 3 completes), ``_load`` falls
        ...
        # 1. Snapshot the current live files into _backup/ before replacing.
        #    Only files that exist are snapshotted; a first-write has nothing
        #    to back up.
        backup_dir = self._dir / _BACKUP_DIR
        live_paths = [scalar_path, faiss_path, emb_path, meta_path]
        has_backup = any(p.exists() for p in live_paths)
        if has_backup:
        ...
        replaced: list[Path] = []  # live files already replaced (for rollback)
        try:
            ...
            # 2. Replace each live file. If one fails, restore whatever was
            #    already replaced from _backup/ so the on-disk state stays
            #    consistent with the pre-write manifest.
            for tmp, live in [
                (tmp_faiss, faiss_path),
                (tmp_emb, emb_path),
                (tmp_scalar, scalar_path),
                (tmp_meta, meta_path),
            ]:
                _write_atomic(tmp, live)
                replaced.append(live)

            # 3. All four replaced successfully — drop the snapshot.
            if has_backup:
                shutil.rmtree(backup_dir, ignore_errors=True)
        except Exception:
            LOGGER.error(
            ...
            # Roll back any live files already replaced to the pre-write
            # versions so on-disk state stays self-consistent.
            if has_backup:
                for live in replaced:
                    bak = backup_dir / live.name
                    try:
                        if bak.exists():
                            _write_atomic(bak, live)
                    except Exception:
                        LOGGER.warning(
                            "ExperienceBank: failed to roll back %s from backup",
                            live, exc_info=True,
                        )
    ...
    def _rebuild_index(self) -> None:
        ...
        Only items carrying an embedding enter the FAISS index; those items
        are tracked in ``_indexed_items`` so ``search_by_embedding`` can map
        FAISS-returned positions back to the correct items. Items without an
        embedding are kept in ``_items`` but are not searchable.
        """
        if not self._items:
