...
def list_session_ids(sessions_dir: Path) -> list[str]:
    """Return all session IDs from ``sessions_dir``, excluding heartbeat directories."""
    if not sessions_dir.exists() or not sessions_dir.is_dir():
        return []
    return [d.name for d in sessions_dir.iterdir() if d.is_dir() and not d.name.startswith("heartbeat_")]
...
def parse_session(session_id: str, sessions_dir: Path) -> list[TraceRecord]:
                ...
                if isinstance(tool_call, dict):
                    ...
                    for s in extracted:
                        if s and s not in skills:
                            skills.append(s)
                messages.append(
                    {"role": "assistant", "content": assistant_content, "reasoning": reasoning, "tool_call": tool_call}
                )
                assistant_content = ""
                reasoning = ""
...
def parse_all_sessions(sessions_dir: Path) -> list[TraceRecord]:
    """Parse all sessions into TraceRecords, skipping unparseable ones."""
    records: list[TraceRecord] = []
    for session_id in list_session_ids(sessions_dir):
        trace_records = parse_session(session_id, sessions_dir)
        if trace_records:
