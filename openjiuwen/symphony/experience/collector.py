...
class ExperienceBaseBuilder:
    ...
    def add(self, trace: TraceRecord) -> None:
        ...
        if not trace.success or not trace.skills:
            ...
            return
        # check-and-snapshot+clear must be ONE atomic critical section,
        # else two producer threads both see >= threshold and one of them
        # snapshots an already-cleared list (empty/partial) or steals
        # traces another thread just appended — TOCTOU on the threshold.
        snapshot: list[TraceRecord] | None = None
        with self._lock:
    ...
    def _requeue_pending(self, records: list[TraceRecord]) -> None:
        """Put records back into the pending buffer for the next flush.

        ``_cluster_and_merge`` runs outside the pending lock (called from
        ``_flush_snapshot``), so this must take the lock to avoid racing
        with concurrent ``add``/``flush`` mutating ``self._pending``.
        """
        if not records:
            return
        with self._lock:
    ...
    def _requeue_or_warn(
        ...
        *,
        ...
        """Recover records from a failed cluster/distill stage.

        Non-force (auto-flush from ``add``): requeue the records so the next
        flush retries them — auto-flush must never silently drop traces.

        Force (explicit ``flush`` during shutdown): requeuing would loop
        forever (the caller wants the buffer drained), so warn loudly that
        these records are being dropped and why.
        """
        if not records:
            return
        if not force:
            self._requeue_pending(records)
            LOGGER.warning(
                "ExperienceBaseBuilder: requeued %d record(s) after failed "
                "stage (%s); will retry on next flush",
                len(records), reason,
            )
        else:
            LOGGER.error(
                "ExperienceBaseBuilder: dropping %d record(s) during forced "
                "flush — stage failed (%s) and requeue would loop forever",
                len(records), reason,
            )
    ...
    def _cluster_and_merge(
        ...
        if not clusters:
            # Clustering produced nothing (e.g. every cluster smaller than
            # min_cluster_size and got marked as noise). Don't silently drop
            # the records — requeue for the next flush, or warn under force.
            self._requeue_or_warn(records, force, reason="clustering yielded no clusters")
            return 0
        ...
        if not valid_patterns:
            # Distillation produced no usable pattern (e.g. every LLM call
            # failed or returned empty). Same recovery as cluster-empty.
            self._requeue_or_warn(records, force, reason="distillation yielded no patterns")
            return 0
