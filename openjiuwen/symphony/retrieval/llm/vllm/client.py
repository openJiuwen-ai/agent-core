...
class LocalVLLMClient(ProgressiveLLMClient):
    ...
    def from_pretrained(
        ...
        *,
        ...
        LOGGER.info(
            "initializing custom local vllm async client model=%s tokenizer=%s dtype=%s device=%s kwargs=%s",
            model_path,
    ...
    def _encode_cached_suffix(
        ...
        *,
        ...
        if full_text.startswith(handle.prefix_text):
            prefix_length = len(handle.prefix_text)
            suffix_rendered = full_text[prefix_length:]
...
def _filter_callable_kwargs(callable_obj: Any, kwargs: Mapping[str, Any]) -> dict[str, Any]:
    try:
...
def _load_chat_template_tokenizer(*, tokenizer_path: str, trust_remote_code: bool) -> Any:
    try:
