...
from .selector import DefaultTopKSelector, GenerateFragmentSelector, LogitSelectionFragmentSelector
                ...
                for index in indices:
                    ...
                    if len(segments) > 1:
                        ...
                        display_names[index] = "/".join(
                            (to_pascal_case(segment) or segment) for segment in segments[-next_depth:]
                        )
                    else:
...
def _initial_visible_display_name(*, entry: Dict[str, str], segments: Sequence[str]) -> str:
    ...
    return to_pascal_case(fallback) or fallback
...
class ProgressiveRetriever:
    ...
    def _complete(
        ...
        *,
        ...
        response_detail: dict[str, object] = {
            "stage": stage,
            "model": model,
    ...
    def _merge_branch_candidates(
        ...
        *,
        ...
        remaining: List[RetrieverCandidate] = []
        for branch_candidates in branch_results:
            if branch_candidates:
                remaining.extend(branch_candidates)
        for candidate in self._dedupe_candidates(remaining):
            dedupe_key = candidate.payload or candidate.item_id
            if dedupe_key in seen:
                continue
            seen.add(dedupe_key)
