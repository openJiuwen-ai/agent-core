...
from openjiuwen.symphony.retrieval.common.prompts import RETRIEVAL_DISCLOSURE_YAML, get_prompt, load_prompt_yaml
from openjiuwen.symphony.shared.naming import to_pascal_case
...
def _build_system_prompt(
    *,
    ...
    if not compact_codes_enabled and flat_list_mode and resolved_top_k != 2:
        return LEGACY_FLAT_NON_COMPACT_SYSTEM_PROMPT
...
def _build_disclosure_prompt_static(
    *,
    ...
    cache_id = hashlib.sha256(
        (f"progressive-disclosure-v1\n{prefix_token_hash}\n{'/'.join(str(code) for code in candidate_codes)}").encode(
            "utf-8"
        )
    ).hexdigest()[:32]
    ...
    if _can_parse_joined_compact_codes(fragment=fragment, parsed_codes=parsed_codes, text=text):
        ...
        if compact != "0" and len(compact) % fragment.code_width == 0:
            ...
            for index in range(0, len(compact), fragment.code_width):
                code_end = index + fragment.code_width
                compact_codes.append(compact[index:code_end])
...
def _parse_non_compact_codes(*, fragment: ExposedFragment, lines: Sequence[str]) -> List[str]:
    ...
    for line in lines:
        ...
        if normalized.lower().startswith("candidate "):
            ...
            candidate_start = len(candidate_prefix)
            candidates.append(normalized[candidate_start:].strip())
...
def _parse_non_compact_code_list(line: str, fragment: ExposedFragment) -> List[str]:
    ...
    meaningful_tokens = [token for token in tokens if not re.fullmatch(r"\d+[\).]?", token)]
    if not meaningful_tokens or len(meaningful_tokens) <= 1:
        return []
    resolved_codes = [
        _match_non_compact_candidate_code(fragment=fragment, candidate=token) for token in meaningful_tokens
    ]
...
def _parse_compact_codes(*, fragment: ExposedFragment, lines: Sequence[str]) -> List[str]:
    ...
    for line in lines:
        ...
        if normalized.lower().startswith("candidate "):
            candidate_prefix = "candidate "
            candidate_start = len(candidate_prefix)
            normalized = normalized[candidate_start:].strip()
...
def _match_label_code(*, fragment: ExposedFragment, code: str) -> SelectableResolution | None:
    ...
    if not text:
        return None
    matches = [
        resolution for resolution in fragment.code_to_resolution.values() if str(resolution.label or "").strip() == text
    ]
    if len(matches) == 1:
        return matches[0]
    return None
...
def _fixed_category_labels() -> Dict[str, str]:
    try:
        from openjiuwen.symphony.retrieval.build.tree.schema import FIXED_ROOT_CATEGORIES
    except Exception:
        return {}
