    ...
    def search_terms(
        ...
        *,
        ...
        if exact:
            ...
            return tuple(LexicalHit(document.key, 0.0) for document in sorted_exact)
        ...
        matched = []
        for document in scope.values():
            document_tokens = set().union(*self._field_tokens[document.key][:3])
            if len(present_set.intersection(document_tokens)) >= required_overlap:
