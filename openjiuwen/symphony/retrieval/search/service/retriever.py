...
from openjiuwen.symphony.retrieval.common.models import RetrieverNode
...
from ..artifacts.loading import LoadedRetrieverIndex, load_retriever_index
from ..runtime.progressive import ProgressiveRetriever
    ...
    def _normalize_candidate_records(
        ...
        *,
        ...
        for index, item in enumerate(candidate_records, start=1):
            record = dict(item)
            record["rank"] = max(1, int(str(record.get("rank") or index)))
    ...
    def _build_public_summary_lines(candidate_records: Sequence[Dict[str, object]]) -> List[str]:
        ...
        for index, record in enumerate(candidate_records, start=1):
            ...
            if score is None:
                ...
                continue
            try:
                lines.append(f"{index}. {label} (source={source}, score={float(str(score)):.4f})")
            except Exception:
