...
from openjiuwen.symphony.retrieval.build.io.items_jsonl import is_passthrough_item_uri


def write_tree_preset(payload: Dict[str, object], path: Path) -> None:
    raw_nodes = payload.get("nodes")
    nodes = sort_tree_nodes(raw_nodes if isinstance(raw_nodes, list) else [])
    lines = ["nodes:"]
    for node in nodes:
