...
class PluginScanner(BaseScanner):
    ...
    @classmethod
    def _plugin_metadata_candidates(cls, path: Path) -> list[Path]:
        metadata_dir = path / ".codex-plugin"
        yaml_names = ("plugin.yaml", "plugin.yml")
        candidates = [metadata_dir / name for name in yaml_names]
        candidates.extend(path / name for name in yaml_names)
        candidates.extend((metadata_dir / "plugin.json", path / "plugin.json"))
        return candidates
    ...
    def scan_item_dir(self, item_dir: Path) -> ScannedItem | None:
        ...
        plugin_file = next(
            (candidate for candidate in self._plugin_metadata_candidates(item_root) if candidate.exists()), None
        )
    ...
    def _parse_simple_yaml_payload(text: str) -> dict[str, Any]:
        ...
        for raw_line in text.splitlines():
            ...
            if not active_section:
                continue
            if indent < 2 or ":" not in line or line.startswith("- "):
                continue
