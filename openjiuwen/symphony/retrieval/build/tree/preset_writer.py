...
from openjiuwen.symphony.shared.rich_compat import Console, RichTree
...
class TreePresetWriter:
    ...
    def node_to_dict(self, node: TreeNode) -> dict:
        payload: dict[str, object] = {"id": node.id, "name": node.name, "description": node.description}
        if node.select_when:
        ...
        if child_nodes:
            payload["children"] = [self.node_to_dict(child) for child in child_nodes]
