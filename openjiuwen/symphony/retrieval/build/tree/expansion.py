...
class TreeExpansionEngine:
    ...
    def process_node(
        ...
        *,
        ...
        if configured_groups:
            ...
            grouped = builder.grouping_engine.split_skills(skills, parent_context, verbose)

        if not grouped:
            self._emit_grouping_failure(
                node,
                skill_count=len(skills),
                depth_reached=depth >= builder.config.max_depth,
            )
            self._emit_leaf(node, skills, verbose=verbose)
            return []
    ...
    def _update_progress(self, *, node_id: str, skill_count: int, depth: int) -> None:
        builder = self._builder
        if builder.state.progress and builder.state.progress_task is not None:
            builder.state.progress.update(
                builder.state.progress_task,
    ...
    def _emit_leaf(self, node: TreeNode, skills: list[dict], *, verbose: bool) -> None:
        self._builder.operations.assign_skills_to_leaf(node, skills)
        if verbose:
    ...
    def _classify_configured_groups(
        ...
        *,
        ...
        initial = builder.grouping_engine.classify_skills(skills, groups, verbose)
        repaired = builder.grouping_engine.validate_and_recover(skills, groups, initial, verbose)
        return builder.grouping_engine.build_groups_from_assignments(groups, repaired)
    ...
    def build_children_from_groups(
        ...
        for group_id, group_data in groups.items():
            ...
            if not child_skills:
                continue
            if len(child_skills) < 2 and not builder.settings.equiv_allow_singleton_groups:
                if len(child_skills) == 1:
                    singleton_triggered = True
                if verbose:
                    skipped_ids = [s["id"] for s in child_skills]
                    console.print(
                        f"[dim]  Skipping singleton group '{group_id}' ({skipped_ids}), "
                        "will reassign into existing groups[/dim]"
                    )
                continue
        ...
        if skill_map:
            if children_to_process:
                ...
                builder.operations.assign_skills_to_leaf(node, skills)
                return []

        if singleton_triggered and children_to_process:
            self.rewrite_node_label_after_singleton(node, children_to_process, verbose)
    ...
    def reassign_skills_to_children(
        ...
        assignments = builder.grouping_engine.classify_skills_single(
            builder.grouping_engine.sorted_skills(unassigned_skills),
            groups,
            verbose=False,
            is_retry=True,
        )
    ...
    def rewrite_node_label_after_singleton(
        ...
        result = builder.llm_runtime.call_llm_json(prompt)
    ...
    def existing_child_groups(self, children: list[TreeNode]) -> list[ChildGroup]:
        builder = self._builder
        return [
            ChildGroup(node=child, skills=builder.repair_engine.collect_subtree_skill_dicts(child))
