...
class TreeGroupingEngine:
    ...
    def sorted_skills(self, skills: list[dict]) -> list[dict]:
        ...
        items = list(skills)
        if not builder.settings.deterministic_prompts:
            return items
        return sorted(items, key=lambda item: (str(item.get("id", "")), str(item.get("name", ""))))

    def iter_group_items(self, groups: dict):
        builder = self._builder
        if not builder.settings.deterministic_prompts:
            return groups.items()
    ...
    def sampling_seed(self, parent_context: Optional[dict], skills_count: int) -> int:
        ...
        payload = "|".join(
            [
                str(builder.settings.discovery_seed),
                str(parent.get("name", "")),
    ...
    def build_groups_from_assignments(self, groups: dict, assignments: dict) -> dict:
        ...
        for group_id, payload in self.iter_group_items(groups):
            ...
            if self._builder.settings.deterministic_prompts:
    ...
    def classify_skills(self, skills: list[dict], groups: dict, verbose: bool = False) -> dict:
        ...
        build_config = self._builder.settings.manager_config.build
        cfg_cap = int(getattr(build_config, "classify_batch_cap", 20) or 20)
        batch_size = min(self._builder.llm_runtime.auto_batch_size(), max(1, cfg_cap))
    ...
    def classify_skills_single(
        ...
        response = self._builder.llm_runtime.call_llm_json(prompt, is_retry=is_retry)
    ...
    def _partition_skills(skills: list[dict], batch_size: int) -> list[list[dict]]:
        ...
        while cursor < len(skills):
            window_end = cursor + batch_size
            windows.append(skills[cursor:window_end])
            cursor += batch_size
        return windows
    ...
    def validate_and_recover(
        ...
        if missing:
            console.print(
                Panel(
                    f"[bold red]{len(missing)}/{len(skills)} skills were placed into "
                    f"fallback group '{fallback_group_id}'.[/bold red]",
    ...
    def discover_groups(self, skills: list[dict], parent_context: Optional[dict], verbose: bool = False) -> dict:
        ...
        response = builder.llm_runtime.call_llm_json(prompt)
    ...
    def split_skills(self, skills: list[dict], parent_context: Optional[dict], verbose: bool = False) -> dict:
        batch_size = self._builder.llm_runtime.auto_batch_size()
    ...
    def _sample_batches(
        ...
        batches = self._partition_skills(shuffled, batch_size)
