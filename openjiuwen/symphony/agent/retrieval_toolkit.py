...
class AgenticSkillRetrievalToolkit:
    ...
    def build_index(
        ...
        *,
        ...
        if not force and self._is_fresh(fingerprint):
            skill_category_paths: list[dict[str, Any]] = []
            state = _build_state(
                status="success",
                stage="reuse",
            ...
            _set_skill_category_paths(state, skill_category_paths)
            self._write_state(state)
            return _result(
                True,
                f"Skill index is fresh. {len(records)} skills indexed.",
                data={**state, "skill_category_paths": skill_category_paths},
            )
        ...
        with TemporaryDirectory(prefix="symphony-skill-index-") as tmp:
            ...
            try:
                ...
                skill_category_paths = _load_skill_category_paths(
                    output_dir,
                ...
                return _result(
                    ...
                    data={
                        **state,
                        ...
                        "elapsed_seconds": elapsed,
                        "skill_category_paths": skill_category_paths,
                    },
                )
    ...
    def load_index_status(
        ...
        *,
        ...
        skill_category_paths: list[dict[str, Any]] = []
        if build.get("status") == "success":
            skill_category_paths = list(build.get("skill_category_paths") or [])
        status = {
...
def _set_skill_category_paths(state: dict[str, Any], paths: Sequence[dict[str, Any]]) -> None:
    build = dict(state.get("build") or {})
    build["skill_category_paths"] = [dict(item) for item in paths]
    state["build"] = build


def _load_skill_category_paths(
    index_dir: str | Path,
    *,
    ...
    if duplicate_worker_ids:
        ...
        raise RuntimeError(f"built index contains duplicate catalog records for Skill IDs: {joined}")
    ...
    if missing:
        ...
        raise RuntimeError(f"built index is missing category paths for updated Skill IDs: {joined}")
    ...
    for worker_id in requested:
        ...
        if not category_path:
            raise RuntimeError(f"built index contains an empty category path for updated Skill ID: {worker_id}")
        paths.append({"skill_id": worker_id, "category_path": category_path})
    return paths
