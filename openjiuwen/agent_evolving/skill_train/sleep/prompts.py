...
_REFLECT = (
    ...
    "Language and anchoring rules:\n"
    "- Match the dominant natural language of the current __TARGET__ document "
    "(prose, headings, and existing bullet rules). Write `content`, `anchor`, "
    "and `rationale` in that same language.\n"
    "- Do not switch to English (or any other language) just because the "
    "failure traces or this instruction are in English.\n"
    "- Keep code tokens, CLI snippets, format codes, and identifiers unchanged "
    "(for example curl, wttr.in, %u, %q, JSON keys).\n"
    "- For `replace` / `delete`, copy `anchor` verbatim from a span that "
    "already appears in the current __TARGET__; never invent an English "
    "heading or example that is not in the document.\n"
    "- Prefer `add` when no reliable verbatim anchor exists.\n\n"
    "# Current __TARGET__\n__CUR_DOC__\n"
    "__PREFS__\n\n"
    "# Recurring failures\n__FAILURES__"
)
