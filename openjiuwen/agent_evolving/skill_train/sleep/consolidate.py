...
def consolidate(
    ...
    *,
    ...
    def _gate_apply(doc: str, edits: List[EditRecord], which: str) -> str:
        ...
        blocked = bool(
            gate_no_regression and any(row["status"] == "regressed" for row in task_deltas)
        )
    ...
    if gate_off:
        ...
        final_deltas = _task_deltas(
            val_tasks, base_pairs, final_pairs, gate_metric, gate_mixed_weight
        )
        blocked_final = bool(
            gate_no_regression and any(row["status"] == "regressed" for row in final_deltas)
        )
        gate = evaluate_gate(
