...
def _ts_dir(clock: Optional[float] = None) -> str:
    when = datetime.now(timezone.utc) if clock is None else datetime.fromtimestamp(clock, tz=timezone.utc)
...
def load_manifest(staging_dir: Path | str) -> Dict[str, Any]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"manifest.json missing under staging directory {staging_dir}; "
            "staging may be incomplete or corrupted"
        )
