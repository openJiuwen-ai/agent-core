...
# Expected backend/transport failures; AttributeError and similar still propagate.
_RUBRIC_SYNTHESIS_ERRORS = (OSError, RuntimeError, TimeoutError, ValueError, TypeError)
...
def _run_read_skill(store: EvolutionStore, name: str) -> str:
    ...
    raise RuntimeError("_run_read_skill cannot be called from a running event loop")
...
def _synthesize_rubrics(backend: Backend, tasks: List[TaskRecord]) -> int:
    ...
    for task in tasks:
        ...
        except _RUBRIC_SYNTHESIS_ERRORS as exc:  # keep the night alive on a single bad call
            logger.warning(
                "[skill_sleep] rubric synthesis failed for %s: %s",
                task.id,
                exc,
                exc_info=True,
            )
