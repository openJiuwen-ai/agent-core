...
def consolidate_groups(
    ...
    **consolidate_kwargs: object,
    ...
    ``evolve_memory`` is declared explicitly so callers cannot silently lose a
    ``True`` value stuffed into ``consolidate_kwargs``.
    """
    if "evolve_memory" in consolidate_kwargs:
        raise TypeError(
            "pass evolve_memory= as a direct keyword to consolidate_groups(); "
            "do not bury it in consolidate_kwargs"
        )
    kwargs = dict(consolidate_kwargs)
    kwargs["evolve_memory"] = evolve_memory
