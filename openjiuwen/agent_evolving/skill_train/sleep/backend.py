...
class Backend:
    ...
    def attempt_with_tools(
        ...
        called = [tool for tool in tools if re.search(rf"(?i)\btool_call\s*:\s*{re.escape(tool)}\b", response)]
...
def _follow_ups_text(task: TaskRecord) -> str:
    ...
    lines = [line[2:].strip() for line in (task.context_excerpt or "").splitlines() if line.startswith("- ")]
    ...
    def reflect(
        ...
        *,
        ...
        for task, result in failures[:12]:
            ...
            follow_ups = _follow_ups_text(task)
            if follow_ups:
                block += "  follow-ups: " + " | ".join(line[2:] for line in follow_ups.splitlines())[:300] + "\n"
            if task.reference:
