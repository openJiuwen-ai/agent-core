...
class MetaSkillInputs:
    """Packed inputs for one meta-skill distillation call."""
...
def run_meta_skill(
    ...
    *,
    prior_memory: str = "",
    system_prompt: str | None = None,
    inputs: MetaSkillInputs | None = None,
) -> Optional[Dict[str, Any]]:
    """Distil updated optimizer memory from two adjacent epochs.

    Prefer passing :class:`MetaSkillInputs`. The positional/keyword form is kept
    for existing call sites and uses the same field names as that dataclass.

    Returns ``None`` when the optimizer fails or yields no memory text.
    """
    if inputs is None:
        inputs = MetaSkillInputs(
            previous_skill=str(previous_skill or ""),
            current_skill=str(current_skill or ""),
            comparison_pairs=list(comparison_pairs or []),
            prior_memory=prior_memory,
            system_prompt=system_prompt,
        )
    prior = (inputs.prior_memory or "").strip() or _NO_PRIOR_MEMORY

    message = UserMessage()
    message.section("Previous Epoch Last-Step Skill", inputs.previous_skill)
    message.section("Current Epoch Last-Step Skill", inputs.current_skill)
    message.section(
    ...
    message.section(
        ...
        format_comparison_text(inputs.comparison_pairs),
    )
    ...
    reply = ask_optimizer(
        OptimizerCall(
            ...
            system=(
                ...
                if inputs.system_prompt is not None
                else load_prompt("meta_skill")
            ),
            user=message.render(),
