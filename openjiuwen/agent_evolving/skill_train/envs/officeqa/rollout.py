...
def _tool_calls_for_messages(message) -> list[dict]:
    """Normalize tool calls to OpenAI chat dict shape for multi-turn replay.

    agent-core ``ToolCall`` is flat (``name`` / ``arguments``), while OpenAI SDK
    and chat APIs use nested ``function``. Accept either and always emit nested.
    """
...
def process_one(
    ...
    *,
    ...
    item_id = str(item["id"])
    pred_dir = os.path.join(out_root, "predictions", safe_fs_id(item_id))
    ...
    try:
        if normalized_search_mode == _CUSTOM_SEARCH_MODE:
            ...
            for turn in range(1, max_tool_turns + 1):
                ...
                response = message.content or ""
                final_response = response
                tool_calls = _tool_calls_for_messages(message)
