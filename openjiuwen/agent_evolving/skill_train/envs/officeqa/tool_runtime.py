...
def resolve_docs_roots(data_dirs: list[str] | tuple[str, ...] | str | None = None) -> list[str]:
    ...
    # Keep repo-root fallback for callers that still pass ``data/...`` relative paths.
    repo_root = Path(__file__).resolve().parents[5]
    env_value = os.environ.get("OFFICEQA_DOCS_DIR", "").strip()
    candidates = _normalize_data_dirs(data_dirs, repo_root)
    candidates.extend(_normalize_data_dirs(data_dirs, package_root))
    candidates.extend(_normalize_data_dirs(env_value, repo_root))
    candidates.extend(_normalize_data_dirs(env_value, package_root))
    candidates.extend([
