...
def _load_document_elements(json_path: str, mtime: float) -> tuple[dict, ...]:
    del mtime  # cache key only; content is always read from disk
...
def _mtime_or_sentinel(path: str) -> float:
    try:
...
def _document_elements(json_path: str) -> tuple[dict, ...]:
    return _load_document_elements(json_path, _mtime_or_sentinel(json_path))


@lru_cache(maxsize=2048)
def _assemble_page_body(json_path: str, page_id: int, mtime: float) -> str:
    del mtime  # cache key only
    rendered: list[str] = []
    for element in _document_elements(json_path):
...
def _page_body(json_path: str, page_id: int) -> str:
    return _assemble_page_body(json_path, page_id, _mtime_or_sentinel(json_path))


def clear_oracle_caches() -> None:
    """Drop cached document/page bodies (e.g. after corpus refresh)."""
    _load_document_elements.cache_clear()
    _assemble_page_body.cache_clear()
...
def build_oracle_parsed_pages_context(
    ...
    *,
    ...
    for anchor in anchors:
        ...
        body = _page_body(str(json_path), anchor.page_num)
        if not body:
            continue
