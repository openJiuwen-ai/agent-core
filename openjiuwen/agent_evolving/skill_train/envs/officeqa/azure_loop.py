...
def run_azure_dialogue(item: dict, skill_content: str, **options: Any) -> _DialogueOutcome:
    ...
    ``diagnostic_instruction``, ``exec_timeout`` (same names as historical callers).
    """
    ...
    exec_timeout = options.get("exec_timeout")
    timeout_val = int(exec_timeout) if exec_timeout is not None else None
    prompts = _compose_azure_prompts(item, skill_content, probe_on, probe_text)
    reply, transcript, meta = _invoke_native_web_search(
        prompts, token_budget, exec_timeout=timeout_val
    )
