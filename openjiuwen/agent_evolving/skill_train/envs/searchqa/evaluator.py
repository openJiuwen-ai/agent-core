...
def _score_one(prediction: str, gold_answers: list[str]) -> tuple[float, float, float]:
    ...
    pred_tokens = pred_norm.split()
    if not pred_tokens:
        # Empty prediction: score 1 only when some gold also normalizes empty.
        # Do not use ``"" in gold`` — that is True for every non-empty string.
        has_empty_gold = any(not _canonical(g).split() for g in gold_answers)
        score = 1.0 if has_empty_gold else 0.0
