...
class EnvAdapter(ABC):
    ...
    def rollout(
        ...
        **kwargs,
        ...
        -------
        list[dict]
            Each dict conforms to :class:`~openjiuwen.agent_evolving.skill_train.types.RolloutResult`:
    ...
    def reflect(
        ...
        **kwargs,
        ...
        Each returned dict conforms to :class:`~openjiuwen.agent_evolving.skill_train.types.RawPatch`:
        ...
        # Local rollouts write conversations under ``rollout/predictions``.
        default_pred = os.path.join(out_dir, "predictions")
        rollout_pred = os.path.join(out_dir, "rollout", "predictions")
        if not kwargs.get("prediction_dir"):
            if os.path.isdir(rollout_pred):
                default_pred = rollout_pred

        return run_minibatch_reflect(
            results=results,
            skill_content=skill_content,
            prediction_dir=kwargs.get("prediction_dir", default_pred),
    ...
    #   1. ``openjiuwen/agent_evolving/skill_train/envs/<env>/prompts/<name>.md``  (env-specific)
    #   2. ``skill_train/prompts/<name>.md``          (generic fallback)
    #
