...
class ChatLLMClient:
    ...
    async def achat(
        ...
        *,
        ...
        """Async single-turn chat (system + user)."""
        prompt = f"{system.strip()}\n\n{user.strip()}"
        text = await invoke_text_with_retry(
            self.llm,
            self.model,
            prompt,
    ...
    async def achat_messages(
        ...
        *,
        ...
        """Async chat completion over an explicit message list."""
        del retries  # single invoke; retries belong to chat()/resilience
