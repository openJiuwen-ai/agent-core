...
``{env}_split`` directories under ``<repo>/data/``.

Missing assets under ``<repo>/data`` are downloaded from Hugging Face
...
# Prefer plain HTTP over HF Xet for large downloads / mirrors. Set once at import
# so download helpers do not mutate process env as a hidden side effect.
os.environ.setdefault("HF_HUB_DISABLE_XET", "1")


def is_id_split_dir(path: str | Path) -> bool:
    """Return True when *path* names an id-only split directory."""
...
def _enable_vt_mode() -> bool:
    ...
    try:
        ...
        if not kernel32.GetConsoleMode(handle, ctypes.byref(mode)):
            _VT.enabled = False
            return False
        _VT.enabled = bool(kernel32.SetConsoleMode(handle, mode.value | 0x0004))
    except (AttributeError, OSError, TypeError, ValueError) as exc:
        logger.debug("[materialize] VT mode unavailable: %s", exc)
        _VT.enabled = False
    return bool(_VT.enabled)


def _emit_progress_text(text: str) -> None:
    """Emit interactive progress text without a direct ``sys.stdout.write`` call."""
    stream = sys.stdout
...
def _progress(msg: str) -> None:
    with _progress_lock:
        _emit_progress_text(f"  [materialize] {msg}\n")
...
def _progress_part(label: str, msg: str) -> None:
    ...
    with _progress_lock:
        if label not in _progress_labels:
            ...
            _emit_progress_text(text + "\n")
            return
        if not _enable_vt_mode():
            _emit_progress_text("\r" + text)
            return
        idx = _progress_labels.index(label)
        n = len(_progress_labels)
        up = n - idx
        _emit_progress_text(f"\033[{up}A\r{text}\n")
        down = n - idx - 1
        if down:
            _emit_progress_text(f"\033[{down}B")
...
def _materialized_ready(  # pylint: disable=too-many-return-statements
    out_dir: Path, id_split_dir: Path, *, required_field: str
) -> bool:
...
def _candidate_dirs(*rel_names: str) -> list[Path]:
    """Only look under <repo>/data (and SKILL_TRAIN_DATA_ROOT)."""
...
def _load_searchqa_parquet_index() -> dict[str, dict]:  # pylint: disable=too-many-locals
    try:
        import pyarrow.parquet as pq
    except ImportError as exc:
        raise FileNotFoundError(
            "SearchQA materialization requires pyarrow to read Hugging Face parquet. Install with: uv add pyarrow"
        ) from exc
    ...
    if not parquet_files:
        try:
            ...
            raise FileNotFoundError(
                ...
                "or place a full catalog at <repo>/data/searchqa_split."
            ) from exc
        ...
        parquet_files = sorted(hub.rglob("*.parquet"))
    if not parquet_files:
        raise FileNotFoundError("SearchQA parquet still missing after download. Check network / HF_ENDPOINT and retry.")
    ...
    for file_idx, path in enumerate(parquet_files, start=1):
        ...
        required = ("key", "question", "context", "answers")
        missing_cols = [name for name in required if name not in names]
        if missing_cols:
            raise FileNotFoundError(f"SearchQA parquet missing columns {missing_cols}: {path}")
        for batch in pf.iter_batches(batch_size=1024, columns=list(required)):
            keys = batch.column("key")
            questions = batch.column("question")
            contexts = batch.column("context")
            answers = batch.column("answers")
            for i in range(batch.num_rows):
                key_s = str(keys[i].as_py())
                scanned += 1
                if key_s in index:
                    continue
                ans = answers[i].as_py()
                ...
                if scanned % 20000 == 0:
                    _progress(f"  scanned {scanned} rows (index={len(index)})")
        _progress(f"  scanned {scanned} rows (index={len(index)})")
    return index


def ensure_materialized_searchqa(id_split_dir: Path | str) -> Path:
    """Hydrate SearchQA id splits into a cached full-payload split directory."""
    ...
    id_splits = _load_id_split(id_split_dir)
    _progress("searchqa: id counts " + " ".join(f"{name}={len(items)}" for name, items in id_splits.items()))
    ...
    if missing:
        raise FileNotFoundError(f"SearchQA materialization missing {len(missing)} ids (e.g. {missing[:3]}).")
    return result
...
def _hf_endpoint() -> str:
    return (
        (os.environ.get("HF_ENDPOINT") or os.environ.get("HUGGINGFACE_HUB_ENDPOINT") or "https://huggingface.co")
        .strip()
        .rstrip("/")
    )
    ...
    for attempt in range(1, _HTTP_RETRIES + 1):
        try:
            with session.get(
                ...
                ranged = response.status_code == 206 or (response.headers.get("accept-ranges") or "").lower() == "bytes"
                return response.url, total, ranged
        except Exception as exc:
            # Non-retryable errors (incl. programming bugs) re-raise immediately.
            last_exc = exc
            if not _is_retryable_http(exc) or attempt >= _HTTP_RETRIES:
                raise
            logger.warning(
                "[materialize] resolve retry %s/%s for %s: %s",
                attempt,
                _HTTP_RETRIES,
                url,
                type(exc).__name__,
    ...
    for attempt in range(1, _HTTP_RETRIES + 1):
        ...
        try:
            ...
            if not _is_retryable_http(exc) or attempt >= _HTTP_RETRIES:
                raise
            logger.warning(
                "[materialize] stream retry %s/%s for %s: %s",
                attempt,
                _HTTP_RETRIES,
                name,
                type(exc).__name__,
...
def _stream_to_file_once(  # pylint: disable=too-many-locals
    url: str,
    dest: Path,
    *,
    token: str = "",
    ...
    with _http_session(token).get(
        ...
        range_requested = existing > 0 or start > 0 or end is not None
        if response.status_code == 200 and range_requested:
            if end is not None:
...
def _download_single(url: str, dest: Path, *, token: str = "", total: int = 0) -> None:
    ...
    if total and partial.stat().st_size < total:
        raise OSError(f"Incomplete download for {dest.name}: {partial.stat().st_size}/{total} bytes")
...
def _unlink_quiet(path: Path) -> None:
    try:
        ...
        _kill_locking_processes(path)
        try:
            path.unlink(missing_ok=True)
        except OSError as exc:
            logger.warning("[materialize] could not remove %s: %s", path, exc)
...
def _download_multipart(url: str, dest: Path, *, token: str = "", total: int) -> None:
    ...
    def fetch(idx: int, start: int, end: int) -> None:
        ...
        if part.stat().st_size != expected:
            raise OSError(f"Incomplete part {idx} for {dest.name}: {part.stat().st_size}/{expected}")

    try:
        with ThreadPoolExecutor(max_workers=len(ranges)) as pool:
            futs = [pool.submit(fetch, i, start, end) for i, (start, end) in enumerate(ranges)]
        ...
        # Drop partials so a later retry cannot mix stale chunks with a new remote.
        for part in parts:
            _unlink_quiet(part)
        _unlink_quiet(tmp)
        raise
...
def _windows_locking_pids(path: Path) -> list[int]:
    ...
    try:
        ...
        rc = rstrtmgr.RmGetList(session, ctypes.byref(needed), ctypes.byref(n_info), infos, ctypes.byref(reboot))
...
def _taskkill_executable() -> str:
    """Resolve an absolute path for ``taskkill`` (G.EDV.05)."""
    found = shutil.which("taskkill")
    if found:
        return found
    windir = os.environ.get("WINDIR") or os.environ.get("SystemRoot") or r"C:\Windows"
    return os.path.join(windir, "System32", "taskkill.exe")
...
def _kill_locking_processes(path: Path) -> list[int]:
    ...
    for pid in _windows_locking_pids(path):
        ...
        try:
            ...
            _progress(f"taskkill PID {pid} timed out after 30s")
            continue
...
def _list_hf_dataset_files(  # pylint: disable=too-many-locals
    repo_id: str, prefix: str = ""
) -> list[str]:
    ...
    for attempt in range(1, _HTTP_RETRIES + 1):
        try:
            ...
            if attempt >= _HTTP_RETRIES:
                break
            logger.warning(
                "[materialize] HF tree list retry %s/%s for %s: %s",
                attempt,
                _HTTP_RETRIES,
                repo_id,
                type(exc).__name__,
            ...
            time.sleep(min(2 * attempt, 10))
    raise FileNotFoundError(f"Failed to list {repo_id} files from {url}: {last_exc}") from last_exc
    ...
    for attempt in range(1, _HTTP_RETRIES + 1):
        ...
        try:
            ...
            if not _is_retryable_http(exc) or attempt >= _HTTP_RETRIES:
                raise
            logger.warning(
                "[materialize] small-file retry %s/%s for %s: %s",
                attempt,
                _HTTP_RETRIES,
                dest.name,
                type(exc).__name__,
...
def _docvqa_validation_parquets() -> list[Path]:  # pylint: disable=too-many-locals,too-many-statements
    try:
        import pyarrow.parquet  # noqa: F401  # pylint: disable=unused-import
    except ImportError as exc:
        raise FileNotFoundError(
            "DocVQA download requires pyarrow to read Hugging Face parquet. Install with: uv add --dev pyarrow"
        ) from exc
    ...
    _progress(f"docvqa: have {len(found)}/6 parquet files, downloading missing {missing}")

    # huggingface_hub + hf-mirror often fails on Xet metadata; stream via HTTP
    # (HF_HUB_DISABLE_XET is set once at module import).
    endpoint = _hf_endpoint()
    token = _hf_token()
        ...
        for attempt in range(1, _HTTP_RETRIES + 1):
            try:
                ...
                if not _is_retryable_http(exc) or attempt >= _HTTP_RETRIES:
                    raise
                logger.warning(
                    "[materialize] docvqa parquet retry %s/%s for %s: %s",
                    attempt,
                    _HTTP_RETRIES,
                    name,
                    type(exc).__name__,
    ...
    with ThreadPoolExecutor(max_workers=_download_file_workers()) as pool:
        ...
        for fut in as_completed(futs):
            ...
            try:
                fut.result()
            except Exception as exc:  # noqa: BLE001 - keep sibling downloads going
                logger.warning("[materialize] docvqa parquet failed %s: %s", name, exc)
    ...
    raise FileNotFoundError(
        ...
        "Fix options:\n"
        "  1) Place images at <repo>/data/docvqa_images/*.png and a catalog at "
        "<repo>/data/docvqa_split/ (or docvqa/splits/), then retry\n"
...
def _load_docvqa_hf_index(wanted: set[str]) -> dict[str, dict]:  # pylint: disable=too-many-locals
    try:
        import pyarrow.parquet as pq
    except ImportError as exc:
        raise FileNotFoundError(
...
def _download_docvqa_from_hf(id_split_dir: Path, out_dir: Path, image_dir: Path) -> Path:
    id_splits = _load_id_split(id_split_dir)
    wanted = {
        str(item.get("questionId") or item.get("id") or "").strip() for items in id_splits.values() for item in items
    }
    wanted.discard("")
    ...
    if missing:
        raise FileNotFoundError(f"DocVQA HF source missing {len(missing)} questionId(s), e.g. {missing[:3]}")
    ...
    def hydrate(row: dict) -> dict:
        ...
        rel_image = str(row.get("image_path") or f"data/docvqa_images/q{qid}_d{doc_id}.png").replace("\\", "/")
...
def ensure_docvqa_images(*, id_split_dir: Path | None = None) -> Path | None:
    """Ensure DocVQA images exist under <repo>/data/docvqa_images."""
...
def ensure_materialized_docvqa(id_split_dir: Path | str) -> Path:
    """Hydrate DocVQA id splits and images into cached training assets."""
    id_split_dir = Path(id_split_dir)
    out_dir = skill_train_data_root() / "docvqa_split"
    ...
    id_splits = _load_id_split(id_split_dir)
    _progress("docvqa: id counts " + " ".join(f"{name}={len(items)}" for name, items in id_splits.items()))
    ...
    if index and images_ready:
        ...
        if missing:
            raise FileNotFoundError(f"DocVQA materialization missing {len(missing)} ids (e.g. {missing[:3]}).")
        return result
...
def _download_officeqa_docs_from_hf(  # pylint: disable=too-many-locals,too-many-statements
    output_dir: Path,
) -> Path:
    token = _hf_token()
    if not token:
    ...
    try:
        remote_files = [path for path in _list_hf_dataset_files("databricks/officeqa", prefix) if path.endswith(".txt")]
    except Exception as exc:  # noqa: BLE001 - surface as FileNotFoundError for callers
        logger.warning("[materialize] officeqa docs listing failed: %s", exc)
        raise FileNotFoundError(
    ...
    if not remote_files:
        raise FileNotFoundError(f"OfficeQA docs: no {prefix}/*.txt found in databricks/officeqa")
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                logger.warning("[materialize] officeqa docs failed %s: %s", relpath, exc)
    ...
    if not src_files:
        ...
        raise FileNotFoundError(f"OfficeQA docs download finished but found 0 treasury_bulletin_*.txt. {detail}")
...
def ensure_officeqa_docs(*, allow_download: bool = True) -> Path | None:
    """Ensure OfficeQA docs under <repo>/data/officeqa_docs_official.
...
def ensure_materialized_officeqa(id_split_dir: Path | str) -> Path:
    """Hydrate OfficeQA id splits and source documents into cached assets."""
    id_split_dir = Path(id_split_dir)
    ...
    id_splits = _load_id_split(id_split_dir)
    _progress("officeqa: id counts " + " ".join(f"{name}={len(items)}" for name, items in id_splits.items()))
    ...
    if missing:
        raise FileNotFoundError(f"OfficeQA materialization missing {len(missing)} ids (e.g. {missing[:3]}).")
    ensure_officeqa_docs()
    return result
