...
class SplitDataLoader(BaseDataLoader):
    ...
    def train_items(self) -> list[dict]:
        return self._bucket("train")

    @property
    def val_items(self) -> list[dict]:
        return self._bucket("val")

    @property
    def test_items(self) -> list[dict]:
        return self._bucket("test")
    ...
    def _absorb_cfg(self, cfg: dict) -> None:
        paths = self.paths
        paths.split_mode = str(cfg.get("split_mode") or paths.split_mode or "ratio")
        ...
        if "split_seed" in cfg:
            raw = cfg.get("split_seed")
            # Keep explicit 0; only fall back when the key is present but empty.
            paths.split_seed = paths.seed if raw in (None, "") else int(raw)
        elif paths.split_seed == 0:
