...
from openjiuwen.agent_evolving.skill_train.utils import extract_json, prediction_item_dir
...
def fmt_minibatch_trajectories(
    ...
    for idx, item in enumerate(items, 1):
        tid = str(item["id"])
        item_dir = prediction_item_dir(prediction_dir, tid)
        conv_path = os.path.join(item_dir, "conversation.json")
        ...
        header = (
            ...
            f"Task: {item.get('task_description') or item.get('instruction') or item.get('question') or ''}\n"
            f"Task type: {item.get('task_type') or item.get('instruction_type') or ''}\n"
        )
        ...
        if not target_prompt:
            prompt_path = os.path.join(item_dir, "target_system_prompt.txt")
        ...
        if not user_prompt:
            user_prompt_path = os.path.join(item_dir, "target_user_prompt.txt")
        ...
        if os.environ.get("REFLACT_CODEX_TRACE_TO_OPTIMIZER", "0") == "1":
            ...
            if not codex_trace_summary:
                codex_trace_summary_path = os.path.join(item_dir, "codex_trace_summary.txt")
        ...
        if os.environ.get("REFLACT_CLAUDE_TRACE_TO_OPTIMIZER", "0") == "1":
            claude_steps_path = os.path.join(item_dir, "claude_trace_steps.txt")
        ...
        if not preview:
            preview_path = os.path.join(item_dir, "spreadsheet_preview.txt")
...
def run_minibatch_reflect(
    ...
    *,
    ...
    ----------
    results : list[dict]
        Rollout result dicts; see :class:`~openjiuwen.agent_evolving.skill_train.types.RolloutResult`.
    skill_content : str
