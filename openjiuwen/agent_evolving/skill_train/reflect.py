...
class ReflectRequest:
    ...
    Field groups are nested so the surface no longer mirrors the upstream
    flat kwargs list; callers can still construct via :meth:`assemble`.
    """

    results: list[dict]
    skill_content: str
    artifact_roots: tuple[str, str]  # prediction_dir, patches_dir
    parallelism: tuple[int, bool]  # workers, failure_only
    batch_knobs: tuple[int, int, int | None]  # minibatch, edit_budget, seed
    analyst_prompts: tuple[str | None, str | None]  # error, success
    context_blocks: tuple[str, str, str, str]  # rejection, traj_mem, step_buf, meta
    update_mode: str = "patch"
    awareness: tuple[bool | None, str | None] = (None, None)
    ...
    def assemble(
        ...
        *,
        ...
        return cls(
            ...
            artifact_roots=(prediction_dir, patches_dir),
            parallelism=(workers, failure_only),
            batch_knobs=(minibatch_size, edit_budget, random_seed),
            analyst_prompts=(error_system, success_system),
            context_blocks=(
                rejection_context,
            ...
            awareness=(skill_aware_reflection, skill_aware_appendix_source),
        )

    @property
    def prediction_dir(self) -> str:
        return self.artifact_roots[0]

    @property
    def patches_dir(self) -> str:
        return self.artifact_roots[1]

    @property
    def workers(self) -> int:
        return self.parallelism[0]

    @property
    def failure_only(self) -> bool:
        return self.parallelism[1]

    @property
    def minibatch_size(self) -> int:
        return self.batch_knobs[0]

    @property
    def edit_budget(self) -> int:
        return self.batch_knobs[1]

    @property
    def random_seed(self) -> int | None:
        return self.batch_knobs[2]

    @property
    def error_system(self) -> str | None:
        return self.analyst_prompts[0]

    @property
    def success_system(self) -> str | None:
        return self.analyst_prompts[1]

    @property
    def rejection_context(self) -> str:
        return self.context_blocks[0]

    @property
    def trajectory_memory_context(self) -> str:
        return self.context_blocks[1]

    @property
    def step_buffer_context(self) -> str:
        return self.context_blocks[2]

    @property
    def meta_skill_context(self) -> str:
        return self.context_blocks[3]

    @property
    def skill_aware_reflection(self) -> bool | None:
        return self.awareness[0]

    @property
    def skill_aware_appendix_source(self) -> str | None:
        return self.awareness[1]
...
def run_minibatch_reflect(request: ReflectRequest) -> list[dict | None]:
    ...
    raw_by_index: dict[int, dict | None] = {}
    pending: list[tuple[int, MinibatchJob]] = []
    for index, job in enumerate(plan.jobs):
        cached = _load_cached_patch(request.patches_dir, job.tag)
        if cached is None:
            pending.append((index, job))
        else:
            raw_by_index[index] = cached

    if not pending:
        return [raw_by_index[i] for i in sorted(raw_by_index)]
    ...
    with ThreadPoolExecutor(max_workers=request.workers) as pool:
        futures = {pool.submit(_execute, job): (index, job) for index, job in pending}
        for done, future in enumerate(as_completed(futures), 1):
            index, job = futures[future]
            try:
                patch = future.result()
            except Exception:  # noqa: BLE001 - one analyst failure must not abort the stage
                logger.exception("[analyst] %s failed; skipping", job.tag)
                continue
            if patch:
                _store_patch(request.patches_dir, job.tag, patch)
                raw_by_index[index] = patch
