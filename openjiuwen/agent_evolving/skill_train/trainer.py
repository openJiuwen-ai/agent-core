...
class TrainPlan:
    ...
    def gate_score(self, hard: float, soft: float, skill: str) -> float:
        return select_gate_score(
            ...
            GateTuning(
                soft=soft,
                metric=cast(GateMetric, self.gate_metric),
                mixed_weight=self.gate_mixed_weight,
            ),
            skill_content=skill,
        )
...
class SkillReflACTTrainer:
    ...
    def train(
        ...
        *,
        ...
        return self._finalize(plan, runtime, started)
    ...
    @staticmethod
    def _run_step(plan: TrainPlan, runtime: TrainRuntime, step: StepId) -> Dict[str, Any]:
        ...
        if plan.use_gate:
            action = progress.adopt(
                evaluate_gate(
                    ...
                    GateTuning(
                        soft=cand_soft,
                        metric=cast(GateMetric, plan.gate_metric),
    ...
    @staticmethod
    def _finalize(plan: TrainPlan, runtime: TrainRuntime, started: float) -> SkillTrainResult:
    ...
    @staticmethod
    def _run_epoch_slow_update(request: SlowUpdateRequest) -> SlowUpdateOutcome:
        """Run the epoch-boundary slow update."""
        return SlowUpdateStage(request).run()

    @staticmethod
    def _run_epoch_meta_skill(request: MetaSkillRequest) -> Optional[List[dict]]:
        """Run the epoch-boundary meta skill stage."""
        return MetaSkillStage(request).run()
