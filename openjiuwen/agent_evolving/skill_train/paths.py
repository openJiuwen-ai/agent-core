...
def skill_train_data_root() -> Path:
    """Return the default dataset root under the repo ``data/`` directory.

    Layout: ``<repo-root>/data/...`` (downloads, splits, docs).
    ...
    # paths.py -> skill_train -> agent_evolving -> openjiuwen -> <repo-root>
...
def resolve_data_path(
    ...
    *env_names: str,
    ...
    2. ``<repo-root>/data/<trimmed>``
    3. ``SKILL_TRAIN_DATA_ROOT`` variants
    4. Legacy paths under ``repo_root`` when provided
    """
