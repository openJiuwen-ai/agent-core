...
def _read_trajectory(rollout_dir: str, task_id: str) -> str:
    ...
    if not predictions_root.is_dir() and root.name == "predictions":
        predictions_root = root
    pred_dir = prediction_item_dir(str(predictions_root), task_id)
    conv_path = Path(pred_dir) / "conversation.json"
