...
"""Shared Ray/VERL execution engine for PPO training adapters."""
...
class PPOBatchEngine:
    """Initialize PPO infrastructure, train one batch, and publish its LoRA."""

    def __init__(
        self,
        *,
        base_model_path: str,
        lora_repo: Optional[LoRARepositoryProtocol | LoRARepository],
        nproc_per_node: int,
        ...
        self._ppo_runner: Any = None
        self._ppo_initialized = False
        self._ppo_config = None
        self._ppo_init_lora_adapter_path = ""

    def close(self) -> None:
        """Stop the current Ray actor when one exists."""

        if self._ppo_runner is not None:
            try:
                import ray
    ...
    def _init_ppo_trainer(self, *, init_lora_adapter_path: str = "") -> None:
        ...
        if not ray.is_initialized():
            ...
            logger.info("Ray initialized for online PPO (GPUs=%s)", self.training_gpu_ids or "default")
        ...
        runner_options = OnlineTaskRunner.options  # type: ignore[attr-defined]  # pylint: disable=no-member
        self._ppo_runner = runner_options(
            name="online_ppo_runner",
            lifetime="detached",
        ).remote()
    ...
    def _run_ppo_training_sync(
        ...
        *,
    ...
    ) -> Optional[LoRAVersionLike]:
        """Convert samples, run PPO steps, and publish one immutable LoRA."""
        ...
        data_cfg = getattr(self._ppo_config, "data", None)
        max_prompt_length = int(data_cfg.max_prompt_length) if data_cfg and data_cfg.get("max_prompt_length") else None
        max_response_length = (
            int(data_cfg.max_response_length) if data_cfg and data_cfg.get("max_response_length") else None
        )
        ...
        raw_prompt_max = max(
            (len((sample.get("trajectory") or {}).get("prompt_ids") or []) for sample in samples), default=0
        )
        raw_response_max = max(
            (len((sample.get("trajectory") or {}).get("response_ids") or []) for sample in samples), default=0
        )
        logger.info(
        ...
        peft_dir = ray.get(self._ppo_runner.export_lora.remote(str(run_dir), self.base_model_path))
        if self.lora_repo is None:
            return None
        scores = [sample.get("judge", {}).get("score", 0.0) for sample in samples]
    ...
    def _sample_chunks(self, samples: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
        ...
        for offset in range(0, len(samples), self.ppo_samples_per_step):
            end = offset + self.ppo_samples_per_step
            chunks.append(samples[offset:end])
        return chunks


__all__ = ["PPOBatchEngine"]
