...
class ManagedTrainingProcess:
    ...
    def _send_signal(self, process: subprocess.Popen, sig: signal.Signals) -> None:
        try:
            ...
            logger.info("Sent %s to %s training process group pid=%s", sig.name, self.name, process.pid)
        except ProcessLookupError:
            logger.debug("%s training process already exited before %s", self.name, sig.name)
