...
def _normalize_rollout_backend(raw: str) -> str:
    ...
    if backend in {"local_repo", "local-repo", "local", "akernel"}:
        return "akernel"
...
def build_task_rollout_local_repo_spec(
    case: SFTTaskCase,
    ...
    *,
    ...
    from ..backends.rollouter.task_rollout_backends import AKernelTaskRolloutBackend

    return AKernelTaskRolloutBackend().build_spec(case, config, index=index)
