...
class TrajectoryUploader:
    ...
    def __init__(
        ...
        *,
        ...
        self.force_wal = force_wal or _env_bool("TRAJECTORY_FORCE_WAL", False)
    ...
    async def upload_now(self, batch: Any) -> None:
        """Upload one batch immediately, writing WAL on failure."""
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if 400 <= resp.status_code < 500:
                    ...
                    logger.warning(
                        "[TrajectoryUploader] drop 4xx batch status=%d protocol=%s sample=%s body=%s",
                        resp.status_code,
                        payload.get("protocol_version"),
                        payload.get("sample_id") or payload.get("raw_id") or payload.get("trajectory_id"),
                        resp.text[:200],
                    )
                    return True
