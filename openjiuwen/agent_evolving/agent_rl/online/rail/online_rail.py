...
class RLOnlineRail(EvolutionRail):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._model_call_scoped_builder = False
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        self._ensure_model_call_builder(ctx)
        await self._apply_latest_lora_model(ctx)

    async def after_model_call(self, ctx: AgentCallbackContext) -> None:
        try:
            if self._model_call_scoped_builder and self._is_empty_model_call(ctx.inputs):
                self._model_call_scoped_builder = False
                self._reset_trajectory_builder()
                return
            await super().after_model_call(ctx)
            await self._flush_model_call_builder(ctx)
        finally:
            self._restore_model(ctx)

    @staticmethod
    def _is_empty_model_call(inputs: Any) -> bool:
        if not isinstance(inputs, ModelCallInputs):
            return False
        return not inputs.messages and inputs.response is None

    def _ensure_model_call_builder(self, ctx: AgentCallbackContext) -> None:
        """Create an RL invoke boundary when the host only forwards model callbacks."""
        if self._builder is not None:
            return
        session_id = self._session_id_from_ctx(ctx) or self._session_id
        ...
        self._builder = TrajectoryBuilder(
            ...
            meta={
                ...
                "fallback_boundary": "model_call",
            },
            max_steps=self._max_trajectory_steps,
        )
        self._last_llm_ref = None
        self._tool_call_parent_refs = {}
        self._model_call_scoped_builder = True
        self._bind_trajectory_trace(ctx)

    async def _flush_model_call_builder(self, ctx: AgentCallbackContext) -> None:
        """Upload one LLM turn when no outer invoke lifecycle is available."""
        if not self._model_call_scoped_builder or self._builder is None:
            return
        try:
            if not self._builder.steps:
                return
    ...
    async def _on_after_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if self._model_call_scoped_builder:
            last_step.meta["fallback_boundary"] = "model_call"
