...
"""Redis-backed store for async online training tasks."""
...
class TrainingTaskStore:
    """Persist the single active training task in Redis."""
    ...
    async def list_tasks(self, *, limit: int = 100) -> list[dict[str, Any]]:
        task_ids = await self._r.zrange(_TASK_INDEX, 0, max(0, int(limit) - 1))
    ...
    async def get_task(self, task_id: str) -> Optional[dict[str, Any]]:
        ...
        out["drain_pending_on_train"] = str(out.get("drain_pending_on_train") or "").lower() in {"1", "true", "yes", "on"}
        ...
        out["allow_partial_last_step"] = str(out.get("allow_partial_last_step") or "").lower() in {"1", "true", "yes", "on"}
        try:
    ...
    async def claim_pending_task(self, *, user_id: str | None, sample_count: int) -> Optional[dict[str, Any]]:
        ...
        if user_id and task.get("user_id") and str(task["user_id"]) not in {"", user_id}:
            return None
        await self._r.hset(_task_key(task["task_id"]), mapping={
            "status": "running",
            "started_at": _now_iso(),
            "updated_at": _now_iso(),
    ...
    async def update_task_status(
        ...
        *,
        ...
        await self._r.hset(_task_key(task_id), mapping={k: str(v) for k, v in payload.items()})
