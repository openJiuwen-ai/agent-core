...
class _BM25Index:
    ...
    def from_dict(cls, payload: Optional[Dict[str, Any]]) -> "_BM25Index":
        ...
        index.tokenizer = str(data.get("tokenizer") or TOKENIZER_VERSION)
        return index
...
class TTSEIndex:
    ...
    def align(self, facts: Sequence[Dict[str, Any]], tips: Sequence[Dict[str, Any]], category_of) -> None:
        fact_ids = {ensure_id(record, "fact") for record in facts}
        tip_ids = {ensure_id(record, "tip") for record in tips}
        fact_docs = set(self._load_bm25(None, "fact").docs.keys())
        tip_docs = set(self._load_bm25(None, "tip").docs.keys())
        if fact_docs == fact_ids and tip_docs == tip_ids:
            logger.info(
    ...
    async def _rebuild_chroma(self, store: Any) -> None:
        ...
        try:
            ...
            self._chroma_stale = False
            logger.info(
                "[TTSERail] chroma rebuilt fingerprint=%s records=%s/%s",
                expected,
                len(payload),
                len(items),
            )
    ...
    async def move(self, record: Dict[str, Any], track: str, old_category: str, new_category: str, store: Any) -> None:
        ...
        await self._upsert_vector(
            ...
            track,
            new_category,
            getattr(store, "_cached_embedding", lambda *_: None)(_norm(text)),
