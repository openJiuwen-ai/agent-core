...
* With an embedding provider: cosine ``soft_cluster`` then per-cluster LLM merge
  (at most ``dream_max_llm_merges`` invokes; cluster size capped by
  ``dream_merge_max_rules``).
...
async def _dream_merge_embedding(
    ...
    *,
    ...
    At most ``config.dream_max_llm_merges`` clusters invoke the merge LLM
    per track; each prompt is further capped by ``dream_merge_max_rules``.
    """
    ...
    logger.info(
        ...
        "llm_budget=%s similarity=cosine threshold=%.3f",
        track,
        len(records),
        len(by_category),
        len(clusters),
    ...
    for cluster in clusters:
        if budget <= 0:
            logger.info(
                "[TTSERail] dream merge budget exhausted track=%s llm_budget=%s",
                track,
                int(config.dream_max_llm_merges),
            )
            break
...
async def dream_merge(
    ...
    *,
    ...
    (at most ``dream_max_llm_merges`` invokes; each cluster capped by
    ``dream_merge_max_rules``, highest-count kept).
