...
def _normalize_bucket(bucket: dict[str, Any]) -> dict[str, Any]:
    bucket["triggerCount"] = _as_count(bucket.get("triggerCount"))
    bucket["experienceSuccessCount"] = _as_count(bucket.get("experienceSuccessCount"))
    return bucket
...
def _read_ledger(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "skills": [_normalize_bucket(s) for s in skills if isinstance(s, dict)],
    }
...
def record_evolution_counts(
    ...
    *,
    ...
    """Increment Suggest-mode trigger / experience-success counters.

    Failures are logged and swallowed so evolution itself is not blocked.
    """
    name = (skill_name or "").strip()
    if not name or (not triggered and not experience_succeeded):
        return

    path = ledger_path or resolve_ledger_path(skills_dirs)
    if path is None:
        logger.warning(
            "[EvolutionSuggestionsLedger] skip counts: cannot resolve ledger path skill=%s",
            name,
        )
        return
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            bucket = _ensure_bucket(ledger, name)
            if triggered:
                bucket["triggerCount"] = _as_count(bucket.get("triggerCount")) + 1
            if experience_succeeded:
                bucket["experienceSuccessCount"] = (
                    _as_count(bucket.get("experienceSuccessCount")) + 1
                )
            bucket["updated_at"] = _now_iso()
            _atomic_write(path, ledger)
            return
    ...
    logger.warning(
        "[EvolutionSuggestionsLedger] failed to record counts skill=%s "
        "triggered=%s succeeded=%s err=%s",
        name,
        triggered,
