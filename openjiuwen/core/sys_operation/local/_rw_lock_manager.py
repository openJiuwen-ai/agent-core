        ...
        try:
            ...
            async with cls._get_state_lock():
                ...
                try:
                    ...
                    # Windows can keep SQLite's file handle alive briefly after
                    # the exclusive transaction is released.  This is cleanup
                    # only: retain and retry the database later without making
                    # ordinary filesystem operations look like failures.
                    sys_operation_logger.debug(
                        "Deferred expired read-write lock database cleanup, lock_file=%s, error=%s",
                        lock_file,
                        exc,
                    )
                    return False
