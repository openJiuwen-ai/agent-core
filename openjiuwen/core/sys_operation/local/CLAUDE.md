...
- **Temporary storage** — databases live in a per-user machine-local directory, `tempfile.gettempdir()/openjiuwen-fs-rwlocks-<uid>` (the sanitized login name replaces `<uid>` where `os.getuid` is unavailable), created 0700. The name is stable for one account, so that account's independently launched processes derive the same database path; a second account on the same host gets its own directory instead of being denied write access to the first one's
- **Idle cleanup** — when the last process-local lease ends, the manager evicts the business singleton, closes its SQLite connection and executor, refreshes the database mtime, and records its cleanup deadline in a process-local min-heap. A per-process task checks only due heap entries; after 2 idle minutes it creates an independent non-singleton connection, verifies that an immediate exclusive write lock succeeds, closes the probe for Windows compatibility, and then deletes the database.
- **Orphan sweep** — the idle heap only tracks locks this process leased, so a hard-killed process would leave its databases on disk forever. Each cleanup tick therefore also scans the lock directory (off the event loop, one bounded batch per tick) for `.db` files this process does not track whose mtime is older than `_orphan_ttl` (24h), and queues them through the same deletion path. **The age bar is deliberately far above `_idle_ttl`**: SQLite only refreshes mtime on write, so a database another live process holds for reads alone looks idle, and unlinking it would put the two processes on different inodes and silently end their mutual exclusion. Age only selects candidates; the exclusive-write-lock probe is what decides.
