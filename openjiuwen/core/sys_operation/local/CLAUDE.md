...
- **Idle cleanup** — when the last process-local lease ends, the manager evicts the business singleton, closes its SQLite connection and executor, refreshes the database mtime, and records its cleanup deadline in a process-local min-heap. A per-process task checks only due heap entries; after 2 idle minutes it creates an independent non-singleton connection, verifies that an immediate exclusive write lock succeeds, closes the probe for Windows compatibility, and then deletes the database. A hard-killed process relies on the OS to release its connections and leaves untracked databases in place.
