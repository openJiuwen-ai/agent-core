...
from openjiuwen.core.context_engine.processor.legacy.budget_guard import effective_context_budget
from openjiuwen.core.context_engine.processor.legacy.compressor.util import build_team_collaboration_reinjected_messages
from openjiuwen.core.context_engine.processor.legacy.offloader.message_summary_offloader import TRUNCATED_MARKER
...
class LegacyRoundLevelCompressorConfig(BaseModel):
    """
    Configuration for LegacyRoundLevelCompressor.
...
class LegacyRoundLevelCompressor(ContextProcessor):
    def __init__(self, config: LegacyRoundLevelCompressorConfig):
    ...
    async def _apply_llm_phase(
        ...
        *,
        ...
        if model_messages is None:
            logger.warning(
                f"[LegacyRoundLevelCompressor] phase={phase_name} skipped "
        ...
        if not replacements:
            logger.warning(f"[LegacyRoundLevelCompressor] phase={phase_name} produced no valid replacements")
            return None

        updated_messages = self._apply_replacements(messages, replacements)
        logger.info(
            f"[LegacyRoundLevelCompressor] phase={phase_name} context_window_tokens "
    ...
    def _wrap_memory_block(self, summary: str, scope: str) -> str:
        return (
            ...
            "processor: LegacyRoundLevelCompressor\n"
    ...
    def _looks_like_ack(message: BaseMessage) -> bool:
        if not isinstance(message, AssistantMessage):
            return False
        return LegacyRoundLevelCompressor._to_text(message.content).strip() in {
