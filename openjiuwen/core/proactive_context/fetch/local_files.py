"""Local Files proactive-context fetch provider."""
...
_SKIPPED_DIRECTORIES = {".git", ".venv", "node_modules", "__pycache__", ".pcs"}
_BATCH_SIZE = 20
...
def _root_dir(config: PCSFetchServiceConfig) -> Path:
...
def _changes(
    ...
    *,
    ...
    for candidate in candidates:
        ...
        if previous_summary is None or previous_summary.get("revision_id") != candidate["revision_id"]:
            changes.append({**candidate, "operation": "upsert"})
    ...
    if initial or len(changes) > max_items:
        changes.sort(key=lambda change: (-int(change["mtime_ns"]), str(change["relative_path"]).casefold()))
    else:
        changes.sort(key=lambda change: (str(change["relative_path"]).casefold(), str(change["relative_path"])))
    return changes
