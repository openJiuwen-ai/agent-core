"""Feishu ``lark-cli`` provider for the embedded proactive-context core.
...
cursor helpers are module-private functions so the public PCS class inventory
...
def _validate_cursor(value: object) -> dict[str, object]:
    ...
    for key, raw_resource in result.items():
        ...
        allowed_resource_keys = {
            "docs": {"items", "documents", "query"},
            "tasks": {"items"},
            "calendar": {"items", "start", "end"},
            "wiki_space": {"nodes"},
        }[key]
...
class FeishuFetchService(ContextFetchService):
            ...
            if isinstance(document_ids, (list, tuple)):
                ...
                for result in found:
                    if emitted >= remaining:
                        break
                    doc_id = _stable_identifier(result, "document_id", "doc_id", "token", "id", fallback="doc")
                    ...
                    if not content:
                        ...
                        raw = _as_object(raw_value, name="document")
                        payload = raw
                        _, content = _content_from_payload(raw)
                    ...
                    current_items[doc_id] = {"revision_id": revision, "title": title}
                    if not isinstance(previous, Mapping) or previous.get("revision_id") != revision:
                        emitted += 1
                        pending.append(
                            (
                                item,
                                {"docs": {"items": deepcopy(current_items), "query": query}},
                            )
                        )
        if "tasks" in resources and remaining - len(pending) > 0:
            pending.extend(await self._fetch_tasks(cursor=cursor))
        if "calendar" in resources and remaining - len(pending) > 0:
            pending.extend(await self._fetch_calendar(source=source, cursor=cursor))
    ...
    async def _fetch_wiki(
        ...
        *,
        ...
        previous = _resource_cursor(cursor, "wiki_space").get("nodes")
        ...
        for node_token, node in current.items():
            ...
            if not isinstance(previous_node, Mapping) or _revision(previous_node) != _revision(node):
                changed_tokens.append(node_token)
        if complete_scan:
            changed_tokens.extend(node_token for node_token in previous_nodes if node_token not in current)
        changed_tokens.sort(
            key=lambda token: (
                _wiki_change_sort_value(current.get(token) or previous_nodes.get(token)),
                token,
            ),
            reverse=True,
        )
        changed_tokens = changed_tokens[:max_items]

        changed: list[tuple[RawChangeItem, Mapping[str, object]]] = []
        running_nodes = deepcopy(previous_nodes)
        for node_token in changed_tokens:
            node = current.get(node_token)
            if node is None:
                old_node = previous_nodes.get(node_token)
                ...
                changed.append(
                    (
                        ...
                        {"wiki_space": {"nodes": deepcopy(running_nodes)}},
                    )
                )
                continue
            ...
            changed.append((item, {"wiki_space": {"nodes": deepcopy(running_nodes)}}))
