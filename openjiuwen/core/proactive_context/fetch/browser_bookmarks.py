"""Windows Microsoft Edge bookmarks provider for the embedded PCS core."""
...
_PAGE_USER_AGENT = (
    ...
    "Chrome/126.0 Safari/537.36 PCSBrowserBookmarks/1.0"
)
    ...
    async def fetch(
        ...
        *,
        ...
        try:
            ...
            changes: list[tuple[str, str, dict[str, str] | None, str]] = []
            for logical_id, (bookmark, fingerprint) in current.items():
                if previous.get(logical_id) != fingerprint:
                    changes.append(("upsert", logical_id, bookmark, fingerprint))
            for logical_id, fingerprint in previous.items():
                if logical_id not in current:
                    changes.append(("delete", logical_id, None, fingerprint))
            changes.sort(
                key=lambda change: (
                    0 if change[0] == "upsert" else 1,
                    -_date_sort_value(change[2]["date_added"]) if change[2] is not None else 0,
                    change[2]["folder_path"].casefold() if change[2] is not None else "",
                    change[2]["title"].casefold() if change[2] is not None else "",
                    change[1],
                )
            )
            ...
            for batch_index in range(0, change_limit, _BATCH_SIZE):
                ...
                for operation, logical_id, bookmark, fingerprint in chunk:
                    if operation == "delete":
