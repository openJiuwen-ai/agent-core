"""GitHub REST/archive provider for the embedded proactive-context core.
...
materialisation; they do not own state or become additional PCS classes.
"""
...
def _marker_path(candidate: Path) -> Path:
    return candidate / ".pcs-marker.json"
...
def _validate_cursor(value: object) -> dict[str, object]:
    ...
    for key in ("issues", "pull_requests", "commits"):
        ...
        if section is None:
            continue
        if not isinstance(section, Mapping) or set(section) - {"updated_at", "seen_ids"}:
            raise _fetch_error(f"GitHub {key} cursor is invalid")
        updated_at = section.get("updated_at")
        seen_ids = section.get("seen_ids", [])
        if updated_at is not None and not isinstance(updated_at, str):
            raise _fetch_error(f"GitHub {key} cursor is invalid")
        if (
            not isinstance(seen_ids, list)
            or len(seen_ids) > 10_000
            or any(not isinstance(item, str) or not item for item in seen_ids)
        ):
            raise _fetch_error(f"GitHub {key} cursor is invalid")
...
def _stable_cursor_part(cursor: Mapping[str, object] | None, key: str) -> tuple[str | None, set[str]]:
    value = cursor.get(key) if isinstance(cursor, Mapping) else None
    if not isinstance(value, Mapping):
        return None, set()
    watermark = value.get("updated_at")
    ids = value.get("seen_ids")
    seen_ids = ids if isinstance(ids, list) else []
    return (watermark if isinstance(watermark, str) else None), {
        str(item) for item in seen_ids if isinstance(item, str)
    }


def _should_emit(updated_at: str | None, stable_id: str, watermark: str | None, seen_ids: set[str]) -> bool:
    if watermark is None:
        return stable_id not in seen_ids
    if updated_at is None:
        return stable_id not in seen_ids
    return updated_at > watermark or (updated_at == watermark and stable_id not in seen_ids)
...
def _cursor_update(
    ...
    previous_watermark = previous_mapping.get("updated_at")
    previous_ids_value = previous_mapping.get("seen_ids", [])
    previous_ids = (
        [item for item in previous_ids_value if isinstance(item, str)] if isinstance(previous_ids_value, list) else []
    )
    current_ids = [stable_id for stable_id, _ in items]
    valid = [(stable_id, timestamp) for stable_id, timestamp in items if timestamp is not None]
    if not valid:
        watermark = previous_watermark if isinstance(previous_watermark, str) else None
        merged_ids = sorted(set(previous_ids + current_ids))[-10_000:]
        return {key: {"updated_at": watermark, "seen_ids": merged_ids}}
    current_watermark = max(timestamp for _, timestamp in valid)
    if isinstance(previous_watermark, str) and previous_watermark > current_watermark:
        return {key: {"updated_at": previous_watermark, "seen_ids": sorted(set(previous_ids))[-10_000:]}}
    watermark = current_watermark
    current_max_ids = [stable_id for stable_id, timestamp in valid if timestamp == watermark]
    if previous_watermark == watermark:
        ids = sorted(set(previous_ids + current_max_ids))[-10_000:]
    else:
        ids = sorted(set(current_max_ids))[-10_000:]
    return {
        key: {
    ...
    async def _fetch_impl(self, *, run_id: str, cursor: dict[str, object] | None) -> list[FetchBatch]:
        ...
        for resource, endpoint, label, is_commit in resource_specs:
            ...
            if resource_limit <= 0 or remaining <= 0:
                continue
            resource_start = len(pending)
            watermark, seen_ids = _stable_cursor_part(old_cursor, resource)
            payloads = await self._list_resource(
                ...
                since=watermark if resource in {"issues", "commits"} else None,
            )
            resource_updates: list[tuple[str, str | None]] = []
            for payload in payloads:
                ...
                if not _should_emit(updated_at, stable_id, watermark, seen_ids):
                    continue
                ...
                pending.append((item, {}, None, None))
                resource_updates.append((stable_id, updated_at))
                remaining -= 1
                if len(resource_updates) >= resource_limit or remaining <= 0:
                    break
            if resource_updates:
                # Attach the cumulative cursor after each item.  This lets a
                # batch acknowledge exactly the items it has published rather
                # than advancing the cursor for later batches.
                resource_cursor: object = old_cursor.get(resource)
                for offset, (stable_id, updated_at) in enumerate(resource_updates):
                    update = _cursor_update(resource, [(stable_id, updated_at)], resource_cursor)
                    resource_cursor = update[resource]
                    item_index = resource_start + offset
                    item, _, path, revision = pending[item_index]
                    pending[item_index] = (item, update, path, revision)
