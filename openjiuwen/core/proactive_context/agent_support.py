...
_PCS_ROUND_LEVEL_PROCESSOR_KEY = "PCSCoreRoundLevelCompressor"
...
# The PCS agent can edit its own candidate output, but it does not need a
...
def _agent_error(
    message: str = "proactive context agent execution failed",
    *,
...
def validate_pcs_messages(messages: Sequence[object]) -> None:
...
def trim_pcs_messages(messages: Sequence[object], budget: int) -> list[object]:
...
def _make_sys_operation(sandbox: Path) -> SysOperation:
    config = LocalWorkConfig(
        shell_allowlist=list(_PCS_SHELL_ALLOWLIST),
        sandbox_root=[str(sandbox)],
        restrict_to_sandbox=True,
        dangerous_patterns=list(_PCS_DANGEROUS_PATTERNS),
    )
    card = SysOperationCard(
        id=f"pcs-agent-sys-{uuid.uuid4().hex}",
...
def _make_context_processor_rail(
    ...
    # Keep a stable PCS-only alias to the existing core class without mutating
    ...
    with _PROCESSOR_REGISTRATION_LOCK:
        ...
        registered = processor_map.get(_PCS_ROUND_LEVEL_PROCESSOR_KEY)
        if registered is None:
            processor_map[_PCS_ROUND_LEVEL_PROCESSOR_KEY] = RoundLevelCompressor
        elif registered is not RoundLevelCompressor:
            raise _agent_error("PCS context processor registry key is occupied", fallback_allowed=False)
    ...
    return ContextProcessorRail(
        processors=(_PCS_ROUND_LEVEL_PROCESSOR_KEY, config),
        preset=False,
    )
...
def _make_agent(
    ...
    sys_operation_rail = SysOperationRail(
        ...
        bash_deny_patterns=list(_PCS_DANGEROUS_PATTERNS),
    )
    ...
    # SOUL.md, memory, skills, ...), which is outside the PCS sandbox contract.
    ...
    agent = create_deep_agent(
        ...
        system_prompt=(
            ...
            "or paths outside the sandbox. This is a disposable PCS sandbox, not a generic "
            ...
            "shell. PCS performs the final validation, so do not repeatedly create temporary "
...
async def _register_agent_callbacks(
    ...
    async def pcs_before_model_call_callback(ctx: AgentCallbackContext) -> None:
        await _before_model_call_context_compression(ctx, state=state)

    async def pcs_after_model_call_callback(ctx: AgentCallbackContext) -> None:
        await _after_model_call_context_compression(ctx, state=state)

    async def pcs_after_react_iteration_callback(ctx: AgentCallbackContext) -> None:
...
def _snapshot_sandbox(sandbox: Path) -> Path:
    baseline = Path(tempfile.mkdtemp(prefix=".pcs-agent-baseline-", dir=str(sandbox.parent)))
    try:
...
async def run_pcs_agent(
    *,
    ...
    try:
        ...
        session_id = f"pcs-agent-{uuid.uuid4().hex}"
...
__all__ = ["run_pcs_agent", "trim_pcs_messages", "validate_pcs_messages"]
