"""Embedded proactive-context Core runtime.

``PCS`` owns the small amount of orchestration needed by the first embedded
...
from openjiuwen.core.proactive_context.context_graph import (
    build_context_graph,
...
from openjiuwen.core.proactive_context.fetch.browser_bookmarks import BrowserBookmarksFetchService
from openjiuwen.core.proactive_context.fetch.feishu import (
    FeishuFetchService,
...
from openjiuwen.core.proactive_context.fetch.github import GitHubFetchService
...
from openjiuwen.core.proactive_context.models import FetchBatch, PCSStatus
...
def _source_fingerprint(config: PCSFetchServiceConfig) -> str:
...
class PCS:
    """Core aggregate for the embedded proactive-context runtime."""

    FetchServiceConfig = PCSFetchServiceConfig
    Config = PCSConfig
    Status = PCSStatus
    Error = BaseError
    ...
    def _status_for_name(name: str) -> StatusCode:
        """Return a PCS-owned status for Host error translation."""
    ...
    def __init__(self, *, home: str | Path) -> None:
        ...
        self._config: PCSConfig | None = None
    ...
    async def set_configuration(self, config: PCSConfig) -> None:
        """Set or replace the complete configuration while PCS is stopped."""
        if not isinstance(config, PCS.Config):
            raise _error(StatusCode.CONTEXT_PROACTIVE_CONFIG_INVALID, "config must be PCS.Config")
        await self._cancel_authorization()
        async with self._state_lock:
            if self._state in {"STARTING", "RUNNING", "STOPPING", "FAILED"}:
                raise _state_error("PCS must be stopped before configuration changes")
    ...
    async def authorize_provider(self, provider: str) -> dict[str, object]:
        ...
        async with self._authorization_lock:
            ...
            if task is not None and challenge is not None and float(challenge.get("expires_monotonic", 0.0)) > now:
                return {
                    "provider": "feishu",
                    "state": "pending",
            ...
            ready, _granted = await _lark_cli_auth_status(required_scopes)
            if ready:
                self._authorization_task = None
                self._authorization_challenge = None
                return {"provider": "feishu", "state": "ready", "verification_url": None, "expires_at": None}
            ...
            task = asyncio.create_task(
                _lark_cli_finish_authorization(device_code, timeout_seconds=timeout_seconds),
                name="pcs-feishu-authorization",
            )
    ...
    async def activate_runtime(self) -> None:
        ...
        async with self._state_lock:
            ...
            if self._state == "STARTING" and self._activation_task is not None:
                ...
                task = asyncio.create_task(self._activate_runtime_impl(), name="pcs-activation")
                self._activation_task = task
            else:
                raise _state_error(f"cannot activate PCS from {self._state}")
        try:
        ...
        try:
            ...
            async with self._state_lock:
                if self._state != "STARTING":
                    raise _state_error("PCS activation was superseded")
                self._state = "RUNNING"
            ...
            wrapped = _state_error("PCS runtime activation failed")
    ...
    async def start_fetch_service(self, service_id: str) -> None:
        ...
        async with self._fetch_lock:
            ...
            if not config.fetching_enabled:
                ...
                raise _state_error("PCS fetching is disabled")
            if self._state not in {"STARTING", "RUNNING"}:
                raise _state_error("PCS runtime is not active")
            ...
            task = asyncio.create_task(self._run_fetch_service(safe_id, stop_event), name=f"pcs-fetch-{safe_id}")
        ...
        *,
        ...
        async with self._state_lock:
            ...
            if not config.enabled:
                raise _state_error("PCS is disabled")
            if self._state != "RUNNING":
                raise _state_error("PCS runtime is not running")
            try:
                pipeline_running = pipeline is not None and pipeline.is_running()
            except Exception:
                pipeline_running = False
            if not pipeline_running:
                raise _state_error("PCS pipeline is not running")
            ...
            async with self._fetch_lock:
                ...
                if service_id is None:
                    ...
                    if not target_ids:
                        raise _state_error("PCS has no enabled fetch service")
                else:
                    safe_id = _safe_service_id(service_id)
                ...
                try:
                    ...
                    for target_id in target_ids:
                        task = asyncio.create_task(
                            ...
                            name=f"pcs-fetch-manual-{target_id}",
                        )
    ...
    async def snapshot(self) -> PCSStatus:
        ...
        if state == "RUNNING" and not pipeline_running:
            ...
            if last_error is None:
                last_error = {
                    ...
                    "message": "PCS pipeline stopped unexpectedly",
                    "operation": "snapshot",
                }
    ...
    async def deactivate_runtime(self, *, timeout_seconds: float = 30.0) -> None:
        ...
        if activation is not None and not activation.done():
            ...
            if remaining > 0:
                try:
                    ...
                    stop_error = _error(StatusCode.CONTEXT_PROACTIVE_RUNTIME_TIMEOUT, "PCS activation stop timed out")
                    activation.cancel()
                except asyncio.CancelledError:
                    current = asyncio.current_task()
                    if current is not None and current.cancelling():
                    ...
                    stop_error = _state_error("PCS activation failed while stopping")
                    stop_error.__cause__ = exc
            else:
                stop_error = _error(StatusCode.CONTEXT_PROACTIVE_RUNTIME_TIMEOUT, "PCS stop timed out")
                activation.cancel()
            ...
            if remaining <= 0:
                ...
                stop_tasks = [
                    asyncio.create_task(
                        ...
                        name=f"pcs-fetch-stop-{service_id}",
                    )
        ...
        if pipeline is not None:
            remaining = deadline - asyncio.get_running_loop().time()
            if remaining <= 0:
                stop_error = stop_error or _error(StatusCode.CONTEXT_PROACTIVE_RUNTIME_TIMEOUT, "PCS stop timed out")
            else:
                pipeline_stop_task = asyncio.create_task(
                    pipeline.stop(timeout_seconds=remaining), name="pcs-pipeline-stop"
                )
                try:
                    ...
                    stop_error = stop_error or _error(
                        StatusCode.CONTEXT_PROACTIVE_RUNTIME_TIMEOUT, "PCS stop timed out"
                    )
                except asyncio.CancelledError:
                    current = asyncio.current_task()
                    if current is not None and current.cancelling():
        ...
        async with self._state_lock:
            ...
            if stop_error is not None:
                ...
                self._set_last_error(_state_error("PCS runtime stop did not complete"), operation="deactivate_runtime")
        if stop_error is not None:
            raise stop_error

    async def _cancel_authorization(self) -> None:
        async with self._authorization_lock:
            task = self._authorization_task
            self._authorization_task = None
            self._authorization_challenge = None
    ...
    def _create_fetch_provider(self, config: PCSFetchServiceConfig) -> ContextFetchService:
    ...
    def _service_config(self, service_id: str) -> PCSFetchServiceConfig:
        config = self._config
        if config is None:
            raise _state_error("PCS has not been configured")
        for service in config.fetch_services:
