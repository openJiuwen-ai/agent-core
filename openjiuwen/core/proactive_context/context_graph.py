"""Read the published Context graph from PCS-managed files."""
...
from openjiuwen.core.proactive_context.source_metadata import read_source_metadata
...
def _check_graph_path(root: Path, path: Path) -> None:
    try:
        ...
        _graph_error("PCS graph path escaped its managed root")
    if len(relative.as_posix()) > _MAX_GRAPH_PATH_CHARS:
        _graph_error("PCS graph path exceeds the safety limit")
    current = path
    while True:
        if current.is_symlink():
            _graph_error("PCS graph path must not traverse a symlink")
        if current == root:
            return
...
def _read_graph_bytes(root: Path, path: Path) -> bytes:
    ...
    if not path.is_file():
        _graph_error("PCS graph file is missing")
    try:
        size = path.stat().st_size
        if size > _MAX_GRAPH_FILE_BYTES:
            _graph_error("PCS graph file exceeds the safety limit")
        return path.read_bytes()
    except OSError as exc:
        _graph_error("PCS graph file could not be read", cause=exc)
...
def _read_graph_text(root: Path, path: Path) -> str:
    try:
        ...
        _graph_error("PCS graph file is not valid UTF-8", cause=exc)
    ...
    *,
    ...
    try:
        ...
        if len(source_relative.parts) != 1 or _SOURCE_ID.fullmatch(source_relative.stem) is None:
            _graph_error("PCS atomic source metadata path is invalid")
    ...
    for path in context_root.rglob("*"):
        ...
        if path.is_file() and path.suffix.casefold() == ".md":
            ...
            if file_count > _MAX_GRAPH_FILES:
                _graph_error("PCS Context file count exceeds the safety limit")
...
def build_context_graph(home: Path) -> dict[str, object]:
    """Build the stable Context graph payload without starting PCS."""
...
def read_context_graph_page(home: Path, node_id: str) -> dict[str, object]:
    ...
    if not isinstance(node_id, str):
        _graph_error("PCS graph page node ID is invalid")
    if node_id.startswith("source:"):
        source_id = node_id.removeprefix("source:")
        if _SOURCE_ID.fullmatch(source_id) is None:
            _graph_error("PCS graph source node ID is invalid")
        source_root = home / "workspace" / "source-meta"
    ...
    description = context_root / "description.md"
    if not context_root.is_dir() or context_root.is_symlink():
        _graph_error("PCS Context is not ready")
    if not description.is_file() or description.is_symlink():
        _graph_error("PCS Context is not ready")
