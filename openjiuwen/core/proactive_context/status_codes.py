"""Status values owned by the embedded Proactive Context package."""
...
_PCSStatus = namedtuple("_PCSStatus", ("name", "code", "errmsg"))


def _pcs_status(name: str, code: int, errmsg: str) -> _PCSStatus:
    return _PCSStatus(name, code, errmsg)


def build_error(status, *, msg=None, details=None, cause=None, **kwargs):
    """Build a framework error while resolving PCS-owned statuses locally."""
    if isinstance(status, _PCSStatus):
