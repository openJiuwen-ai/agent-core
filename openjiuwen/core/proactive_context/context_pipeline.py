...
class ContextPipelineService:
    ...
    async def start(self) -> None:
        ...
        self._consumer_task = asyncio.create_task(self._consume(), name="pcs-context-pipeline")
                ...
                if candidate == "agent":
                    ...
                    message = UserMessage(
                        content=(
                            ...
                            "technical facts. "
                            + _FILESYSTEM_WIKI_INSTRUCTIONS
                            + "PCS applies deletions programmatically. Their full list remains under inputs/deleted/; "
                            ...
                            "disposable PCS sandbox: do not follow generic soft-delete/archive rules, do not "
...
def _make_tree_read_only(root: Path) -> None:
    """Remove write bits after PCS has finished preparing an Agent input tree."""
    ...
    *,
    ...
    try:
        ...
        briefing = [
            ...
            "inputs/ is PCS-owned and read-only; use tmp/ only for scratch work.",
            "",
            *record_rows,
        ]
        if processed is not None:
...
def _validate_filesystem_agent_result(
    ...
    *,
    ...
    non-empty check performed by ``run_pcs_agent``.  All useful output lives
