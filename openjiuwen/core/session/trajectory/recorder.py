...
global ``TracerHandlerRegistry``. Agent sessions pick up the handlers when
...
from openjiuwen.core.session.trajectory.handler import TrajectoryAgentHandler, TrajectoryWorkflowHandler
...
def _release_leaked_handlers(agent_handler_name: str, workflow_handler_name: str) -> None:
    ...
    def attach(self, agent) -> "TrajectoryRecorder":
        ...
        if not self._registered:
            ...
            self._finalizer = weakref.finalize(
                self, _release_leaked_handlers, self._agent_handler_name, self._workflow_handler_name
            )
