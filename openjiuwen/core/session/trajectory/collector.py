...
class _TraceBuffer:
    ...
    def __init__(self, trajectory_id: str, binding: "_TraceBinding | None" = None,
                 parent_buffer: "_TraceBuffer | None" = None, session_dir: str | None = None):
...
class TrajectoryCollector:
    ...
    def bind_trace(self, trace_id: str, session_id: str | None = None, agent_name: str | None = None) -> None:
        ...
        self._bindings[trace_id] = _TraceBinding(
            session_id=session_id, agent_name=agent_name, parent_trace_id=parent
        )
        ...
        *,
        ...
        async with self._lock:
            ...
            if buffer.steps_since_snapshot >= max(1, self._config.snapshot_every_n_steps):
                ...
                document = self._build_document(buffer, final_metrics=None)
    ...
    def _sanitize(self, field: str, text: str | None, truncated_fields: list[str]) -> str | None:
        ...
        Redaction runs first so truncation cannot split a secret and leave a
        recognizable prefix behind.
        """
