...
class RoutingStrategyName(str, Enum):
    """Supported model-group routing strategies."""

    OrderedFailover = "ordered-failover"
    TagFiltered = "tag-filtered"


class ResolvedRoutingConfig(BaseModel):
    """Routing policy selected for a resolved model group."""

    strategy: Optional[RoutingStrategyName] = None
...
class ResolvedModelGroup(BaseModel):
    ...
    routing: ResolvedRoutingConfig = Field(default_factory=ResolvedRoutingConfig)
