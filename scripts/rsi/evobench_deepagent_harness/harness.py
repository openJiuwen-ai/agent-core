...
class SubmissionCheckpointRail(DeepAgentRail):
    ...
    def _parse_audit_rows(
        ...
        *,
        ...
        for row in rows:
            ...
            if (
                ground_id not in expected_ids
                or ground_id in parsed
    ...
    async def _semantic_release_violation(self, content: str) -> str:
        ...
        if (
            ...
            or not isinstance(dependent, list)
            or not isinstance(unlisted, bool)
            or any(not isinstance(item, str) or item not in removed_ids for item in present + dependent)
        ):
            self.audit_parse_failures += 1
