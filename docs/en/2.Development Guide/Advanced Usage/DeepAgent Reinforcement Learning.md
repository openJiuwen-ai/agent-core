...
**Runtime note**: Standard rollout uses **`openjiuwen.harness.DeepAgent`** as the outer runtime. [`AgentFactory`](../API%20Docs/openjiuwen.agent_evolving/agent_rl/offline/runtime.md) builds and configures each task's agent with **`DeepAgentConfig`** (single-round rollout: **`enable_task_loop=False`**); tools attach to **`DeepAgent`**. `ParallelRuntimeExecutor` owns one shared `TrajectorySpanProcessor`, and `run_agent_and_collect_trajectory()` temporarily registers **`RLRail`** on each Agent before reading its canonical trajectory. A custom `agent_factory` must return an object that supports **`register_rail`** / **`invoke`** (for example **`DeepAgent`**).
