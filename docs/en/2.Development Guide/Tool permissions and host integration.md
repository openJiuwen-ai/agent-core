...
A title names the risk that triggered the confirmation: `title_risk_detected` frames it, and the name itself is the first of these that applies — the matched built-in rule's entry in `rule_risk_labels`, then `risk_interpreter_sink`, then `finding_shell_too_complex`, then the finding names (`finding_*`), and last the confirmation's own category (`risk_path`, `risk_network`, `risk_finding`, `risk_shell`). Where no risk is named at all the whole title comes from `title_tool` or `title_generic` instead.

`rule_risk_labels` is a table rather than a field per rule, because the built-in rule set grows between releases. A host's table replaces the default one whole; a rule id it does not list simply falls back to the next name above, so a host's table going out of date costs a named risk rather than raising.

Not every string in the ASK body is a template. The summary of a `path` or `network` confirmation is a path or a URL, a `shell` summary is the tool name and the command, and a `finding` over a command that reads or writes files is the file access; only the `tool` summary and the risk name that prefixes the remaining `finding` summaries are prose, and those are the ones `prompt_texts` covers.
