...
* **enable_memory_expiration**(bool, 可选)：是否启用过期记忆清理；为 `True` 时，引擎在调用 `add_messages` 时会自动清理超过保留时长的记忆。默认值：`False`。
* **memory_expiration_seconds**(int, 可选)：记忆保留时长（秒）；超过此时长的记忆将在过期清理时被清除；必须大于 0。默认值：`604800`（7 天）。
...
>>>     memory_expiration_seconds=7 * 24 * 60 * 60,  # 7 天
>>> )
```
