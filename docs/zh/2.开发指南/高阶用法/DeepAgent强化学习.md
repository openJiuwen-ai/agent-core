...
**运行时说明**：默认 rollout 以 **`DeepAgent`** 为外层运行时。[`AgentFactory`](../API文档/openjiuwen.agent_evolving/agent_rl/offline/runtime.md) 按任务构建并配置智能体（**`DeepAgentConfig`**，训练场景下 **`enable_task_loop=False`** 以单轮 rollout）；工具挂在 **`DeepAgent`** 上。`ParallelRuntimeExecutor` 持有一个共享 `TrajectorySpanProcessor`，`run_agent_and_collect_trajectory()` 为每个 Agent 临时注册 **`RLRail`**，再读取 canonical trajectory。自定义 `agent_factory` 必须返回支持 **`register_rail`** / **`invoke`** 的对象（例如 **`DeepAgent`**）。
