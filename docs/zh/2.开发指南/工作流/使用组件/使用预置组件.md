...
`KnowledgeRetrievalComponent` 是一个预置组件，用于将知识检索功能集成到 openJiuwen 工作流中。它封装了 `SimpleKnowledgeBase`，并通过 `VectorStoreConfig` 中的 `store_provider` 字段支持多种向量存储后端（Milvus、ChromaDB、PGVector）。

## 配置说明

创建 `KnowledgeRetrievalCompConfig` 来配置组件。主要字段包括：

- `kb_configs`：`KnowledgeBaseConfig` 列表，定义要检索的知识库。
- `retrieval_config`：`RetrievalConfig`，控制 top_k、score_threshold 等参数。
- `vector_store_config`：`VectorStoreConfig`，其中 `store_provider` 可设置为 `StoreType.Milvus`、`StoreType.Chroma` 或 `StoreType.PGVector`。
- `vector_store_additional_config`：传递给向量存储构造函数的额外关键字参数。
- `embed_config`：`EmbeddingConfig`，嵌入模型配置（vector/hybrid 索引类型时必填）。
- `result_separator`：将检索到的文本连接成 `context` 时使用的分隔符。默认值：`"\n\n"`。
- `include_metadata`：设置为 `True` 时输出中包含 `results_with_metadata`。

## 示例：RAG 工作流（Start → KnowledgeRetrieval → LLM → End）

以下示例使用 `KnowledgeRetrievalComponent` 构建一个简单的检索增强生成（RAG）流水线：
