...
# HTTP 请求组件（HTTPRequest Component）

`HTTPRequestComponent` 是一个预置组件，用于在工作流中发起 HTTP 请求并解析响应。它支持常见的 HTTP 方法，可配置请求头、查询参数、请求体、多种认证方式（Basic/Bearer/API Key/Digest/AWS）、响应格式解析（JSON/Text/Binary/自动检测）、重试、限流与超时，并实现了 `invoke`、`stream`、`collect`、`transform` 四种能力。

## 配置说明

通过 `HttpComponentConfig` 配置组件，其 `request_params` 字段为 `HttpRequestParamConfig`，主要字段如下：

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `url` | str | （必填） | 请求目标 URL，支持 `{{key}}` 占位符，运行时用输入值替换。 |
| `method` | str | `GET` | HTTP 方法。 |
| `headers` | Dict[str, str] \| str | `{}` | 请求头，可为字典或 `{{key}}` 模板字符串。 |
| `query_parameters` | Dict[str, Any] | `{}` | 查询参数，值支持 `{{key}}` 占位符。 |
| `body` | `HttpRequestBodyConfig` \| None | `None` | 请求体配置。 |
| `authentication` | `HttpAuthConfig` \| None | `None` | 认证配置，`type` 取值：`none`/`basic`/`bearer`/`api_key`/`digest`/`aws`。 |
| `response_handling` | `HttpResponseHandlingConfig` | 默认实例 | 响应处理：成功/失败状态码、响应格式（`JSON`/`TEXT`/`BINARY`/`AUTODETECT`）、数据提取属性、`response_mode`。 |
| `advanced_options` | `HttpAdvancedOptionsConfig` | 默认实例 | 高级选项（代理、SSL 校验等）。 |
| `retry_config` | `HttpRetryConfig` | 默认实例 | 重试配置（开关、最大次数、重试状态码、退避策略）。 |
| `rate_limit_config` | `HttpRateLimitConfig` | 默认实例 | 限流配置（`enabled`、`requests_per_unit`、`unit` second/minute/hour）。 |
| `timeout` | float | `60.0` | 请求超时（秒），范围 1.0–300.0。 |
| `max_response_byte_size` | int | `10485760` | 响应体最大字节数（默认 10 MB），超出将报错。 |
...
|------|------|------|
| `method` | str | 覆盖请求方法。 |
| `headers` | Dict[str, str] | 追加到配置的请求头。 |
| `query_parameters` | Dict | 追加到配置的查询参数。 |
| `body` | `HttpRequestBodyConfig` \| dict | 请求体。 |
| `authentication` | `HttpAuthConfig` \| dict | 认证配置。 |
| （占位符值） | Any | 用于替换 `url`/`headers`/`query_parameters` 中的 `{{key}}` 占位符。 |
...
| `body` | Any | 解析后的响应体：JSON → dict/list，TEXT → str，BINARY → bytes；可按 `response_data_property` 提取。 |
| `url` | str | 最终响应 URL。 |
| `ok` | bool | 是否成功（状态码命中成功集且不在失败集）。 |

> 当 `response_handling.response_mode` 为 `on-success` 且响应失败时，输出为空对象；为 `on-error` 且响应成功时，输出为空对象。

## 示例：HTTP 请求工作流（Start → HTTPRequest → End）
...
# 2. 配置 HTTP 请求组件
http_component = HTTPRequestComponent(HttpComponentConfig(request_params={
    "url": "https://api.github.com/repos/octocat/Hello-World",
    "method": "GET",
    "headers": {"Accept": "application/vnd.github+json"},
}))

# 3. 组装工作流图：Start → HTTPRequest → End
end = End({"responseTemplate": "{{statusCode}}"})

flow.set_start_comp("start", start)
flow.add_workflow_comp("http", http_component)
flow.set_end_comp("end", end, inputs_schema={"statusCode": "${http.statusCode}"})
...
async def main():
    session = create_workflow_session(session_id="http_session")
    result = await flow.invoke({}, session)
    print(result.result)
