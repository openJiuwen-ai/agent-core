...
`add_workflow_comp` 还支持以下关键字参数（均为可选，需以关键字形式传入），用于为单个业务组件配置重试、超时与异常处理：

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `max_retries` | int | `0` | 组件执行失败时的最大重试次数，`0` 表示不重试。 |
| `timeout` | float | `-1.0` | 单个组件执行的超时时间（秒），`<= 0` 表示不限制超时。 |
| `exception_config` | `ExceptionConfig` | `None` | 组件异常处理配置，用于错误恢复。 |

其中 `ExceptionConfig`（来自 `openjiuwen.core.workflow.workflow_config`）字段如下：

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `handle_type` | str | `"interrupt"` | 异常处理类型，框架仅识别该字段。 |
| （任意附加字段） | — | — | 模型开启 `extra="allow"`，可附加自定义字段供 `component_error_recovery` 处理器使用。 |

下面的示例为大模型组件配置 2 次重试、30 秒超时与中断式异常处理：
...
workflow.add_workflow_comp(
    ...
    max_retries=2,
    timeout=30.0,
    exception_config=ExceptionConfig(handle_type="interrupt"),
)
```
