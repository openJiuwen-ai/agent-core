...
| [UserInputGuardrail](./guardrail/builtin.md#class-openjiuwencoresecurityguardrailuserinputguardrail) | 用户输入验证护栏，监控用户输入事件。            |


### 快速开始

1. **实现检测后端**：继承 `GuardrailBackend` 实现自定义检测逻辑
2. **配置护栏**：使用内置护栏类（如 `UserInputGuardrail`）并设置检测后端
