...
各维度优化器的公共骨架：bind 过滤可优化 Operator 并返回数量（0 触发上层软退出）；add_trajectory/get_trajectories 缓存轨迹供 backward；update() 返回 Updates，由 Trainer.apply_updates 统一应用。
...
调用子类 _update() 得到 Updates，清空轨迹后返回；异常时清空轨迹并抛出 TOOLCHAIN_OPTIMIZER_UPDATE_EXECUTION_ERROR。
...
LLM 调用维度优化器基类：仅优化暴露 system_prompt / user_prompt 的算子；domain 为 `"llm"`，default_targets 为 `["system_prompt", "user_prompt"]`。子类实现 _backward / _update 完成提示词优化逻辑。
...
基于 LLM 的指令（提示词）优化器：backward 阶段用 LLM 对 bad cases 生成文本梯度写入 TextualParameter；update 阶段用 LLM 生成优化后的 system/user 提示词，返回 Updates。
...
继承自 LLMCallOptimizerBase，targets 默认即 system_prompt、user_prompt；_backward 为每个绑定算子生成文本梯度并写入对应 TextualParameter；_update 根据梯度生成优化后的 system/user 提示词并保持占位符一致，返回 (operator_id, target) -> 新内容的 Updates。
