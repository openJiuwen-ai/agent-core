...
### 6.3 因果交接完整性与动作可推出性（2026-08-24）

本轮使用同一条真实 GLM-5.2 失败轨迹连续回放，而不是只用人工构造 JSON 验收。
`v12-v15` 依次暴露了审计输入截断、已通过兄弟诊断被修复覆盖、supported 假设静默
丢失、以及“与任务不冲突”被错误当成“任务要求这样做”等通用问题。

最终协议 `generic_behavior_causal_v16` 完成以下修复：

1. 调查计划禁止用评分器期望值定义因果假设；修复提示会收到控制器的具体冲突，而不是
   盲目重写。
2. 审计只接收公开任务、选中假设、对应评估、实际引用的控制器证据和因果交接，避免整包
   截断；模型漏审时只拒绝漏掉的诊断。
3. 控制器维护全局假设评估账本。删除冗余残余诊断前会迁移其评估；每个 supported 假设
   必须被选择、包含或明确标成不可行动。
4. 评分器依赖的交接会被单独降级，已验证兄弟诊断保持不变。审计修复只替换被拒绝项，
   不得覆盖已批准项。
5. 新增动作可推出性审计。动作必须来自公开任务条款、任务可见不变量或通用运行安全
   不变量；仅仅“可能符合某种解释”不够，输出错误也不能推出任意修复算法。
6. Analyzer 默认输出预算由 8192 增至 16384，避免多假设、多诊断 JSON 因长度结束而丢项。

真实 `v16` 回放中，模型观察到的“保留原趋势”解释被保留为记录但标记为
`non_actionable`，没有生成 Issue；系统只保留“最终制品修改后未校验实际交付对象”这一
任务可见根因，独立审计状态为 `approved`。下游只编译出 1 个优化假设，并严格绑定被选中
假设 `h4`。这证明 Analyzer 到 Hypothesis Compiler 的语义链路已恢复，但仍不等同于候选
在官方评分上已经提分。

本轮相关回归为 `454 passed, 2 skipped`，Ruff 与 `git diff --check` 均通过。完整 64 题
实验应在新的小规模端到端候选评测确认动作实际触发后再启动，避免把 Analyzer 回放通过
误报为 Harness 已提分。
