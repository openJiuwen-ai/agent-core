...
from openjiuwen.deepagents.rails import MemoryRail
...
class TestMemoryRailE2E(unittest.IsolatedAsyncioTestCase):
    """MemoryRail 真实端到端调用测试。"""

    async def asyncSetUp(self):
        await Runner.start()
        self._tmp_dir = tempfile.TemporaryDirectory(prefix="memory_rail_e2e_")
        self._work_dir = Workspace().root_path
        self._sys_operation_id = f"memory_rail_sysop_{uuid.uuid4().hex}"
        ...
        memory_dir = Path(self._work_dir) / "memory"
    ...
    async def test_01_memory_rail_basic_invoke(self):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_02_write_memory_tool(self):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_03_memory_search_tool(self):
        ...
        self.assertEqual(search_result.get("result_type"), "answer")
        self.assertIn("Python", search_result.get("output", ""), "Should find Python in memory")

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_04_read_memory_tool(self):
        ...
        self.assertIsInstance(result, dict)
        self.assertEqual(result.get("result_type"), "answer")
        self.assertIn("lisi@example.com", result.get("output", ""), "Should find email in memory")

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_05_edit_memory_tool(self):
        ...
        self.assertIn("13900139000", result.get("output", ""), "Should find updated phone number")

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_06_memory_get_tool(self):
        ...
        self.assertIsInstance(result, dict)
        self.assertEqual(result.get("result_type"), "answer")
        self.assertIn("100", result.get("output", ""), "Should find budget info")

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_07_write_memory_append_mode(self):
        ...
        self.assertTrue(
            "登录" in output and "注册" in output,
            "Should find at least one feature in memory"
        )

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_08_update_user_profile(self):
        """测试更新 User.md 用户画像。"""
        self._require_api_config()
        ...
        output = result.get("output", "")
        self.assertTrue(
            "王明" in output or "产品经理" in output,
            "Should find user info in memory"
        )

    @pytest.mark.asyncio
    @pytest.mark.skip("need llm and embedding")
    async def test_09_write_memory_md_file(self):
        ...
        self.assertIsInstance(result, dict)
        self.assertEqual(result.get("result_type"), "answer")
        self.assertIn("192.168.1.100", result.get("output", ""), "Should find server IP")
