...
运行方式::
    ...
    python -m pytest tests/system_tests/rsi/auto_harness/test_e2e_full_cycle.py -v -s
"""
...
API_BASE = os.getenv("API_BASE") or os.getenv("OPENJIUWEN_API_BASE") or _settings.get("apiBase", "")
API_KEY = os.getenv("API_KEY") or os.getenv("OPENJIUWEN_API_KEY") or _settings.get("apiKey", "")
MODEL_NAME = os.getenv("MODEL_NAME") or os.getenv("OPENJIUWEN_MODEL") or _settings.get("model", "GLM-5")
MODEL_PROVIDER = os.getenv("MODEL_PROVIDER") or os.getenv("OPENJIUWEN_PROVIDER") or _settings.get("provider", "OpenAI")
MODEL_TIMEOUT = int(os.getenv("MODEL_TIMEOUT", "120"))

CI_GATE_YAML = str(Path(__file__).parent / "ci_gate_e2e.yaml")
...
class TestAutoHarnessE2EFullCycle(
    ...
    def _require_llm_config(self) -> None:
        if not API_KEY or not API_BASE:
            self.skipTest("E2E requires API_KEY and API_BASE in environment.")
    ...
    def _make_config(self) -> AutoHarnessConfig:
        ...
        return AutoHarnessConfig(
            ...
            experience_dir=str(Path(self._workspace) / ".auto_harness/experience"),
