...
def test_external_endpoint_uses_jiuwen_provider():
    ...
    assert _external_cli_provider_name(config, cli_agent="codex") == "jiuwen"
...
def test_external_endpoint_provider_is_independent_of_host(api_base: str):
    assert _external_cli_provider_name(_client_config(api_base=api_base), cli_agent="codex") == "jiuwen"
...
def test_official_endpoint_without_api_base_keeps_protocol_label():
    ...
    assert _external_cli_provider_name(config, cli_agent="codex") == "OpenAI"


@pytest.mark.level0
def test_non_codex_cli_keeps_configured_provider():
    config = _client_config()
    assert _external_cli_provider_name(config, cli_agent="claude") == "OpenAI"


@pytest.mark.level0
def test_team_model_config_conversion_uses_resolved_provider():
    member_model = _member_model(_client_config())
    external = _team_model_config_to_external(member_model, cli_agent="codex")
...
def test_resolved_provider_name_is_codex_bare_key_safe():
    ...
    assert _BARE_KEY_RE.fullmatch(_external_cli_provider_name(_client_config(), cli_agent="codex"))
