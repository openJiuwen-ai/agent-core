...
from openjiuwen.symphony.agent.retrieval_toolkit import _load_skill_category_paths, _record_hashes
...
def test_toolkit_disables_lexical_equivalence_split_by_default() -> None:
...
def test_toolkit_reports_missing_index(tmp_path: Path) -> None:
    ...
    assert status["skill_category_paths"] == []


def test_build_plan_tracks_added_and_updated_skill_ids(tmp_path: Path) -> None:
    previous = [
...
def test_category_path_loader_returns_variable_depth_paths_in_request_order(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        paths = _load_skill_category_paths(
            tmp_path,
            worker_ids=("three-levels", "two-levels", "three-levels"),
        )

    assert paths == [
        {"skill_id": "three-levels", "category_path": ["L1", "L2", "L3"]},
        {"skill_id": "two-levels", "category_path": ["L1", "L2"]},
    ]
...
def test_category_path_loader_rejects_invalid_final_catalog(
    ...
    with (
        ...
        _load_skill_category_paths(tmp_path, worker_ids=(worker_id,))
...
def test_incremental_build_returns_and_persists_final_category_path(tmp_path: Path) -> None:
    ...
    expected = [{"skill_id": "new", "category_path": ["Productivity", "Documents"]}]
    assert result["success"] is True
    assert result["data"]["skill_category_paths"] == expected
    assert toolkit.check_build_status()["data"]["skill_category_paths"] == expected
    state = json.loads((tmp_path / "state.json").read_text(encoding="utf-8"))
    assert state["build"]["skill_category_paths"] == expected
...
def test_missing_incremental_category_path_preserves_previous_index(tmp_path: Path) -> None:
    ...
    assert toolkit.check_build_status()["data"]["skill_category_paths"] == []
