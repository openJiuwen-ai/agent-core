    ...
    assert evidence["port_mappings"] == [
        {
            ...
            "target_input": "file",
            "target_type": "file",
            "match_reason": ("source output URL can satisfy target file-like input that accepts remote references"),
            "match_method": "semantic_overlap_match",
        }
    ]
...
async def test_graph_builder_materializes_accepted_exact_io_match():
    ...
    assert edge.source == "capability:source"
    assert edge.target == "capability:target"
    assert edge.type == "can_feed"
    assert edge.confidence == 0.9


def test_graph_lookup_emits_text_term_lookup():
...
def _candidates(*skills: Fingerprint):
    return CandidateGenerator().generate(SkillRegistry(skills={skill.id: skill for skill in skills}))
