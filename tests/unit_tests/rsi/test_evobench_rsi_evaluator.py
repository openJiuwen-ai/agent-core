...
async def test_candidate_ignores_h0_cache_and_runs_official_protocol(
    ...
    policy_model = tmp_path / "policy.yaml"
    judge_model = tmp_path / "judge.yaml"
    _write_model_config(policy_model, model="policy-model")
    _write_model_config(judge_model, model="judge-model")
    ...
    eval_ref_path = await rsi_evaluator.evaluate_batch(
        ...
        judge_model_config=str(judge_model),
    )
