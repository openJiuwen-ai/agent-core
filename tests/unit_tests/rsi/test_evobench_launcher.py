...
def test_load_local_model_expands_portable_environment_refs(tmp_path: Path, monkeypatch) -> None:
    path = _write_model(
        tmp_path / "model.yaml",
        base="${RSI_TEST_MODEL_BASE}",
        key="${RSI_TEST_MODEL_KEY}",
        model="portable-model",
    )
    monkeypatch.setenv("RSI_TEST_MODEL_BASE", "https://model.example/v1")
    monkeypatch.setenv("RSI_TEST_MODEL_KEY", "test-key")

    model = launcher.load_local_model(path)

    assert model.api_base == "https://model.example/v1"
    assert model.api_key == "test-key"


def test_load_local_model_rejects_missing_environment_ref(tmp_path: Path, monkeypatch) -> None:
    path = _write_model(
        tmp_path / "model.yaml",
        base="https://model.example/v1",
        key="${RSI_MISSING_MODEL_KEY}",
        model="portable-model",
    )
    monkeypatch.delenv("RSI_MISSING_MODEL_KEY", raising=False)

    try:
        launcher.load_local_model(path)
    except ValueError as error:
        assert "RSI_MISSING_MODEL_KEY" in str(error)
    else:
        raise AssertionError("unresolved model environment variable was accepted")
