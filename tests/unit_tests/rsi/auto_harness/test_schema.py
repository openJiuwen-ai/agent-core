...
class TestAutoHarnessConfig:
    ...
    def test_experience_dir_from_data_dir(self):
        ...
        assert cfg.resolved_experience_dir == str(Path("/tmp/ah") / "experience")
    ...
    def test_explicit_experience_dir_takes_precedence(
        ...
        assert cfg.resolved_experience_dir == ("/tmp/custom-exp")

    def test_worktrees_dir_from_data_dir(self):
        cfg = AutoHarnessConfig(data_dir="/tmp/ah")
        assert cfg.worktrees_dir == str(Path("/tmp/ah") / "worktrees")

    def test_runs_dir_from_data_dir(self):
        cfg = AutoHarnessConfig(data_dir="/tmp/ah")
        assert cfg.runs_dir == str(Path("/tmp/ah") / "runs")

    def test_runtime_extensions_dir_from_data_dir(self):
        cfg = AutoHarnessConfig(data_dir="/tmp/ah")
        assert cfg.runtime_extensions_dir == str(Path("/tmp/ah") / "runtime_extensions")

    def test_cache_repo_dir_from_data_dir(self):
        cfg = AutoHarnessConfig(data_dir="/tmp/ah")
        assert cfg.cache_repo_dir == str(Path("/tmp/ah") / "repo" / "agent-core")
    ...
    def test_build_paths_includes_runtime_extensions_dir(
        ...
        assert paths.runtime_extensions_dir == str(Path("/tmp/ah") / "runtime_extensions")
    ...
    def test_cache_repo_dir_uses_upstream_repo(self):
        ...
        assert cfg.cache_repo_dir == str(Path("/tmp/ah") / "repo" / "custom-repo")
    ...
    def test_build_project_profile_prefers_explicit_immutable_files(
        ...
        cfg = AutoHarnessConfig(immutable_files=["custom/file.py"])
        profile = cfg.build_project_profile()
        assert profile.immutable_files == ["custom/file.py"]

    def test_resolve_gitcode_token_direct(self):
        cfg = AutoHarnessConfig(gitcode_token="my-token")
    ...
    def test_resolve_gitcode_token_custom_env(
        ...
        cfg = AutoHarnessConfig(gitcode_token_env="MY_TOKEN")
    ...
    def test_resolve_gitcode_username_prefers_explicit(
        ...
        assert cfg.resolve_gitcode_username() == ("bot-user")

    def test_resolve_gitcode_username_falls_back_to_fork_owner(
        self,
    ):
        cfg = AutoHarnessConfig(fork_owner="fallback-owner")
        assert cfg.resolve_gitcode_username() == ("fallback-owner")
    ...
    def test_resolve_ci_gate_python_executable_prefers_configured(
        ...
        cfg = AutoHarnessConfig(ci_gate_python_executable="/tmp/python3.11")
        assert cfg.resolve_ci_gate_python_executable() == ("/tmp/python3.11")
...
class TestVenvPythonCandidates:
    ...
    def test_unix_candidates(self):
        ...
        assert str(candidates[0]) == ("/tmp/project/.venv/bin/python")
    ...
    def test_windows_candidates(self):
        ...
        assert candidates[0] == (__import__("pathlib").Path("/tmp/project") / ".venv" / "Scripts" / "python.exe")

    def test_resolve_ci_gate_venv_found(self, tmp_path):
        """When the platform venv python exists, it is returned."""
        import sys

        cfg = AutoHarnessConfig(workspace=str(tmp_path))
        if sys.platform == "win32":
...
class TestLoadFromDict:
    ...
    def test_extensions_section(self):
        data = {
            "extensions": {
                "stage_registrars": ["pkg.stage:register"],
                "pipeline_registrars": ["pkg.pipeline:register"],
            }
        }
        cfg = AutoHarnessConfig.load_from_dict(data)
        assert cfg.stage_registrars == ["pkg.stage:register"]
        assert cfg.pipeline_registrars == ["pkg.pipeline:register"]
    ...
    def test_ci_gate_section(self):
        ...
        assert cfg.ci_gate_install_command == ("uv sync --active --group dev --extra cli")
...
class TestLoadAutoHarnessConfig:
    ...
    def test_missing_file_returns_defaults(self, tmp_path):
        cfg = load_auto_harness_config(str(tmp_path / "nonexistent.yaml"))
        assert cfg.git_remote == ""
        assert cfg.session_budget_secs == 900000.0
    ...
    def test_missing_file_bootstraps_with_detected_local_repo(
        ...
        assert cfg.suggested_local_repo == str(repo.resolve())
        content = (tmp_path / "auto_harness" / "config.yaml").read_text(encoding="utf-8")
        assert '# local_repo: "/home/user/code/agent-core"' in content
...
class TestLocalRepoHelpers:
    def test_placeholder_local_repo_detected(self):
        assert is_placeholder_local_repo("/home/user/code/agent-core")
        assert not is_placeholder_local_repo("/home/snape/code/gitcode/agent-core")
