...
class TestSlugify:
    def test_basic(self):
        assert _slugify("fix timeout bug") == ("fix-timeout-bug")
...
class TestWorktreeManager:
    ...
    async def test_prepare_with_local_repo(
        ...
        # 验证 fetch 被调用
        fetch_calls = [c for c in calls if c[0][0] == "fetch"]
        assert len(fetch_calls) == 1
        assert fetch_calls[0][1] == str(local)

        # 验证 worktree add 被调用
        wt_calls = [c for c in calls if c[0][0] == "worktree" and c[0][1] == "add"]
        assert len(wt_calls) == 1

        # 验证 git config user.name 被设置
        config_calls = [c for c in calls if c[0][0] == "config" and "user.name" in c[0]]
    ...
    async def test_prepare_deletes_existing_branch_before_add(
        ...
        calls = []
        branch_ref = "refs/heads/auto-harness/fix-timeout"
        ...
        prune_idx = calls.index(("worktree", "prune"))
        ...
        add_idx = next(idx for idx, call in enumerate(calls) if call[:2] == ("worktree", "add"))
    ...
    async def test_prepare_removes_managed_worktree_for_existing_branch(
        ...
        calls = []
        branch_ref = "refs/heads/auto-harness/fix-timeout"
        stale_wt = Path(cfg.worktrees_dir) / "old-fix-timeout"
    ...
    async def test_prepare_rejects_unmanaged_worktree_for_existing_branch(
        ...
        calls = []
        branch_ref = "refs/heads/auto-harness/fix-timeout"
        unmanaged_wt = tmp_path / "foreign" / "fix-timeout"
        ...
        async def fake_run_git(*args, cwd, env=None):
            ...
            if args[:3] == (
                ...
                return 0, (f"worktree {unmanaged_wt}\nHEAD deadbeef\nbranch {branch_ref}\n")
            return 0, "ok"
    ...
    async def test_prepare_without_local_repo_clones(
        ...
        # 验证 clone 被调用
        clone_calls = [c for c in calls if c[0][0] == "clone"]
    ...
    async def test_cleanup(self, tmp_path):
        ...
        remove_calls = [c for c in calls if c[0][0] == "worktree" and c[0][1] == "remove"]
    ...
    async def test_cleanup_nonexistent_path(
        ...
        # 不应抛异常
        await mgr.cleanup(str(tmp_path / "nonexistent"))
    ...
    async def test_prepare_adds_fork_remote(
        ...
        # 验证 remote add 被调用
        remote_add = [c for c in calls if c[0][0] == "remote" and c[0][1] == "add"]
    ...
    async def test_prepare_uses_task_scoped_git_auth_env(
        ...
        assert all(env.get("GIT_CONFIG_KEY_2") == "http.https://gitcode.com/.extraheader" for env in seen_envs)
