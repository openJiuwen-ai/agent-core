...
def test_extract_verify_related_files_finds_test_paths():
    ci_result = {
        "errors": ("FAILED tests/unit_tests/rsi/auto_harness/test_schema.py::test_x"),
        "gates": [],
    }
    assert extract_verify_related_files(ci_result) == ["tests/unit_tests/rsi/auto_harness/test_schema.py"]
...
def test_derive_legacy_related_test_files_requires_edit_and_reference():
    ...
    assert result == ["tests/unit_tests/rsi/auto_harness/test_schema.py"]


def test_is_allowed_documentation_file_limits_docs_layout():
    assert is_allowed_documentation_file("docs/en/guide.md")
    assert not is_allowed_documentation_file("docs/auto-harness-agent-design.md")
    assert not is_allowed_documentation_file("README.md")
