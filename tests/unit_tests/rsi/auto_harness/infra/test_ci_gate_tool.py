...
class TestCIGateRunnerMatchGates(IsolatedAsyncioTestCase):
    ...
    def test_normalize_make_test_uses_bash_shell(self):
        ...
        with patch(
            ...
            tool = CIGateRunner("/tmp", python_executable="/tmp/python3.11")
            with patch(
    ...
    def test_normalize_make_test_preserves_flags(self):
        ...
        with patch(
            ...
            with patch(
                ...
                normalized = tool._normalize_command("make test TESTFLAGS=tests/unit_tests/harness/")
        assert normalized == ("/tmp/python3.11 -m pytest tests/unit_tests/harness/")
    ...
    def test_normalize_shell_prefixed_make_test_uses_configured_python(self):
        ...
        with patch(
            ...
            with patch(
                ...
                normalized = tool._normalize_command('PATH="/tmp/bin:$PATH" make test')
        assert normalized == ('PATH="/tmp/bin:$PATH" /tmp/python3.11 -m pytest')
    ...
    def test_normalize_python_module_command_uses_configured_python(self):
        ...
        with patch(
            ...
            normalized = tool._normalize_command("python -m pytest -q")
    ...
    def test_resolve_python_executable_prefers_configured_path(self):
        ...
        with patch(
            ...
            assert tool._resolve_python_executable() == ("/tmp/python3.11")
...
class TestCIGateRunnerInvoke(IsolatedAsyncioTestCase):
    ...
    async def test_run_gate_rewrites_make_test(self):
        ...
        assert mock_exec.await_args.args[2] == "/tmp/python3.11 -m pytest"
    ...
    async def test_run_gate_rewrites_shell_prefixed_make_test(self):
        ...
        assert result["passed"] is True
        assert mock_exec.await_args.args[2] == 'PATH="/tmp/bin:$PATH" /tmp/python3.11 -m pytest'

    async def test_run_gate_executes_install_command_once(self):
        tool = CIGateRunner.__new__(CIGateRunner)
        tool._workspace = "/tmp"
        tool._python_executable = "/tmp/python3.11"
        tool._install_command = "uv sync --active --group dev --extra cli"
        ...
        assert mock_exec.await_args_list[0].args[2] == ("uv sync --active --group dev --extra cli")
