...
class TestMergeRuntimeExtensions:
    ...
    def test_zero_conflict(self, tmp_path: Path):
        ...
        result = merge_runtime_extensions([ext_a, ext_b], tmp_path)
        assert result.runtime_ext.extension_name == "merged_extensions"
        ...
        assert (merged_root / "tools" / "a_only.py").is_file()
        assert (merged_root / "tools" / "b_only.py").is_file()
        assert (merged_root / "tools" / "helper_b.py").is_file()
        ...
        for kind in ("rails", "tools"):
            for spec in data.get("resources", {}).get(kind, []):
                if spec.get("type") == "package":
                    assert spec["module"].startswith("openjiuwen.extensions.harness.merged_extensions")
    ...
    def test_conflict_file(self, tmp_path: Path):
        ...
        ext_a = _write_minimal_ext(
            tmp_path,
            extension_name="ext_a",
            extra_files={"tools/helper.py": 'VALUE = "from_a"\n'},
        )
        ext_b = _write_minimal_ext(
            tmp_path,
            extension_name="ext_b",
            extra_files={"tools/helper.py": 'VALUE = "from_b"\n'},
        )
        result = merge_runtime_extensions([ext_a, ext_b], tmp_path)
        merged_root = Path(result.runtime_ext.runtime_path)
        # Both renamed
        assert (merged_root / "tools" / "helper__ext_a.py").is_file()
        assert (merged_root / "tools" / "helper__ext_b.py").is_file()
        # rename_map only has the conflict
        assert ("ext_a", "tools/helper.py") in (result.rename_map)
        assert ("ext_b", "tools/helper.py") in (result.rename_map)
        assert result.rename_map[("ext_a", "tools/helper.py")] == "tools/helper__ext_a.py"
    ...
    def test_conflict_same_tool_path_manifest_modules(
        ...
        with open(merged_root / "harness_config.yaml", encoding="utf-8") as fh:
            data = yaml.safe_load(fh)
        modules = [s["module"] for s in data["resources"].get("tools", []) if s.get("type") == "package"]
    ...
    def test_skill_conflict(self, tmp_path: Path):
        ...
        for root, name in [
            ...
            (ext_root / "tools" / "__init__.py").write_text("", encoding="utf-8")
            (ext_root / "__init__.py").write_text("", encoding="utf-8")
            (ext_root / "skills" / "my_skill").mkdir(parents=True)
            (ext_root / "skills" / "my_skill" / "SKILL.md").write_text(
        ...
        ext_a = RuntimeExtensionArtifact(
            ...
            config_path=str(root_a / "ext_a" / "harness_config.yaml"),
        )
        ext_b = RuntimeExtensionArtifact(
            extension_name="ext_b",
            runtime_path=str(root_b / "ext_b"),
            config_path=str(root_b / "ext_b" / "harness_config.yaml"),
        )
        result = merge_runtime_extensions([ext_a, ext_b], tmp_path)
        merged_root = Path(result.runtime_ext.runtime_path)
        assert (merged_root / "skills" / "my_skill__ext_a").is_dir()
        assert (merged_root / "skills" / "my_skill__ext_b").is_dir()
        ...
        with open(
            ...
            data = yaml.safe_load(fh)
        assert data["resources"]["skills"]["dirs"] == ["skills/"]
    ...
    def test_deterministic(self, tmp_path: Path):
        ...
        result1 = merge_runtime_extensions([ext_a, ext_b], tmp_path / "run1")
        result2 = merge_runtime_extensions([ext_a, ext_b], tmp_path / "run2")
    ...
    def test_absolute_import_rewrite(self, tmp_path: Path):
        ...
        ext_a = _write_minimal_ext(
            ...
            extension_name="ext_a",
            extra_files={
                "tools/demo_tool.py": ("from openjiuwen.extensions.harness.ext_a.tools.helper import VALUE\n")
            },
        )
        ext_b = _write_minimal_ext(
            tmp_path,
            extension_name="ext_b",
            extra_files={"tools/other.py": "X=1\n"},
        )
        (tmp_path / "ext_b" / "tools" / "helper.py").unlink()
        result = merge_runtime_extensions([ext_a, ext_b], tmp_path)
        merged_root = Path(result.runtime_ext.runtime_path)
        demo_tool = merged_root / "tools" / "demo_tool.py"
        content = demo_tool.read_text(encoding="utf-8")
        assert ("openjiuwen.extensions.harness.merged_extensions.tools.helper") in content
