...
class TestOrchestratorRunSession(
    ...
    async def test_session_selects_meta_pipeline_before_running(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            async def _fake_pipeline_stream(
                ...
                assert orch.runtime.selected_pipeline == (META_EVOLVE_PIPELINE)
                selection = orch.artifacts.require("pipeline_selection")
                assert selection.pipeline_name == (META_EVOLVE_PIPELINE)
                yield orch._msg("pipeline running")

            orch._run_pipeline_stream = _fake_pipeline_stream
            chunks = await _collect(orch.run_session_stream(tasks=None))

            msg_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "message"]
            assert f"Session pipeline: {META_EVOLVE_PIPELINE}" in msg_chunks
    ...
    async def test_session_respects_extended_pipeline_preference(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            msg_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "message"]
            assert f"Session pipeline: {EXTENDED_EVOLVE_PIPELINE}" in msg_chunks
    ...
    async def test_extended_assess_and_plan_keep_workspace(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            orch.worktree_mgr.prepare_readonly_snapshot = AsyncMock(return_value=f"{d}/worktrees/assess")
            ...
            async def _fake_assess_stream(
                ...
                seen_workspaces.append(ctx.orchestrator.config.workspace)
                yield OutputSchema(
                    type="llm_output",
                    index=0,
                    payload={"content": "# Report"},
                )
                yield StageResult(artifacts={"gap_analysis": GapAnalysisArtifact(gaps=[Gap(id="g1", feature="t1")])})
            ...
            with (
                ...
                await _collect(orch.run_session_stream(tasks=None))
    ...
    async def test_direct_tasks_run_assess_and_plan(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                ...
                chunks = await _collect(orch.run_session_stream(tasks=[OptimizationTask(topic="t1")]))
    ...
    async def test_competitor_signal_selects_extended_pipeline(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            selection = orch._select_session_pipeline([OptimizationTask(topic=("吸收 hermes 的动态能力创建机制"))])

            assert selection.pipeline_name == (EXTENDED_EVOLVE_PIPELINE)
    ...
    async def test_extended_pipeline_runs_extension_task_pipeline(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            async def _fake_isolated(_orch, design):
                ...
                yield orch._msg(f"ext:{design.extension_name}")
            ...
            with patch.object(
                ...
                chunks = await _collect(orch.run_session_stream(tasks=[OptimizationTask(topic="吸收 hermes")]))

            message_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "message"]
            assert f"Session pipeline: {EXTENDED_EVOLVE_PIPELINE}" in message_chunks
            assert "ext:hermes" in message_chunks
            session_results = orch.artifacts.require("session_results")
            assert isinstance(session_results, SessionResultsArtifact)
    ...
    async def test_session_stream_passthroughs_ext_assess_and_plan_chunks(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            llm_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "llm_output"]
            message_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "message"]
    ...
    async def test_extended_pipeline_caps_extension_design_tasks(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            message_chunks = [c.payload["content"] for c in chunks if getattr(c, "type", "") == "message"]
            assert "Session pipeline: extended_evolve_pipeline" in message_chunks
    ...
    async def test_extended_pipeline_skips_failed_constraint_dependencies(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            async def _fake_isolated(_orch, design):
                ...
                orch.record_cycle_result(
                    CycleResult(
                        success=(design.extension_name != "guard"),
                        summary=design.extension_name,
                    )
                )
                return
...
class TestTaskPipeline(IsolatedAsyncioTestCase):
    async def test_prepare_task_runtime_creates_task_session_and_fix_agent(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            orch.experience_store.search = AsyncMock(return_value=[])
            orch.worktree_mgr.prepare = AsyncMock(return_value=f"{d}/worktrees/task-1")
            orch.git.set_workspace = Mock()
            orch.ci_gate.set_workspace = Mock()
            orch.git.list_dirty_files = AsyncMock(return_value=["openjiuwen/harness/tools/filesystem.py"])
            ...
            assert runtime.preexisting_dirty_files == ["openjiuwen/harness/tools/filesystem.py"]

            first_call = create_task_agent.call_args_list[0]
            assert first_call.kwargs["workspace_override"] == f"{d}/worktrees/task-1"
            assert "enable_task_loop" not in first_call.kwargs
            edit_safety_rail = first_call.kwargs["edit_safety_rail"]
            assert edit_safety_rail is runtime.edit_safety_rail

            second_call = create_task_agent.call_args_list[1]
            assert second_call.kwargs["workspace_override"] == f"{d}/worktrees/task-1"
            assert second_call.kwargs["edit_safety_rail"] is edit_safety_rail
            assert second_call.kwargs["enable_task_loop"] is False
            assert second_call.kwargs["enable_task_planning"] is False
            assert second_call.kwargs["enable_progress_repeat"] is False
    ...
    async def test_resolve_task_result_from_artifact(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            result = PRTaskPipeline._resolve_task_result(orch, task)
