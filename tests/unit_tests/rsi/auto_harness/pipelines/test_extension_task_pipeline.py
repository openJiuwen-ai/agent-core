...
def _design(name: str = "demo_ext") -> ExtensionDesign:
    module_base = f"openjiuwen.extensions.harness.{name}"
    return ExtensionDesign(
        ...
        file_plan={
            ...
            "manifest": (f"openjiuwen/extensions/harness/{name}/harness_config.yaml"),
        },
...
def _write_extension_scaffold(
    ...
    ext_root = wt_path / "openjiuwen" / "extensions" / "harness" / name
...
async def test_extension_task_pipeline_runs_end_to_end(
    ...
    orch.worktree_mgr.prepare = AsyncMock(return_value=str(wt_path))
    ...
    with (
        ...
        mock_commit_round.side_effect = _fake_commit_round
    ...
    messages = [item.payload["content"] for item in chunks if getattr(item, "type", "") == "message"]
    assert "Implemented extension: demo_ext" in messages
    assert "Verified extension scaffold: demo_ext" in messages
    assert orch.results[-1].success is True
    orch.worktree_mgr.cleanup.assert_awaited_once_with(str(wt_path))
...
async def test_extension_ready_payload_points_to_session_runtime_root(
    ...
    task = OptimizationTask(topic="runtime-extension:first_ext")
    ctx = TaskContext(
        orchestrator=orch,
        task=task,
        runtime=TaskRuntime(
    ...
    ctx.put_artifact(
        ...
        RuntimeExtensionArtifact(
            ...
            config_path=str(session_root / "first_ext" / "harness_config.yaml"),
        ),
    )
    ...
    assert ready.payload["session_runtime_path"] == str(session_root)
    assert ready.payload["extension_runtime_path"] == str(session_root / "first_ext")
    assert [item["extension_name"] for item in ready.payload["runtime_extensions"]] == ["first_ext", "second_ext"]
