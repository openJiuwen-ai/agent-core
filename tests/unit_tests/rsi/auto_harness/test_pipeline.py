...
class TestPipelineBuilders:
    ...
    def test_extended_pipeline_uses_pipeline_class(self):
        ...
        spec = pipeline_registry.require(EXTENDED_EVOLVE_PIPELINE)
    ...
    def test_builtin_stage_registry_uses_stage_classes(self):
        ...
        assert registry.require("assess_ext").stage_cls is ExtendAssessStage
        assert registry.require("plan_ext").stage_cls is ExtendPlanStage
        assert registry.require("implement_ext").stage_cls is ExtendImplementStage
        assert registry.require("verify_ext").stage_cls is ExtendVerifyStage
    ...
    def test_build_stage_registry_loads_registrars(self):
        cfg = AutoHarnessConfig(
            stage_registrars=[("tests.unit_tests.rsi.auto_harness.test_pipeline:register_test_stage")]
        )
        registry = build_stage_registry(cfg)
    ...
    def test_build_pipeline_registry_loads_registrars(
        ...
        cfg = AutoHarnessConfig(
            stage_registrars=[("tests.unit_tests.rsi.auto_harness.test_pipeline:register_test_stage")],
            pipeline_registrars=[("tests.unit_tests.rsi.auto_harness.test_pipeline:register_test_pipeline")],
        )
        stage_registry = build_stage_registry(cfg)
        pipeline_registry = build_pipeline_registry(
            cfg,
        ...
        assert pipeline_registry.require("custom_pipeline").pipeline_cls is _DummyPipeline
