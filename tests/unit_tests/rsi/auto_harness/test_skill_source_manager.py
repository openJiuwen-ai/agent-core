...
class TestRepoNameFromUrl(TestCase):
    def test_github_url(self):
        assert _repo_name_from_url("https://github.com/anthropics/skills.git") == "anthropics-skills"

    def test_baoyu_url(self):
        assert _repo_name_from_url("https://github.com/JimLiu/baoyu-skills.git") == "JimLiu-baoyu-skills"
...
class TestScanSkills(TestCase):
    def test_scan_finds_skills(self):
        ...
        config = AutoHarnessConfig(
            ...
            community_skill_repos=["https://github.com/anthropics/skills.git"],
        )
    ...
    def test_scan_empty_cache(self):
        config = AutoHarnessConfig(
            data_dir=self._tmp_dir,
            community_skill_repos=["https://github.com/nonexistent/repo.git"],
        )
        result = scan_skills(config)
        assert result == {}
...
class TestExtensionDesignSkillSource(TestCase):
    ...
    def test_from_dict(self):
        ...
        config = AutoHarnessConfig.load_from_dict(
            {
                "data_dir": "/tmp/ah",
                "community_skill_repos": ["https://github.com/test/skills.git"],
                "community_skill_cache_dir": "/tmp/skills-cache",
            }
        )
        assert config.community_skill_repos == ["https://github.com/test/skills.git"]
