...
class TestImplementStageHelpers(
    ...
    async def test_run_implement_stream_manages_session_lifecycle(
        ...
        assert session.pre_run_inputs == {"query": agent.query}
    ...
    async def test_stage_emits_ready_message_before_agent_summary(
        ...
        ctx = TaskContext(
            orchestrator=SimpleNamespace(
                ...
                git=_FakeGit(status_text=(" M openjiuwen/harness/cli/cli.py")),
            ),
        ...
        items = [item async for item in stage.stream(ctx)]

        assert items[0].type == "message"
        assert items[0].payload["content"] == "任务准备就绪: 补全 auto-harness 文档"
        assert items[1].type == "llm_output"
        assert "任务完成总结" in items[1].payload["content"]
        assert agent.session is None
    ...
    async def test_stage_passes_runtime_task_session_to_agent(
        ...
        ctx = TaskContext(
            orchestrator=SimpleNamespace(
                artifacts=SimpleNamespace(),
                git=_FakeGit(status_text=(" M openjiuwen/harness/tools/filesystem.py")),
            ),
    ...
    async def test_stage_uses_git_changes_even_if_rail_is_empty(
        ...
        ctx = TaskContext(
            orchestrator=SimpleNamespace(
                ...
                git=_FakeGit(
                    status_text="",
                    diff_files=["openjiuwen/harness/tools/filesystem.py"],
                ),
            ),
            ...
            runtime=TaskRuntime(
                related=[],
                wt_path="/tmp/worktree",
                edit_safety_rail=SimpleNamespace(edited_files=lambda: []),
                preexisting_dirty_files=[],
        ...
        assert isinstance(items[-1], StageResult)
        assert items[-1].status == "success"
        assert items[-1].artifacts["code_change"].edited_files == ["openjiuwen/harness/tools/filesystem.py"]
    ...
    async def test_stage_fails_with_task_failed_error_before_git_check(
        ...
        ctx = TaskContext(
            ...
            runtime=TaskRuntime(
                related=[],
                wt_path="/tmp/worktree",
                edit_safety_rail=SimpleNamespace(edited_files=lambda: ["openjiuwen/harness/tools/filesystem.py"]),
                preexisting_dirty_files=[],
        ...
        assert "No allowed repo file was changed" not in items[-1].error
    ...
    async def test_stage_fails_when_git_reports_no_repo_edits(
        ...
        assert isinstance(items[-1], StageResult)
        assert items[-1].status == "failed"
        assert "No allowed repo file was changed" in items[-1].error
        assert ctx.task.status.value == "failed"
    ...
    def test_extract_repo_edit_candidates_tolerates_stripped_status_prefix(
        ...
        assert edited_files == ["openjiuwen/harness/tools/filesystem.py"]
    ...
    async def test_stage_ignores_preexisting_dirty_files(
        ...
        ctx = TaskContext(
            ...
            runtime=TaskRuntime(
                ...
                wt_path="/tmp/worktree",
                edit_safety_rail=SimpleNamespace(edited_files=lambda: []),
                preexisting_dirty_files=["openjiuwen/harness/tools/filesystem.py"],
                task_agent=_StreamingAgent(),
                commit_agent=None,
            ),
        )
