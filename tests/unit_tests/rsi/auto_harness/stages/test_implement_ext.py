...
def _build_design() -> ExtensionDesign:
    return ExtensionDesign(
        ...
        file_plan={
            "root": ("openjiuwen/extensions/harness/demo_ext"),
            "manifest": ("openjiuwen/extensions/harness/demo_ext/harness_config.yaml"),
        },
...
def test_implement_ext_prompt_guides_tool_skill_ppt_extension(
    ...
    prompt = _build_implement_ext_prompt(
        design,
        extension_root=(tmp_path / "huawei_ppt_generator"),
        config_path=(tmp_path / "huawei_ppt_generator" / "harness_config.yaml"),
    )
...
def _write_scaffold(wt_path: Path) -> None:
    ...
    ext = wt_path / "openjiuwen" / "extensions" / "harness" / "demo_ext"
...
def _make_task_context(
    ...
    *,
    ...
    agent = _make_mock_agent(wt_path) if with_agent else None
    runtime = TaskRuntime(
        related=[],
        wt_path=str(wt_path),
...
async def test_implement_ext_fails_without_agent(
    ...
    ctx = _make_task_context(tmp_path, with_agent=False)
    ctx.put_artifact(
        "extension_target",
        _build_design(),
    )
...
async def test_implement_ext_scopes_nested_agent_stage_events(
    ...
    chunks = [item async for item in ExtendImplementStage().stream(ctx)]

    scoped = next(item for item in chunks if isinstance(item, OutputSchema) and item.type == "stage_result")
