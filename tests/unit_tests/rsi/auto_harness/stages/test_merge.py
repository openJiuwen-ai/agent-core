...
def _make_merge_result(
    ...
    art = _make_artifact(runtime_path=runtime_path, config_path=config_path)
...
class TestMergeEvent:
    ...
    def test_failed_event(self):
        event = _merge_event("failed", error="boom", repair_rounds=3)
...
class TestBuildMergeFixPrompt:
    ...
    def test_prompt_contains_merge_summary(self):
        ...
        result.rename_map = {("ext_a", "tools/helper.py"): "tools/helper__ext_a.py"}
...
class TestMergeActivationBlock:
    ...
    async def test_static_pass_no_agent_created(self):
        ...
        with (
            ...
            events = []
            async for chunk in block.stream(orchestrator, [task]):
                events.append(chunk)

        # Verify event sequence: running -> success
        statuses = [e.payload["status"] for e in events if isinstance(e, OutputSchema)]
    ...
    async def test_one_round_repair(self):
        ...
        async def fake_static(*args, **kwargs):
            ...
            if call_count["static"] == 1:
                return ExtStaticCheckResult(errors=["import error"])
            return ExtStaticCheckResult(errors=[], rails_count=1)
    ...
    async def test_exhausted_rounds(self):
        ...
        async def fake_static(*args, **kwargs):
            return ExtStaticCheckResult(errors=["always fails"])
        ...
        with (
            ...
            with pytest.raises(MergedExtensionError):
                async for _ in block.stream(orchestrator, [task]):
                    pass
    ...
    async def test_merge_hard_error_fails_fast(self):
        ...
        with mock.patch(
            ...
            assert any(isinstance(e, OutputSchema) and e.payload["status"] == "failed" for e in events)
...
class TestSchemaGate:
    ...
    def test_payload_has_required_fields(self):
        event = _merge_event("failed", error="test", repair_rounds=2)
