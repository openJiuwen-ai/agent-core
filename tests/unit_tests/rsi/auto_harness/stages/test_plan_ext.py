        ...
        GapAnalysisArtifact(
            gaps=[
                Gap(
                    ...
                    gap_description=("每 5 次工具执行自动报告进度和 token 花销"),
                    impact=0.9,
                    feasibility=0.8,
                )
            ]
        ...
        GapAnalysisArtifact(
            gaps=[
                Gap(
                    ...
                    gap_description=("生成华为风格 PPT，包含模板规范和文件生成能力"),
                    impact=0.9,
                    feasibility=0.8,
                )
            ]
...
def test_fallback_design_infers_tool_skill_for_ppt_generation():
    design = _build_design(
        Gap(
            ...
            feature="huawei_ppt_generator",
            gap_description=("生成华为风格 PPT，包含模板规范和文件生成能力"),
            suggested_approach=("创建 PPT 生成 Tool 和华为风格 Skill"),
        )
    )
...
def test_parse_extension_designs_preserves_execution_fields():
    ...
    assert designs[1].depends_on == ["huawei_filename_guard"]
    ...
    designs = _build_fallback_designs(
        [
            Gap(
                ...
                gap_description=("所有文件写入前必须强制检查文件名后缀"),
                impact=0.4,
                feasibility=0.4,
            ),
            Gap(
