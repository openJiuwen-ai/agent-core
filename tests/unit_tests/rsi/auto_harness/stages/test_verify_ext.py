...
def _write_scaffold(wt_path: Path) -> ExtensionBuildArtifact:
    ...
    module_base = "openjiuwen.extensions.harness.demo_ext"
    manifest.write_text(
...
def test_verify_ext_prompt_requires_artifact_level_acceptance(
    ...
    build = ExtensionBuildArtifact(
        ...
        config_path=str(tmp_path / "huawei_ppt_generator" / "harness_config.yaml"),
    )
...
async def test_verify_ext_reuses_generated_test_after_fix(
    ...
    class FakeAgent:
        ...
        async def stream(self, inputs, session=None):
            ...
            if "测试文件必须写入: " in query:
                ...
                raw_path = query.split("测试文件必须写入: ", 1)[1].splitlines()[0]
    ...
    async def _fake_run_pytest_file(
        *,
        ...
        seen_test_contents.append(Path(test_file).read_text(encoding="utf-8"))
    ...
    assert any(isinstance(item, _CIResult) and item.passed for item in items)
...
async def test_verify_ext_agent_turn_uses_fresh_open_session():
    ...
    class FakeAgent:
        ...
        async def stream(self, inputs, session=None):
            ...
            emitter = session._inner.stream_writer_manager().stream_emitter()
