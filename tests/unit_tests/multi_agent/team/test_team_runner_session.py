...
async def test_runneder_run_agent_team_recovers_team_and_child_state(isolated_checkpointer):
    ...
    result1 = await Runner.run_agent_team(team, {"payload": "first"}, session=session_id, base=True)
    result2 = await Runner.run_agent_team(team, {"payload": "second"}, session=session_id, base=True)
