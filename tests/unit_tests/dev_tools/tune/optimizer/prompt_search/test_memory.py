...
from openjiuwen.dev_tools.tune.optimizer.prompt_search.memory import JsonlPromptMemory, NullPromptMemory
...
def test_jsonl_memory_persists_and_reloads(tmp_path):
    ...
    hits = reloaded.search_similar(PromptTaskSpec(objective="summarize tickets into action items"))
...
def test_jsonl_memory_ranks_by_relevance_and_reward(tmp_path):
    ...
    hits = mem.search_similar(PromptTaskSpec(objective="summarize a support ticket"), top_k=2)
...
def test_null_memory_is_noop(tmp_path):
    ...
    assert mem.search_similar(PromptTaskSpec(objective="x")) == []
...
def test_mark_applied_removes_from_pending_and_persists_across_reload(tmp_path):
    ...
    hits = reloaded.search_similar(PromptTaskSpec(objective="summarize tickets"))
