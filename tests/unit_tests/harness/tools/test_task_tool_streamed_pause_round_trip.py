...
async def _round_trip(store: _StateStore) -> _RoundTrip:
    ...
    trip.final = await _tool(trip.answering_parent).invoke(
        replayed.arguments, session=session, tool_call_id=_OUTER_CALL_ID
    )
    return trip
