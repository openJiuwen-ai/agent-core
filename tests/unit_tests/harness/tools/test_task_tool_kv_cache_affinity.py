...
def _make_tool(*, enabled: bool = True, subagent: _FakeSubAgent) -> TaskTool:
    ...
    return TaskTool(
        ToolCard(id="task_tool", name="task_tool", description="task"),
        parent,
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("subagent_type", ["verification_agent", "browser_agent"])
async def test_sticky_subagent_uses_child_session_prepare_and_suspend(subagent_type: str) -> None:
    ...
    assert child.get_session_id() == f"parent_session_sub_{subagent_type}"
...
async def test_ephemeral_subagent_releases_without_prefetch() -> None:
    ...
    assert re.fullmatch(
        ...
        subagent.sessions[0].get_session_id(),
    )
...
@pytest.mark.asyncio
async def test_failure_releases_child_and_preserves_original_exception() -> None:
