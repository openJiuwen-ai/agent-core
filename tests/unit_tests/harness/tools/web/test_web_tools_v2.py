...
def _header_for(recorder, host: str, header: str) -> str:
    """Return one header sent on the first recorded request against ``host``."""
    for call in recorder.calls:
        if host in call["url"]:
            return (call["headers"] or {}).get(header, "")
    raise AssertionError(f"no request was recorded against {host}; urls={[c['url'] for c in recorder.calls]}")
...
@pytest.mark.parametrize(
    ...
    [("example", "en-US"), ("\u4f8b\u5b50", "zh-CN")],
)
async def test_duckduckgo_via_jina_keeps_the_query_language(monkeypatch, query, expected_prefix):
    """Only the User-Agent is neutralized for the reader.

    The reader fetches DuckDuckGo server-side, so the query-aligned
    Accept-Language still belongs on the request; dropping it alongside the
    User-Agent would silently send every CJK search to an English SERP.
    """
    monkeypatch.setenv("FREE_SEARCH_DDG_ENABLED", "true")
    monkeypatch.setenv("FREE_SEARCH_BING_ENABLED", "false")
    markdown = "[Example Title](https://example.com/page1)\n[Another](https://example.org/page2)\n"
    ...
    recorder = _patch_request(monkeypatch, handler)
    tool = WebFreeSearchTool(language="cn")
    await tool.invoke({"query": query, "max_results": 5})

    assert "Chrome" not in _user_agent_for(recorder, "r.jina.ai")
    assert _header_for(recorder, "r.jina.ai", "Accept-Language").startswith(expected_prefix)
