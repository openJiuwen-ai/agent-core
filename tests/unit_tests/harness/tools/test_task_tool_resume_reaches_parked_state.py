...
# The id of the model-visible call the delegation runs under; the replay that
# carries the answer back reuses it, which is what makes the sub-session
# reproducible across a parent rebuild.
_DELEGATING_CALL_ID = "call_1"
...
async def _ask_then_answer(store, answer, interrupt_again: bool = False):
    ...
    # ``tool_call_id`` is what AbilityManager passes for the call the model
    # made, and the handler replays that same call, so both runs carry it.
    ...
    first = await _tool(asking_parent).invoke(
        ...
        session=session,
        tool_call_id=_DELEGATING_CALL_ID,
    )
    ...
    second = await _tool(answering_parent).invoke(
        _resume_arguments(answer),
        session=session,
        tool_call_id=_DELEGATING_CALL_ID,
    )
