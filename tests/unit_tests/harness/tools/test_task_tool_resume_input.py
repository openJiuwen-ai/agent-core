...
async def test_resumed_call_reaches_the_interrupted_sub_session() -> None:
    ...
    # Both runs carry the id of the model-visible call: AbilityManager supplies
    # it on the first, and the handler replays that same call on the second.
    await tool.invoke(
        {"subagent_type": "code", "task_description": "run task"},
        session=session,
        tool_call_id="call_1",
    ...
    await tool.invoke(
        _resume_tool_call({"action": "allow_once"}).arguments,
        session=session,
        tool_call_id="call_1",
    )
