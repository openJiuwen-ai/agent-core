...
@pytest.mark.parametrize(
    ...
    [
        ...
        # the whole of what the ability layer renders on success, dropping every
        # sibling field -- so skill_directory reaches the model only by being
        # rendered into it. The bodies still differ in media detection, which is
        # what this pins: the directory must not depend on it either way.
...
async def test_skill_tool_always_tells_the_model_the_skill_directory(
    ...
    # Rendered through the ability layer rather than read off the result, so
    # the assertion is about the text the model actually receives.
    model_text = AbilityManager._render_tool_result(
        skill_tool,
        ToolCall(id="call-1", type="function", name="skill_tool", arguments="{}"),
        skill_res,
    )
