...
class TestPowerShellToolResolveHeadRatio:
    """POWER_SHELL_TOOL_HEAD_RATIO must override the default only when head_ratio is omitted."""

    def test_env_overrides_default_when_omitted(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "0.9")
        assert PowerShellTool._resolve_head_ratio(None) == 0.9

    def test_default_when_omitted_without_env(self, monkeypatch) -> None:
        monkeypatch.delenv("POWER_SHELL_TOOL_HEAD_RATIO", raising=False)
        assert PowerShellTool._resolve_head_ratio(None) == 0.6

    def test_explicit_value_wins_over_env(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "0.9")
        assert PowerShellTool._resolve_head_ratio(0.4) == 0.4

    def test_clamped_to_unit_range(self, monkeypatch) -> None:
        monkeypatch.delenv("POWER_SHELL_TOOL_HEAD_RATIO", raising=False)
        assert PowerShellTool._resolve_head_ratio(1.7) == 1.0
        assert PowerShellTool._resolve_head_ratio(-0.2) == 0.0

    def test_invalid_env_falls_back_to_default(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "not-a-number")
        assert PowerShellTool._resolve_head_ratio(None) == 0.6

    def test_invalid_raw_falls_back_to_env(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "0.8")
        assert PowerShellTool._resolve_head_ratio("abc") == 0.8

    def test_parse_inputs_env_override_when_omitted(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "0.85")
        parsed = PowerShellTool._parse_inputs({"command": "Write-Output hi"})
        assert parsed.head_ratio == 0.85

    def test_parse_inputs_explicit_beats_env(self, monkeypatch) -> None:
        monkeypatch.setenv("POWER_SHELL_TOOL_HEAD_RATIO", "0.85")
        parsed = PowerShellTool._parse_inputs({"command": "Write-Output hi", "head_ratio": 0.3})
