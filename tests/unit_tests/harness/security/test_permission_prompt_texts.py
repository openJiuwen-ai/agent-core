...
_FINDING_CALL = ("bash", {"command": "echo hi"})
_FINDING_RESULT = {
...
def test_default_finding_labels_are_unchanged(reason: str, expected_label: str) -> None:
    ...
    命令取 ``echo hi``：命中的风险若同时读写文件，摘要改由文件访问摘要给出路径，
    风险名便不出现在摘要里。
    """
    message = _rail()._build_message(
        _tool_call("bash", {"command": "echo hi"}),
        _ask(matched_rule="tiered_policy:defaults.*", findings=_finding(reason, "HIGH")),
    )

    assert message.startswith(f"{expected_label}: echo hi")


@pytest.mark.parametrize(
    ("call", "result_kwargs", "category", "expected_title"),
    [
        pytest.param(_PATH_CALL, _PATH_RESULT, "path",
                     "检测到受保护的文件路径访问，需要确认后才能执行", id="path"),
        pytest.param(_NETWORK_CALL, _NETWORK_RESULT, "network",
                     "检测到需确认的网络访问，需要确认后才能执行", id="network"),
        pytest.param(_FINDING_CALL, _FINDING_RESULT, "finding",
                     "检测到含重定向或命令替换等结构，需要确认后才能执行", id="finding"),
        pytest.param(_SHELL_CALL, _SHELL_RESULT, "shell",
                     "检测到需确认的命令执行，需要确认后才能执行", id="shell"),
...
@pytest.mark.parametrize(
    ("matched_rule", "expected_title"),
    [
        pytest.param("builtin[shell_privilege_escalation]",
                     "检测到提权，需要确认后才能执行", id="builtin_rule_id"),
        pytest.param("rules[shell_data_exfiltration]",
                     "检测到文件外发，需要确认后才能执行", id="operator_rule_id"),
        pytest.param("builtin[interpreter_sink]",
                     "检测到管道汇入解释器，需要确认后才能执行", id="interpreter_sink"),
        pytest.param("builtin[shell_too_complex]",
                     "检测到命令结构过复杂，需要确认后才能执行", id="too_complex"),
        pytest.param("builtin[brand_new_rule]",
                     "检测到需确认的命令执行，需要确认后才能执行", id="unlisted_rule_falls_back"),
    ],
)
def test_default_rule_risk_labels_are_unchanged(
    matched_rule: str, expected_title: str
) -> None:
    """命中的规则点名风险，默认名称逐字不变；未登记的规则退回分类名，不报错。"""
    metadata = _rail()._build_interrupt_metadata(
        _tool_call("bash", {"command": "sudo rm -rf /"}), _ask(matched_rule=matched_rule),
    )
...
def _sentinel_texts() -> PermissionPromptTexts:
    return PermissionPromptTexts(
        title_risk_detected="TITLE-{risk}",
        risk_path="PATH",
        risk_network="NETWORK",
        risk_finding="FINDING",
        risk_shell="SHELL",
        risk_interpreter_sink="INTERPRETER-SINK",
        rule_risk_labels={"shell_privilege_escalation": "PRIVILEGE-ESCALATION"},
...
def test_host_titles_replace_the_builtin_titles(
    ...
    # ``finding`` names the matched finding rather than the category, and the
    # two titles that name no risk are whole templates of their own.
    expected = {
        "finding": "TITLE-FINDING-RISKY",
        "tool": "TITLE-TOOL",
        "generic": "TITLE-GENERIC",
    }.get(category, f"TITLE-{category.upper()}")
    assert metadata["ask_title"] == expected
...
def test_host_rule_risk_labels_replace_the_builtin_table() -> None:
    """宿主给出的规则风险名整体替换默认表；表中没有的规则退回分类名。"""
    rail = _rail(_sentinel_texts())
    call = _tool_call("bash", {"command": "sudo rm -rf /"})

    listed = rail._build_interrupt_metadata(
        call, _ask(matched_rule="builtin[shell_privilege_escalation]"),
    )
    unlisted = rail._build_interrupt_metadata(
        call, _ask(matched_rule="builtin[shell_data_exfiltration]"),
    )

    assert listed["ask_title"] == "TITLE-PRIVILEGE-ESCALATION"
    assert unlisted["ask_title"] == "TITLE-SHELL"
    assert not any(ord(ch) in CJK_RANGE for ch in unlisted["ask_title"])
...
def test_every_field_declares_its_own_placeholders() -> None:
    ...
    templated = {spec.name for spec in fields(PermissionPromptTexts)} - {"rule_risk_labels"}
    assert set(_FIELD_PLACEHOLDERS) == templated
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("risk_path", "50{}% risk", id="positional_in_a_field_that_takes_none"),
...
def test_shipped_texts_pass_their_own_validation(texts: PermissionPromptTexts) -> None:
    ...
    for spec in fields(texts):
        if spec.name == "rule_risk_labels":
            # A table of names, not a template: every value is used whole.
            assert all(
                isinstance(key, str) and isinstance(value, str)
                for key, value in texts.rule_risk_labels.items()
            )
            continue
...
def test_a_rule_label_table_of_the_wrong_shape_raises_at_construction() -> None:
    """规则风险名表不经 ``str.format``，但类型仍在构造处检查。"""
    with pytest.raises(BaseError) as excinfo:
        PermissionPromptTexts(rule_risk_labels={"shell_privilege_escalation": 1})

    assert excinfo.value.status is StatusCode.DEEPAGENT_CONFIG_PARAM_ERROR
    assert "PermissionPromptTexts.rule_risk_labels" in str(excinfo.value)

    with pytest.raises(BaseError):
        PermissionPromptTexts(rule_risk_labels="not a mapping")  # type: ignore[arg-type]
