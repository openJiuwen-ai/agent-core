...
class _FakeTrainingTaskStore:
    ...
    async def create_task(self, payload: dict):
        ...
        task = {"task_id": task_id, "status": "pending", "user_id": payload.get("user_id", "")}
    ...
    async def request_stop(self, task_id: str):
        ...
        if task is None:
            return None
        task["status"] = "canceled" if task["status"] == "pending" else "stopping"
        return task
...
def _build_test_gateway(upstream_client: _FakeUpstreamClient, lora_repo: _FakeLoRARepo):
    ...
    return build_gateway_app(
        ...
        training_task_store=_FakeTrainingTaskStore(),
...
def test_training_task_api_creates_lists_and_stops_task():
    app = _build_test_gateway(_FakeUpstreamClient(), _FakeLoRARepo({}))

    with TestClient(app) as client:
        created = client.post(
            "/v1/training/tasks",
            headers={"Authorization": "Bearer gw-token"},
            json={"task_id": "task-a", "user_id": "user-1"},
        )
        listed = client.get("/v1/training/tasks", headers={"Authorization": "Bearer gw-token"})
        stopped = client.patch(
            "/v1/training/tasks/task-a",
            headers={"Authorization": "Bearer gw-token"},
