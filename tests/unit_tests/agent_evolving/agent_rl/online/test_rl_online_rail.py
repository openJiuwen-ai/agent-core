...
@pytest.mark.asyncio
async def test_rl_online_rail_uploads_when_only_model_callbacks_fire():
    from openjiuwen.agent_evolving.agent_rl.online.rail.online_rail import RLOnlineRail
    ...
    await rail.before_model_call(AgentCallbackContext(agent=agent, inputs=ModelCallInputs()))
    await rail.after_model_call(AgentCallbackContext(
        agent=agent,
        inputs=ModelCallInputs(
    ...
    assert len(uploader.batches) == 1
    assert uploader.batches[0].session_id == "fallback-session"
    assert uploader.batches[0].samples[0].response_text == "a"
    assert uploader.batches[0].samples[0].meta["fallback_boundary"] == "model_call"
