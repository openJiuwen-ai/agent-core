...
def test_drain_mode_waits_for_training_task_before_fetching_samples():
    from openjiuwen.agent_evolving.agent_rl.online.scheduler.online_training_scheduler import (
        OnlineTrainingScheduler,
    )

    store = _FakeStore()
    store.samples = [{"sample_id": f"s{i}", "user_id": "u1"} for i in range(12)]
    scheduler = OnlineTrainingScheduler(redis_url="", drain_pending_on_train=True, min_samples_for_training=12)
...
def test_drain_mode_training_task_claims_all_pending_samples_once():
    from openjiuwen.agent_evolving.agent_rl.online.scheduler.online_training_scheduler import (
        OnlineTrainingScheduler,
    )
    ...
    assert task_store.claims == [{"user_id": "u1", "sample_count": 12}]
    assert task_store.updates[-1]["status"] == "succeeded"
    assert store.trained == [[f"s{i}" for i in range(12)]]
