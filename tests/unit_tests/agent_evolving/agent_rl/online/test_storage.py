...
class _FakeRedis:
    ...
    async def set(self, key: str, value: Any):
        self._hashes[key]["_value"] = value
        return True

    async def get(self, key: str):
        return self._hashes.get(key, {}).get("_value")
...
@pytest.mark.asyncio
async def test_training_task_store_single_active_task_and_stop_flow():
    ...
    task = await store.create_task({"task_id": "task-1", "user_id": "u1"})

    assert task["status"] == "pending"
    with pytest.raises(RuntimeError):
        await store.create_task({"task_id": "task-2"})

    claimed = await store.claim_pending_task(user_id="u1", sample_count=12)
    assert claimed is not None
    assert claimed["status"] == "running"
    assert claimed["sample_count"] == 12

    stopping = await store.request_stop("task-1")
    assert stopping is not None
    assert stopping["status"] == "stopping"

    done = await store.update_task_status("task-1", status="canceled")
    assert done is not None
    assert done["status"] == "canceled"

    second = await store.create_task({"task_id": "task-2"})
    assert second["task_id"] == "task-2"


@pytest.mark.asyncio
async def test_training_task_store_pending_stop_cancels_and_releases_active():
    from openjiuwen.agent_evolving.agent_rl.storage.training_task_store import TrainingTaskStore
    ...
    assert (await store.create_task({"task_id": "task-2"}))["status"] == "pending"
