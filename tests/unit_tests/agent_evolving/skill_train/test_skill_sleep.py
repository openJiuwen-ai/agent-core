...
def test_mine_skips_segments_without_resolvable_skill_hint() -> None:
    """No segment tool + ambiguous session skills + no intent skill → skip."""
    digests = [
        SessionDigest(
            trace_id="s1",
            project="p",
...
def test_learned_markers_roundtrip() -> None:
    ...
    assert LEARNED_START not in doc
    assert LEARNED_END not in doc
    assert "Prefer brief weather fallbacks." in doc
    assert "Learned preferences" not in doc
    ...
    assert "Ask for location when missing." in updated
    assert LEARNED_START not in updated and LEARNED_END not in updated


def test_reflect_prompt_requires_skill_language_match() -> None:
    ...
    prompt = render(
        "reflect",
        {
            ...
            "__CUR_DOC__": "# 天气查询技能\n- 无需 API Key\n",
            "__PREFS__": "",
            "__FAILURES__": "- UV missing",
        },
    )
    assert "dominant natural language of the current skill" in prompt
    assert "Do not switch to English" in prompt
    assert "copy `anchor` verbatim" in prompt
    assert "# 天气查询技能" in prompt


def test_learned_migrates_legacy_marked_region() -> None:
    legacy = (
        "# Skill\n\n"
        f"{LEARNED_START}\n"
    ...
    assert "Prefer brief weather fallbacks." in extract_learned(legacy)
    updated, applied, unmatched = apply_edits_detailed(
        legacy,
        [EditRecord(target="skill", op="add", content="Ask for location when missing.")],
    )
    assert applied and not unmatched
    assert LEARNED_START not in updated
    assert "Learned preferences" not in updated
    assert "Prefer brief weather fallbacks." in updated
    assert "Ask for location when missing." in updated
...
def test_consolidate_accepts_improving_edit() -> None:
    ...
    assert LEARNED_START not in result.new_skill
...
def test_apply_edits_unmatched() -> None:
    ...
    assert applied == []
    assert unmatched
    assert new_doc.startswith("#")
    assert LEARNED_START not in new_doc
...
def test_classify_user_turn() -> None:
    ...
    assert classify_user_turn("你好世界") == "task"
    assert classify_user_turn("好的没问题") == "task"
...
def test_segment_mine_inherits_session_skill_when_turns_lack_tool() -> None:
    """Legacy digests without tool turns still mine when skills_used is unique."""
    digest = SessionDigest(
        trace_id="s4",
        project="p",
...
def test_mine_recovers_skill_hint_from_intent_when_no_skill_tool() -> None:
    """Downloads/.trace style: user names the skill, agent never calls skill_tool."""
    digest = SessionDigest(
        trace_id="bc2246ac75d2",
        project="p",
        user_prompts=["使用 tianqi 技能 杭州的天气", "输出不对，要给出UV指数的数据"],
        assistant_finals=["杭州：多云 26°C", "已补充"],
        skills_used=[],
        n_user_turns=2,
        n_assistant_turns=2,
        turns=[
            _turn("user", "使用 tianqi 技能 杭州的天气"),
            _turn("assistant", "杭州：多云 26°C"),
            _turn("user", "输出不对，要给出UV指数的数据"),
            _turn("assistant", "已补充"),
        ],
    )
    tasks = mine([digest], max_tasks=10, val_fraction=0.0, seed=1)
    assert len(tasks) == 1
    task = tasks[0]
    assert task.skill_hint == "tianqi"
    assert task.outcome == "fail"
    assert task.reference_kind == "rubric"
    assert "UV" in task.reference or "紫外" in task.reference


def test_mine_normalizes_tianqi_weather_alias_from_intent() -> None:
    digest = SessionDigest(
        trace_id="68dcd296fbc4",
        project="p",
        user_prompts=["使用 tianqi-weather 技能 杭州的天气", "输出不对，要给出空气质量数据"],
        assistant_finals=["杭州晴", "好的"],
        skills_used=[],
        n_user_turns=2,
        n_assistant_turns=2,
        turns=[
            _turn("user", "使用 tianqi-weather 技能 杭州的天气"),
            _turn("assistant", "杭州晴"),
            _turn("user", "输出不对，要给出空气质量数据"),
            _turn("assistant", "好的"),
        ],
    )
    tasks = mine([digest], max_tasks=10, val_fraction=0.0, seed=1)
    assert len(tasks) == 1
    assert tasks[0].skill_hint == "tianqi"
    assert "空气质量" in tasks[0].reference


def test_mine_skips_when_no_skill_signal_at_all() -> None:
    digest = SessionDigest(
        trace_id="noskill",
        project="p",
        user_prompts=["杭州的天气", "输出不对，要给出UV指数"],
        assistant_finals=["多云", "已补充"],
        skills_used=[],
        n_user_turns=2,
        n_assistant_turns=2,
        turns=[
