...
def test_evaluate_gate_reject_accept_accept_new_best() -> None:
    ...
    rejected = evaluate_gate("cand", 0.5, state, 10)
    ...
    accepted = evaluate_gate("cand", 0.6, state, 10)
    ...
    new_best = evaluate_gate("cand", 0.9, state, 10)
...
def test_select_gate_score_hard_soft_mixed() -> None:
    assert select_gate_score(0.8, GateTuning(soft=0.2, metric="hard")) == 0.8
    assert select_gate_score(0.8, GateTuning(soft=0.2, metric="soft")) == 0.2
    assert select_gate_score(
        0.8, GateTuning(soft=0.2, metric="mixed", mixed_weight=0.5)
    ) == pytest.approx(0.5)
    assert select_gate_score(
        1.0, GateTuning(soft=0.0, metric="mixed", mixed_weight=0.25)
    ) == pytest.approx(0.75)
    assert select_gate_score(
        0.0, GateTuning(soft=1.0, metric="mixed", mixed_weight=1.5)
    ) == pytest.approx(1.0)
    assert select_gate_score(
        1.0, GateTuning(soft=0.0, metric="mixed", mixed_weight=-1.0)
    ) == pytest.approx(1.0)
