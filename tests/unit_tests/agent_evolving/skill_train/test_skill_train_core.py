...
from openjiuwen.agent_evolving.skill_train.gate import GateState, evaluate_gate, select_gate_score
from openjiuwen.agent_evolving.skill_train.skill_patch import apply_patch

FIXTURES = Path(__file__).resolve().parent / "evaluator_fixtures.json"
...
class TestGate:
    def test_accept_new_best(self):
        ...
        gate = evaluate_gate("candidate", 0.8, state, global_step=2)
        assert gate.action == "accept_new_best"
        assert gate.best_score == pytest.approx(select_gate_score(0.8))
    ...
    def test_reject(self):
        ...
        gate = evaluate_gate("candidate", 0.3, state, global_step=2)
...
class TestParityRunner:
    def test_searchqa_evaluator_parity(self):
        ...
        if report.get("skipped"):
            pytest.skip(report.get("reason", "Skill not available"))
...
class TestEnvRegistry:
    def test_builtin_envs_registered(self):
        ...
        assert not ({"alfworld", "spreadsheetbench", "livemathematicianbench"} & registered), (
            "removed envs should not remain registered"
        )
    ...
    def test_var_keyword_kwargs_forwarded(self):
        from openjiuwen.agent_evolving.skill_train.registry import get_env_adapter

        adapter = get_env_adapter(
            "searchqa",
            split_mode="split_dir",
            split_dir="/tmp/searchqa_split",
            workers=8,
            exec_timeout=90,
        )
        assert adapter.dataloader.split_mode == "split_dir"
        assert adapter.dataloader.split_dir == "/tmp/searchqa_split"
        assert adapter.workers == 8
        assert adapter.exec_timeout == 90
...
class TestLlmInvokePolicy:
    def test_make_policy_and_timeout_override(self):
        ...
        policy = make_llm_invoke_policy(
            ...
            total_budget_secs=200,  # raised to cover every attempt
            max_attempts=5,
        )
        assert policy.attempt_timeout_secs == 300.0
        assert policy.total_budget_secs == 300.0 * 5 + 30.0
...
class TestConfigDefaults:
    def test_skill_reflact_base_defaults(self):
...
class TestEpochEndHooks:
    def test_epoch1_placeholder_and_meta_skip(self, tmp_path, monkeypatch):
        from openjiuwen.agent_evolving.skill_train.epoch_stages import MetaSkillRequest, SlowUpdateRequest
