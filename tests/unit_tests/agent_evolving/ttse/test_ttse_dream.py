...
@pytest.mark.asyncio
async def test_dream_max_llm_merges_caps_embedding_path(tmp_path):
    """Embedding path must stop after dream_max_llm_merges cluster LLM calls."""
    # Three mutually isolated near-dup pairs → three soft clusters.
    dims = 6
    ...
    for i in range(3):
        ...
        near[i * 2] = 0.99
        near[i * 2 + 1] = 0.01
        mapping[f"pair{i}-a"] = base
        mapping[f"pair{i}-b"] = near
    emb = FakeEmbedding(mapping)
    ...
    cfg = TTSEConfig(
        ...
        dream_min_rules=1,
        dream_soft_lo=0.72,
        dream_max_llm_merges=2,
        dream_purge_tips_enabled=False,
        dream_prune_enabled=False,
    )
    store, cfg = _make_store(tmp_path, cfg=cfg, embedding=emb)
    for i in range(3):
        await store.add_record_direct("fact", f"pair{i}-a", count=2, save=False)
        await store.add_record_direct("fact", f"pair{i}-b", count=2, save=False)
    await store.save()
    llm = ScriptedLLM(handler)
    await run_dream_pass(
        store,
