...
from openjiuwen.agent_evolving.ttse.index import _BM25Index, TOKENIZER_VERSION, _rrf
...
def test_from_dict_drops_foreign_tokenizer():
    loaded = _BM25Index.from_dict(
        {
            "N": 1,
            "sum_dl": 1.0,
            "tokenizer": "old-tokenizer",
            "docs": {"abc": {"dl": 1, "tf": {"x": 1}}},
            "df": {"x": 1},
            "postings": {"x": [["abc", 1]]},
        }
    )
    ...
    assert loaded.tokenizer == TOKENIZER_VERSION


@pytest.mark.asyncio
async def test_align_rebuilds_when_only_categories_change(tmp_path):
    devops = "software-engineering-devops"
    store = await _office_store(tmp_path, [HIT, *NOISE])
    office = await store.index.retrieve(store, category=OFFICE, query="PresentBench slides.md", top_k=2)
    assert HIT in {record["text"] for record in office.facts}
    for record in store.facts:
        record["category"] = devops
    store.index.align(store.facts, store.tips, store.record_category)
    assert store.index._chroma_stale is True
    moved = await store.index.retrieve(store, category=devops, query="PresentBench slides.md", top_k=2)
    assert HIT in {record["text"] for record in moved.facts}
    leftover = await store.index.retrieve(store, category=OFFICE, query="PresentBench slides.md", top_k=2)
    assert HIT not in {record["text"] for record in leftover.facts}
    assert (tmp_path / "bm25" / f"{devops}_fact.json").is_file()
    assert not (tmp_path / "bm25" / f"{OFFICE}_fact.json").is_file()


class _HalfEmbedding:
    def __init__(self, hit_text: str, model: str = "half-v1") -> None:
        self.hit_text = hit_text
        self.model = model

    async def embed_query(self, text: str) -> List[float]:
        if text == self.hit_text:
            return [1.0, 0.0]
        raise RuntimeError("embed skip")

    async def embed_documents(self, texts: List[str]) -> List[List[float]]:
        del texts
        raise RuntimeError("batch skip")


@pytest.mark.asyncio
async def test_rebuild_chroma_skips_fingerprint_when_partial(tmp_path, monkeypatch):
    store = await _office_store(tmp_path, [HIT, *NOISE], embedding=_HalfEmbedding(HIT))
    fake = _FakeChroma()
    monkeypatch.setattr(store.index, "_chroma_store", lambda: fake)
    store.index._chroma_stale = True
    store.index._chroma_fingerprint = ""
    await store.index.rebuild_chroma(store)
    assert 0 < len(fake.docs) < len(store.facts)
    assert store.index._chroma_stale is True
    assert store.index._chroma_fingerprint == ""


@pytest.mark.asyncio
async def test_move_recomputes_embedding_on_cache_miss(tmp_path, monkeypatch):
    devops = "software-engineering-devops"
    table = _office_vectors(HIT, *NOISE)
    store = await _office_store(tmp_path, [HIT, *NOISE], embedding=DictEmbedding(table, model="move-v1"))
    fake = _FakeChroma()
    monkeypatch.setattr(store.index, "_chroma_store", lambda: fake)
    store.index._chroma_stale = False
    store.index._chroma_fingerprint = "move-v1"
    store._emb_cache.clear()
    called: List[str] = []
    original = store.embedding_of

    async def spy(text: str):
        called.append(text)
        return await original(text)

    store.embedding_of = spy
    updated = await store.set_categories([(HIT, "fact", devops)])
    assert updated == 1
    assert HIT in called
    hit_id = next(record["id"] for record in store.facts if record["text"] == HIT)
    item = next(doc for doc in fake.docs if doc.get("id") == hit_id)
    assert item["metadata"]["category"] == devops


@pytest.mark.asyncio
async def test_chroma_retries_after_transient_failure(tmp_path):
    store = await _office_store(tmp_path, [HIT, *NOISE])
    client = object()
    store.index._chroma_failed = False
    store.index._chroma = client
    store.index._mark_chroma_transient(RuntimeError("busy"))
    assert store.index._chroma is None
    assert store.index._chroma_failed is False
    assert store.index._chroma_store() is None
    store.index._chroma = client
    store.index._chroma_retry_at = 0.0
    assert store.index._chroma_store() is client
    store.index._chroma_failed = True
    store.index._chroma_retry_at = 0.0
    assert store.index._chroma_store() is None
