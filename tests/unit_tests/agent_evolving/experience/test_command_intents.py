...
def test_build_rebuild_command_prompt_uses_swarmskill_creator_for_swarm_skill():
    ...
    assert f"MUST call write_file or edit_file on the absolute target path `{team_skill_md}`" in prompt
    assert "Do NOT call todo_complete" in prompt
...
def test_build_rebuild_command_prompt_prefers_skill_md_path_from_context():
    ...
    assert f"MUST call write_file or edit_file on the absolute target path `{external_md}`" in prompt
    assert "Do NOT mark Write/Confirm" in prompt
