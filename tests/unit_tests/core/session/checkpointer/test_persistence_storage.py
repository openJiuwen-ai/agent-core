...
from openjiuwen.core.session.checkpointer.persistence import AgentGroupStorage, AgentStorage
...
async def test_persistence_agent_group_storage_save_recover_exists_and_clear():
    kv_store = MockKVStore()
    storage = AgentGroupStorage(kv_store)
    ...
    await storage.clear("group-1", "session-group")
    assert await storage.exists(session) is False
