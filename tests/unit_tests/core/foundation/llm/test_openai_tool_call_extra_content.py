...
class _ExtraContentModel(BaseModel):
    google: dict[str, str]
...
@pytest.mark.asyncio
async def test_pydantic_tool_call_extra_content_is_converted_to_dict() -> None:
    raw_tool_call = _raw_tool_call(
        call_id="call-1",
        name="list_files",
        arguments="{}",
        index=0,
        extra_content=_ExtraContentModel(
...
def test_stream_tool_call_explicit_empty_extra_content_replaces_previous_value() -> None:
    first = _client()._parse_stream_chunk(
        _stream_chunk(
            _raw_tool_call(
                call_id="call-1",
    ...
    merged = first + second
    assert merged.tool_calls is not None
    assert merged.tool_calls[0].extra_content == {}
    assert _serialize(merged)["tool_calls"][0]["extra_content"] == {}


def test_parallel_stream_tool_calls_merge_by_index_and_keep_metadata_position() -> None:
    first = _client()._parse_stream_chunk(
        _stream_chunk(
            _raw_tool_call(
                call_id="call-1",
    ...
    merged = first + second
    assert merged.tool_calls is not None
    assert [call.arguments for call in merged.tool_calls] == [
        '{"city":"Paris"}',
        '{"city":"London"}',
    ]
    assert merged.tool_calls[0].extra_content == {
    ...
    assert merged.tool_calls[1].extra_content is None

    serialized_calls = _serialize(merged)["tool_calls"]
    assert serialized_calls[0]["extra_content"] == {
        "google": {"thought_signature": "signature-a"}
    }
