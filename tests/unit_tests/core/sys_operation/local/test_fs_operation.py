...
@pytest.mark.asyncio
async def test_stale_closed_connection_self_heals_on_next_lease(work_dir):
    """Verify read/write recovers when the cached connection was closed underneath the lease."""
    file_path = Path(work_dir) / "self_heal_target.txt"
    lock_file = ReadWriteLockManager.get_lock_file(file_path)
    ...
    # Simulate a stale holder: the connection is closed while the entry stays
    # cached, as observed when idle cleanup races a lease in the field.
    lock._file._lock._con.close()
...
@pytest.mark.asyncio
async def test_stale_registry_singleton_self_heals_for_new_leases(work_dir):
    """A new lease heals a closed connection even when the stale singleton stays registered."""
    file_path = Path(work_dir) / "stale_registry_target.txt"
    lock_file = ReadWriteLockManager.get_lock_file(file_path)

    async with FsOperation._file_lock(file_path, "write", timeout=1.0):
        file_path.write_text("v1", encoding="utf-8")
    lock = ReadWriteLockManager.get_lock(file_path)
    inner = lock._file._lock

    # Field-failure shape: the connection dies and the cache entry is removed
    # while the singleton registry still hands out the old instance.
    inner._con.close()
    ReadWriteLockManager._locks.pop(lock_file, None)
    ReadWriteLockManager._idle_lru.pop(lock_file, None)

    async with FsOperation._file_lock(file_path, "write", timeout=1.0):
        file_path.write_text("v2", encoding="utf-8")
        fresh = ReadWriteLockManager.get_lock(file_path)
        assert fresh._file._lock is inner
...
@pytest.mark.asyncio
async def test_ttl_cleanup_close_is_atomic_with_cache_removal(work_dir, monkeypatch):
    """A lease acquired right after TTL cleanup builds a fresh live connection."""
    monkeypatch.setattr(ReadWriteLockManager, "_idle_ttl", 0.0)
    file_path = Path(work_dir) / "atomic_close_target.txt"
    lock_file = ReadWriteLockManager.get_lock_file(file_path)

    async with FsOperation._file_lock(file_path, "write", timeout=1.0):
        file_path.write_text("v0", encoding="utf-8")
    old_lock = ReadWriteLockManager.get_lock(file_path)
    old_inner = old_lock._file._lock

    await asyncio.sleep(0.01)
    await ReadWriteLockManager.cleanup_expired_locks()

    assert old_lock._closed
    assert lock_file not in ReadWriteLockManager._locks

    async with FsOperation._file_lock(file_path, "write", timeout=1.0):
        file_path.write_text("v1", encoding="utf-8")
        fresh_lock = ReadWriteLockManager.get_lock(file_path)
        assert fresh_lock is not old_lock
        assert fresh_lock._file._lock is not old_inner
        assert not fresh_lock._closed
...
@pytest.mark.asyncio
async def test_concurrent_cleanup_and_lease_never_hits_closed_connection(work_dir, monkeypatch):
    """A lease racing the TTL cleanup never operates on a closed connection."""
    monkeypatch.setattr(ReadWriteLockManager, "_idle_ttl", 0.0)
    file_path = Path(work_dir) / "race_cleanup_target.txt"
    ...
    async def write_again():
        async with FsOperation._file_lock(file_path, "write", timeout=1.0):
            file_path.write_text("v1", encoding="utf-8")

    await asyncio.gather(
        ReadWriteLockManager.cleanup_expired_locks(),
        write_again(),
    )
