...
from openjiuwen.core.proactive_context import agent_support
from openjiuwen.core.proactive_context.status_codes import StatusCode
...
def test_pcs_shell_patterns_reject_escape_and_process_commands(command: str) -> None:
    assert any(re.search(pattern, command) for pattern in agent_support._PCS_DANGEROUS_PATTERNS)


@pytest.mark.parametrize("command", ["head", "tail", "wc", "touch", "Test-Path", "Copy-Item", "Move-Item"])
def test_pcs_shell_allowlist_contains_local_file_helpers(command: str) -> None:
    assert command in agent_support._PCS_SHELL_ALLOWLIST
...
def test_validate_message_continuity_accepts_complete_tool_group() -> None:
    ...
    agent_support.validate_pcs_messages(messages)
...
def test_validate_pcs_messages_rejects_broken_tool_group(messages: list[object]) -> None:
    with pytest.raises(Exception):
        agent_support.validate_pcs_messages(messages)


def test_trim_pcs_messages_keeps_tool_group_together() -> None:
    group = _tool_message_group()
    messages = [UserMessage(content="old"), *group, UserMessage(content="new")]
    kept = agent_support.trim_pcs_messages(messages, budget=2)
...
@pytest.mark.parametrize(
    ...
    [
        ("pcs-unknown-model", 200_000, 90_000, 60_000),
...
def test_context_processor_uses_core_round_compressor_and_model_context_window(
    ...
    assert processor_name == agent_support._PCS_ROUND_LEVEL_PROCESSOR_KEY
...
async def test_context_processor_uses_core_compressor_after_forked_registry_pollution() -> None:
    ...
    model_request = ModelRequestConfig(model="pcs-unknown-model", max_tokens=3)
...
def test_context_processor_internal_registration_is_idempotent_under_concurrency() -> None:
    ...
    model_request = ModelRequestConfig(model="pcs-unknown-model")
    ...
    processor_key = agent_support._PCS_ROUND_LEVEL_PROCESSOR_KEY
...
async def test_real_inner_callback_manager_registers_named_callbacks_and_cleans_exactly(tmp_path: Path) -> None:
    ...
    assert [callback.__name__ for callback in registered_callbacks] == [
        "pcs_before_model_call_callback",
        "pcs_after_model_call_callback",
        "pcs_after_react_iteration_callback",
    ]
...
async def test_real_inner_callback_manager_rolls_back_callback_written_before_registration_error(
    ...
    def fail_after_second_callback_write(message: object, *args: object, **kwargs: object) -> None:
        ...
        if "Registered callback" in str(message) and "pcs_after_model_call_callback" in str(message):
...
async def test_iteration_reminder_fires_after_complete_tool_group_at_20_40_60_80_only() -> None:
    ...
    assert all(message.startswith("[message from PCS system]\n") for message in pushed)
    ...
    agent_support.validate_pcs_messages(history)
...
@pytest.mark.asyncio
async def test_run_pcs_agent_creates_unique_session_and_returns_text(
    ...
    output = await agent_support.run_pcs_agent(
    ...
    assert session.session_id.startswith("pcs-agent-")
    assert len(session.session_id) > len("pcs-agent-")
...
@pytest.mark.asyncio
async def test_run_pcs_agent_repairs_in_same_session_after_first_validation_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
...
@pytest.mark.asyncio
async def test_run_pcs_agent_repairs_in_same_session_after_invoke_error_with_closed_history(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
...
async def test_run_pcs_agent_repairs_for_allowlisted_invoke_status_with_closed_history(
...
@pytest.mark.asyncio
async def test_run_pcs_agent_clean_redo_after_invoke_error_without_context(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
...
@pytest.mark.asyncio
async def test_run_pcs_agent_seeds_closed_history_and_uses_only_current_user_query(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
...
async def test_run_pcs_agent_repairs_bounded_output_validation_errors(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    first_output: object,
) -> None:
...
@pytest.mark.asyncio
async def test_run_pcs_agent_clean_redo_restores_baseline_after_second_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
    ...
    assert not (sandbox / "dirty-first.txt").exists()
    assert not (sandbox / "dirty-second.txt").exists()
    assert list(tmp_path.glob(".pcs-agent-baseline-*")) == []
...
async def test_clean_redo_restore_failure_is_non_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(BaseError) as raised:
        await agent_support.run_pcs_agent(
            model_client=cast(Any, object()),
            model_request=cast(Any, object()),
            sandbox_path=sandbox,
            messages=[UserMessage(content="summarize")],
...
@pytest.mark.asyncio
async def test_run_pcs_agent_unclosed_tool_group_skips_in_place_repair(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    events: list[tuple[str, Any]] = []
    created = _patch_agent_runtime(
...
async def test_run_pcs_agent_always_cleans_session_and_rail_on_disk_or_cancelled_failure(
...
@pytest.mark.asyncio
async def test_run_pcs_agent_configures_explicit_sandbox(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    events: list[tuple[str, Any]] = []
    ...
    assert "pcs_provenance_manifest.json" not in system_prompt
