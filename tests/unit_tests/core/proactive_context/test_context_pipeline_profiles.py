...
def _config(profile: str) -> PCSConfig:
...
def _assert_new_wiki_prompt(prompt: str) -> None:
    ...
    assert "pcs_provenance_manifest.json" not in prompt
...
def test_agent_sandbox_rejects_removed_manifest_contract(tmp_path: Path) -> None:
    ...
    (sandbox / "pcs_provenance_manifest.json").write_text("{}", encoding="utf-8")
...
async def test_agent_publication_keeps_unmapped_aggregate_page_without_manifest(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / "workspace" / "pcs_provenance_manifest.json").exists()
...
async def test_filesystem_agent_noop_for_non_empty_run_is_repairable_and_falls_back(
    ...
    monkeypatch.setattr(context_pipeline, "run_pcs_agent", noop_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = [
        json.dumps(
            {
...
async def test_filesystem_agent_undeclared_root_is_a_non_fallback_security_error(
    ...
    monkeypatch.setattr("openjiuwen.core.proactive_context.context_pipeline.run_pcs_agent", unsafe_filesystem_agent)
    with pytest.raises(Exception) as raised:
        await service._filesystem_with_fallback(
            processed=processed,
            sandbox=sandbox,
...
async def test_filesystem_agent_content_validation_can_fallback_to_balanced(
    ...
    monkeypatch.setattr("openjiuwen.core.proactive_context.context_pipeline.run_pcs_agent", failed_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = [
        json.dumps(
            {
...
async def test_filesystem_rules_fallback_discards_failed_candidate(
    ...
    monkeypatch.setattr(context_pipeline, "run_pcs_agent", failed_agent)
...
async def test_filesystem_candidate_prepare_disk_error_is_non_fallback(
    ...
    monkeypatch.setattr("openjiuwen.core.proactive_context.context_pipeline._prepare_agent_candidate", fail_prepare)
    processed = {
        "documents": [
            {
                "logical_id": "notes/one",
...
async def test_filesystem_non_model_agent_statuses_do_not_fallback(
    ...
    monkeypatch.setattr(context_pipeline, "run_pcs_agent", fail_agent)
