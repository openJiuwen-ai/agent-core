...
def test_config_normalizes_service_order_and_is_frozen():
    config = PCSConfig.from_dict(_valid_config(_local_service("z"), _local_service("a")))
...
def test_fetching_enabled_is_required_and_serialized():
    ...
    with pytest.raises(JiuwenValidationError):
        PCSConfig.from_dict(raw)

    raw["fetching_enabled"] = False
    config = PCSConfig.from_dict(raw)
...
def test_fetch_service_interval_defaults_to_three_hours():
    ...
    config = PCSConfig.from_dict(_valid_config(service))

    assert config.fetch_services[0].interval_seconds == 10_800.0


def test_credentials_are_not_in_repr():
    config = PCSConfig.from_dict(_valid_config(_github_service("mock-secret")))
...
def test_config_rejects_unknown_fields_and_duplicate_service_ids():
    with pytest.raises(JiuwenValidationError):
        PCSConfig.from_dict({**_valid_config(), "unexpected": True})

    with pytest.raises(JiuwenValidationError):
        PCSConfig.from_dict(_valid_config(_local_service("same"), _local_service("same")))


def test_service_id_is_bounded_to_128_safe_segment_characters():
    with pytest.raises(JiuwenValidationError):
        PCSConfig.from_dict(_valid_config(_local_service("a" * 129)))

    config = PCSConfig.from_dict(_valid_config(_local_service("a" * 128)))
...
def test_feishu_rejects_embedded_access_tokens():
    ...
    with pytest.raises(JiuwenValidationError):
        PCSConfig.from_dict(_valid_config(service))


def test_config_normalizes_paths_and_isolated_input():
    original = _local_service()
    config = PCSConfig.from_dict(_valid_config(original))
