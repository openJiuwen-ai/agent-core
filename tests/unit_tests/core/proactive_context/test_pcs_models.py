...
from openjiuwen.core.proactive_context.models import FetchBatch, PCSStatus, RawChangeItem
...
def test_pcs_status_is_frozen_and_copies_nested_state():
...
def test_status_reports_fetching_switch():
    status = PCSStatus(
        ...
        fetch_service_errors={},
        context_root="C:/pcs/workspace/context",
        context_ready=True,
        last_error=None,
    )
...
def test_pcs_status_last_error_has_a_bounded_public_shape():
    with pytest.raises(ValidationError):
        PCSStatus(
            configured=True,
            enabled=True,
...
def test_pcs_status_rejects_unknown_fetch_service_state():
    with pytest.raises(ValidationError):
        PCSStatus(
            configured=True,
            enabled=True,
...
def test_pcs_status_rejects_nested_last_error_values():
    with pytest.raises(ValidationError):
        PCSStatus(
            configured=True,
            enabled=True,
...
def test_pcs_status_rejects_non_string_last_error_message():
    with pytest.raises(ValidationError):
        PCSStatus(
            configured=True,
            enabled=True,
