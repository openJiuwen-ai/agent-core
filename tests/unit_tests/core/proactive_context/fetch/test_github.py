...
def github_config(
    ...
    *,
    ...
    return PCSFetchServiceConfig.model_validate(
        {
...
async def test_github_fetches_enabled_resources_and_materializes_code(
    ...
    import openjiuwen.core.proactive_context.fetch.github as github_module
...
async def test_github_default_limit_is_25_and_resources_are_switchable(
    ...
    assert len(batches[0].next_cursor["issues"]["seen_ids"]) == 20
    assert len(batches[1].next_cursor["issues"]["seen_ids"]) == 25
...
async def test_github_fetch_removes_same_service_crash_candidate_first(
    ...
    (candidate / ".pcs-marker.json").write_text(json.dumps({"run_id": "crashed-run"}), encoding="utf-8")
...
async def test_github_rejects_unsafe_archive_and_wrong_run_cannot_commit_or_abort(
    ...
    candidate = tmp_path / "materialized-sources" / "github" / "github-demo" / "candidate"
    candidate.mkdir(parents=True)
    (candidate / ".pcs-marker.json").write_text(json.dumps({"run_id": "run-a"}))
...
@pytest.mark.asyncio
async def test_github_cursor_merges_same_timestamp_seen_ids(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import openjiuwen.core.proactive_context.fetch.github as github_module
    ...
    assert second_cursor["issues"]["seen_ids"] == [
