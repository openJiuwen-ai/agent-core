...
def _set_responses(monkeypatch: pytest.MonkeyPatch, responses: dict[str, list[Response]]) -> None:
    import openjiuwen.core.proactive_context.fetch.zhihu_reader as module
...
async def test_zhihu_paginates_reads_body_sorts_and_builds_cursor(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    assert batches[-1].next_cursor == {
        ...
        "watermark_timestamp": 30.0,
        "same_timestamp_ids": ["c"],
    }
...
@pytest.mark.asyncio
async def test_zhihu_stops_pagination_after_run_limit(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
