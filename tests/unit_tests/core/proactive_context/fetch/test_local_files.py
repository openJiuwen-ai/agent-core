...
from openjiuwen.core.proactive_context.fetch import local_files
...
def _config(root: Path, *, max_items: int | None = None) -> PCSFetchServiceConfig:
...
def test_local_files_skips_fixed_directories_and_symlink_paths(tmp_path: Path):
    root = tmp_path / "source"
    root.mkdir()
    for dirname in (".git", ".venv", "node_modules", "__pycache__", ".pcs"):
