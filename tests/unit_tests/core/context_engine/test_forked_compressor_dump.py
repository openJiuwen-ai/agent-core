...
from openjiuwen.core.context_engine.processor.compressor.forked.compression_dump import (
    FORKED_COMPRESSION_DUMP_DIR_ENV,
)
...
async def test_dump_uses_env_var_when_config_dir_unset(tmp_path, monkeypatch):
    monkeypatch.setenv(FORKED_COMPRESSION_DUMP_DIR_ENV, str(tmp_path))
...
async def test_dump_uses_session_workspace_default_dir(tmp_path):
    ...
    expected_dir = workspace / "context" / "s-7_context" / "forked_compression_logs"
