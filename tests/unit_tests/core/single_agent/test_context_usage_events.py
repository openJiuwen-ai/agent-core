...
@pytest.mark.asyncio
async def test_react_agent_emits_pre_and_post_context_usage_events() -> None:
    ...
    await agent._emit_context_usage(ctx, window, phase="pre_call")
    ...
    assert [event.type for event in events] == ["context.usage", "context.usage"]
    assert events[0].payload["phase"] == "pre_call"
    assert events[1].payload["phase"] == "post_call"
    assert events[0].payload["sequence"] == events[1].payload["sequence"] == 0
    assert events[1].payload["parts"]["skills"]["category"] == "skills"
    assert events[1].payload["context_window"]["input_tokens"] == 100
    assert events[1].payload["kv_cache"]["session"]["weighted_hit_rate"] == 0.4
    assert events[1].payload["session_kv_cache_hit_rate"] == 0.4
    assert sum(part["tokens"] for part in events[1].payload["parts"].values()) == 100
    assert events[1].payload["parts"]["messages"]["source"] == "provider_usage_residual"
    assert events[0].payload["context_snapshot_id"] == events[1].payload["context_snapshot_id"]
    assert events[0].payload["context_owner_id"] == events[1].payload["context_owner_id"]
