...
import openjiuwen.core.personal_context.personal_context as personal_context_module
from openjiuwen.core.common.exception.errors import BaseError
from openjiuwen.core.personal_context.config import PersonalContextConfig
from openjiuwen.core.personal_context.personal_context import PersonalContext
...
async def test_fetching_disabled_starts_pipeline_without_provider_tasks(
    ...
    monkeypatch.setattr(
        "openjiuwen.core.personal_context.personal_context.ContextPipelineService",
        RunningPipeline,
    )
    personal_context = PersonalContext(home=tmp_path)
...
async def test_concurrent_activate_waits_for_one_pipeline_start(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.personal_context.ContextPipelineService", FakePipeline)
    personal_context = PersonalContext(home=tmp_path)
...
async def test_deactivate_during_starting_consumes_internal_activation_cancel(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.personal_context.ContextPipelineService", DelayedPipeline)
    personal_context = PersonalContext(home=tmp_path)
    await personal_context.set_configuration(_config())
...
async def test_deactivate_retains_activation_task_when_cancel_does_not_finish(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.personal_context.ContextPipelineService", StubbornPipeline)
    personal_context = PersonalContext(home=tmp_path)
...
async def test_reactivation_replaces_pipeline_queue(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.personal_context.ContextPipelineService", QueuePipeline)
    personal_context = PersonalContext(home=tmp_path)
    await personal_context.set_configuration(
        PersonalContextConfig.from_dict(
            {
