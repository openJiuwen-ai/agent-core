...
import openjiuwen.core.personal_context.context_pipeline as context_pipeline
from openjiuwen.core.common.exception.errors import BaseError
from openjiuwen.core.personal_context.config import PersonalContextConfig
from openjiuwen.core.personal_context.context_pipeline import (
...
from openjiuwen.core.personal_context.models import FetchBatch, RawChangeItem
from openjiuwen.core.personal_context.source_metadata import upsert_source_metadata
from openjiuwen.core.personal_context.status_codes import StatusCode, build_error
...
async def test_balanced_agent_failure_falls_back_to_rules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.Model", _FakeDirectModel)
    await service.start()
    await _submit_run(queue, _batch())
...
async def test_deterministic_processing_does_not_prevent_filesystem_agent_attempt(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", fail_agent)
    _FakeDirectModel.instances.clear()
...
async def test_agent_success_validates_pages_and_does_not_serialize_raw_snapshot(
    ...
    monkeypatch.setattr(
        "openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", successful_agent
    )
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = []
    monkeypatch.setattr(context_pipeline, "Model", _FakeDirectModel)
...
async def test_filesystem_agent_undeclared_root_is_a_non_fallback_security_error(
    ...
    monkeypatch.setattr(
        "openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", unsafe_filesystem_agent
    )
    with pytest.raises(Exception) as raised:
        await service._filesystem_with_fallback(
            processed=processed,
...
async def test_filesystem_agent_content_validation_can_fallback_to_balanced(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", failed_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = [
        json.dumps(
            {
...
async def test_filesystem_candidate_prepare_disk_error_is_non_fallback(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline._prepare_agent_candidate", fail_prepare)
    processed = {
        "documents": [
            {
                "logical_id": "notes/one",
...
async def test_filesystem_fallback_downshifts_after_deterministic_processing(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", mixed_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = [
        json.dumps(
            {
...
async def test_filesystem_agent_without_page_output_falls_back_before_publish(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", long_path_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = ["not-json"]
    monkeypatch.setattr(context_pipeline, "Model", _FakeDirectModel)
    await service.start()
...
async def test_filesystem_agent_rejects_undeclared_context_files(
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", unsafe_agent)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = ["not-json"]
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.Model", _FakeDirectModel)
    await service.start()
...
async def test_non_model_agent_error_does_not_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("openjiuwen.core.personal_context.context_pipeline.run_personal_context_agent", fail_path)
    _FakeDirectModel.instances.clear()
    _FakeDirectModel.outputs = []
    monkeypatch.setattr(context_pipeline, "Model", _FakeDirectModel)
    await service.start()
