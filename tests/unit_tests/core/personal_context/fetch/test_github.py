...
from openjiuwen.core.personal_context.config import PersonalContextFetchServiceConfig
from openjiuwen.core.personal_context.fetch.github import GitHubFetchService
...
async def test_github_fetches_enabled_resources_and_materializes_code(
    ...
    import openjiuwen.core.personal_context.fetch.github as github_module
    ...
    batches = [batch async for batch in provider.fetch(run_id="run-a", cursor=None)]
...
async def test_github_default_limit_is_25_and_resources_are_switchable(
    ...
    assert len(batches[0].next_cursor["issues"]["latest_ids"]) == 20
    assert len(batches[1].next_cursor["issues"]["latest_ids"]) == 25
...
async def test_github_fetch_removes_same_service_crash_candidate_first(
    ...
    batches = [batch async for batch in provider.fetch(run_id="run-new", cursor=None)]
...
@pytest.mark.asyncio
async def test_github_total_limit_is_shared_fairly_across_enabled_list_resources(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import openjiuwen.core.personal_context.fetch.github as github_module
    ...
    assert counts == {"readme": 1, "issues": 6, "pull_requests": 6, "commits": 6}
    assert len(items) == 19
...
async def test_github_rejects_unsafe_archive_and_wrong_run_cannot_commit_or_abort(
    ...
    FakeSession.responses = {
        "https://api.github.com/repos/acme/demo": [FakeResponse({"default_branch": "main", "sha": "a" * 40})],
        f"https://api.github.com/repos/acme/demo/zipball/{'a' * 40}": [FakeResponse({}, content=archive)],
    }
    monkeypatch.setattr(github_module.aiohttp, "ClientSession", FakeSession)
    provider = GitHubFetchService(github_config(tmp_path, resources=["code"]), home=tmp_path)
    with pytest.raises(Exception):
        _ = [batch async for batch in provider.fetch(run_id="run-a", cursor=None)]
...
async def test_github_cursor_merges_same_timestamp_latest_ids(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = [batch async for batch in provider.fetch(run_id="run-a", cursor=None)]
    ...
    second = [batch async for batch in provider.fetch(run_id="run-b", cursor=first_cursor)]
    ...
    assert set(second_cursor["issues"]) == {
        "latest_updated_at",
        "latest_ids",
    ...
    assert second_cursor["issues"]["latest_ids"] == [
    ...
    third = [batch async for batch in provider.fetch(run_id="run-c", cursor=second_cursor)]
...
async def test_github_rejects_malformed_resource_cursor(tmp_path: Path) -> None:
    provider = GitHubFetchService(github_config(tmp_path, resources=["issues"]), home=tmp_path)
    with pytest.raises(Exception):
        _ = [batch async for batch in provider.fetch(run_id="run-a", cursor={"issues": "bad"})]
...
async def test_github_issues_continue_history_and_prioritize_new_changes(
    ...
    for run_id in ("run-a", "run-b", "run-c"):
        batches = [batch async for batch in service.fetch(run_id=run_id, cursor=cursor)]
    ...
    assert isinstance(cursor["issues"]["latest_updated_at"], str)
    assert isinstance(cursor["issues"]["latest_ids"], list)
    assert cursor["issues"]["history_before_updated_at"] is None or isinstance(
        cursor["issues"]["history_before_updated_at"], str
    )
    assert isinstance(cursor["issues"]["history_boundary_ids"], list)
    assert cursor["issues"]["history_complete"] is True
    ...
    first = [batch async for batch in priority_service.fetch(run_id="priority-a", cursor=None)]
    ...
    second = [batch async for batch in priority_service.fetch(run_id="priority-b", cursor=first_cursor)]
