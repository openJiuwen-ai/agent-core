...
class TestConditionBase:
    ...
    def setup_method(self):
        ...
        # Set Session's state method to return this mock_state
        self.mock_session.state.return_value = self.mock_state
...
class TestCondition(TestConditionBase):
    ...
    def test_condition_base_class(self):
        ...
        self.mock_state.get_inputs.return_value = {}
        result = condition(self.mock_session)
    ...
    def test_condition_with_tuple_result(self):
        ...
        class TestConditionWithTuple(Condition):
            def invoke(self, inputs: Input, session: BaseSession) -> tuple:
...
class TestFuncCondition(TestConditionBase):
    ...
    def test_func_condition_invoke(self):
        ...
        # Test invoke method
        result = func_condition.invoke({}, self.mock_session)
        assert result is True
...
class TestAlwaysTrue(TestConditionBase):
    ...
    def test_always_true_invoke(self):
        ...
        result = always_true.invoke({}, self.mock_session)
        assert result is True

        # Test again to ensure it always returns True
        result = always_true.invoke({"key": "value"}, self.mock_session)
        assert result is True
...
class TestArrayCondition(TestConditionBase):
    ...
    def test_array_condition_invoke_within_limit(self):
        ...
        # Test invoke method
        result, updates = array_condition.invoke(inputs, self.mock_session)
    ...
    def test_array_condition_invoke_beyond_limit(self):
        ...
        # Test invoke method
        result = array_condition.invoke(inputs, self.mock_session)
...
class TestNumberCondition(TestConditionBase):
    ...
    def test_number_condition_invoke_within_limit(self):
        ...
        # Test invoke method
        result = number_condition.invoke(inputs, self.mock_session)
...
class TestExpressionCondition(TestConditionBase):
    ...
    def test_expression_condition_invoke_with_true_result(self):
        ...
        # Set up mock data
        expression = "${a} > 5 && ${b} < 10"
        self.mock_state.get_global.side_effect = lambda x: 6 \
            if x == "a" else 8 if x == "b" else None

        # Create ExpressionCondition instance
        expr_condition = ExpressionCondition(expression)

        # Test invoke method
        result = expr_condition.invoke({}, self.mock_session)
    ...
    def test_expression_condition_invoke_with_false_result(self):
        ...
        # Set up mock data
        expression = "${a} > 5 && ${b} < 10"
        self.mock_state.get_global.side_effect = lambda x: 4 \
            if x == "a" else 8 if x == "b" else None
    ...
    def test_expression_condition_trace_info(self):
        ...
        trace_info = expr_condition.trace_info(self.mock_session)
    ...
    def test_expression_preprocessing_operators(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: 6 \
            if x == "a" else 8 if x == "b" else 3 if x == "c" else None
    ...
    def test_expression_preprocessing_functions(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: [] if x == "empty_list" \
            else [1, 2, 3] if x == "non_empty_list" else None
    ...
    def test_expression_arithmetic_operators(self):
        ...
        self.mock_state.get_global.side_effect = lambda \
                x: 6 if x == "a" else 5 if x == "b" else 4 if x == "c" else 4 if x == "d" else None
    ...
    def test_expression_comparison_operators(self):
        ...
        self.mock_state.get_global.side_effect = lambda \
                x: 5 if x == "a" or x == "b" else 10 if x == "c" else 20 if x == "d" else None
        expr_condition_1 = ExpressionCondition(expression_1)
        result_1 = expr_condition_1.invoke({}, self.mock_session)
        assert result_1 is True

        # Test in, not in
        expression_2 = "${a} in ${list} && ${b} not_in ${list}"
        self.mock_state.get_global.side_effect = lambda x: 1 \
            if x == "a" else 4 if x == "b" \
            else [1, 2, 3] if x == "list" else None
        expr_condition_2 = ExpressionCondition(expression_2)
        result_2 = expr_condition_2.invoke({}, self.mock_session)
        assert result_2 is True

        # Test is, is not
        expression_3 = "${a} is None && ${b} is not None"
        self.mock_state.get_global.side_effect = lambda x: None if x == "a" \
            else "value" if x == "b" else None
        expr_condition_3 = ExpressionCondition(expression_3)
        result_3 = expr_condition_3.invoke({}, self.mock_session)
    ...
    def test_expression_boolean_operators(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: 4 if x == "a" else 8 \
            if x == "b" else 3 if x == "c" else None
    ...
    def test_expression_unary_operators(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: -5 \
            if x == "a" else 5 if x == "b" else None
    ...
    def test_expression_func_calls(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: [1, 2, 3] \
            if x == "list" else None
    ...
    def test_complex_nested_expressions(self):
        ...
        # Set up mock data
        expression = ("(((${a} > 5 and ${b} < 10) or "
                      "(${c} == 3 and ${d} != 4)) and (len(${list}) > 0))")
        self.mock_state.get_global.side_effect = lambda \
                x: 4 if x == "a" else 8 if x == "b" else 3 if x == "c" else 5 if x == "d" \
            else [1, 2, 3] if x == "list" else None
    ...
    def test_prevent_object_method_escape(self):
        ...
        assert ("prohibited" in str(excinfo.value)
                or "Disallowed operation" in str(excinfo.value))
    ...
    def test_prevent_resource_exhaustion(self):
        ...
        # Use a value that should trigger our MAX_COLLECTION_SIZE check
        expression = "${large_list}[0:2000]"
    ...
    def test_large_list_literal_protection(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: [0] * 100001 \
            if x == "very_large_list" else None
    ...
    def test_expression_length_limit(self):
        ...
        # This will be much longer than allowed
        too_long_expression = "${a} > 0" * (MAX_EXPRESSION_LENGTH // 8 + 1)
    ...
    def test_ast_nesting_depth_limit(self):
        ...
        self.mock_state.get_global.side_effect = lambda x: 5 \
            if x == "a" else 50 if x == "b" else None
        result = expr_condition.invoke({}, self.mock_session)
        assert result is True
    ...
    def test_string_literal_preservation_with_boolean_identifier(self):
        ...
        result2 = expr_condition2.invoke({}, self.mock_session)
    ...
    def test_single_quote_string_literal_preservation(self):
        ...
        result1 = expr_condition1.invoke({}, self.mock_session)
        ...
        result3 = expr_condition3.invoke({}, self.mock_session)
        ...
        result4 = expr_condition4.invoke({}, self.mock_session)
